...
public class RdsBillListView extends HrBillListView implements BillEditListener2, ChangeListener {
	...
	private InfoItemVO[] getJobItems() throws BusinessException {
		...
		if (jobItems == null) {
			jobItems = getService().queryByCondition(
					InfoItemVO.class,
					" pk_infoset in (select pk_infoset from hr_infoset where pk_infoset_sort = '"
							+ TRNConst.INFOSET_SORT_PSNDOC + "' and infoset_code = 'hi_psnjob' )");
		}
		return jobItems;
	}
	...
	private void setDimiCellEditable(BillModel billModel, int selRow) throws BusinessException {
		// 可编辑的字段
		String[] defaultAttr = new String[] { PsnJobVO.BEGINDATE, PsnJobVO.ENDDATE, PsnJobVO.CLERKCODE,
				PsnJobVO.PK_ORG, PsnJobVO.PK_PSNCL, PsnJobVO.PK_DEPT, PsnJobVO.TRNSTYPE, PsnJobVO.TRNSREASON,
				PsnJobVO.MEMO };
	...
	private void addTabActions() {
		...
		for (Action action : actions) {
			FuncPermissionState state = getModel().getContext().getFuncInfo()
					.getButtonPermissionState((String) action.getValue(INCAction.CODE));
	...
	public void afterEdit(BillEditEvent e) {
		...
		if (TRNConst.Table_NAME_TRIAL.equals(e.getTableCode())) {
			...
			if (TrialVO.TRIALRESULT.equals(e.getKey())) {
				...
				if (((RdsPsninfoModel) getModel()).getEditType() == RdsPsninfoModel.INSERT && trialResult != null
						&& trialResult == 2) {
					// 插入试用记录时不能选择延长使用期
					MessageDialog.showWarningDlg(getModel().getContext().getEntranceUI(), null,
							ResHelper.getString("6009tran", "06009tran0095"));
					getBillListPanel().getBodyBillModel(e.getTableCode()).setValueAt(e.getOldValue(), e.getRow(),
							TrialVO.TRIALRESULT);
					return;
				}
				// 转正通过或转正未通过时试用结束
				getBillListPanel().getBodyBillModel(e.getTableCode()).setValueAt(
						UFBoolean.valueOf(trialResult != null && (trialResult == 1 || trialResult == 3)), e.getRow(),
						TrialVO.ENDFLAG);
			}
			...
			if (PartTimeVO.ENDFLAG.equals(e.getKey())) {
				...
				if (endflag != null && endflag.booleanValue()) {
					getBillListPanel().getBodyBillModel(e.getTableCode()).setValueAt(UFBoolean.FALSE, e.getRow(),
							PartTimeVO.POSTSTAT);
				}
			}
		}
		if (ArrayUtils.contains(new String[] { TRNConst.Table_NAME_DEPTCHG, TRNConst.Table_NAME_DIMISSION,
				TRNConst.Table_NAME_PART }, e.getTableCode())) {
			afterEditPsnjob(e);
		}
	...
	private void afterEditPsnjob(BillEditEvent e) {
		...
		PsnJobVO psn = (PsnJobVO) getBillListPanel().getBodyBillModel(e.getTableCode()).getBodyValueRowVO(e.getRow(),
		...
		try {
			if (PsnJobVO.PK_GROUP.equals(e.getKey())) {
				...
				if (pk_post != null) {
					setValue(null, e, new String[] { PsnJobVO.PK_PSNCL, PsnJobVO.PK_POST, PsnJobVO.PK_JOBGRADE,
							PsnJobVO.PK_POSTSERIES, PsnJobVO.PK_JOB, PsnJobVO.SERIES, PsnJobVO.PK_JOBRANK });
				}
				...
				String pk_post = psn.getPk_post();
				if (pk_post != null) {
					setValue(null, e, new String[] { PsnJobVO.PK_POST, PsnJobVO.PK_JOBGRADE, PsnJobVO.PK_POSTSERIES,
							PsnJobVO.PK_JOB, PsnJobVO.SERIES, PsnJobVO.PK_JOBRANK });
				}
				String values = (String) e.getValue();
				nc.itf.uap.IUAPQueryBS query = NCLocator.getInstance().lookup(IUAPQueryBS.class);

				Collection<OrgVO> c = query.retrieveByClause(OrgVO.class, "pk_org ='" + values.toString() + "'");

				OrgVO[] orgVOs = c.toArray(new OrgVO[0]);
				String pk_corp;
				if (null != orgVOs && orgVOs.length > 0) {
					pk_corp = orgVOs[0].getPk_corp();
				} else {
					pk_corp = "";
				}
				
                //根据组织设置上级区域以及发薪公司  add by ward 2018年10月31日15:57:48 begin
                setValue(pk_corp,e,"jobglbdef3");//发薪公司
                setValue(orgVOs[0].getPk_fatherorg(),e,"jobglbdef5");//所属区域
              //根据组织设置上级区域以及发薪公司  add by ward 2018年10月31日15:57:48 end

				/*
				 * int jobglbdef3 =
				 * getBillListPanel().getBodyBillModel(e.getTableCode
				 * ()).getBodyColByKey("jobglbdef3");
				 */
				getBillListPanel().getBodyTable(e.getTableCode()).setValueAt(pk_corp, e.getRow(), 16);
				/*
				 * getBillListPanel().getBodyBillModel(e.getTableCode()).setValueAt
				 * (pk_corp, e.getRow(), "jobglbdef3");
				 */
				/*
				 * getBillListPanel().getBodyBillModel(e.getTableCode()).
				 * setCellEditable(e.getRow(), jobglbdef3, true);
				 */
                ...
                //根据部门设置所属群组  add by ward 2018年10月31日15:32:06 begin
                String sql="select glbdef1 from org_dept where pk_dept='"+psn.getPk_dept()+"'";
                IUAPQueryBS query=NCLocator.getInstance().lookup(IUAPQueryBS.class);
                String jobglbdef4=(String) query.executeQuery(sql, new ColumnProcessor());
                setValue(jobglbdef4,e,"jobglbdef4");
              //根据部门设置所属群组  add by ward 2018年10月31日15:32:06 end
				...
				if (post != null) {
					...
					setValue(null, e, new String[] { PsnJobVO.PK_POSTSERIES, PsnJobVO.PK_JOB, PsnJobVO.PK_JOBGRADE,
							PsnJobVO.PK_JOBRANK, PsnJobVO.SERIES });// 岗位序列
				}

				if (post == null || post.getPk_job() != null) {
					BillEditEvent event = new BillEditEvent(getBillListPanel().getBodyItem(e.getTableCode(),
							PsnJobVO.PK_JOB), post == null ? null : post.getPk_job(), PsnJobVO.PK_JOB, e.getRow(),
							e.getPos());
				...
				String pk_post = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.PK_POST + "_ID");
				if (job != null) {
					String defaultlevel = "";
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
				...
				String pk_job = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.PK_JOB + "_ID");
				String pk_post = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.PK_POST + "_ID");
				...
				String series = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.SERIES + "_ID");
				String pk_job = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.PK_JOB + "_ID");
				String pk_post = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
				...
				JobLevelVO jobgrade = pk_jobgrage == null ? null : getService().queryByPk(JobLevelVO.class,
						pk_jobgrage, true);
				if (jobgrade != null) {
					String series = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(
							e.getRow(), PsnJobVO.SERIES + "_ID");
					String pk_job = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(
							e.getRow(), PsnJobVO.PK_JOB + "_ID");
					String pk_postseries = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(
							e.getRow(), PsnJobVO.PK_POSTSERIES + "_ID");
					String pk_post = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(
							e.getRow(), PsnJobVO.PK_POST + "_ID");
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
				...
				// 岗位
				String pk_post1 = (String) e.getValue();
	...
	private void setJobGradeItemEdit(String strTabCode, int row) {
		String series = (String) getBillListPanel().getBodyBillModel(strTabCode).getValueAt(row,
				PsnJobVO.SERIES + "_ID");
		String pk_job = (String) getBillListPanel().getBodyBillModel(strTabCode).getValueAt(row,
				PsnJobVO.PK_JOB + "_ID");
		String pk_postseries = (String) getBillListPanel().getBodyBillModel(strTabCode).getValueAt(row,
				PsnJobVO.PK_POSTSERIES + "_ID");
		String pk_post = (String) getBillListPanel().getBodyBillModel(strTabCode).getValueAt(row,
				PsnJobVO.PK_POST + "_ID");
	...
	private void setBodyItemEdit(String strTabCode, int iRowIndex, boolean isEdit, String... strBodyItemKeys) {
		...
		BillModel billModel = strTabCode == null ? getBillListPanel().getBodyBillModel() : getBillListPanel()
				.getBodyBillModel(strTabCode);
		if (billModel == null) {
			return;
		}
	...
	private boolean beforeEditPsnjob(BillEditEvent e) {
		...
		if (PsnJobVO.TRNSTYPE.equals(e.getKey())) {
			...
			String powerSql = HiSQLHelper.getPsnPowerSql(pk_group, HICommonValue.RESOUCECODE_ORG,
			...
			where += " and pk_adminorg in(select pk_org from hr_relation_org where pk_hrorg = '"
			...
			String pk_group = psn.getPk_group();
			((UIRefPane) bi.getComponent()).getRefModel().setPk_group(pk_group);
			String powerSql = HiSQLHelper.getPsnPowerSql(pk_group, HICommonValue.RESOUCECODE_PSNCL,
			...
			String powerSql = HiSQLHelper.getPsnPowerSql(psn.getPk_group(), HICommonValue.RESOUCECODE_DEPT,
			...
			// 职等
			BillItem item = (BillItem) e.getSource();
			String pk_jobrank = (String) (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(
					e.getRow(), PsnJobVO.PK_JOBRANK + "_ID");
			...
			String pk_jobgrade = (String) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
					PsnJobVO.PK_JOBGRADE + "_ID");
			...
			if (TRNConst.Table_NAME_PART.equals(e.getTableCode())) {
				Boolean isEnd = (Boolean) getBillListPanel().getBodyBillModel(e.getTableCode()).getValueAt(e.getRow(),
						PsnJobVO.ENDFLAG);
	...
	private PsnJobVO createNewDimission(PsnJobVO srcVO) {
		...
		psnjob.setPk_post(null);
		psnjob.setPk_postseries(null);
		psnjob.setPk_job(null);
		psnjob.setPk_jobgrade(null);
		psnjob.setPk_jobrank(null);
		psnjob.setSeries(null);
		...
		psnjob.setPk_job_type(null);
		psnjob.setWorktype(null);
	...
	public void editLine(String curTabCode, boolean isInJob) throws BusinessException {
		...
		if (TRNConst.Table_NAME_TRIAL.equals(curTabCode)) {
			...
			if (selvo.getAttributeValue("lastflag") != null
					&& !((UFBoolean) selvo.getAttributeValue("lastflag")).booleanValue()) {
				// 历史的是否在岗不能修改
			...
			if (TRNConst.Table_NAME_DIMISSION.equals(curTabCode)) {
				...
				if (selvo.getAttributeValue("lastflag") != null
					...
					setDimiCellEditable(bm, selRow);
					if (selvo.getAttributeValue(PsnJobVO.TRNSEVENT) != null
							&& ((Integer) selvo.getAttributeValue(PsnJobVO.TRNSEVENT)) == 5) {
						bm.setCellEditable(selRow, PsnJobVO.TRNSTYPE, false);
					}
				}
			}
	...
	public CircularlyAccessibleValueObject getBodySelectVO() {
		...
		CircularlyAccessibleValueObject vo = getBillListPanel().getBodyBillModel(tabCode).getBodyValueRowVO(row,
				getCurClassName());
		if (vo == null) {
			return null;
		}
	...
	public String getMataTabCode() {
		if (TRNConst.Table_NAME_DEPTCHG.equals(getCurrentTabCode())
				|| TRNConst.Table_NAME_DIMISSION.equals(getCurrentTabCode())) {
	...
	public void initUI() {
		...
		setBillListPanelValueSetter(new MDBillListPanelValueSetter() {
			...
			public void setBodyData(BillListPanel listPanel, Object selectedData) {
				...
				PsndocAggVO aggvo = ((RdsPsninfoModel) getModel()).querySubVO((PsndocAggVO) selectedData,
						new String[] { getCurrentTabCode() });
			...
			public void setHeaderRowData(BillListPanel listPanel, Object rowData, int row) {
				...
				IBusinessEntity be = getBillListPanel().getBillListData().getBillTempletVO().getHeadVO()
	...
	public boolean isHisInf() {
		...
		if (jobVO == null || !TRNConst.Table_NAME_DEPTCHG.equals(getCurrentTabCode()))
			return false;
		...
		if (!jobVO.getPk_hrorg().equals(getModel().getContext().getPk_org())) {
			...
			for (int i = 1; i < rowSize - row; i++) {
				pk_hrorg = (String) getBillListPanel().getBodyBillModel(TRNConst.Table_NAME_DEPTCHG).getValueAt(
						row + i, PsnJobVO.PK_HRORG);
				if (!getModel().getContext().getPk_org().equals(pk_hrorg))
					return true;
			}
		}
		return false;
