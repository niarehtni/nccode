...
public class SegDetailServiceImpl implements ISegDetailService {
	...
	private OTSChainNode getOvertimeSegChainNodeByOTReg(OvertimeRegVO otRegVO, AggSegRuleVO ruleAggVO)
		...
		for (SegRuleTermVO term : terms) {
			...
			UFDouble originStart = term.getStartpoint();
			UFDouble originend = term.getEndpoint() == null ? new UFDouble(24) : term.getEndpoint();
			UFDouble originSeghours = originend.sub(originStart); // 原始分段時長

			if (beginTerm != null) {
				if (!term.getPk_segruleterm().equals(beginTerm.getPk_segruleterm())) {
					continue;
				} else {
					term = beginTerm;
					beginTerm = null;
			...
			segvo.setRulehours(originSeghours); // 还原原始分段时长
						...
						for (SegRuleTermVO term : terms) {
							if (term.getPk_segruleterm().equals(pk_lastterm)) {
								...
								if (lastHours.doubleValue() < ruleHours.doubleValue()) {
									retTerm = (SegRuleTermVO) term.clone();
