...
public class PsndocDataManager extends DefaultAppModelDataManager implements AppEventListener, IAppModelDataManagerEx,
	...
	public IPsndocQryService getQueryService() {
		if (queryService == null) {
			queryService = (NCLocator.getInstance().lookup(IPsndocQryService.class));
		}
		return queryService;
	}
	...
	protected String getSQL4Tree() throws Exception {
		...
		if ((superVO instanceof OrgVO)) {
			...
			if ("other_node_msaos".equals(orgVO.getPrimaryKey())) {
				String strOtherPsnjobByHrorg = ManagescopeFacade.queryOtherPsnjobByHrorg4MsAOS(getContext().getPk_org(),
			...
			String grpInSql = " select pk_keypsn_group from hi_keypsn_group where pk_org = '" + getContext().getPk_org()
					+ "' ";
	...
	public String getTableAlias(String strTableName) {
		return getModel().getUsedTablesOfQuery().containsKey(strTableName)
				? (String) getModel().getUsedTablesOfQuery().get(strTableName) : strTableName;
	}
	...
	public void handleEvent(AppEvent event) {
		...
		if (ArrayUtils.contains(
				new String[] { "Data_Deleted", "Data_Inserted", "Selected_Data_Changed", "Multi_Selection_Changed" },
				event.getType())) {
			...
			if ((info instanceof RowSelectionOperationInfo)) {
				...
				((RowSelectionOperationInfo) newInfo)
						.setSelectionState(((RowSelectionOperationInfo) info).getSelectionState());
			}
	...
	public void queryData() throws Exception {
		...
		String[] strPk_psndocs = getQueryService().queryPsndocPks(getContext(),
				new String[] { getTableAlias(PsnJobVO.getDefaultTableName()) + "." + "pk_psnjob" }, strFrom, strWhere,
				getOrderby(getModel().getSortFields()), getModel().getUsedTablesOfQuery(),
	...
	public SuperVO[] querySubVO(String strTabCode, String strAddtionalWhere) throws BusinessException {
		...
		if (((getModel().getBusinessInfoSet().contains(strTabCode))
				|| (WainfoVO.getDefaultTableName().equals(strTabCode))) && (psndocAggVO != null)) {
			...
			if ("60070psninfo".equals(getContext().getNodeCode())) {
				...
				if (PsnJobVO.getDefaultTableName().equals(strTabCode)) {
					...
					strWhere = strWhere + " and pk_psnjob in (select pk_psnjob from hi_psnjob where pk_org = '" + pk_org
							+ "' and pk_psnorg = '" + psndocAggVO.getParentVO().getPsnOrgVO().getPrimaryKey() + "') ";
					...
					// 只显示当前组织关系的数据
					strWhere = strWhere + " and pk_psnorg = '" + psndocAggVO.getParentVO().getPsnOrgVO().getPrimaryKey()
							+ "'";
				}
				...
				if ((UIState.ADD == getModel().getUiState() || UIState.EDIT == getModel().getUiState())
					...
					// 显示所有信息时只显示转档的数据
					// strWhere +=
					// " and pk_psnorg in ( select pk_psnorg from hi_psnorg
					// where pk_psndoc='"
