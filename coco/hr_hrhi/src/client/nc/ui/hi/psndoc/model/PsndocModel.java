...
public class PsndocModel extends BillManageModel implements IQueryMode {
	...
	public BillCodeContext getPsndocClerkCodeContext() {
		try {
			return HiCacheUtils.getBillCodeContext("6007psndoc_clerkcode", getContext().getPk_group(),
	...
	public BillCodeContext getPsndocCodeContext() {
		try {
			return HiCacheUtils.getBillCodeContext("6007psndoc_code", getContext().getPk_group(),
					getContext().getPk_org());
		} catch (BusinessException ex) {
			Debug.error(ex.getMessage(), ex);
		}
	...
	public String[] getHiddenKeys() throws BusinessException {
		if (hiddenKeys == null) {
			hiddenKeys = NCLocator.getInstance().lookup(IPsndocQryService.class).queryHiddenKeys(getContext());
		}
		return hiddenKeys;
	}
	...
	public void saveKeyPsn(String[] refPKs) throws BusinessException {
		...
		PsndocAggVO[] agg = NCLocator.getInstance().lookup(IPsndocService.class).saveKeyPsn(refPKs,
	...
	public void saveKeyPsn(String wherePart) throws BusinessException {
		...
		PsndocAggVO[] agg = NCLocator.getInstance().lookup(IPsndocService.class).saveKeyPsnByCondition(wherePart,
				(KeyPsnGrpVO) getCurrTypeOrgVO());
	...
	public void deleteKeyPsn() throws Exception {
		...
		NCLocator.getInstance().lookup(IPsndocService.class).deleteKeyPsn(vos.toArray(new PsndocVO[0]),
				pks.toArray(new String[0]), grp.getPk_keypsn_group());
	...
	public String[] queryNotNullSubset(PsndocAggVO psndocAggVO, HashMap<String, String> showTab)
		...
		return NCLocator.getInstance().lookup(IPsndocQryService.class).queryNotNullSubset(psndocAggVO, showTab);
	}

	public String[] validateSubNotNull(PsndocAggVO psndocAggVO, HashMap<String, String> showTab)
			throws BusinessException {
		return NCLocator.getInstance().lookup(IPsndocQryService.class).validateSubNotNull(psndocAggVO, showTab);
	}
