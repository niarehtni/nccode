...
public class PsndocWadocSynNHIServiceImpl implements IPsndocWadocSynNHIService {
	...
	@Override
	public void synNHIByWadocQueryVO(WadocQueryVO[] wadocQueryVOs) throws Exception {
		...
		for (String pk_psndoc : psndocPKs) {
			...
			if (cutterVOs.length > 0) {
				PsndocDefVO[] nhiVOs = service.queryByCondition(PsndocDefUtil.getPsnLaborVO().getClass(), strCondition);
				// 原始（切分後）VOs
				WadocQueryVO[] finalQueryVOs = getOriginNHIVOs(nhiVOs);
				if (wadocQueryVOs.length > 0) {
					for (int i = 0; i < cutterVOs.length; i++) {
						finalQueryVOs = WadocQueryVOCutUtils.getCombinedVOs(finalQueryVOs, cutterVOs[i],
								MoneyCalcTypeEnum.NEWER);
					}
				}
				PsndocDefVO[] nhiSaveVOs = getSaveVOsByCombinedVOs(finalQueryVOs, nhiVOs);
				...
				if (nhiSaveVOs != null && nhiSaveVOs.length > 0) {
					...
					if (nhiVOs != null && nhiVOs.length > 0) {
						for (PsndocDefVO vo : nhiVOs) {
							if ((vo.getBegindate().isSameDate(startDate) || vo.getBegindate().after(startDate))
									&& (vo.getEnddate().isSameDate(endDate) || vo.getEnddate().before(endDate))) {
								if (!updatedList.contains(vo.getPk_psndoc_sub())) {
									service.delete(vo);
								}
							}
						}
	...
	private PsndocDefVO[] getSaveVOsByCombinedVOs(WadocQueryVO[] finalQueryVOs, PsndocDefVO[] nhiVOs) throws Exception {
		...
		if (finalQueryVOs != null && finalQueryVOs.length > 0) {
			for (WadocQueryVO vo : finalQueryVOs) {
				...
				newVO.setBegindate(UFLiteralDate.getDate(vo.getBegindate().toDate()));
				newVO.setEnddate(UFLiteralDate.getDate(vo.getEnddate().toDate()));
	...
	private PsndocDefVO filledVOs(PsndocDefVO newVO, PsndocDefVO comVO) {
		...
		if (comVO == null) {
			...
			if (newVO.getBegindate().isSameDate(comVO.getBegindate())
				...
				comVO.setAttributeValue("glbdef2", newVO.getAttributeValue("glbdef2"));
				returnedVO = comVO;
			}
			// 起始時間在源VO區間內，從源VO取除人員PK、時間、金額以外屬性值（Insert）
				...
				for (int i = 1; i < 10; i++) {
					if (i != 2) {
						newVO.setAttributeValue("glbdef" + String.valueOf(i),
								comVO.getAttributeValue("glbdef" + String.valueOf(i)));
					}
					newVO.setLastflag(UFBoolean.TRUE);
				}
				returnedVO = newVO;
	...
	private PsndocDefVO getPeriodContainedNHIVO(PsndocDefVO[] nhiVOs, WadocQueryVO vo) {
		if (nhiVOs != null && nhiVOs.length > 0) {
			for (PsndocDefVO nhivo : nhiVOs) {
				if ((vo.getBegindate().isSameDate(UFDate.getDate(nhivo.getBegindate().toDate()))
						|| vo.getBegindate().after(UFDate.getDate(nhivo.getBegindate().toDate())))
						&& (vo.getEnddate().isSameDate(UFDate.getDate(nhivo.getEnddate().toDate()))
								|| vo.getEnddate().before(UFDate.getDate(nhivo.getEnddate().toDate())))) {
					return nhivo;
				}
			}
		}
	...
	private WadocQueryVO[] getOriginNHIVOs(PsndocDefVO[] nhiVOs) {
		...
		if (nhiVOs != null) {
			for (PsndocDefVO vo : nhiVOs) {
				...
				newVO.setNmoney((vo.getAttributeValue("glbdef2") == null || vo.getAttributeValue("glbdef2").equals("~"))
						? UFDouble.ZERO_DBL : (UFDouble) vo.getAttributeValue("glbdef2"));
			}
		}
		return null;
	}

	private WadocQueryVO[] getVOsByPsndoc(WadocQueryVO[] wadocQueryVOs, String pk_psndoc) {
		List<WadocQueryVO> vos = new ArrayList<WadocQueryVO>();
