...
public class PaydataServiceImpl implements IPaydataManageService,
		...
		if (NCLocator.getInstance().lookup(IAmoSchemeQuery.class)
			...
			if (WaLoginVOHelper.isSubClass(waLoginVO)) {
				throw new BusinessException(ResHelper.getString("60130paydata",
						"060130paydata0516")/* @res "选择的薪资方案父方案已经分摊制单,不能取消发放！" */);
			} else {
				throw new BusinessException(ResHelper.getString("60130paydata",
						"060130paydata0458")/* @res "选择的薪资方案已经分摊制单,不能取消发放！" */);
			}
		}
	}
	...
	private void checkReData(WaLoginVO waLoginVO) throws BusinessException {
		...
		if (!b) {
			throw new BusinessException(ResHelper.getString("60130paydata",
					"060130paydata0459")/* @res "补发数据尚未传递过来，无法继续！" */);
		}
	...
	public void reTotal(WaLoginVO waLoginVO) throws BusinessException {
		...
		if (state == WaState.CLASS_WITHOUT_RECACULATED
			...
			if (!unitClassQuery.isUnitAllCheckOut(waLoginVO)) {
				throw new BusinessException(ResHelper.getString("60130paydata",
						"060130paydata0460")/* @res "参与汇总的方案有的尚未结帐！" */);
			}
			...
			throw new BusinessException(ResHelper.getString("60130paydata",
					"060130paydata0461")/* @res "当前状态 不能汇总！" */);// new
		}
	...
	private void validateHealth(WaLoginContext loginContext,
		...
		String sql2 = "select bd.pk_psndoc from bd_psndoc bd left join wa_data wad on bd.pk_psndoc = wad.PK_PSNDOC where wad.PK_WA_CLASS = '"
				...
				+ cperiod
				+ "' and wad.CYEAR='" + cyear + "'";
		List<String> pk_psndocs = (ArrayList<String>) new BaseDAO().executeQuery(sql2,
		...
		//查出已经进行了二代健保计算的人员
		String sql3 = "select vbdef4 FROM DECLARATION_BUSINESS " 
				+ " where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" 
				+ " union "
				+ " select vbdef4 FROM DECLARATION_NONPARTTIME " 
				+ " where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" 
				+ " union "
				+ " select vbdef4 FROM DECLARATION_PARTTIME " 
				+ " where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" ;
		List<String> psndocHealthReady = (ArrayList<String>) new BaseDAO().executeQuery(sql3,
				new ColumnListProcessor());
		//过滤出未进行过滤的人员
		...
		if(psndocNeedHealthSet!=null && psndocNeedHealthSet.size() > 0){
			throw new BusinessException("二代健保計算未全部完成,請先進行二代健保計算!");
		}
	}
	/**
	 * 模拟计算的逻辑得出所有的人员,查询需要进二代健保计算的人,并过滤出本次真正需要进行计算的人员
	 * @return
	 * @throws BusinessException 
	 */
		...
		for (String pk_org : legalOrgs) {
			...
			if (DECLAREFORM_50 == waclassType) {
				...
				if(null == payDate){
					...
					throw new BusinessException("本人力資源组织下的法人組織:"+value+",還未進行二代健保計算,請先點擊二代健保計算,再進行審核!");
				}
				// 区分兼职人员和非兼职人员
	...
	private UFDate findPayDateInDeclaration(String pk_wa_class, String pk_org,
		...
		//查詢三張表中的是否有值,只要有一条即可
		UFDate result = null;
		String sql = "select top 1 pay_date FROM DECLARATION_BUSINESS " +
				" where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" ;
		...
		if(null == result){
			sql= " select top 1 pay_date FROM DECLARATION_NONPARTTIME " +
					" where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" ;
			...
			if(null == result){
				sql= " select top 1 pay_date FROM DECLARATION_PARTTIME " +
						" where vbdef1 = '"+pk_org+"' and vbdef2 = '"+pk_wa_class+"' and vbdef3 = '"+pk_wa_period+"'" ;
				value = (String) dao.executeQuery(sql,new ColumnProcessor());
				try{
					result = new UFDate(value);
