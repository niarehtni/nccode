 ...
 *
 */
public class ShopTaRegListQueryCtrl extends IQueryController {
	...
	public void simpleValueChanged(DatasetCellEvent dataLoadEvent) {
		...
		if (colIndex == ds.nameToIndex(ShopOverTimeRegConsts.FD_BEGINDATE) || colIndex == ds.nameToIndex(ShopOverTimeRegConsts.FD_ENDDATE)) {
			...
			if (oldValue.equals(newValue)) {
				appCxt.addAppAttribute(ShopOverTimeRegConsts.SESSION_DATE_CHANGE, UFBoolean.FALSE);
			} else {
				// 开始日期/结束日期发生了变化
				appCxt.addAppAttribute(ShopOverTimeRegConsts.SESSION_DATE_CHANGE, UFBoolean.TRUE);
			}
		}
	}
	/**
	...
	private void setDefaultConditions(Dataset ds, Row selRow) {
		...
		if (ds.nameToIndex(ShopOverTimeRegConsts.FD_BEGINDATE) > -1) {
			// 开始日期
			selRow.setValue(ds.nameToIndex(ShopOverTimeRegConsts.FD_BEGINDATE), String.valueOf(dates[0]));
		}
		if (ds.nameToIndex(ShopOverTimeRegConsts.FD_ENDDATE) > -1) {
			if(dates[1].after(dates[0].getDateAfter(60))){
				dates[1] = dates[0].getDateAfter(60);
			}
			// 结束日期
			selRow.setValue(ds.nameToIndex(ShopOverTimeRegConsts.FD_ENDDATE), String.valueOf(dates[1]));
		}
