package nc.bs.hrsms.ta.sss.monthreport;
...
//			viewOrderVOs = viewOrder.queryViewOrder(pk_org, fun_type, report_type, true);
	...
	public static void removeCol(LfwView widget){
		// 表格
		GridComp grid = (GridComp) widget.getViewComponents().getComponent(MonthReportConsts.GRID_MONTH_REPORT);
		...
		// 数据集
		Dataset ds = widget.getViewModels().getDataset(MonthReportConsts.DATASET_MONTH_REPORT);
		// 去掉多余字段
	...
	public static void buildDsAndGrid(LfwView widget, String pk_org) throws LfwRuntimeException {
		...
		ViewOrderVO[] viewOrderVOs = queryViewOrder(pk_org, ViewOrderVO.FUN_TYPE_MONTH, ViewOrderVO.REPORT_TYPE_PSN,
				true);
...
//				String[] mReportFields = new String[]{"ACTUALWORKDAYS", "ACTUALWORKHOURS", "WORKDAYS", "WORKHOURS"};
	 ...
	 */
	public static void resetData(Dataset ds, String pk_dept, FromWhereSQL fromWhereSQL, String year, String month,
...
//		fromWhereSQL = null;
		...
		try {
			monthStatVOs = service.queryByConditionAndDept(pk_dept, containsSubDepts, fromWhereSQL, year, month,
					showNoDataRecord);
		} catch (BusinessException e) {
			new HrssException(e).deal();
		}
		...
		for(Row r:row){
			if(r.getUFBoolean(ds.nameToIndex("isapprove")).booleanValue()){
				r.setString(ds.nameToIndex("approvestatus"), "已审核");
			}else{
				r.setString(ds.nameToIndex("approvestatus"), "未审核");
			}
		}
	}
