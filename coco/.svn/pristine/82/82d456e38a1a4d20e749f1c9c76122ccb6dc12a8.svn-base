...
public class AddSumIncomeTaxView extends UIDialog implements ActionListener,
	...
	private UILabel getGranttypeLable() {
		if (granttypeLable == null) {
			try {
				granttypeLable = new UILabel();
	...
	private UILabel getDeclarenumLable() {
		if (declarenumLable == null) {
			try {
				declarenumLable = new UILabel();
	...
	private UILabel getReasonLable() {
		if (reasonLable == null) {
			try {
				reasonLable = new UILabel();
	...
	private UILabel getBusinessnoLabel() {
		if (businessnoLabel == null) {
			try {
				businessnoLabel = new UILabel();
	...
	private UILabel getCostnoLabel() {
		if (costnoLabel == null) {
			try {
				costnoLabel = new UILabel();
	...
	private UILabel getProjectnoLabel() {
		if (projectnoLabel == null) {
			try {
				projectnoLabel = new UILabel();
	...
	private UILabel granttypeLable;// 凭单填发格式
	private UILabel declarenumLable = null;// 申报次数
	private UILabel reasonLable = null;// 重复申报原因
	private UILabel businessnoLabel = null;// 业别代号
	private UILabel costnoLabel = null;// 费用别代号
	private UILabel projectnoLabel = null;// 项目代号
