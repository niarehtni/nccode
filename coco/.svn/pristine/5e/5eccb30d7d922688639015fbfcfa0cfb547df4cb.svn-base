...
public class ExportTextDlg4TW extends HrDialog {
	 ...
	 * @param parent
	 */
	public ExportTextDlg4TW(Container parent, String title,
			LoginContext context, boolean enableWaclass, String initWaClass) {
		super(parent);
	...
	private String[] getOrgsByVAT(IReportExportService exportSrv)
			throws BusinessException {
		String[] pk_orgs = null;
		if (this.getChkSumReport().isSelected()) {
			if (!StringUtils.isEmpty((String) this.getCboUniqCode()
					.getSelectdItemValue())) {
				pk_orgs = exportSrv.getAllOrgByVATNumber((String) this
						.getCboUniqCode().getSelectdItemValue());
			}
		} else {
	...
	private void initWaCalss(String[] pk_orgs) {
		...
		try {
			...
			MessageDialog.showErrorDlg(this, ResHelper.getString(
					"twhr_datainterface", "DataInterface-00060"), e
					.getMessage()); // 錯誤
		}
	}

	private void initYearPeriods(String[] pk_orgs, IUAPQueryBS qry)
			throws BusinessException {
		String orgsInString = "";
		...
		String sql = "SELECT DISTINCT wadata.cyear + wadata.cperiod"
				+ " FROM   wa_data AS wadata"
				+ "  INNER JOIN wa_periodstate AS stat ON stat.pk_wa_class = wadata.pk_wa_class "
				+ " WHERE  ( stat.pk_org in (" + orgsInString
				+ ")) AND stat.payoffflag='Y' AND stat.cpaydate != ''"
				+ " ORDER BY wadata.cyear + wadata.cperiod";
		ArrayList<Object[]> rslt = (ArrayList<Object[]>) qry.executeQuery(sql,
	...
	private void initSumComps(IReportExportService exportSrv)
		...
		if (StringUtils.isEmpty(this.getContext().getPk_org())) {
			...
			this.getCboUniqCode().removeAllItems();
			this.getCboUniqCode().addItem(
					exportSrv.getOrgVATNumber(this.getContext().getPk_org()));
	...
	private void initCboVoucherType() {
		if (this.getCboVoucherType().getItemCount() == 0) {
			this.getCboVoucherType().addItem(
					ResHelper.getString("twhr_datainterface",
							"DataInterface-00065") + "[1]");// 免填發
			this.getCboVoucherType().addItem(
					ResHelper.getString("twhr_datainterface",
							"DataInterface-00066") + "[2]");// 電子憑單
			this.getCboVoucherType().addItem(
					ResHelper.getString("twhr_datainterface",
							"DataInterface-00067") + "[3]");// 紙本憑單
		}
	}
	...
	private void initCboYear() {
		if (this.getCboAccYear().getListeners(YearChangeHandler.class).length == 0) {
			this.getCboAccYear().addActionListener(
					new YearChangeHandler(this.getCboStartPeriod(), this
							.getCboEndPeriod()));
		}
	...
	public void closeOK() {
		start_period = StringUtils.leftPad(
				StringUtils.trimToEmpty(this.getCboStartPeriod()
						.getSelectedItem().toString()), 4, "0");
		end_period = StringUtils.leftPad(
				StringUtils.trimToEmpty(this.getCboEndPeriod()
						.getSelectedItem().toString()), 4, "0");
		wa_class = this.getRefWaClass().getRefPK();
		wa_classes = this.getRefWaClass().getRefPKs();
		vouchType = String
				.valueOf(this.getCboVoucherType().getSelectedIndex() + 1);
		vatNumber = (String) this.getCboUniqCode().getSelectdItemValue();
		cyear = (String) this.getCboAccYear().getSelectdItemValue();

		IReportExportService checkSrv = NCLocator.getInstance().lookup(
		...
		try {
			...
			MessageDialog.showErrorDlg(this, ResHelper.getString(
					"twhr_datainterface", "DataInterface-00060"), e1
					.getMessage());
			return;
		}
		...
		try {
			// 校驗期間資料
			int count = checkSrv.checkPeriodWaDataExists(wa_classes,
					this.getStart_period(), this.getEnd_period());

			if (count == 0) {
				MessageDialog
						.showErrorDlg(this, ResHelper.getString(
								"twhr_datainterface", "DataInterface-00060"),
								ResHelper.getString("twhr_datainterface",
										"DataInterface-00075"));
				return;
			}
		} catch (BusinessException e) {
			int btnRst = MessageDialog.showYesNoCancelDlg(
					this,
					ResHelper.getString("twhr_datainterface",
							"DataInterface-00073"),
					ResHelper.getString("twhr_datainterface",
							"DataInterface-00074") + e.getMessage());
	...
	protected UIPanel createCenterPanel() {
		...
		this.getLblWaClass()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00026")); // 薪資方案
		this.getLblStartPeriod()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00010"));// 起始期間
		this.getLblEndPeriod()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00011"));// 截止期間
		this.getLblAccYear()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00064"));
		this.getLblVoucherType()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00068"));// 憑單填發方式
		this.getLblSumReport().setText("");
		this.getChkSumReport()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00071"));// 法人合併申報
		this.getChkSumReport().setBorderPainted(false);
		this.getLblUniqCode()
				.setText(
						ResHelper.getString("twhr_datainterface",
								"DataInterface-00072"));// 統一編號
