...
public class HealthCaculateAction extends PayDataBaseAction {
	...
	public HealthCaculateAction() {
		...
		putValue("ShortDescription", ResHelper.getString("60130paydata", "60130paydata-001") + "(Ctrl+F)");
	...
	public void doActionForExtend(ActionEvent e) throws Exception {
		...
		// 发放日期
		DateLarborDelDlg dlg = new DateLarborDelDlg(getEntranceUI(), "发放日期", "发放日期");

		dlg.initUI();

		if (dlg.showModal() == 0) {
			putValue("message_after_action", ResHelper.getString("60130paydata", "60130paydata-005"));
			return;
		}
		...
		/*
		 * String payDateTWSetting =
		 * SysInitQuery.getParaString(this.getContext() .getPk_org(), "TWHR07");
		 */
		if (payDate == null) { // payDateTWSetting.equals("發放日期") remarked by
								// ssx on 2017/7/4, for 二代健保強制限制發放日期不能為空
			MessageDialog.showErrorDlg(SwingUtilities.getWindowAncestor(getWaContext().getEntranceUI()),
					ResHelper.getString("60130paydata", "60130paydata-007"),
					ResHelper.getString("60130paydata", "60130paydata-006")); // 薪資發放日期不允許為空。
			return;
		}
		...
		// 计算和二代健保相关的薪资项
		// final RecacuTypeChooseDialog chooseDialog = new
		// RecacuTypeChooseDialog(getParentContainer(), WA);
		...
		new SwingWorker<Boolean, Void>() {
			...
			protected Boolean doInBackground() throws Exception {
				try {
					...
					nhiSrv.updateExtendNHIInfo(getWaContext().getPk_group(), getWaContext().getPk_org(), getWaContext()
							.getPk_wa_class(), getWaContext().getWaLoginVO().getPk_periodscheme(), getWaContext()
					...
					for (SuperVO payfileVO : payfileVos) {
						if (null != payfileVO) {
							if (null != payfileVO.getAttributeValue(PayfileVO.CACULATEFLAG)
									&& ((UFBoolean) (payfileVO.getAttributeValue(PayfileVO.CACULATEFLAG)))
											.booleanValue()) {
			...
			protected void done() {
				if (error != null) {
					ShowStatusBarMsgUtil.showErrorMsg(ResHelper.getString("60130paydata", "060130paydata0335")/*
																											 ...
																											 */, error,
							getContext());
					// MessageDialog.showErrorDlg(getParentContainer(), null,
					// error);
	...
	protected boolean isActionEnable() {
		/*
		 * boolean enable = super.isActionEnable(); if (enable) {
		 * //检查计算回写的表中有无数据 try { enable = !getIsHealthCaculate();
		 * 
		 * } catch (BusinessException e) { Debug.debug(e.getStackTrace());
		 * 
		 * e.printStackTrace(); }
		 * 
		 * 
	...
	public boolean getIsHealthCaculate() throws BusinessException {
		// 计算人力资源下的所有法人组织
		Set<String> legalOrgs = LegalOrgUtilsEX.getOrgsByLegal(getContext().getPk_org(), getContext().getPk_group());
		int sum = 0;
		for (String pk_org : legalOrgs) {
			// 查询这个法人组织在子表中有无数据

			String sqlStr = "select count(*) as sumb from  declaration_business main " + " where main.vbdef1 = '"
					+ pk_org + "'" + " and main.vbdef2 = '" + getWaContext().getPk_wa_class() + "'"
			...
			sqlStr = "select count(*) as sumb from  declaration_company main " + " where main.vbdef1 = '" + pk_org
					+ "'" + " and main.vbdef2 = '" + getWaContext().getPk_wa_class() + "'" + " and main.vbdef3 = '"
					+ getWaContext().getWaLoginVO().getPeriodVO().getPk_wa_period() + "' and dr = 0";
			...
			sqlStr = "select count(*) as sumb from  declaration_nonparttime main " + " where main.vbdef1 = '" + pk_org
					+ "'" + " and main.vbdef2 = '" + getWaContext().getPk_wa_class() + "'" + " and main.vbdef3 = '"
					+ getWaContext().getWaLoginVO().getPeriodVO().getPk_wa_period() + "' and dr = 0";
			...
			sqlStr = "select count(*) as sumb from  declaration_parttime main " + " where main.vbdef1 = '" + pk_org
					+ "'" + " and main.vbdef2 = '" + getWaContext().getPk_wa_class() + "'" + " and main.vbdef3 = '"
					+ getWaContext().getWaLoginVO().getPeriodVO().getPk_wa_period() + "' and dr = 0";
