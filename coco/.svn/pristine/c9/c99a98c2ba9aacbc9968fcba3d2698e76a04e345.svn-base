...
public interface ILeaveQueryService extends IApproveBillQueryService<LeaveRegVO,LeaveCommonVO> {
	...
	/**
	 * 根据客户端的业务日期，查找加班转调休的转入年度（期间）
	 * 若加班转调休为按年结算，则返回长度为1的数组，元素即业务日期的所属考勤年度
	...
	/**
	 * 根据人员、休假类别、年度(期间)查询有效的休假单据,上面方法的批量查询
	 * 用于假期计算的休假信息浏览
	 * @param pk_org
	 * @param pk_psndoc
	 * @param pk_leaveType
	...
	/**
	 * 查询人员范围内,日期范围内的生效的单据，以登记表VO形式返回（），
	 * Map的key是pk_psndoc，value是单据数组
	 * @param pk_org
	 * @param psndocInSQL
	 * @param beginDate
	 * @param endDate
