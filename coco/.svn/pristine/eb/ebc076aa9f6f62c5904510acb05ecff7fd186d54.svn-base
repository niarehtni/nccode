...
public class MainViewController implements IController {
	...
	public void onDataLoad(DataLoadEvent dataLoadEvent) throws BusinessException {
		try {
			...
			if (hrFromWhereSQL == null) {
				...
				hrFromWhereSQL = new FromWhereSQL(fromWhereSQL.getFrom(), fromWhereSQL.getWhere(), fromWhereSQL,
						fromWhereSQL.getAttrpath_alias_map());

			}
			psnJobVOs = ServiceLocator.lookup(IPsndocQryService.class).queryPsndocVOsByDeptPK(pk_mng_dept,
					isIncludeSubDept, hrFromWhereSQL);
			// 去掉多余的工作记录
	...
	public void plugininMain(Map<String, Object> keys) throws BusinessException {
		try {
			PsnJobVO[] psnJobVOs = null;
			// 获得选择的管理部门
			String pk_mng_dept = SessionUtil.getPk_mng_dept();
			...
			FromWhereSQL hrFromWhereSQL = new FromWhereSQL(fromWhereSQL.getFrom(), fromWhereSQL.getWhere(),
					fromWhereSQL, fromWhereSQL.getAttrpath_alias_map());
			psnJobVOs = ServiceLocator.lookup(IPsndocQryService.class).queryPsndocVOsByDeptPK(pk_mng_dept,
					isIncludeSubDept, hrFromWhereSQL);
