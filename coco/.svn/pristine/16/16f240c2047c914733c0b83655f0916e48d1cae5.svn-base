...
public class TaiwanNHICalculator extends BaseDAOManager {
	...
	try {
	    ...
	    List<PsnLaborDataVO> laborLastMonthData = new ArrayList<PsnLaborDataVO>();
	...
	if (healthVOs != null && healthVOs.length > 0) {
		...
		if (newVO.length > 0) {
		    ...
		    newVO[0].setIncludelastmonth(UFBoolean.FALSE);
		    try{
	...
	if (healDataByPsn != null && healDataByPsn.size() > 0) {
		...
		for (Map data : healDataByPsn) {
			...
			if (!((String) data.get("psndocid")).equals(healthid)) {
			    ...
			    // 雇主
			    if ("HT02".equals(this.getRefList().get(
				    getKey(new DefdocVO(), psnType, (String) data.get("pk_org"))))) {
				healVO.setHealthAmount_Org(UFDouble.ZERO_DBL);
			    }
			    // 低收入戶
			    else if ("HT03".equals(this.getRefList().get(
				    getKey(new DefdocVO(), psnType, (String) data.get("pk_org"))))) {
			    ...
			    // 研替
			    else if ("HT05".equals(this.getRefList().get(
				    getKey(new DefdocVO(), psnType, (String) data.get("pk_org"))))) {
				healVO.setPersonCount(UFDouble.ZERO_DBL);
				healVO.setHealthAmount(UFDouble.ZERO_DBL);
				healVO.setHealthAmount_Org(UFDouble.ZERO_DBL);
			    ...
			    // 留停
			    else if ("HT07".equals(this.getRefList().get(
				    getKey(new DefdocVO(), psnType, (String) data.get("pk_org"))))) {
				healVO.setHealthAmount(UFDouble.ZERO_DBL);
				healVO.setHealthAmount_Org(UFDouble.ZERO_DBL);
				healVO.setHealthAmount_Psn(UFDouble.ZERO_DBL);
...
try{
	...
	if (nhiHelpAmountMap.size() > 0) {
			...
			for (Map<String, Object> healdata : healDataByPsn) {
				if (!insertedSubPK.contains((String) healdata.get("pk_psndoc_sub"))) {
					...
					if (help.getKey().equals(healid)) {
						...
						if(null != healdata.get("healthid") && 
								null != healdata.get("psndocid") && 
		...
		if (insList.size() > 0) {
			this.getBaseDao().insertVOList(insList);
	...
	if (laborData != null && laborData.size() > 0) {
		...
		if (pk_psndoc.equals(vo.getPk_psndoc())) {
		    if ("LT01".equals(this.getRefList().get(
			    getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) {
			if ("HT02".equals(this.getRefList().get(getKey(new DefdocVO(), psnType, vo.getPk_legalOrg())))) {
			    return true;
			}
		    }
		}
	...
	if (laborLastMonthData != null && laborLastMonthData.size() > 0) {
		...
		NhiCalcVO[] newVO = getNhiCalcVOByPsn(nhiVOExists, vo.getPk_psndoc(), false);
		if (newVO.length > 0) {
    ...
    private NhiCalcVO[] getNhiCalcVOByPsn(NhiCalcVO[] nhiCalcVOs, String pk_psndoc, boolean isHealth) {
	...
	if (nhiCalcVOs != null && nhiCalcVOs.length > 0) {
		...
		if (isHealth) {
		    ...
		    if (vo.getPk_psndoc().equals(pk_psndoc)
			    && ((vo.getLaborrange() != null && vo.getLaborrange().doubleValue() > 0) || (vo
				    .getRetirerange() != null && vo.getRetirerange().doubleValue() > 0))) {
			retList.add(vo);
		    }
		}
	...
	strSQL += "         def.glbdef14 retirebegin, def.glbdef15 retireend, nhi.pk_nhicalc, case when nhi.laborrange is null then 'N' else 'Y' end iflabor , ";
	...
	if (laborData != null && laborData.size() > 0) {
		...
		UFLiteralDate laborbegindate = labor.get("laborbegindate") == null ? null : new UFLiteralDate((String) labor.get("laborbegindate"));
		...
		UFLiteralDate retirebegindate = labor.get("retirebegindate") == null ? null : new UFLiteralDate((String) labor.get("retirebegindate"));
	...
	if ("LT01".equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 雇主
	    // 就业
	    vo.setEmpInsAmount_Org(UFDouble.ZERO_DBL);
	    vo.setEmpInsAmount_Psn(UFDouble.ZERO_DBL);
	} else if ("LT04"
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 职灾劳工
	    // 普通事故
	    vo.setCommonAccAmount_Org(UFDouble.ZERO_DBL);
	    vo.setCommonAccAmount_Psn(UFDouble.ZERO_DBL);
	    // 就业
		...
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 外籍劳工
		...
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 育婴留停
	    // 公司承担部分全为0
		...
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 建教生
	    // 个人及公司只缴纳普通事故（就业全为0）
	    vo.setLaborInsAmount_Psn(vo.getLaborInsAmount_Psn().sub(vo.getEmpInsAmount_Psn())); // 劳保个人负担合计（不含就业）
	    vo.setLaborInsAmount_Org(vo.getLaborInsAmount_Org().sub(vo.getEmpInsAmount_Org())); // 劳保公司负担合计（不含就业）
	    vo.setEmpInsAmount_Psn(UFDouble.ZERO_DBL); // 就业
		...
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 不參加就業保險
	    vo.setEmpInsAmount_Psn(UFDouble.ZERO_DBL); // 就业
	    vo.setEmpInsAmount_Org(UFDouble.ZERO_DBL); // 就业
	} else if ("LT13"
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 外籍配偶
	    // 按一般勞工，不做特殊處理
	} else if ("LT14"
		.equals(this.getRefList().get(getKey(new DefdocVO(), vo.getNhiPsnType(), vo.getPk_legalOrg())))) { // 陸籍配偶
	    // 按一般勞工，不做特殊處理
	}
    }
    ...
    private List<Map> getMapDataByPsn(List<Map> laborData, String pk_psndoc) throws BusinessException{
	...
	if (this.getPsnList() != null && this.getPsnList().size() > 0) {
		...
		data.setPk_legalOrg(getLegalOrgListWithPsnListOrder().get(i));
		...
		data.setAveNum((UFDouble) getBaseDocValueByKey(BaseDocVO.class.getName() + "_TWNP0004",
			data.getPk_legalOrg())); // 平均眷口數
		nhiData.add(data);
	    }
	}
