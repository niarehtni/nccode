...
public class DeptServiceImpl extends TreeBaseServiceAggVOAdapter<AggHRDeptVO> implements IDeptManageService, IDeptQueryService
{
    ...
    {
        return new Validator[]{new AggBDUniqueRuleValidate(), new AggBDTreeUpdateLoopValidator(), new DeptInsertValidator()};
    }
    
    @Override
    protected Validator[] getUpdateValidator(AggHRDeptVO oldVO)
    {
        return new Validator[]{new AggBDUniqueRuleValidate(), new AggBDTreeUpdateLoopValidator(), new DeptUpdateValidator()};
    }
    ...
    public AggHRDeptVO[] queryByCondition(LoginContext context, String condition) throws BusinessException
    {
        ...
        // 查询时按照displayorder排序
        AggHRDeptVO[] aggVOs =
            queryByConditionWithoutVisibleWithOrder(condition, HRDeptVO.DISPLAYORDER + "," + HRDeptVO.CODE,
                new String[]{"depthistory.effectdate"});
     ...
     *************************************************************************** */
    @Override
    public AggHRDeptVO[] queryByCondition(LoginContext context, String condition, String typePk) throws BusinessException
    {
        ...
        if (StringUtils.isEmpty(typePk))
        {
            ...
            String orgSql = NCLocator.getInstance().lookup(IPostQueryService.class).getInSQL(leftPKs.toArray(new String[0]));
    ...
    public HRDeptVersionVO[] queryFatherDeptVersionVOs(String[] pk_vids) throws BusinessException
    {
        ...
        String[] pk_fatherorgs = SuperVOHelper.getAttributeValueArray(versionVOs, HRDeptVersionVO.PK_FATHERORG, String.class);
    ...
    private void handleDeptHistory(AggHRDeptVO... deptVOs) throws BusinessException
    {
        ...
        for (AggHRDeptVO aggDeptVO : deptVOs)
        {
            ...
            for (DeptHistoryVO historyVO : historyVOs)
            {
                ...
                if (DeptChangeType.MERGE.equalsIgnoreCase(historyVO.getChangetype()))
                {// 如果是合并类型
                
                    if (UFBoolean.TRUE.equals(historyVO.getIsreceived()))
                    {// 如果是接收部门
                     // 设定接收部门和组织
    ...
    public AggHRDeptVO[] queryChildDeptVOs(String pk_dept, String orderBy) throws BusinessException
    {
        ...
        /*
         * String condition = " innercode like ? and innercode <> ?"; SQLParameter parameter = new
         * SQLParameter(); parameter.addParam(innerCode + "%"); parameter.addParam(innerCode);
         */
        
        return queryByCondition(null, condition, orderBy);
        /*
         * @SuppressWarnings("unchecked") Collection<HRDeptVO> c =
         * getDeptDao().getBaseDAO().retrieveByClause(HRDeptVO.class, condition, orderBy, parameter); if (null
         * == c || c.isEmpty()) { return new HRDeptVO[0]; } return c.toArray(new HRDeptVO[c.size()]);
         */
    }
    ...
    public HRDeptVO[] queryDeptVOsByOrgPK(String pk_org, boolean includeCanceled) throws BusinessException
    {
        ...
        String condition = HRDeptVO.PK_ORG + " = '" + pk_org + "' and " + HRDeptVO.ENABLESTATE + " = " + IPubEnumConst.ENABLESTATE_ENABLE;
        if (!includeCanceled)
        {
     ...
     * @throws BusinessException
     */
    public AggHRDeptVO[] queryDeptInfoByOrgAdminPKandCondition(String pk_orgadmin, boolean includeCanceled, String wherePart)
            throws BusinessException
    {
        AggHRDeptVO[] rets = getDeptDao().queryDeptInfoByOrgAdminPKandCondition(pk_orgadmin, includeCanceled, wherePart);
        return rets;
    }
    ...
    private OrgVO initOrgVO(HRDeptVO deptVO)
    {
        ...
        // 部门类型，其业务单元的报表职能默认选中
        orgVO.setAttributeValue(OrgTypeManager.getInstance().getOrgTypeByID(IOrgConst.REPORTORGTYPE).getFieldname(), UFBoolean.TRUE);
    ...
    protected void beforeInsert(AggHRDeptVO aggVO) throws BusinessException
    {
        ...
        vo.setVname(ResHelper.getString("6005dept", "06005dept0346")/* @res "初始版本" */);
...
//        vo.setOrgtype17(UFBoolean.TRUE);
    ...
    protected void afterInsert(AggHRDeptVO oldAggVO, AggHRDeptVO newAggVO) throws BusinessException
    {
        ...
        HRDeptVersionVO oldDeptVersionVO = getOMCommonQueryService().queryByPK(HRDeptVersionVO.class, deptVersionVO.getPk_dept());
        ...
        // 新增时部门主管都没有PK_ORG,因为部门主管的pk_org为该部门主键，所以需要在插入部门信息后重新更新 zhangqiano 2015-11-21
        ...
        if (!StringUtil.isEmptyWithTrim(newVO.getPrincipal()))
        {
            ...
            DeptManager[] deptManagers = getOMCommonQueryService().queryByCondition(DeptManager.class, condition.toString());
            DeptManager dupManagerVO = SuperVOHelper.getSuperVOByValue(deptManagers, DeptManager.PK_PSNDOC, newVO.getPrincipal());
        ...
        OrgVersionVO orgVersionVO = getOMCommonQueryService().queryByPK(OrgVersionVO.class, newOrgVersionVO.getPk_vid());
...
//        new DeptTypeServiceAdapterBasic().execEachDeptInsertOperate(deptVO, planBudgetVO);
    }
    ...
    protected void afterInsertImportVO(AggHRDeptVO oldAggVO, AggHRDeptVO newAggVO) throws BusinessException
    {
        ...
        // BDPKLockUtil.lockSuperVO(newOrgVersionVO);// 乐观锁
        // 因为第一次插入，不可能有部门版本历史
        // OrgVersionVO orgVersionVO = getOMCommonQueryService().queryByPK(OrgVersionVO.class,
    ...
    protected void beforeUpdate(AggHRDeptVO aggVO) throws BusinessException
    {
        ...
        if (StringUtil.isEmptyWithTrim(vo.getPrincipal()))
        {
            ...
            // 查询组织主管里的部门负责人信息
            DeptManager[] managers = getOMCommonQueryService().queryByCondition(DeptManager.class, condition.toString());
        ...
        if (createDate != null)
        {
            ...
            DeptHistoryVO[] historyVOs = getOMCommonQueryService().queryByCondition(DeptHistoryVO.class, condition.toString());
            if (!ArrayUtils.isEmpty(historyVOs))
            {
    ...
    public AggHRDeptVO update(AggHRDeptVO vo, boolean blChangeAuditInfo) throws BusinessException
    {
    	...
    	String DocsDept = DeptVo.getPk_dept();
    	String DocsOrg = DeptVo.getPk_org();
    	String DocsGroup = DeptVo.getPk_group();
    	String queryPsndocsSql = "SELECT * FROM bd_psndoc WHERE dr=0 AND enablestate=2 AND pk_group='" + DocsGroup + "' AND pk_org='" + DocsOrg +
    			"' AND pk_psndoc IN (SELECT pk_psndoc FROM hi_psnjob WHERE dr=0 AND pk_dept = '" + DocsDept + "' ) " +
    			"AND glbdef11 NOT IN (SELECT glbdef1 FROM org_dept WHERE pk_dept = '" + DocsDept +"')";
    	ArrayList<PsndocVO> psndocs =
                (ArrayList<PsndocVO>) NCLocator.getInstance().lookup(IPersistenceHome.class)
                    .executeQuery(queryPsndocsSql, new BeanListProcessor(PsndocVO.class));
        ...
        // 检索OldVO: oldVO只关心主表
        AggHRDeptVO oldVO = getMDQueryService().queryBillOfVOByPK(getEntityClass(), vo.getParentVO().getPrimaryKey(), false);
      ...
      //更新人員基礎檔案群組修改后記錄
        updateDataAfterHRAlter(psndocs);
    ...
    protected void afterUpdate(AggHRDeptVO oldAggVO, AggHRDeptVO newAggVO) throws BusinessException
    {
        ...
        // 部門群組有更新時，同步更新人員基本資料所屬部門對應的群組
        String newGroupStr = (String)newVO.getAttributeValue("glbdef1");
        String oldGroupStr = (String)oldVO.getAttributeValue("glbdef1");
        if(newGroupStr!=null&&!newGroupStr.equals(oldGroupStr)){
        	getDeptDao().updateJobGroup(newGroupStr,newVO.getPk_dept());
        }
    }
    ...
    protected void afterDelete(AggHRDeptVO aggVO) throws BusinessException
    {
        ...
        CacheProxy.fireDataDeleted(new DefaultGetBizInfoByMDUtil().getTableName(deptVersionVO), deptVersionVO.getPk_vid());// 通知缓存
    ...
    private boolean isAutoGenerateBillCode(String billType, String pk_group, String pk_org) throws BusinessException
    {
        BillCodeContext billCodeContext =
            NCLocator.getInstance().lookup(IBillcodeManage.class).getBillCodeContext(billType, pk_group, pk_org);
    ...
    public void createDeptStruVersion(String pk_org, String vname, UFDate vstartdate) throws BusinessException
    {
        ...
        HRDeptStruVersionVO[] deptStruVersionVOs = getOMCommonQueryService().queryByCondition(HRDeptStruVersionVO.class, condition);
    ...
    public AggHRDeptVO createDeptVersion(AggHRDeptVO aggVO) throws BusinessException
    {
        ...
        String condition = HRDeptVersionVO.PK_DEPT + " = '" + deptVO.getPrimaryKey() + "' and pk_vid = '" + deptVO.getPk_vid() + "'";
        ...
        for (int i = 0; i < orgtypevos.length; i++)
        {
            ...
            String conditionVersion =
                eachorgTypeVO.getPKFieldName() + " = '" + eachorgTypeVO.getPrimaryKey() + "' and " + IOrgVersionConst.PK_VID + " = '"
            ...
            try
            {
                ...
                {
                    // 查询出需要生成新版本的deptvo的旧版本数据，更新旧版本的venddate字段为新版本生效日期-1
                    SuperVO oldEachOrgTypeVersionVO = eachTypevc.toArray((SuperVO[]) Array.newInstance(czz, eachTypevc.size()))[0];
                    ...
                    eachorgTypeVO = new DeptTypeServiceAdapterBasic().execEachDeptUpdateOperate(newDeptVO, eachorgTypeVO);
    ...
    @Override
    public PostVO[] queryPostAdjustVOMapForDeptCopy(LoginContext context, String[] deptPKs, String pk_org) throws BusinessException
    {
        // 获取待复制岗位
        PostVO[] toBeCopiedPostVOs = null;
        AggPostVO[] aggPostVOs = getPostQueryService().queryAggPostVOsByDeptPKArray(deptPKs, " order by pk_dept, postcode");
        ...
        CrossOrgCopyFilter.filterByExcludeAttributs(IMetaDataIDConst.POST, new String[]{"pk_dept"}, toBeCopiedPostVOs, context, false,
            false);
            ...
            PostAdjustVO[] adjustedPostVO, PostAdjustVO[] savedPostVO) throws BusinessException
    {
        ...
        // 获得有关岗位的VO
        String[] allPostPKs = SuperVOHelper.getNotNullDistinctAttributeValueArray(psnJobVOs, PsnJobVO.PK_POST, String.class);
        
        PostVO[] allPostVOs = null;
        
        if (ArrayUtils.isEmpty(allPostPKs))
        {// 如果人都没有设置岗位
        ...
        // 获得有关任职类型的VO
        String[] allJobModePKs = SuperVOHelper.getNotNullDistinctAttributeValueArray(psndocVOs, PsnJobVO.JOBMODE, String.class);
        ...
        for (int i = 0; i < personAdjustVOs.length; i++)
        {
            ...
            if (postCache.containsKey(pk_oldpost))
            {// 如果岗位被迁移过去了
            ...
            SuperVOHelper.copyMultiLangAttribute(deptVO, personAdjustVO, new String[]{HRDeptVO.NAME}, new String[]{"newdeptname"});
        ...
        {
            if (personAdjustVO.getPk_newpost() == null)
            {// 岗位自动匹配
     ...
     * @return
     */
    private PersonAdjustVO[] initPersonAdjustVO(PsndocVO[] psndocVOs, HRDeptVO[] mergedDeptVOs, PostVO[] allPostVOs, DefdocVO[] allJobModes)
    {
        ...
        for (int i = 0; i < psndocVOs.length; i++)
        {
            ...
            SuperVOHelper.copyMultiLangAttribute(psndocVOs[i], personAdjustVOs[i], new String[]{PsndocVO.NAME}, new String[]{"psnname"});
            HRDeptVO deptVO = deptVOMap.get(psndocVOs[i].getPsnJobVO().getPk_dept());
            if (deptVO != null)
            {
                personAdjustVOs[i].setPk_olddept(deptVO.getPk_dept());
                SuperVOHelper.copyMultiLangAttribute(deptVO, personAdjustVOs[i], new String[]{HRDeptVO.NAME}, new String[]{"olddeptname"});
            }
            ...
            if (postVO != null)
            {
                ...
                SuperVOHelper
                    .copyMultiLangAttribute(postVO, personAdjustVOs[i], new String[]{PostVO.POSTNAME}, new String[]{"oldpostname"});
            }
            DefdocVO jobMode = defdocVOMap.get(psndocVOs[i].getPsnJobVO().getJobmode());
            if (jobMode != null)
            {
                SuperVOHelper.copyMultiLangAttribute(jobMode, personAdjustVOs[i], new String[]{DefdocVO.NAME}, new String[]{"jobmode"});
            }
    ...
    private HRDeptVO[] buildTemplateDepts(HRDeptVO[] sourceDeptVOs) throws BusinessException
    {
        ...
        {
            @Override
            public int compare(HRDeptVO o1, HRDeptVO o2)
            {// 让部门按照上下级的关系插入到数据库
             // 注释中的上下级不代表直接上下级，是在树中的层级
                if (o1.getInnercode() == null)
                {// 当o1的上级部门为空时，o2为空那么是平级，o2不为空为下级
                    return o2.getInnercode() == null ? 0 : -1;
                }
                if (o2.getInnercode() == null)
                {// 当o1的上级部门不为空时，o2为空，那么o1为o2的下级
                    return 1;
                }
        ...
        //
        String[] clearAttributes =
            new String[]{
                HRDeptVO.PRINCIPAL,
                HRDeptVO.ADDRESS,
                HRDeptVO.TEL, 
                HRDeptVO.CREATOR,
                HRDeptVO.CREATIONTIME,
                HRDeptVO.MODIFIER,
                HRDeptVO.MODIFIEDTIME};
     ...
     * @throws BusinessException
     */
    private void saveToBeCopiedDeptVOs(DeptCopyWrapperVO wrapperVO, DeptCopyConfirmVO confirmVO, HRDeptVO[] sourceDeptVOs,
            Map<String, String> deptPKBackupMap) throws BusinessException
    {
        ...
        for (int i = 0; i < sourceDeptVOs.length; i++)
        {
            ...
            //
            DeptCopyAdjustVO adjustVO = SuperVOHelper.getSuperVOByValue(adjustVOs, HRDeptVO.PK_DEPT, sourceDeptVOs[i].getPk_dept());
            if (adjustVO == null)
            {
                continue;
            }
            ...
            //
            if (deptPKBackupMap.containsKey(confirmVO.getPk_org() + deptVO.getPk_fatherorg())) deptVO.setPk_fatherorg(deptPKBackupMap
                .get(confirmVO.getPk_org() + deptVO.getPk_fatherorg()));
            else
    ...
    private AggPostVO[] buildTemplatePostVOs(HRDeptVO[] sourceDeptVOs) throws BusinessException
    {
        ...
        // 得到原始的PostVO、过滤撤销岗位
        AggPostVO[] sourcePostVOs = getPostQueryService().queryAggPostVOsByDeptPKArray(deptPKs, "  and " + PostVO.HRCANCELED + " = 'N'");
        ...
        String[] clearAttributes =
            new String[]{PostVO.CREATOR, PostVO.CREATIONTIME, PostVO.MODIFIER, PostVO.MODIFIEDTIME, PostVO.HRCANCELDATE};
    ...
    private void clearChildPK(AggPostVO[] sourcePostVOs) throws BusinessException
    {
        for (AggPostVO sourcePostVO : sourcePostVOs)
        {
            ...
            if (!ArrayUtils.isEmpty(allChildren)) for (CircularlyAccessibleValueObject child : allChildren)
            {
     ...
     * @throws BusinessException
     */
    private void saveToBeCopiedAggPostVOs(DeptCopyWrapperVO wrapperVO, DeptCopyConfirmVO confirmVO, AggPostVO[] sourcePostVOs,
            Map<String, String> deptPKBackupMap) throws BusinessException
    {
        // 首先对岗位进行排序，这样才能维护内部码
        ...
        Collections.sort(postVOList, new Comparator<AggPostVO>()
        {
            ...
            @Override
            public int compare(AggPostVO aggVO1, AggPostVO aggVO2)
            {// 让岗位按照上下级的关系重新排列
             // 注释中的上下级不代表直接上下级，是在树中的层级
                PostVO postVO1 = (PostVO) aggVO1.getParentVO();
                PostVO postVO2 = (PostVO) aggVO2.getParentVO();
                if (postVO1.getInnercode() == null)
                {// 当o1的上级岗位为空时，o2为空那么是平级，o2不为空为下级
                    return postVO2.getInnercode() == null ? 0 : -1;
                }
                if (postVO2.getInnercode() == null)
                {// 当o1的上级岗位不为空时，o2为空，那么o1为o2的下级
                    return 1;
                }
        ...
        boolean isAutoGenerateBillCode = isAutoGenerateBillCode("post", wrapperVO.getContext().getPk_group(), confirmVO.getPk_org());
        ...
        {
            // AggPostVO sourcePostVO = AggVOHelper.clone(sourcePostVOs[i], AggPostVO.class);
            ...
            if (isAutoGenerateBillCode)
            {
                String billcode =
                    NCLocator
                        ...
                        .getBillCode_RequiresNew(IOrgBillCodeConst.BILL_CODE_POST, wrapperVO.getContext().getPk_group(),
                            confirmVO.getPk_org(), toBeCopiedPostVO);
            ...
            /*
             * // 把在目标组织不可见的岗位序列设置成null if (StringUtils.isNotBlank(toBeCopiedPostVO.getPk_postseries())) {
             * LoginContext context = wrapperVO.getContext(); String pk_bu = context.getPk_org(); OrgVO org =
             * NCLocator.getInstance().lookup(IAOSQueryService.class).queryHROrgByOrgPK(pk_bu); if (org ==
             * null) throw new IllegalStateException("查询不到bu：pk = " + pk_bu + " 所对应的HR组织！");
             * context.setPk_org(org.getPk_org());
             * CrossOrgCopyFilter.filterByIncludeAttributs("039385a6-3e60-489c-b257-f48a1b7ab046", new
             * String[]{ PostVO.PK_POSTSERIES, PostVO.PK_JOB}, new Object[]{toBeCopiedPostVO}, context, false,
             * false); context.setPk_org(pk_bu); }
             */
            // 上级岗位不为空，且其上级岗位不在复制范围内
            if (StringUtils.isNotBlank(toBeCopiedPostVO.getSuporior()))
            {
                if (postPKBackupMap.containsKey(confirmVO.getPk_org() + toBeCopiedPostVO.getSuporior())) toBeCopiedPostVO
                    .setSuporior(postPKBackupMap.get(confirmVO.getPk_org() + toBeCopiedPostVO.getSuporior()));
                else
                {
                    PostVO supPostVO = getOMCommonQueryService().queryByPK(PostVO.class, toBeCopiedPostVO.getSuporior());
                    if (!supPostVO.getPk_org().equals(confirmVO.getPk_org())) toBeCopiedPostVO.setSuporior(null);
                }
            }
            // 执行插入
    ...
    public static String getFlowCode(String prefix) throws BusinessException
    {
        ...
        String whereSql =
            PostVO.POSTCODE + " like '" + prefix + "%' and len(" + PostVO.POSTCODE + ") = 18 order by " + PostVO.POSTCODE + " desc";
        SuperVO[] vos = NCLocator.getInstance().lookup(IPersistenceRetrieve.class).retrieveByClause(null, PostVO.class, whereSql);
    ...
    @Override
    public AggHRDeptVO rename(AggHRDeptVO aggdeptVO, DeptHistoryVO historyVO, boolean updateCareer) throws BusinessException
    {
        ...
        // 重置所属组织版本ID
        String conOrg =
            OrgVersionVO.PK_ORG + " = '" + ((HRDeptVO) aggdeptVO.getParentVO()).getPk_org() + "' order by " + OrgVersionVO.VNO + " desc";
        OrgVersionVO[] orgVersionVOs = getOMCommonQueryService().queryByCondition(OrgVersionVO.class, conOrg);
        if (!ArrayUtils.isEmpty(orgVersionVOs))
        {
            historyVO.setPk_org_v(orgVersionVOs[0].getPk_vid());
    ...
    public AggHRDeptVO[] shift(AggHRDeptVO aggDeptVO, DeptHistoryVO historyVO) throws BusinessException
    {
        ...
        // 维护变更操作后所属组织版本ID更行。
        String conOrg = OrgVersionVO.PK_ORG + " = '" + historyVO.getPk_org() + "' order by " + OrgVersionVO.VNO + " desc";
        OrgVersionVO[] orgVersionVOs = getOMCommonQueryService().queryByCondition(OrgVersionVO.class, conOrg);
        if (!ArrayUtils.isEmpty(orgVersionVOs))
        {
            historyVO.setPk_org_v(orgVersionVOs[0].getPk_vid());
            ...
            PostAdjustVO[] postAdjustVOs) throws BusinessException
    {
        ...
        if (ArrayUtils.isEmpty(postAdjustVOs))
        {// 如果列表为空
            return new PostAdjustVO[0];
        }
        
        Map<String, PostAdjustVO> postAdjustVOCache = SuperVOHelper.buildAttributeToVOMap(PostVO.PK_POST, postAdjustVOs);
        ...
        for (PostVO postVO : postVOs)
        {
            ...
            if (UFBoolean.TRUE.equals(postAdjustVO.getMergedpost()))
            {// 如果是待复制部门的岗位
        ...
        DefaultMutableTreeNode[] roots =
            SuperVOHelper.buildMultiTreeWithSuperVOs(mergedPosts.toArray(new PostVO[0]), PostVO.PK_POST, PostVO.SUPORIOR);
        ...
        for (PostVO postVO : mergedPosts)
        {
            ...
            savedPostVOs[index] = SuperVOHelper.createSuperVOFromSuperVO((PostVO) resultVO.getParentVO(), PostAdjustVO.class);
    ...
    private int updateIDAndPID(DefaultMutableTreeNode root, String[] ids, int offset, Map<String, Integer> postAdjustIndexCache)
            throws BusinessException
    {
    ...
    @Override
    public void rollbackPostForDeptMerge(PostAdjustVO[] savedPostVOs, PostAdjustVO[] backupPostVOs) throws BusinessException
    {
        if (!ArrayUtils.isEmpty(savedPostVOs))
        {// 已经保存的数据
            List<String> toBeDelPosts = new ArrayList<String>();
            for (int i = 0; i < savedPostVOs.length; i++)
            {
                if (UFBoolean.TRUE.equals(savedPostVOs[i].getMergedpost()))
                {// 如果是待合并的
        ...
        if (!ArrayUtils.isEmpty(backupPostVOs))
        {// 原有的岗位数据
    ...
    public AggHRDeptVO[] merge(DeptMergeWrapperVO wrapperVO) throws BusinessException
    {
        ...
        // step 0. 处理人员相关信息
        // 更新人员的岗位，职务，部门，组织信息
        String[] mergedDeptPKs = SuperVOHelper.getAttributeValueArray(wrapperVO.getMergedDeptHistoryVOs(), "pk_dept", String.class);
        ...
        if (!ArrayUtils.isEmpty(personAdjustVOs))
        {
            ...
            {
                throw new BusinessException(ResHelper.getString("6005dept", "06005dept0378"/* @res "虚拟部门[{0}]下不能有人员!" */, ((HRDeptVO)takeOverDeptVO.getParentVO()).getMultilangName()));
            }
            // 循环中查任职记录的优化
            // 任职记录主键
            ...
            for (PersonAdjustVO personAdjustVO : personAdjustVOs)
            {
                if (psnjobPks.contains(personAdjustVO.getPk_psnjob())) continue;
                psnjobPks.add(personAdjustVO.getPk_psnjob());
            }
            PsnJobVO[] tmpPsnjobVOs = getOMCommonQueryService().queryByPKs(PsnJobVO.class, psnjobPks.toArray(new String[0]));
            // 任职记录主键对应VO的map
            ...
            for (PersonAdjustVO personAdjustVO : personAdjustVOs)
            {
                if (null == personAdjustVO.getPk_newpost() || postPks.contains(personAdjustVO.getPk_newpost())) continue;
            ...
            if (tmpPostVOs != null)
            {
                for (PostVO tmpPostVO : tmpPostVOs)
                {
                    if (StringUtils.isEmpty(tmpPostVO.getPk_job()) || jobPks.contains(tmpPostVO.getPk_job())) continue;
            ...
            {
                // **********
                // PsnJobVO psnjobVO = getOMCommonQueryService().queryByPK(PsnJobVO.class,
                ...
                {
                    // *************
                    // PostVO postVO = getOMCommonQueryService().queryByPK(PostVO.class, pk_post);
                    // ************
                ...
                // ************
                // JobVO jobVO = getOMCommonQueryService().queryByPK(JobVO.class, pk_job);
                // ************
        ...
        if (!deptPKList.isEmpty())
        {
            ...
            {
                if (ArrayUtils.isEmpty(childDeptAggVOs))
                {  // 所有被合并部门都没有子部门，跳出循环
                    break;
                }
            ...
            for (int i = 0; i < mergedDeptVOs.length; i++)
            {
                ...
                if (ArrayUtils.isEmpty(childDeptVOs)) continue;
        ...
        for (int i = 0; i < mergedDeptVOs.length; i++)
        {
            ...
            /*
             * HRDeptVO[] childDeptVOs = getOMCommonQueryService().queryByCondition(HRDeptVO.class,
             * HRDeptVO.PK_FATHERORG + " = '" + mergedDeptVOs[i].getPk_dept() + "'");
             */
            HRDeptVO[] childDeptVOs = (HRDeptVO[]) AggVOHelper.getParentVOArrayFromAggVOs(childDeptAggVOs, HRDeptVO.class);
            
            // 业务日志
            JFQueryOrgLogUtils.writeQueryDeptCopyLog("89788d4d-679b-42ac-b16f-e72381068f93", mergedDeptVOs[i], "MergerOpeDept");
        ...
        // String conditon =
        // PostVO.PK_DEPT + " in (" + deptPks + ") and (" + PostVO.HRCANCELED + " = 'N' or " +
        // PostVO.HRCANCELED + " = '~' )";
        // // 增加排序，先撤销下级岗位
        // conditon += " order by " + PostVO.INNERCODE + " desc";
        // @SuppressWarnings("unchecked")
        // List<AggPostVO> postList = (List<AggPostVO>) getMDQueryService().queryBillOfVOByCond(PostVO.class,
        // conditon, false);
        ...
        // .cancel((PostVO) aggPostVO.getParentVO(), wrapperVO.getTakeOverDeptHistoryVO().getEffectdate(),
        // false);
        // }
        // }
    ...
    private void cancelPostsOfDept(String[] mergedDeptPKs, UFLiteralDate effectiveDate) throws BusinessException
    {
        ...
        String deptPks = isc.getInSQL(mergedDeptPKs);
        String conditon =
            PostVO.PK_DEPT + " in (" + deptPks + ") and (" + PostVO.HRCANCELED + " = 'N' or " + PostVO.HRCANCELED + " = '~' )";
        // 增加排序，先撤销下级岗位
        conditon += " order by " + PostVO.INNERCODE + " desc";
        @SuppressWarnings("unchecked")
        List<AggPostVO> postList = (List<AggPostVO>) getMDQueryService().queryBillOfVOByCond(PostVO.class, conditon, false);
        if (postList != null && postList.size() > 0)
        {
            for (AggPostVO aggPostVO : postList)
            {
    ...
    @Override
    public AggHRDeptVO[] cancel(AggHRDeptVO aggvo, DeptHistoryVO historyVO, boolean disableDept) throws BusinessException
    {
        HRDeptVO vo = (HRDeptVO) aggvo.getParentVO();
        ...
        if (!ArrayUtils.isEmpty(childDeptVOs))
        {
            ...
            {
                if (UFBoolean.TRUE.equals(childDeptVO.getHrcanceled()))
                {// 如果部门已经撤销，不需要处理
                    continue;
                }
                ...
                // 记录所属组织的最新版本ID。
                String conOrgCancel = OrgVersionVO.PK_ORG + " = '" + childDeptVO.getPk_org() + "' order by " + OrgVersionVO.VNO + " desc";
                OrgVersionVO[] orgVersionCanVOs = getOMCommonQueryService().queryByCondition(OrgVersionVO.class, conOrgCancel);
                if (!ArrayUtils.isEmpty(orgVersionVOs))
                {
                ...
                String[] attrs =
                    new String[]{
                        DeptHistoryVO.CHANGENUM,
                        DeptHistoryVO.APPROVENUM,
                        DeptHistoryVO.APPROVEDEPT,
                        DeptHistoryVO.EFFECTDATE,
                        DeptHistoryVO.MEMO};
                SuperVOHelper.batchSyncCopyAttributes(new SuperVO[]{historyVO}, new SuperVO[]{childHistoryVO}, attrs);
        ...
        for (HRDeptVO deptVO : canceledDeptVOs)
        {
             ...
             * updateDeptVO.getParentVO()).getEnablestate() == IPubEnumConst.ENABLESTATE_ENABLE) {
             * TreeBaseService<DeptVO> baseService = new TreeBaseService<DeptVO>(IOrgMetaDataIDConst.DEPT,
             * ITreePersistenceUtil.METADATA_PERSISTENCE_STYLE); baseService.disableVO((DeptVO)
             * updateDeptVO.getParentVO()); } }
             */
    ...
    @Override
    public AggHRDeptVO[] uncancel(AggHRDeptVO aggVO, DeptHistoryVO historyVO, boolean includeChildDept, boolean includePost,
            boolean enableDept) throws BusinessException
    {
        HRDeptVO vo = (HRDeptVO) aggVO.getParentVO();
        // 判断上级部门是否处于撤销状态
        ...
        if (includeChildDept)
        {
            ...
            if (!ArrayUtils.isEmpty(childAggDeptVOs))
            {
                ...
                for (HRDeptVO childDeptVO : childDeptVOs)
                {
                    ...
                    // 更新部门所属组织的版本ID。
                    String conOrgUncan =
                        OrgVersionVO.PK_ORG + " = '" + childHistoryVO.getPk_org() + "' order by " + OrgVersionVO.VNO + " desc";
                    OrgVersionVO[] orgVersionUncanVOs = getOMCommonQueryService().queryByCondition(OrgVersionVO.class, conOrgUncan);
                    if (!ArrayUtils.isEmpty(orgVersionVOs))
                    {
     ...
     */
    @Override
    public DeptTransDeptVO[] queryTransDeptInf(DeptTransRuleVO ruleVO, DeptTransRefNameVO[] refNameVOs, LoginContext context)
            throws BusinessException
    {
        //
        if (ruleVO == null || StringUtils.isEmpty(ruleVO.getOldorg()) || StringUtils.isEmpty(ruleVO.getNeworg())) return null;
        //
     ...
     * @throws BusinessException
     */
    private DeptTransDeptVO[] queryTransDeptInf(DeptTransRuleVO ruleVO, DeptTransRefNameVO[] refNameVOs) throws BusinessException
    {
        // 转移部门主键临时表
        InSQLCreator isc = new InSQLCreator();
        ...
        if (!StringUtils.isEmpty(ruleVO.getAimdept())) supDeptVO = getServiceTemplate().queryByPk(HRDeptVO.class, ruleVO.getAimdept());
        ...
        for (HRDeptVO deptVO : deptVOs)
        {
            ...
            if (prefixcode != null) transDeptVO.setAimdeptcode(prefixcode + deptcode);
            else
    ...
    private String getCodePrefix(String prefixcode, HROrgVO hrOrgVO, HRDeptVO supDeptVO)
    {
        ...
        else if (ResHelper.getString("6005dept", "06005dept0180")/* @res "目标业务单元编码" */.equals(prefixcode))
        {
            // 目标业务单元编码
            return hrOrgVO.getCode();
        }
        else if (ResHelper.getString("6005dept", "06005dept0181")/* @res "目标业务单元中上级部门编码" */.equals(prefixcode) && supDeptVO != null)
        {
            // 目标业务单元中上级部门编码
     ...
     * @return
     */
    private DeptTransDeptVO getNamePrefix(String prefixname, HROrgVO hrOrgVO, HRDeptVO supDeptVO, DeptTransDeptVO transDeptVO,
            HRDeptVO deptVO)
    {
        //
        ...
        else if (ResHelper.getString("6005dept", "06005dept0183")/* @res "目标业务单元名称" */.equals(prefixname))
        {
            // 目标业务单元名称
        ...
        else if (ResHelper.getString("6005dept", "06005dept0184")/* @res "目标业务单元简称" */.equals(prefixname))
        {
            // 目标业务单元简称
        ...
        else if (ResHelper.getString("6005dept", "06005dept0185")/* @res "目标业务单元中上级部门名称" */.equals(prefixname))
        {
            // 目标业务单元中上级部门名称
            if (supDeptVO == null)
            {
     ...
     * @throws SQLException
     */
    public DeptTransPostVO[] queryTransPostInf(DeptTransDeptVO transDeptVO, DeptTransRuleVO ruleVO, DeptTransRefNameVO[] refNameVOs,
            LoginContext context) throws BusinessException
    {
        //
        PostVO[] postVOs;
        AggPostVO[] aggPostVOs =
            getServiceTemplate().queryByCondition(AggPostVO.class,
    ...
    public DeptTransRefNameVO[] queryDeptTransRefInf(DeptTransRuleVO ruleVO) throws BusinessException
    {
        ...
        try
        {
            ...
            if (refNameVO != null) listRefNameVOs.add(refNameVO);
        ...
        if (listRefNameVOs.size() > 0) refNameVOs = listRefNameVOs.toArray(new DeptTransRefNameVO[0]);
        return refNameVOs;
    }
    ...
    {
        if (StringUtils.isEmpty(defdocPk) || StringUtils.isEmpty(defdoclist) || refNameVOs == null) return defdocPk;
     ...
     */
    @Override
    public DeptTransValidateVO validateDeptForTrans(DeptTransDeptVO[] transDeptVos, String pk_org) throws BusinessException
    {
        DeptTransValidateVO validateVO = new DeptTransValidateVO();
        
        HRDeptVO[] deptVOs;
        AggHRDeptVO[] aggDeptVOs = queryByCondition(null, "pk_org = '" + pk_org + "' and enablestate = 2 and hrcanceled = 'N'");
        if (aggDeptVOs == null || aggDeptVOs.length < 1)
        {
            return validateVO;
        }
     ...
     */
    @Override
    public DeptTransValidateVO validatePostForTrans(DeptTransDeptVO[] transDeptVos, String pk_org) throws BusinessException
    {
        DeptTransValidateVO validateVO = new DeptTransValidateVO();
        ...
        AggPostVO[] aggPostVOs =
            getServiceTemplate().queryByCondition(
                AggPostVO.class,
                PostVO.PK_ORG + " = '" + pk_org + "' and " + PostVO.HRCANCELED + " = 'N'" + " order by len(" + PostVO.INNERCODE + "),"
                    + PostVO.POSTCODE);
     ...
     */
    @Override
    public DeptTransItemVO[] queryItemSetByOrg(String pk_group, String pk_org, String pk_trnstype) throws BusinessException
    {
        // 查询调配项目
        TrnTransItemVO[] trnTransItemVOs =
            TrnDelegator.getIItemSetQueryService().queryItemSetByOrg(TRNConst.TRNSITEM_BEANID, pk_group, pk_org, pk_trnstype);
        if (ArrayUtils.isEmpty(trnTransItemVOs)) return null;
        List<DeptTransItemVO> transItemVOs = new ArrayList<DeptTransItemVO>();
        DeptTransItemVO deptTransItemVO;
        
        InfoSetVO infoSetVO = NCLocator.getInstance().lookup(IInfoSetQry.class).queryInfoSetByPk(trnTransItemVOs[0].getPk_infoset());
        ...
        for (TrnTransItemVO trnTransItemVO : trnTransItemVOs)
        {
            ...
            if ("oldpk_org".equals(itemkey) || "newpk_org".equals(itemkey) || "oldpk_psncl".equals(itemkey)
                ...
                || "oldpk_job".equals(itemkey) || "newpk_job".equals(itemkey) || "oldseries".equals(itemkey) || "newseries".equals(itemkey)
                || "oldpk_post".equals(itemkey) || "newpk_post".equals(itemkey) || "oldpk_postseries".equals(itemkey)
                || "newpk_postseries".equals(itemkey) || "oldpk_jobrank".equals(itemkey) || "newpk_jobrank".equals(itemkey)
                || "oldpk_jobgrade".equals(itemkey) || "newpk_jobgrade".equals(itemkey)) continue;
    ...
    private void dealTransItemInf(DeptTransItemVO deptTransItemVO, InfoItemVO[] itemVOs) throws BusinessException
    {
        ...
        {
            deptTransItemVO.setItemname(StringUtil.replaceAllString(ResHelper.getString("6005dept", "06005dept0350")/*
                                                                                                                     * @
                                                                                                                     * res
                                                                                                                     * "原{0}"
                                                                                                                     */, "{0}",
                deptTransItemVO.getItemname()));
        }
        else
        {
            deptTransItemVO.setItemname(StringUtil.replaceAllString(ResHelper.getString("6005dept", "06005dept0351")/*
                                                                                                                     * @
                                                                                                                     * res
                                                                                                                     * "目标{0}"
                                                                                                                     */, "{0}",
        ...
        RefInfoVO refInfoVO =
            (StringUtils.isBlank(ref_model_name) ? null : new SimpleDocServiceTemplate("refinfo")
     ...
     */
    @Override
    public GeneralVO[] queryTransPsnInf(DeptTransRuleVO ruleVO, DeptTransRefNameVO[] refNameVOs, DeptTransDeptVO[] transDeptVOs,
            DeptTransItemVO[] transItemVOs) throws BusinessException
    {
        // 转移部门主键临时表
        InSQLCreator isc = new InSQLCreator();
        String temptable = isc.getInSQL(ruleVO.getPk_depts());
        String cond =
            " ismainjob = 'Y' and ( " + SQLHelper.getNullSql("endflag") + " or endflag ='N' ) and psntype = 0 and " + PsnJobVO.PK_DEPT
                + " in (" + temptable + " )";
        ...
        for (PsnJobVO psnJobVO : psnJobVOs)
        {
            ...
            if (!ArrayUtils.isEmpty(transItemVOs)) for (DeptTransItemVO transItemVO : transItemVOs)
            {
                // 根据调配项目设置
    ...
    public void reSetPsnInVO(GeneralVO transPsnJobVO, PsnJobVO psnJobVO, DeptTransDeptVO[] transDeptVOs)
    {
        ...
        for (DeptTransDeptVO transDeptVO : transDeptVOs)
        {
            if (transDeptVO.getOlddeptpk().equals(psnJobVO.getPk_dept()))
            {
                ...
                if (ArrayUtils.isEmpty(transPostVOs)) continue;
        ...
        try
        {
            ...
            for (int i = 0; i < sourceDeptVOs.length; i++)
            {
                ...
                for (int j = i + 1; j < sourceDeptVOs.length; j++)
                {
                    ...
                    if (pk_fatherorg != null && pk_fatherorg.equals(pk_dept))
                    {// 修改所有子部门
            ...
            if (!ArrayUtils.isEmpty(sourcePostVOs))
            {
                ...
                savePostVOs(ruleVO.getNeworg(), ruleVO.getContext().getPk_group(), sourcePostVOs, deptPKBackupMap, old_newPostpkmap,
                    insertPostVOs);
            }
            // 新旧人员工作记录主键
            ...
            // 3.1 转移在职人员
            doTransPsnInf(ruleVO, insertPostVOs, deptPKBackupMap, old_newPostpkmap, transItemVOs, transPsnInfVOs, psndocPKMap);
            ...
            DeptTransDeptVO[] transDeptVOs) throws BusinessException
    {
        ...
        for (DeptTransDeptVO transDeptVO : transDeptVOs)
        {
            if (transDeptVO.getOlddeptpk().equals(deptdocVO.getPk_dept()))
            {
                ...
                SuperVOHelper.copyMultiLangAttribute(transDeptVO, deptdocVO, new String[]{DeptTransDeptVO.AIMDEPTNAME},
                    new String[]{HRDeptVO.NAME});
    ...
    public void reSetTransPostInf(AggPostVO[] sourcePostVOs, DeptTransRuleVO ruleVO, DeptTransRefNameVO[] refNameVOs,
            DeptTransDeptVO[] transDeptVOs) throws BusinessException
    {
        if (ArrayUtils.isEmpty(sourcePostVOs)) return;
        ...
        for (PostVO postVO : postVOs)
        {
            for (DeptTransDeptVO transDeptVO : transDeptVOs)
            {
                ...
                if (!transDeptVO.getOlddeptpk().equals(postVO.getPk_dept()) || ArrayUtils.isEmpty(transPostVOs)) continue;
                ...
                for (DeptTransPostVO transPostVO : transPostVOs)
                {
                    if (transPostVO.getOldPostpk().equals(postVO.getPk_post()))
                    {
                        ...
                        SuperVOHelper.copyMultiLangAttribute(transPostVO, postVO, new String[]{DeptTransPostVO.AIMPOSTNAME},
                            new String[]{PostVO.POSTNAME});
     ...
     * @throws BusinessException
     */
    private void savePostVOs(String pk_org, String pk_group, AggPostVO[] sourcePostVOs, Map<String, String> deptPKBackupMap,
            Map<String, String> old_newPostpkmap, PostVO[] insertPostVOs) throws BusinessException
    {
        // 首先对岗位进行排序，这样才能维护内部码
        List<AggPostVO> postVOList = new ArrayList<AggPostVO>();
        Collections.addAll(postVOList, sourcePostVOs);
        ...
        {
            BillCodeContext billCodeContext =
                NCLocator.getInstance().lookup(IBillcodeManage.class).getBillCodeContext("post", pk_group, pk_org);
            ...
            else
            {
                billcode =
                    NCLocator.getInstance().lookup(IHrBillCode.class)
                        .getLeveledBillCode("post", pk_group, pk_org, (PostVO) sourcePostVOs[0].getParentVO(), sourcePostVOs.length);
            }
        }
        ...
        if (!aggPostVOList.isEmpty())
        {
            ...
            for (int i = 0; i < newAggPostVOs.length; i++)
            {
                ...
                for (int j = i + 1; j < sourcePostVOs.length; j++)
                {
                    ...
                    if (suporior != null && suporior.equals(toBeCopiedPostPK[i]))
                    {// 修改所有子岗位
    ...
    {
        if (ArrayUtils.isEmpty(transPsnInfVOs)) return;
        String oldPkPsnjob;
        // 是否同步工作履历
        ...
        {
            PsnJobVO psnjob =
                buildTransPsns(transPsnInfVO, ruleVO, insertPostVOs, deptPKBackupMap, old_newPostpkmap, transItemVOs, psnJobMap);
            ...
            // if (getIManagescopeFacade().isEntityUsed(ManagescopeTypeEnum.psnMajorjob, oldPkPsnjob)) {
            // String[] pkHrorgs = getIManagescopeFacade().queryHrorgsByAssgidPsnorgAndBusiregion(
            // psnjob.getPk_psnorg(), psnjob.getAssgid(), ManagescopeBusiregionEnum.psndoc);
            // pk_hrorg = pkHrorgs[0];
            // } else {
            // String[] pkHrorgs =
        ...
        Map<String, String> map =
            ManagescopeFacade.queryHrOrgByPsnjobAndDept(psnjobList.toArray(new String[0]), deptList.toArray(new String[0]));
        ArrayList<String> orgList = new ArrayList<String>();
            ...
            Map<String, PsnJobVO> psnJobMap) throws BusinessException
    {
        ...
        if (!ArrayUtils.isEmpty(transItemVOs)) for (DeptTransItemVO transItemVO : transItemVOs)
        {
            ...
            {
                psnjob.setAttributeValue(transItemVO.getItemkey().substring(3), transPsnInfVO.getAttributeValue(transItemVO.getItemkey()));
            }
        }
        // 部门信息
        ...
        if (!ArrayUtils.isEmpty(insertPostVOs)) for (PostVO insertPostVO : insertPostVOs)
        {
            // 岗位序列
            ...
            Map<String, String> old_newPostpkmap) throws BusinessException
    {
        ...
        String temptable = isc.getInSQL(ruleVO.getPk_depts());
        String cond =
            " ismainjob = 'N' and ( " + SQLHelper.getNullSql("endflag") + " or endflag ='N' ) and assgid > 1 and " + PsnJobVO.PK_DEPT
                + " in (" + temptable + " )";
        // 未结束的兼职记录
     ...
     */
    @SuppressWarnings("unused")
    private void doTransDimissionPsnInf(DeptTransRuleVO ruleVO, Map<String, String> deptPKBackupMap) throws BusinessException
    {
        // 转移部门主键临时表
        InSQLCreator isc = new InSQLCreator();
        String temptable = isc.getInSQL(ruleVO.getPk_depts());
        String cond = " ismainjob = 'Y' and  lastflag = 'Y' and endflag = 'Y'  and " + PsnJobVO.PK_DEPT + " in (" + temptable + " )";
        // 未结束的兼职记录
        PsnJobVO[] psnJobVOs = getServiceTemplate().queryByCondition(PsnJobVO.class, cond);
        // 无离职人员直接返回
        if (ArrayUtils.isEmpty(psnJobVOs))
    ...
    private void doTransPoiPsnInf(DeptTransRuleVO ruleVO, Map<String, String> deptPKBackupMap) throws BusinessException
    {
        ...
        String temptable = isc.getInSQL(ruleVO.getPk_depts());
        String cond =
            " ismainjob = 'Y' and  lastflag = 'Y'  and ( " + SQLHelper.getNullSql("endflag") + " or endflag ='N' ) and psntype = 1  and "
                + PsnJobVO.PK_DEPT + " in (" + temptable + " )";
        
        // 未结束的兼职记录
        PsnJobVO[] psnJobVOs = getServiceTemplate().queryByCondition(PsnJobVO.class, cond);
        // 无相关人员直接返回
        if (ArrayUtils.isEmpty(psnJobVOs)) return;
     ...
     * @return
     */
    private void cancelOldDeptHistoryVOs(DeptTransRuleVO ruleVO, AggHRDeptVO[] newAggDeptVOs, Map<String, String> deptPKBackupMap)
            throws BusinessException
    {
        ...
        for (int i = 0; i < sourceDeptVOs.length; i++)
        {
            ...
            // 记录源部门所属组织的版本主键
            // HROrgVO hrorgVO = getOMCommonQueryService().queryByPK(HROrgVO.class,
            ...
            /*
             * 跨业务单元转移时，不执行停用 // 执行停用 if (((HRDeptVO) updateDeptVO.getParentVO()).getEnablestate() ==
             * IPubEnumConst.ENABLESTATE_ENABLE) { updateDeptVO = disable(updateDeptVO); }
             */
        ...
        // 新部门历史记录的是否结束 = 'Y'
        getDeptDao().getBaseDAO().updateVOArray(sourceHistoryVOCache.toArray(new DeptHistoryVO[0]), new String[]{DeptHistoryVO.ISRECEIVED});
    ...
    public HRDeptVO[] queryManagedDeptsByPersonPK(String pk_psndoc, boolean isPrincipal) throws BusinessException
    {
        ...
        HROrgManagerVO[] managerVOs =
            getOMCommonQueryService().queryByCondition(HROrgManagerVO.class, new String[]{HRDeptVO.PK_DEPT}, condition, null);
    ...
    public String getDeptVid(String pk_dept, UFDate date) throws BusinessException
    {
        ...
        String querySql =
            " select pk_vid from org_dept_v where pk_dept ='" + pk_dept + "' and '" + strDate + "' < venddate and '" + strDate
                + "'>= vstartdate; ";
    ...
    public DeptManager queryOrgManagerVOByOrgIDAndCuserID(String pk_org, String cuserid) throws BusinessException
    {
        ...
        if (c != null && c.size() > 0) return c.toArray(new DeptManager[0])[0];
     ...
     */
    @Override
    public AggHRDeptVO[] batchUpdateDeptMain(SuperVO updateVO, String strFieldCode, String[] strPk_depts) throws BusinessException
    {
        ...
        // return getDeptDao().batchUpdateDeptMain(updateVO,strFieldCode,strPk_depts);
    }
            ...
            if (needCreateVersion)
            {
                ...
                if (oldversionStartDate.isSameDate(vstartdate, Calendars.getGMTDefault()))
                {
                    warningMsg = "由于" + ResHelper.getString("org", "CreateVersionDialog-000002")/*
                                                                                                 ...
                                                                                                 */;
                    warningMsg +=
                        ResHelper.getString("6005dept", "06005dept0366",
                            new String[]{MultiLangHelper.getName(deptVO), VOUtils.getDocName(OrgVO.class, deptVO.getPk_org())})
            ...
            if (needCreateVersion)
            {
                for (AggHRDeptVO canceledVO : canceledVOs)
                {
                    ...
                    if (oldversionStartDate.isSameDate(vstartdate, Calendars.getGMTDefault()))
                    {
                        ...
                        warningMsg +=
                            ResHelper.getString("6005dept", "06005dept0366",
                                new String[]{MultiLangHelper.getName(mainVO), VOUtils.getDocName(OrgVO.class, mainVO.getPk_org())})
                        /* @res "部门：[{0}]  所属组织：[{1}] 生成新版本失败！" */;
                    }
                    else if (!oldversionStartDate.isSameDate(vstartdate, Calendars.getGMTDefault())
                        && oldversionStartDate.compareTo(vstartdate) > 0)
    ...
    protected AggHRDeptVO dbUpdateImportVO(AggHRDeptVO oldVO, AggHRDeptVO vo) throws BusinessException
    {
        ...
        {
            InnerCodeUtil.generateInnerCodeAfterChangePosition(getBizInfoUtil().getTableName((SuperVO) vo.getParentVO()), idName, pidName,
                vo.getParentVO().getPrimaryKey(), newParentPK);
        }
    ...
    public String getDisabledOrgDeptnames(HRDeptVO[] deptVOs) throws BusinessException
    {
        ...
        HRAdminOrgVO[] adminOrgVOs =
            (HRAdminOrgVO[]) NCLocator.getInstance().lookup(IPersistenceRetrieve.class)
    ...
    {
        if (isAutoGenerateBillCode(vo) && !getBillCodeContext(vo).isPrecode() && JFCommonValue.LEVELED_CODE_TMP.equals(vo.getCode()))
        {
            IHrBillCode iBillcode = NCLocator.getInstance().lookup(IHrBillCode.class);
            String[] codes = iBillcode.getLeveledBillCode(JFCommonValue.NBCR_DEPT, vo.getPk_group(), vo.getPk_org(), vo, 1);
     ...
     * 
     */
    private void updateDataAfterHRAlter(ArrayList<PsndocVO> psndocs) throws BusinessException
    {
	    if (psndocs != null || psndocs.size() > 0)
	    {
	    	BaseDAO dao = new BaseDAO();
	    	String updatePsnDocsSql = "UPDATE bd_psndoc SET glbdef11 = ? WHERE pk_psndoc = ?";
	    	String updatePsnJobSql = "update hi_psnjob set jobglbdef4 = ? WHERE pk_psndoc = ?";
	    	String glbdef11 = "",psndoc="";
	        for (int i = 0; i < psndocs.size(); i++)
	        {
	        	SQLParameter parameter = new SQLParameter();
	        	glbdef11 = psndocs.get(i).getAttributeValue("glbdef11")==null?"":(String)psndocs.get(i).getAttributeValue("glbdef11");
	        	psndoc = psndocs.get(i).getPk_psndoc()==null?"":psndocs.get(i).getPk_psndoc();
	        	parameter.addParam(glbdef11);
	            parameter.addParam(psndoc);
	            dao.executeUpdate(updatePsnDocsSql, parameter);
	            dao.executeUpdate(updatePsnJobSql, parameter);
	        }
	    }
    }
