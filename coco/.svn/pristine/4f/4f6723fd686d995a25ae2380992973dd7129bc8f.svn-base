...
public class DelSubSetAction extends DelLineAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		...
		if (rows == null || rows.length == 0) {
			putValue(MESSAGE_AFTER_ACTION, ResHelper.getString("6007psn",
					"06007psn0107")/*
									 * @ res "请选择要删除的行"
									 */);
			return;
		...
		if (HICommonValue.FUNC_CODE_REGISTER.equals(getContext().getNodeCode())
			...
			if (rows.length == 1 && ObjectUtils.equals(psndocAggVO.getParentVO().getPsnJobVO().getPrimaryKey(),
		...
		if (HICommonValue.FUNC_CODE_REGISTER.equals(getContext().getNodeCode()) && getCardPanel().getBillCardPanel()
				.getBodyPanel().getTable().getName().equals(PsndocDefTableUtil.getGroupInsuranceTablename())
				&& getModel().getUiState() == UIState.EDIT) {
			PsndocAggVO psndocAggVO = (PsndocAggVO) getModel().getSelectedData();
			IPsndocSubInfoService4JFS srv = NCLocator.getInstance().lookup(IPsndocSubInfoService4JFS.class);
			if (srv.isExistsGroupInsCalculateResults(psndocAggVO.getParentVO().getPk_psndoc(),
					(String) psndocAggVO.getTableVO(PsndocDefTableUtil.getGroupInsuranceTablename())[0]
		...
		if (CtrtVO.getDefaultTableName().equals(getValue(ActionConst.HR_ACTION_TABKEY))) {
			...
			if (rowCount > 0) {
				getCardPanel().getBillCardPanel().getBillModel(CtrtVO.getDefaultTableName()).setValueAt(1, 0,
						CtrtVO.CONTTYPE);
			}
		}
	}
	...
	protected boolean isActionEnable() {
		...
		if (ArrayUtils
				.contains(new String[] { HICommonValue.FUNC_CODE_EMPLOYEE, HICommonValue.FUNC_CODE_PSN_INFO,
			...
			if ((CtrtVO.getDefaultTableName().equals(getValue(ActionConst.HR_ACTION_TABKEY))
					&& !HiCacheUtils.isModuleStarted("6011"))
					|| (CapaVO.getDefaultTableName().equals(getValue(ActionConst.HR_ACTION_TABKEY))
							&& !HiCacheUtils.isModuleStarted("6004"))
					|| (TrainVO.getDefaultTableName().equals(getValue(ActionConst.HR_ACTION_TABKEY))
							&& !HiCacheUtils.isModuleStarted("6025"))
					|| (AssVO.getDefaultTableName().equals(getValue(ActionConst.HR_ACTION_TABKEY))
							&& !HiCacheUtils.isModuleStarted("6029"))) {
