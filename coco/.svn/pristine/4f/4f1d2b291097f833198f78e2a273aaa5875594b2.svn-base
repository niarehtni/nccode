...
public class DelSubSetAction extends DelLineAction {
	...
	public void doAction(ActionEvent e) throws Exception {
		...
		int[] rows = getCardPanel().getBillCardPanel().getBodyPanel()
				.getTable().getSelectedRows();

		if (rows == null || rows.length == 0) {
			putValue(MESSAGE_AFTER_ACTION,
					ResHelper.getString("6007psn", "06007psn0107")/*
																 * @res
																 * "请选择要删除的行"
																 */);
			return;
		...
		if (HICommonValue.FUNC_CODE_REGISTER.equals(getContext().getNodeCode())
				&& PsnJobVO.getDefaultTableName().equals(
						getValue(ActionConst.HR_ACTION_TABKEY))
				&& getModel().getUiState() == UIState.EDIT) {

			// 最新的任职记录不能在子表中删除
			PsndocAggVO psndocAggVO = (PsndocAggVO) getModel()
					.getSelectedData();
			BillModel billModel = getCardPanel().getBillCardPanel()
					.getBillModel(PsnJobVO.getDefaultTableName());
			if (rows.length == 1
					&& ObjectUtils.equals(psndocAggVO.getParentVO()
				...
				throw new ValidationException(ResHelper.getString("6007psn",
						"06007psn0108")/* @res "最新任职记录不能删除" */);
			}

			ArrayList<Integer> al = new ArrayList<Integer>();
			for (int row : rows) {
				if (ObjectUtils.equals(psndocAggVO.getParentVO().getPsnJobVO()
						.getPrimaryKey(),
		...
		if (HICommonValue.FUNC_CODE_REGISTER.equals(getContext().getNodeCode())
			...
			PsndocAggVO psndocAggVO = (PsndocAggVO) getModel()
					.getSelectedData();
			IPsndocSubInfoService4JFS srv = NCLocator.getInstance().lookup(
					IPsndocSubInfoService4JFS.class);
			if (srv.isExistsGroupInsCalculateResults(psndocAggVO.getParentVO()
					.getPk_psndoc(),
					(String) psndocAggVO.getTableVO(PsndocDefTableUtil
							.getGroupInsuranceTablename())[0]
		...
		if (CtrtVO.getDefaultTableName().equals(
				getValue(ActionConst.HR_ACTION_TABKEY))) {
			// 处理合同子集增加的第一条记录必须是签订
			int rowCount = getCardPanel().getBillCardPanel()
					.getBillTable(CtrtVO.getDefaultTableName()).getRowCount();
			if (rowCount > 0) {
				getCardPanel().getBillCardPanel()
						.getBillModel(CtrtVO.getDefaultTableName())
	...
	protected boolean isActionEnable() {
		...
		if (ArrayUtils.contains(new String[] {
				...
				&& getModel().getBusinessInfoSet().contains(
						getValue(ActionConst.HR_ACTION_TABKEY))) {
			if ((CtrtVO.getDefaultTableName().equals(
					getValue(ActionConst.HR_ACTION_TABKEY)) && !HiCacheUtils
					.isModuleStarted("6011"))
					|| (CapaVO.getDefaultTableName().equals(
							getValue(ActionConst.HR_ACTION_TABKEY)) && !HiCacheUtils
							.isModuleStarted("6004"))
					|| (TrainVO.getDefaultTableName().equals(
							getValue(ActionConst.HR_ACTION_TABKEY)) && !HiCacheUtils
							.isModuleStarted("6025"))
					|| (AssVO.getDefaultTableName().equals(
							getValue(ActionConst.HR_ACTION_TABKEY)) && !HiCacheUtils
			...
			if (HICommonValue.FUNC_CODE_KEYPSN.equals(getModel().getNodeCode())
					&& KeyPsnVO.getDefaultTableName().equals(
							getValue(ActionConst.HR_ACTION_TABKEY))) {
				return getModel().getUiState() == UIState.EDIT
						|| getModel().getUiState() == UIState.ADD;
			}
		...
		return super.isActionEnable()
				&& ActionHelper.isBusiSetStarted(this)
				&& !WainfoVO.getDefaultTableName().equals(
						getValue(ActionConst.HR_ACTION_TABKEY))
				&& !BminfoVO.getDefaultTableName().equals(
						getValue(ActionConst.HR_ACTION_TABKEY));
	}
