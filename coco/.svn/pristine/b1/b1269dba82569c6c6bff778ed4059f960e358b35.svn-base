	...
	public void doAction(ActionEvent arg0) throws Exception {
		...
		if (isGen == UIDialog.ID_OK) {
			...
			for (String pk_legalOrg : pkRealOrgSet) {
				...
				//微...调以兼容只有健保 Ares.Tank 2018-11-1 11:27:28
				...
				String sql = "select 3 as move, '" + TWNHHI01.getTextvalue() + "' as insnum,"
						...
						+ " case doc2.code  " + " when 'LT06' then 'Y' " + " when 'LT13' then '1'  "
						+ " when 'LT14' then '2' " + " else '' end as insforeign, "
						...
						+ " ps.birthdate as birthdate, " + " case when g2.glbdef2 is null then g3.glbdef16 "
						+ " else g2.glbdef2 end as salary, " + " (SELECT top 1 glbdef16 FROM "
						...
						+ " and isnull(g2.insuranceform,3) = 3 and g3.insuranceform = 3 "
				...
				if (list != null && list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						...
						for (int j = 0; j < 14; j++) {
							...
							case 10:
								cell.setCellValue(
										map.get("salary") == null ? dfInt.format(0) : dfInt.format(map.get("salary")));
								break;
							case 11:
								cell.setCellValue(map.get("oldsalary") == null ? dfInt.format(0)
										: dfInt.format(map.get("oldsalary")));
								break;
							case 12:
								cell.setCellValue(map.get("newsalary") == null ? dfInt.format(0)
										: dfInt.format(map.get("newsalary")));
								break;
							case 13:
				...
				try {
					...
					outstream.close();
					/*
					 * Runtime CRuntime = Runtime.getRuntime(); Process CProce =
					 * CRuntime.exec("cmd /c start " + filepath); // 关闭进程
					 * CProce.waitFor(); CProce.destroy();
					 */
				} catch (Exception e) {
