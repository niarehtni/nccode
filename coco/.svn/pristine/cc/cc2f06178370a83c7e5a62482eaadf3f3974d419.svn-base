...
public class PsndocDataManager extends DefaultAppModelDataManager implements
		AppEventListener, IAppModelDataManagerEx, ITypeSupportModelDataManager,
	...
	public IPsndocQryService getQueryService() {
		if (queryService == null) {
			queryService = ((IPsndocQryService) NCLocator.getInstance().lookup(
	...
	protected String getSQL4IncludeChildren() {
		...
		if ((superVO == null)
				|| (((superVO instanceof OrgVO)) && ("other_node"
						.equals(superVO.getPrimaryKey())))) {

			return "";
		}
		String strSQL = "";
		String strPsnJobTableAlias = getTableAlias(PsnJobVO
				.getDefaultTableName());
	...
	protected String getSQL4Tree() throws Exception {
		...
		if ((superVO instanceof OrgVO)) {
			...
			if ("other_node_msaos".equals(orgVO.getPrimaryKey())) {
				String strOtherPsnjobByHrorg = ManagescopeFacade
						.queryOtherPsnjobByHrorg4MsAOS(
								getContext().getPk_org(),
								ManagescopeBusiregionEnum.psndoc);

				strSQL = " " + tableAlias + ".pk_psnjob in"
						+ strOtherPsnjobByHrorg;
				...
				String sql = "";
				if (("60070psninfo".equals(getModel().getContext()
						.getNodeCode()))
						&& (isIncludeChildHR != null)
					...
					String gkSql = ((IPsndocService) NCLocator.getInstance()
							...
							"@@@@Z710000000006M2K", getModel().getContext()
									.getPk_org(), true);

					sql = AOSSQLHelper.getAllBUInSQLByHROrgPK(orgVO
							.getPrimaryKey());
					if (!StringUtils.isEmpty(gkSql)) {
						sql = sql + " and pk_adminorg in (" + gkSql + ")";
					} else {
						sql = AOSSQLHelper.getChildrenBUInSQLByHROrgPK(orgVO
								.getPrimaryKey());
					}
				} else {
					sql = AOSSQLHelper.getChildrenBUInSQLByHROrgPK(orgVO
			...
			if (getModel().isIncludeChildDepts()) {
				...
				strSQL = " "
						+ tableAlias
						+ ".pk_dept in("
						+ AOSSQLHelper.getAllDeptInSQLByHROrgPK(
								deptVO.getPrimaryKey(), true) + ")";
			} else {
				strSQL = " " + tableAlias + ".pk_dept='"
						+ deptVO.getPrimaryKey() + "'";
			}
			...
			if (getModel().isBlIncludeChildPsncl()) {
				...
				strSQL = " " + tableAlias + ".pk_psncl='"
						+ psnClVO.getPrimaryKey() + "'";
			}
			if ("60070psninfo".equals(getModel().getContext().getNodeCode())) {
				...
				String orgPart = " "
						+ tableAlias
						+ ".pk_org in ("
						+ AOSSQLHelper.getAllBUInSQLByHROrgPK(getContext()
								.getPk_org()) + ") ";
				strSQL = strSQL + " and " + orgPart + " ";
			} else if ("60070poi".equals(getModel().getContext().getNodeCode())) {

				strSQL = strSQL + " and " + tableAlias + ".pk_hrorg = '"
						+ getContext().getPk_org() + "' ";
			}
		} else if ((superVO instanceof KeyPsnGrpVO)) {

			KeyPsnGrpVO grp = (KeyPsnGrpVO) superVO;
			String inSql = " select pk_psnorg from hi_psndoc_keypsn where pk_keypsn_grp = '"
			...
			if ((isIncludeChildHR != null) && (isIncludeChildHR.booleanValue())) {
				...
				sql = AOSSQLHelper.getChildrenBUInSQLByHROrgPK(getContext()
						.getPk_org());

			} else {
				sql = AOSSQLHelper.getAllBUInSQLByHROrgPK(getContext()
						.getPk_org());
			}
			...
			grpInSql = grpInSql
					+ " and "
					+ KeyPsnGroupSqlUtils.getKeyPsnGroupPowerSql(KeyPsnGrpVO
							.getDefaultTableName());

			String inSql = " select pk_psnorg from hi_psndoc_keypsn where pk_keypsn_grp in ( "
					+ grpInSql + " ) ";
			if (!getModel().isShowHisKeyPsn()) {
	...
	public String getSQLAll() throws Exception {
		...
		String strLastWhereSqlQueryDialog = getModel()
				.getLastWhereSqlQueryDialog();
		...
		if ((!"60070poi".equals(getModel().getContext().getNodeCode()))
				&& (!"60070psninfo".equals(getModel().getContext()
						.getNodeCode()))) {

			String strPsnJobTableName = getTableAlias(PsnJobVO
					.getDefaultTableName());

			strSQL = strSQL + " and ( " + strPsnJobTableName + "." + "pk_hrorg"
					+ " = '" + getContext().getPk_org() + "' and "
					+ strPsnJobTableName + "." + "lastflag" + " = 'Y' and "
					+ strPsnJobTableName + "." + "ismainjob" + " = 'Y' ) ";

			if (getModel().isBlShowOnJobPsn()) {
				strSQL = strSQL + " and ( " + strPsnJobTableName + "."
						+ "endflag" + " = 'N' ) ";
			} else {
				strSQL = strSQL + " and ( " + strPsnJobTableName + "."
						+ "endflag" + " = 'Y' ) ";
		...
		if ("60070employee".equals(getModel().getContext().getNodeCode())) {
			...
			strSQL = strSQL
					...
					+ strPsnJobTableName
					+ ".pk_psnorg  not in ( "
					+ KeyPsnGroupSqlUtils.getKeyPsnSql(getModel().getContext()
							.getPk_org()) + " ) ) ";
		}
		...
		String strPsnJobTableName = getTableAlias(PsnJobVO
				.getDefaultTableName());
		strSQL = strSQL + " and " + strPsnJobTableName
				+ ".pk_org in (select pk_adminorg from org_admin_enable)";
	...
	protected String getSQLExtWhere() {
		...
		String strSQL = psnorg + "." + "indocflag" + "='"
				+ getModel().getIndocFlag() + "' and " + psnorg + "."
				+ "psntype" + "=" + getModel().getPsnType();

		if (!"60070psninfo".equals(getModel().getContext().getNodeCode())) {

			strSQL = strSQL + "  and " + psnjob + "." + "lastflag" + "='"
					+ UFBoolean.TRUE + "' ";
		}

		if (ArrayUtils.contains(new String[] { "60070register" }, getModel()
				.getContext().getNodeCode())) {
			strSQL = strSQL + " and " + psnjob + "." + "endflag" + "='"
					+ UFBoolean.FALSE + "'";
		}
		return strSQL;
	}
	...
	public String getTableAlias(String strTableName) {
		return getModel().getUsedTablesOfQuery().containsKey(strTableName) ? (String) getModel()
				.getUsedTablesOfQuery().get(strTableName) : strTableName;
	}
	...
	public void handleEvent(AppEvent event) {
		...
		if (ArrayUtils.contains(new String[] { "Data_Deleted", "Data_Inserted",
				"Selected_Data_Changed", "Multi_Selection_Changed" },
				event.getType())) {
			...
			for (int i = 0; i < objs.length; i++) {
				newObjs[i] = ((PsndocAggVO) objs[i]).getParentVO()
						.getPsnJobVO();
			}

			RowOperationInfo newInfo = new RowOperationInfo(
					info.getRowIndexes(), newObjs);
			if ((info instanceof RowSelectionOperationInfo)) {
				...
				((RowSelectionOperationInfo) newInfo)
						.setSelectionState(((RowSelectionOperationInfo) info)
								.getSelectionState());
			}

			AppEvent newEvent = new AppEvent(event.getType(),
					event.getSource(), newInfo);
			getPaginationDelegator().handleEvent(newEvent);

			if (ArrayUtils.contains(new String[] { "Data_Inserted",
					"Selected_Data_Changed" }, event.getType())) {
				for (int i = 0; i < objs.length; i++) {
					PsndocAggVO agg = (PsndocAggVO) objs[i];
					getPaginationModel()
							.update(agg.getParentVO().getPsnJobVO()
									.getPk_psnjob(), agg);
				}
	...
	public void queryData() throws Exception {
		...
		String strPsnOrgTableName = getTableAlias(PsnOrgVO
				.getDefaultTableName());
		strFrom = strFrom + " inner join hi_psnorg " + strPsnOrgTableName
				+ " on bd_psndoc.pk_psndoc = " + strPsnOrgTableName
				+ ".pk_psndoc ";

		String strPsnJobTableName = getTableAlias(PsnJobVO
				.getDefaultTableName());
		strFrom = strFrom + " inner join hi_psnjob " + strPsnJobTableName
				+ " on " + strPsnOrgTableName + ".pk_psnorg = "
				+ strPsnJobTableName + ".pk_psnorg";

		HashMap<String, String> hashUsedTablesOfQuery = getModel()
				.getUsedTablesOfQuery();
		for (String tabName : hashUsedTablesOfQuery.keySet()) {
			if (!ArrayUtils.contains(
				...
				if (PsndocAggVO.hashBusinessInfoSet.contains(tabName)) {
					...
					strFrom = strFrom + " left outer join " + tabName + " "
							+ alias + " on " + strPsnOrgTableName
							+ ".pk_psnorg = " + alias + ".pk_psnorg ";

				} else {
					strFrom = strFrom + " left outer join " + tabName + " "
							+ alias + " on bd_psndoc.pk_psndoc = " + alias
							+ ".pk_psndoc ";
				}
			}
		}

		strFrom = strFrom + " left outer join org_orgs org_orgs  on "
				+ strPsnJobTableName + ".pk_org = org_orgs.pk_org ";

		strFrom = strFrom + " left outer join org_dept org_dept  on "
				+ strPsnJobTableName + ".pk_dept = org_dept.pk_dept ";

		String[] strPk_psndocs = getQueryService()
				.queryPsndocPks(
						getContext(),
						new String[] { getTableAlias(PsnJobVO
								.getDefaultTableName()) + "." + "pk_psnjob" },
						strFrom, strWhere,
						getOrderby(getModel().getSortFields()),
						getModel().getUsedTablesOfQuery(),
						getModel().getResourceCode());

		getPaginationModel().setObjectPks(strPk_psndocs);
	}

	public SuperVO[] querySubVO(String strTabCode, String strAddtionalWhere)
			throws BusinessException {
		if ((getModel().getCurrentPkPsndoc() == null)
				|| (getModel().getCurrentPkPsndoc().trim().length() == 0)) {
			return null;
		}
		...
		if (PsnJobVO.getDefaultTableName().equals(strTabCode)) {
			...
			strAddtionalWhere = strAddtionalWhere
					+ " hi_psndoc_ctrt.isrefer = 'Y' ";
		} else if (KeyPsnVO.getDefaultTableName().equals(strTabCode)) {

			if ((getModel().getCurrTypeOrgVO() != null)
					&& ((getModel().getCurrTypeOrgVO() instanceof KeyPsnGrpVO))) {
				strAddtionalWhere = strAddtionalWhere
						+ " hi_psndoc_keypsn.pk_keypsn_grp = '"
						+ ((KeyPsnGrpVO) getModel().getCurrTypeOrgVO())
								.getPk_keypsn_group() + "' ";
				...
				strAddtionalWhere = strAddtionalWhere
						...
						+ KeyPsnGroupSqlUtils
								.getKeyPsnGroupPowerSql(KeyPsnGrpVO
										.getDefaultTableName()) + " ) ";
			}

		} else if (CapaVO.getDefaultTableName().equals(strTabCode)) {
			strAddtionalWhere = strAddtionalWhere
					+ " hi_psndoc_capa.lastflag = 'Y' ";
		} else if (TrainVO.getDefaultTableName().equals(strTabCode)) {
			PsndocAggVO psndocAggVO = (PsndocAggVO) getModel()
					.getSelectedData();
			if ((psndocAggVO != null)
					&& (psndocAggVO.getParentVO().getPsnJobVO() != null)) {
				String pk_psnjob = psndocAggVO.getParentVO().getPsnJobVO()
						.getPk_psnjob();
				strAddtionalWhere = strAddtionalWhere
						+ " hi_psndoc_train.pk_psnjob = '" + pk_psnjob + "' ";
			}
		}
		...
		if (((getModel().getBusinessInfoSet().contains(strTabCode)) || (WainfoVO
				.getDefaultTableName().equals(strTabCode)))
				&& (psndocAggVO != null)) {

			if ("60070psninfo".equals(getContext().getNodeCode())) {

				String pk_org = psndocAggVO.getParentVO().getPsnJobVO()
						.getPk_org();
				if (PsnJobVO.getDefaultTableName().equals(strTabCode)) {
					...
					strWhere = strWhere
							...
							+ pk_org
							+ "' and pk_psnorg = '"
							+ psndocAggVO.getParentVO().getPsnOrgVO()
									.getPrimaryKey() + "'";

				} else if (PsndocAggVO.hashPsnJobInfoSet.contains(strTabCode)) {

					strWhere = strWhere
							+ " and pk_psnjob in (select pk_psnjob from hi_psnjob where pk_org = '"
							+ pk_org
							+ "' and pk_psnorg = '"
							+ psndocAggVO.getParentVO().getPsnOrgVO()
									.getPrimaryKey() + "') ";

				} else if (!strTabCode.equals(PsndocDefTableUtil
						.getPsnNHIDetailTablename())
						&& !strTabCode.equals(PsndocDefTableUtil
				...
				UFBoolean ismainjob = psndocAggVO.getParentVO().getPsnJobVO()
						.getIsmainjob();
				...
				if ((UIState.ADD == getModel().getUiState() || UIState.EDIT == getModel()
						.getUiState())
						&& (!strTabCode.equals(PsndocDefTableUtil
								.getPsnNHIDetailTablename())
								&& !strTabCode.equals(PsndocDefTableUtil
								...
								&& !strTabCode.equals(PsndocDefTableUtil
										.getPsnHealthInsExTablename())
								&& !strTabCode.equals(PsndocDefTableUtil
										.getGroupInsuranceDetailTablename()) && !strTabCode
									.equals(PsndocDefTableUtil
											.getGroupInsuranceTablename()))) {
					// 编辑态还是只显示当前组织关系的数据
					strWhere += " and pk_psnorg = '"
							+ psndocAggVO.getParentVO().getPsnOrgVO()
									.getPrimaryKey() + "'";
				} else if (!strTabCode.equals(PsndocDefTableUtil
						.getPsnNHIDetailTablename())
		...
		IBean bean = MDBaseQueryFacade.getInstance().getBeanByFullName(
				"hrhi." + strTabCode);
		IAttribute attribute = MDBaseQueryFacade.getInstance()
				.getAttributeByFullName("hrhi." + strTabCode + ".recordnum");
		...
		SuperVO[] subVOs = getQueryService().querySubVO(clazzVO, strWhere,
				strOrder);
		return subVOs;
	}
	...
	public boolean getValueOFParaMaintainCadre() throws Exception {
		return getQueryService().getValueOFParaMaintainCadre(getContext())
				.booleanValue();
	}
}
