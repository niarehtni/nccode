...
public class PsndocSubInfoService4JFSImpl implements IPsndocSubInfoService4JFS {
		...
		if (groupSettings != null && groupSettings.size() > 0 && setVOs != null && setVOs.length > 0) {
			for (Entry<String, Map<String, GroupInsSettingVO>> grpSets : groupSettings.entrySet()) {
				...
				for (GroupInsuranceSettingVO setvo : setVOs) {
					int i = 0;
					for (String idins : idinslists) {
						if ((setvo.getCgrpinsid() + setvo.getCgrpinsrelid()).equalsIgnoreCase(idins)) {
							i++;
						}
					}
					// 包含这个险种-身份的数据就更新金额
					for (FamEmpGroupInsDetailVO famempvo : famemplist) {
						if ((famempvo.getInsurancecode() + famempvo.getIdentitycode()).equals(setvo.getCgrpinsid()
							...
							for (int x = 0; x < i; x++) {
								if (x == 0) {
									// 把余数放在第一条中
									famempvo.setInsuranceamount(new UFDouble((insuranceamount / i)
											+ (insuranceamount % i)));
								} else {
									famempvo.setInsuranceamount(new UFDouble((insuranceamount / i)));
								}
		...
		if (nhiFinalVOs != null && nhiFinalVOs.length > 0) {
			for (NhiCalcVO vo : nhiFinalVOs) {
				...
				if ((vo.getLaborrange() != null && !vo.getLaborrange().equals(vo.getOldlaborrange()) && !UFDouble.ZERO_DBL
					...
					newLaborVO.setAttributeValue("legalpersonorg", vo.getPk_corp());
				...
				if (vo.getHealthrange() != null && !vo.getHealthrange().equals(vo.getOldhealthrange())
					...
					// Ares.Tank 劳保法人组织和投保形态设设置
					newHealVO.setAttributeValue("legalpersonorg", vo.getPk_corp());
					// 按理来说,此处应该用枚举来set值的,为什么用数字呢?
					// 1.因为没有枚举类 2.hrtaovertimereg.bmf这个元数据是有点毛病 -->3.数字比中文更适合编码
					// // 定调资--加保,同步劳保级距--保薪调整
					if (state) {
	...
	//
	// private int getAvgSalary(String pk_wa_class, String startdate, String
	// cbaseDate, String pk_psndoc,
	// Map<String, List<String>> periodItem, List<String> waperioddates) throws
	// DAOException {
	// // 获取第一条的入职日期
	// List<PsnJobVO> psnjobs = (List<PsnJobVO>)
	// this.getBaseDao().retrieveByClause(PsnJobVO.class,
	...
	// " where cyear||'-'||cperiod = '"
	// + entry.getKey() + "' and pk_psndoc = '" + pk_psndoc +
	...
	// "select sum(nhisalary) nhisalary from (" + strTotelSQL + ") A", new
	// ColumnProcessor());
	// if (rstObj != null) {
	...
	public void generatePsnNHI(String pk_org, String[] pk_psndocs) throws BusinessException {
		...
		SimpleDocServiceTemplate service = new SimpleDocServiceTemplate("TWHRGLBDEF");
		for (String pk_psndoc : pk_psndocs) {
			pkLegalOrg = LegalOrgUtilsEX.findLegalOrgByPsn(pk_psndoc, new UFLiteralDate());
			...
			for (String pk_psndoc : pk_psndocs) {
				...
				if (flag.booleanValue()) {
					...
					if (sixmonth(pk_psndoc, pk_org).booleanValue()) {
						avgSalary = getavgsalary(cbaseDate, basemonthdate, waperiodlist);
					} else {
						avgSalary = getavgsalary(cbaseDate, basemonthdate, waperiodlist);
					}
				} else {
					...
					if (avgmonsalary.doubleValue() - (actmontsalary.multiply(0.6).doubleValue()) > 0) {
						...
						actmontsalary.multiply(0.6);
					}
				}
				UFDouble salary = avgSalary.multiply(30);
				if (salary == null) {
					continue;
				}
	...
	public void renewGroupIns(String pk_org, String[] pk_psndocs, UFDate effectivedate) throws BusinessException {
		...
		if (pk_psndocs != null && pk_psndocs.length > 0) {
			for (String pk_psndoc : pk_psndocs) {
				...
				List<PsndocWaVO> wavos = (List<PsndocWaVO>) this.getBaseDao().retrieveByClause(PsndocWaVO.class,
						"pk_psndoc='" + pk_psndoc + "' and pk_wa_item in (" + waitemInSQL + ") and enddate is null");
				UFDouble sumSalary = UFDouble.ZERO_DBL;
				for (PsndocWaVO psndocwa : wavos) {
