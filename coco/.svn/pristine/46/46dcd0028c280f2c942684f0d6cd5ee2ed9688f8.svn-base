		...
		if (changeDayTypeCheckBox) {
			...
			if (null == firstDate || null != secondDate) {
				...
				try {
					 ...
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" )); wae.addMsg("1", firstDate +
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" ); throw wae; } if
					 ...
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" )); wae.addMsg("1", secondDate +
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" ); throw wae; }
					 */

					// 保存两个日期都有考勤档案的人员
					// Set<String> psndocListHasBothDate = new HashSet<>();//
					// 这年头都没人用set的吗,连个set的包都要自己引
					 ...
					 * ("10140wcb", "010140wcb0010") @res "节假日说明" ); String memo
					 * = " "; if (memoDialog.showModal() == UIDialog.ID_OK) {
					 * memo = memoDialog.getHolidayMemo(); }
					 * 
					 * 
					 * //TODO: 導出人員考勤檔案不存在的人員Excel,而不是顯示在提示框上 int isForce =
					 * MessageDialog.showOkCancelDlg(null,
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0016"),
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0016")); if (1 != isForce) { // 不同意,什么也不改
					 * return; } }
					 */
					// 增加了时间,不进行考勤的校验
			...
			if (null != changeDate) {
				...
				try {
					 ...
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" )); wae.addMsg("1", changeDate +
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0015")
					 * 
					 * @res "人员考勤档案为空,不会进行任何操作" ); throw wae; }
					 ...
					 * // 保存有考勤档案的人员
					 * 
					 * // 遍历各个人员,看是否考勤档案是否都存在 for (String pk_psndoc :
					 * pk_psndocs) { if (null ==
					 * changeTbmpsndocVOMap.get(pk_psndoc) ||
					 * changeTbmpsndocVOMap.get(pk_psndoc).isEmpty()) {
					 * psndocListWithNoDate.add(pk_psndoc); } else{// 两个考勤档案都存的
					 ...
					 * 導出人員考勤檔案不存在人員的Excel(因為數量較多),而不是顯示在提示框上 int isForce =
					 * MessageDialog.showOkCancelDlg(null,
					 * ResHelper.getString("twhr_psncalendar",
					 * "psncalendar-0016"),
					 * ResHelper.getString("twhr_psncalendar",
