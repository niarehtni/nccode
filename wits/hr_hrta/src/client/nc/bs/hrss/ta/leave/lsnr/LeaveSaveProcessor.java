			...
			if (!Boolean.TRUE.equals(result)) {
				...
				if (!islactation.booleanValue()) {
					...
					if (formComp != null) {
						...
						//如果是哺乳假，不计算时长add by zhenmx 2018-03-14 modify 哺乳假默认时长不能设为0，会导致保存校验不通过，设置默认时长为lactationhour（哺乳假单据没有时长）
						headVO.setSumhour(new UFDouble(lactationhour));
							for( int i=0;i<leavebVOs.length;i++){
								leavebVOs[i].setLeavehour(new UFDouble(lactationhour));
							}
						//add end
					}
