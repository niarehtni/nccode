...
public class BmfileManageServiceImpl implements IBmfileManageService, IBmfileQueryService {
	...
	public void batchAddPsn(BmDataVO[] vos, BmDataVO[] classInfo, BmLoginContext context) throws BusinessException {
		...
		//当在批量新增的时候，险种的社保代理公司以最后新增的公司为准 by hepingyang 
		String def1 = null;
		String cyear = null;
		String cperiod = null;
		...
		for(BmDataVO vo : vos){
			...
			arrayList.add(pk_psndoc);
			def1 = vo.getDef1();
			cyear = vo.getCyear();
			cperiod = vo.getCperiod();
		}
			String arra = arrayList.toString().replace("[", "(").replace("]", ")");
			String str = "update bm_data set def1='"+ def1 +"' where pk_psndoc in "+ arra +" and cyear="+ cyear +" and cperiod="+ cperiod +"";
			getBmfileDao().getBaseDao().executeUpdate(str);
	...
	public BmDataVO[] batchEditBmData(BmDataVO[] vos, BmLoginContext loginContext) throws BusinessException {
		...
		for(BmDataVO vo : vos){
			...
			String pk_psndoc = "'"+vo.getPk_psndoc()+"'";
			arryList.add(pk_psndoc);
			cyear = vo.getCyear();
			cperiod = vo.getCperiod();
		}
		
		if(null != def1){
			String arra = arryList.toString().replace("[", "(").replace("]", ")");
			String str = "update bm_data set def1='"+ def1 +"' where pk_psndoc in "+ arra +" and cyear="+ cyear +" and cperiod="+ cperiod +"";
			getBmfileDao().getBaseDao().executeUpdate(str);
		}
