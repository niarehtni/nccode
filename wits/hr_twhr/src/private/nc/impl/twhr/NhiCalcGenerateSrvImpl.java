...
public class NhiCalcGenerateSrvImpl implements INhiCalcGenerateSrv {
			...
			if (laborSetting != null) {
				...
				int countDays = getLaborCountDays(
						...
						(UFLiteralDate) (laborSetting.getAttributeValue("REALLABORENDDATE") == null ? nhivo
								...
								.getAttributeValue("REALLABORENDDATE")), curLaborOutMap.get(pk_psndoc));
				...
				countDays = getRetireCountDays(beginDateOfMonth, endDateOfMonth,
						(laborSetting.getAttributeValue("glbdef14") == null ? nhivo.getBegindate() : new UFDate(
								((UFLiteralDate) laborSetting.getAttributeValue("glbdef14")).toDate())),
						(laborSetting.getAttributeValue("glbdef15") == null ? nhivo.getEnddate() : new UFDate(
								((UFLiteralDate) laborSetting.getAttributeValue("glbdef15")).toDate())),
						curRetireOutMap.get(pk_psndoc));
	...
	private int getLaborCountDays(UFLiteralDate beginDate, UFLiteralDate endDate, UFBoolean isCurMonthOut) {
		if (beginDate.after(endDate)) {
			return 0;
		}
		int countDays = 0;
		int addDays = beginDate.getDaysMonth(); // 附加天数：当前月为2月时，整月加保需补齐30天
		if (isCurMonthOut.booleanValue()) {
			// 如果当月退保，则附加天数为0，即以实际加保天数为准
			addDays = 0;
		} else {
			addDays = (addDays >= 30) ? 0 : (30 - addDays);
		}
		int beginDayOfMonth = 30 - (beginDate.getDay() > 30 ? 30 : beginDate.getDay()) + 1;
		int endDayOfMonth = 30 - (endDate.getDay() > 30 ? 30 : endDate.getDay()) + 1;

		countDays = (beginDayOfMonth > 30 ? 30 : beginDayOfMonth) - (endDayOfMonth > 30 ? 30 : endDayOfMonth) + 1;
		countDays = beginDate.getDaysMonth() < 30 ? (countDays + addDays) : countDays;
		countDays = countDays > 30 ? 30 : countDays;

		return countDays;
	}

	private int getRetireCountDays(UFLiteralDate beginDateOfMonth, UFLiteralDate endDateOfMonth, UFDate beginDate,
			UFDate endDate, UFBoolean isCurMonthOut) {
		if (beginDate.after(endDate)) {
			return 0;
		}

		UFLiteralDate beginD = beginDate.toUFLiteralDate(ICalendar.BASE_TIMEZONE).before(beginDateOfMonth) ? beginDateOfMonth
				: beginDate.toUFLiteralDate(ICalendar.BASE_TIMEZONE);

		UFLiteralDate endD = endDate.toUFLiteralDate(ICalendar.BASE_TIMEZONE).after(endDateOfMonth) ? endDateOfMonth
				: endDate.toUFLiteralDate(ICalendar.BASE_TIMEZONE);

		int countDays = 0;

		if (beginD.getDay() == 1 && endD.getDay() == endD.getDaysMonth()) {
			// 全月在保
			countDays = 30;
		} else if (beginD.getDay() > 1 && endD.getDay() == endD.getDaysMonth()) {
			// 非當月第一天加保，當月未退或最後一天退保
			countDays = 30 - beginD.getDay() + 1;
		} else {
			// 其他所有情況
			countDays = endD.getDay() - beginD.getDay() + 1;
		}
