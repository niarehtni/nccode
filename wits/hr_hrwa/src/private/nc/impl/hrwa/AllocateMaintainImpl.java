...
public class AllocateMaintainImpl extends AceAllocatePubServiceImpl implements
		IAllocateMaintain {

	@Override
	public void delete(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		super.pubdeleteBills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] insert(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		return super.pubinsertBills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] update(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		return super.pubupdateBills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] query(IQueryScheme queryScheme)
			throws BusinessException {
		return super.pubquerybills(queryScheme);
	}

	@Override
	public AggAllocateOutVO[] save(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		return super.pubsendapprovebills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] unsave(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		return super.pubunsendapprovebills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] approve(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
		return super.pubapprovebills(clientFullVOs, originBills);
	}

	@Override
	public AggAllocateOutVO[] unapprove(AggAllocateOutVO[] clientFullVOs,
			AggAllocateOutVO[] originBills) throws BusinessException {
	...
	public AggAllocateOutVO[] queryBillsByCondition(WaLoginContext context,
			String condition, String orderSQL) throws BusinessException {
		AggAllocateOutVO[] bills = null;
		StringBuilder whereSQL = new StringBuilder();
		if (null != context) {
			if (StringUtils.isNotEmpty(context.getPk_org())) {
				whereSQL.append(" and pk_org='").append(context.getPk_org())
						.append("' ");
			}
			if (StringUtils.isNotEmpty(context.getPk_wa_class())) {
				whereSQL.append(" and pk_wa_calss='")
						.append(context.getPk_wa_class()).append("' ");
			}
			if (StringUtils.isNotEmpty(context.getWaLoginVO().getPeriodVO()
					.getCyear())
					|| StringUtils.isNotEmpty(context.getWaLoginVO()
							.getPeriodVO().getCperiod())) {
				whereSQL.append(" and cperiod='")
						.append(context.getWaLoginVO().getPeriodVO().getCyear())
						.append(context.getWaLoginVO().getPeriodVO()
								.getCperiod()).append("' ");
			}
		}
		...
		List<AllocateOutHVO> hvoList = (List<AllocateOutHVO>) getAppDao()
				.getBaseDao().retrieveByClause(AllocateOutHVO.class,
						whereSQL.substring(4).toString(), orderSQL);
	...
	@Override
	public AggAllocateOutVO[] genPjShareDetail(WaLoginContext context)
			throws BusinessException {
		...
		// null);
		DataVO[] payDatas = new PaydataDAO().queryByCondition(context, null,
				null);
		if (ArrayUtils.isEmpty(classItemVOs)) {
			// 此方案期间没有薪资发放项目！
			ExceptionUtils.wrappBusinessException(ResHelper.getString(
					"allocate", "0allcate-ui-000016"));
		}
		if (ArrayUtils.isEmpty(payDatas)) {
			// 此方案期间没有薪资发放数据！
			ExceptionUtils.wrappBusinessException(ResHelper.getString(
					"allocate", "0allcate-ui-000017"));
		}
		// 分摊方案档案
		Map<String, DefdocVO> shareVOMap = queryBasicInfo(DefdocVO.class,
				getDefdocCondition(WaConstant.DEF_CODE_SHARE, null),
				new String[] { DefdocVO.PK_DEFDOC });
		// 员工专案代码分摊
		Map<String, List<ProjSalaryHVO>> psnProjSaVosMap = queryMapListInfo(
				ProjSalaryHVO.class, getProjCodeWhere(context),
				new String[] { "pk_psndoc" });
		// 考勤期间起止时间
		PeriodVO[] periodArr = getAppDao().retrieveByClause(PeriodVO.class,
				getPeriodWhere(context));
		if (ArrayUtils.isEmpty(periodArr)) {
			// 没有找到考勤期间
			ExceptionUtils.wrappBusinessException(ResHelper.getString(
					"allocate", "0allcate-ui-000019"));
		}
		PeriodVO periodVO = periodArr[0];
		String[] psndocPks = StringPiecer.getStrArray(payDatas,
				DataVO.PK_PSNDOC);

		// 加班时数统计
		Map<String, List<OvertimeRegVO>> psnPjOtHoursMap = queryMapListInfo(
				OvertimeRegVO.class, getProjOverTimeWhere(context, periodVO),
				new String[] { OvertimeRegVO.PK_PSNDOC });
		// 员工工作日历工时数
		Map<String, UFDouble> psnMonthWkHourMap = qryWkHoursFromPsnCalendar(
				context, periodVO, psndocPks);
		// 考勤期间内的正常工时数(组织日历去除法定节假日与公休)，没有排班情况下引用
		Map<String, UFDouble> psnMonthWkHours = qryMonthWkHoursByOrgCalander(
				context, periodVO, psndocPks);
		// 员工休假期间休假信息
		Map<String, UFDouble> psnLeaveHoursMap = null; // qryPsnLeaveHours(context,periodVO);
		// 外部人员专案工时信息
		Map<String, Map<String, UFDouble>> psnPjWkHoursMap = qryPsnWkHoursOutersys(
				context, periodVO, psndocPks);
		...
		for (DataVO vo : payDatas) {
			...
			for (WaClassItemVO itemVO : classItemVOs) {
				...
				try {
					itemMny = (obj == null ? UFDouble.ZERO_DBL : new UFDouble(
							String.valueOf(obj)));
				} catch (java.lang.Exception e) {
					Logger.error("Error!!! classitem:" + itemkey + " value:"
							+ String.valueOf(obj));
					continue;
				}
				DefdocVO shareVO = shareVOMap.get(itemVO.getDef1());
				if (null == shareVO
						|| shareVO.getCode().toUpperCase()
								.equals(WaConstant.SHARE_NOT_SYS__SHARED)
						|| itemMny.compareTo(UFDouble.ZERO_DBL) == 0) {
					continue;
				}

				if (shareVO.getCode().toUpperCase()
						.equals(WaConstant.SHARE_MONTH_WORKHOURS)) {
					// 全月工时分摊
					...
					List<AggAllocateOutVO> aggList = getMonthHoursShare(
							psnMonthWkHourMap, psnMonthWkHours,
							psnLeaveHoursMap, psnPjWkHoursMap, pjbaseHVO);
					if (!aggList.isEmpty()) {
						billList.addAll(aggList);
					}
				} else if (shareVO.getCode().toUpperCase()
						.equals(WaConstant.SHARE_PROJ_CODE)) {
					// 专案代码分摊
					...
					List<AggAllocateOutVO> aggList = getProjSalaryShare(
							psnProjSaVosMap, pjCodeHVO);
					if (!aggList.isEmpty()) {
						billList.addAll(aggList);
					}
				} else if (shareVO.getCode().toUpperCase()
						.equals(WaConstant.SHARE_PROJ_OVERHOURS)) {
					// 专案加班时数分摊
					...
					List<AggAllocateOutVO> aggList = getProjOverTimeShare(
							psnPjOtHoursMap, pjOtHVO);
					if (!aggList.isEmpty()) {
						billList.addAll(aggList);
					}

				} else if (shareVO.getCode().toUpperCase()
						.equals(WaConstant.SHARE_FULL_COSTCENTER)) {
					// 全额摊至成本中心
					AllocateOutHVO fullCostCenterHVO = (AllocateOutHVO) baseHVO
							.clone();
					...
					fullCostCenterHVO
							.setDef2(getRoundNumber(itemMny, isTWArea));
					AggAllocateOutVO billVO = new AggAllocateOutVO();
					billVO.setParentVO(fullCostCenterHVO);
					billList.add(billVO);
				} else if (shareVO.getCode().toUpperCase()
						.equals(WaConstant.SHARE_NOT_SHARED)) {
					// 不分摊
					AllocateOutHVO notShareHVO = (AllocateOutHVO) baseHVO
							.clone();
		...
		if (!billList.isEmpty()) {
			...
			// }
			returnBills = insert(billList.toArray(new AggAllocateOutVO[0]),
					null);
		}
		return returnBills;
	}

	protected List<AggAllocateOutVO> getMonthHoursShare(
			Map<String, UFDouble> psnMonthWkHourMap,
			Map<String, UFDouble> psnMonthWkHours,
			Map<String, UFDouble> psnLeaveHoursMap,
			Map<String, Map<String, UFDouble>> psnPjWkHoursMap,
		...
		if (null == psnLeaveHoursMap) {
			...
			Map<String, UFDouble> projWkHoursMap = psnPjWkHoursMap
					.get(pk_psndoc);
			if (MapUtils.isEmpty(projWkHoursMap)) {
				...
				UFDouble projWkHours = calculateSum(projWkHoursMap.values()
				...
				if (projWkHours.compareTo(psnCalMonthWkHours) < 0) {
					for (String projectPK : projWkHoursMap.keySet()) {
						...
						UFDouble shareMny = wkHours.multiply(itemMny).div(
								fullMonthWkHours);
						shareMny = new UFDouble(getRoundNumber(shareMny,
								isTWArea));
						AggAllocateOutVO aggVO = new AggAllocateOutVO();
						AllocateOutHVO hvo = (AllocateOutHVO) baseHVO.clone();
					...
					costcenterMny = new UFDouble(getRoundNumber(costcenterMny,
							isTWArea));
		...
		if (null != projWkHoursMap && !projWkHoursMap.isEmpty()) {
			...
			if (fullMonthWkHours.compareTo(projWkHours) > 0) {
				...
				try {
					PsndocVO psndocVO = getAppDao().retrieveByPK(
					...
					ExceptionUtils.wrappBusinessException(ResHelper.getString(
							...
							"0allcate-ui-000018",
							null,
							new String[] { psndocVO.getCode(),
									projWkHours.toString(),
									psnMonthWkHours.toString() }));
	...
	protected List<AggAllocateOutVO> getProjSalaryShare(
			Map<String, List<ProjSalaryHVO>> psnProjSaVosMap,
			AllocateOutHVO baseHVO) throws BusinessException {
		List<AggAllocateOutVO> list = new ArrayList<AggAllocateOutVO>();
		List<ProjSalaryHVO> tempList = psnProjSaVosMap.get(baseHVO
				.getPk_psndoc());
	...
	protected List<AggAllocateOutVO> getProjOverTimeShare(
			Map<String, List<OvertimeRegVO>> psnPjOtHoursMap,
			AllocateOutHVO baseHVO) throws BusinessException {
		List<AggAllocateOutVO> list = new ArrayList<AggAllocateOutVO>();
		List<OvertimeRegVO> tempList = psnPjOtHoursMap.get(baseHVO
				.getPk_psndoc());
		if (null != tempList && !tempList.isEmpty()) {
			UFDouble itemMny = (StringUtils.isEmpty(baseHVO.getDef2()) ? UFDouble.ZERO_DBL
					: new UFDouble(baseHVO.getDef2()));
			...
			for (OvertimeRegVO vo : tempList) {
				...
				UFDouble overHour = (null == vo.getOvertimehour() ? UFDouble.ZERO_DBL
						: vo.getOvertimehour());
			...
			if (!pjOtHoursMap.isEmpty()) {
				...
				for (String project : pjOtHoursMap.keySet()) {
					...
					UFDouble shareMny = otHours.multiply(itemMny).div(
							totalOtHours);
					if (len == 1) {
	...
	protected WaClassItemVO[] qryClassItems(WaLoginContext context)
			throws DAOException {
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer
				.append("select wa_item.itemkey, wa_item.iitemtype, wa_item.ifldwidth,wa_item.category_id ");
		sqlBuffer
				.append(", wa_classitem.iflddecimal, wa_item.defaultflag, wa_item.iproperty, wa_classitem.* ");
		sqlBuffer.append(", 'Y' editflag from wa_classitem, wa_item ");
		sqlBuffer
				.append(" where wa_classitem.pk_wa_item = wa_item.pk_wa_item and wa_classitem.pk_wa_class = ? ");
		sqlBuffer
				.append(" and wa_classitem.cyear = ?  and wa_classitem.cperiod = ?  ");
		sqlBuffer.append(" and wa_classitem.pk_org='")
				.append(context.getPk_org()).append("' ");
		sqlBuffer.append(" and wa_item.mid = 'N' and wa_classitem.dr <> 1 ");
		sqlBuffer
				.append(" and wa_classitem.def1 in (select bd_defdoc.pk_defdoc from bd_defdoc ");
		sqlBuffer
				.append(" inner join bd_defdoclist on bd_defdoc.pk_defdoclist=bd_defdoclist.pk_defdoclist ");
		sqlBuffer
				.append(" and bd_defdoclist.code='WITSCS' and upper(bd_defdoc.code) in ('A','B','C','D','Z') )");
		sqlBuffer.append(" order by icomputeseq");

		SQLParameter parameter = WherePartUtil.getCommonParameter(context
				.getWaLoginVO());
		WaClassItemVO[] classItemVOs = ((WaClassItemVO[]) getAppDao()
				.executeQueryVOs(sqlBuffer.toString(), parameter,
						WaClassItemVO.class));
		return classItemVOs;
	}
	...
	protected <T extends SuperVO> Map<String, T> queryBasicInfo(
			Class<T> voClass, String condition, String[] fields)
			throws DAOException {
	...
	protected <T extends SuperVO> Map<String, List<T>> queryUnionVOList(
			Class<T> voClass, String condition, String[] fields)
			throws DAOException {
		T[] vos = getAppDao().retrieveByClause(voClass, condition);
	...
	protected String getPeriodWhere(WaLoginContext context) {
		...
		periodBuf.append(" and timeyear='")
				.append(context.getWaLoginVO().getPeriodVO().getCaccyear())
				.append("' ");
		periodBuf.append(" and timemonth='")
				.append(context.getWaLoginVO().getPeriodVO().getCaccperiod())
				.append("' ");
	...
	protected String getProjCodeWhere(WaLoginContext context) {
		...
		where.append(" and pk_wa_calss='").append(context.getClassPK())
				.append("' ");
		where.append(" and cperiod='")
				.append(context.getWaLoginVO().getPeriodVO().getCyear())
				.append(context.getWaLoginVO().getPeriodVO().getCperiod())
				.append("' ");
	...
	protected String getProjOverTimeWhere(WaLoginContext context,
			PeriodVO periodVO) {
		StringBuilder where = new StringBuilder();
		where.append(" pk_org='").append(context.getPk_org()).append("' ");
		where.append(" and approve_time>='")
				.append(periodVO.getBegindate().toStdString())
				.append(" 00:00:00' ");
		where.append(" and approve_time<='")
				.append(periodVO.getEnddate().toStdString())
				.append(" 23:59:59' ");
	...
	protected Map<String, UFDouble> qryWkHoursFromPsnCalendar(
			WaLoginContext context, PeriodVO periodVO, String[] pk_psndocs)
			throws BusinessException {
		Map<String, UFDouble> psnWkHoursMap = new HashMap<String, UFDouble>();

		IShiftQueryService shiftService = NCLocator.getInstance().lookup(
				IShiftQueryService.class);
		// Map<String, ShiftVO> shiftPKMap =
		// shiftService.queryShiftVOMapByOrg(context.getPk_org());
		Map<String, ShiftVO> shiftPKMap = shiftService
				.queryShiftVOMapByHROrg(context.getPk_org());
		IPsnCalendarQueryService psnCalendarService = NCLocator.getInstance()
				.lookup(IPsnCalendarQueryService.class);
		Map<String, Map<UFLiteralDate, AggPsnCalendar>> psnDateCalendarMap = psnCalendarService
				.queryCalendarVOByPsnInSQLForProcess(context.getPk_org(),
						periodVO.getBegindate(), periodVO.getEnddate(),
						pk_psndocs);
		if (null != psnDateCalendarMap && !psnDateCalendarMap.isEmpty()) {
			for (String pk_psndoc : psnDateCalendarMap.keySet()) {
				...
				Map<UFLiteralDate, AggPsnCalendar> dateCalandarMap = psnDateCalendarMap
						.get(pk_psndoc);
				if (null != shiftPKMap && !shiftPKMap.isEmpty()
						&& null != dateCalandarMap
					...
					for (UFLiteralDate date : dateCalandarMap.keySet()) {
						...
						if (null == hvo
								|| StringUtils.isEmpty(hvo.getPk_shift())
								|| null == shiftPKMap.get(hvo.getPk_shift())
								|| hvo.getPk_shift().equals(ShiftVO.PK_GX)) {
							continue;
						}
						ShiftVO shiftVO = shiftPKMap.get(hvo.getPk_shift());
						UFDouble shiftHour = (null == shiftVO ? UFDouble.ZERO_DBL
								: shiftVO.getGzsj());
	...
	protected Map<String, UFDouble> qryMonthWkHoursByOrgCalander(
			WaLoginContext context, PeriodVO periodVO, String[] psndocPks)
			throws BusinessException {
		...
		Map<String, UFDouble> psnWkHoursMap = new HashMap<String, UFDouble>();
		IHRHolidayQueryService holidayService = NCLocator.getInstance().lookup(
				IHRHolidayQueryService.class);
		Map<String, Map<String, Integer>> psnWOrkDayTypeMap = holidayService
				.queryPsnWorkDayTypeInfo(context.getPk_org(), psndocPks,
						periodVO.getBegindate(), periodVO.getEnddate());
		if (null != psnWOrkDayTypeMap && !psnWOrkDayTypeMap.isEmpty()) {
			for (String pk_psndoc : psnWOrkDayTypeMap.keySet()) {
				int wkdays = 0;
				Map<String, Integer> dateWorkTypeMap = psnWOrkDayTypeMap
						.get(pk_psndoc);
				if (null != dateWorkTypeMap && !dateWorkTypeMap.isEmpty()) {
					for (String date : dateWorkTypeMap.keySet()) {
						...
						if (null != wkType
								&& wkType.intValue() == HolidayVO.DAY_TYPE_WORKDAY) {
							wkdays++;
						}
					}
				}
	...
	protected Map<String, UFDouble> qryPsnLeaveHours(WaLoginContext context,
			PeriodVO periodVO) throws DAOException {
		...
		sql.append(" and approve_time >='")
				.append(periodVO.getBegindate().toStdString())
				.append(" 00:00:00' ");
		sql.append(" and approve_time <='")
				.append(periodVO.getEnddate().toStdString())
				.append(" 23:59:59' ");
		sql.append(" and leavebegintime >='")
				.append(periodVO.getBegindate().toStdString())
				.append(" 00:00:00' ");
		sql.append(" and leavebegintime <='")
				.append(periodVO.getEnddate().toStdString())
				.append(" 23:59:59' ");
		sql.append(" and leaveendtime >='")
				.append(periodVO.getBegindate().toStdString())
				.append(" 00:00:00' ");
		sql.append(" and leaveendtime <='")
				.append(periodVO.getEnddate().toStdString())
		...
		Map<String, UFDouble> psnLeaveHoursMap = (Map<String, UFDouble>) getAppDao()
	...
	protected Map<String, Map<String, UFDouble>> qryPsnWkHoursOutersys(
			WaLoginContext context, PeriodVO periodVO, String[] psndocPks)
			throws BusinessException {
		StringBuilder sql = new StringBuilder(
				"select pk_psndoc,pk_project,sum(isnull(workhour,0)) as workhour from (");
		...
		sql.append(" and pk_psndoc in ").append(
				InSqlManager.getInSQLValue(psndocPks));
		...
		Map<String, Map<String, UFDouble>> psnPjWkHoursMap = (Map<String, Map<String, UFDouble>>) getAppDao()
				.getBaseDao().executeQuery(sql.toString(), param,
						new ResultSetProcessor() {
							...
							public Object handleResultSet(ResultSet rs)
								...
								while (rs.next()) {
									...
									UFDouble workhour = (rs.getString(3) == null ? UFDouble.ZERO_DBL
											: new UFDouble(rs.getString(3)));
									Map<String, UFDouble> pjWkHourMap = map
											.get(pk_psndoc);
									if (null == pjWkHourMap) {
	...
	protected <T extends SuperVO> Map<String, List<T>> queryMapListInfo(
			Class<T> voClass, String condition, String[] fields)
			throws DAOException {
	...
	@Override
	public List<AllocateCsvVO> transferToCsvInfo(Object[] datas)
			throws BusinessException {
		return transferToCsv(datas);
	}

	@Override
	public List<AllocateCsvVO> transferToCsvInfo(AggAllocateOutVO[] bills)
			throws BusinessException {
	 ...
	 */
	protected List<AllocateCsvVO> transferToCsv(Object[] datas)
			throws BusinessException {
		...
		if (!ArrayUtils.isEmpty(datas)) {
			...
			classItemBuf.append(" pk_wa_classitem in ").append(
					InSqlManager.getInSQLValue(classItemPk));
			Map<String, WaClassItemVO> itemVOMap = queryBasicInfo(
					WaClassItemVO.class, classItemBuf.toString(),
					new String[] { WaClassItemVO.PK_WA_CLASSITEM });
			StringBuilder filerItemWhere = new StringBuilder();
			filerItemWhere
					.append(" pk_wa_item in (select pk_wa_item  from wa_classitem where ");
			filerItemWhere.append(classItemBuf).append(") ");
			Map<String, WaItemVO> pkItemVOMap = queryBasicInfo(WaItemVO.class,
					filerItemWhere.toString(),
			...
			Map<String, DefdocVO> projVOMap = queryBasicInfo(DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_PROJ, null),
					new String[] { DefdocVO.PK_DEFDOC });
			// 借方科目档案
			Map<String, DefdocVO> debitAccVOMap = queryBasicInfo(
					DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_DEBIT_ACC, null),
					new String[] { DefdocVO.PK_DEFDOC });
			// 借方供应商档案
			Map<String, DefdocVO> debitVenVOMap = queryBasicInfo(
					DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_DEBIT_VEN, null),
					new String[] { DefdocVO.PK_DEFDOC });
			// 贷方科目档案
			Map<String, DefdocVO> creditAccVOMap = queryBasicInfo(
					DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_CREDIT_ACC, null),
					new String[] { DefdocVO.PK_DEFDOC });
			// 贷方供应商档案
			Map<String, DefdocVO> creditVenVOMap = queryBasicInfo(
					DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_CREDIT_VEN, null),
					new String[] { DefdocVO.PK_DEFDOC });
			// 社保厂商档案
			Map<String, DefdocVO> ssmfVOMap = queryBasicInfo(DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_SSMF, null),
					new String[] { DefdocVO.PK_DEFDOC });

			// 社保供應商代碼
			Map<String, DefdocVO> smvenVOMap = queryBasicInfo(DefdocVO.class,
					getDefdocCondition(WaConstant.DEF_CODE_BMVEN, null),
					new String[] { DefdocVO.PK_DEFDOC });

			// 员工薪资档案成本中心
			StringBuilder costcenterBuf = new StringBuilder();
			costcenterBuf.append(" pk_costcenter in ").append(
					InSqlManager.getInSQLValue(costCenterPk));
			Map<String, CostCenterVO> costCenterVOMap = queryBasicInfo(
					CostCenterVO.class, costcenterBuf.toString(),
					new String[] { CostCenterVO.PK_COSTCENTER });
			// 人员基本信息
			StringBuilder psnBuf = new StringBuilder();
			psnBuf.append(" pk_psndoc in ").append(
					InSqlManager.getInSQLValue(psndocPk));
			Map<String, PsndocVO> psnVOMap = queryBasicInfo(PsndocVO.class,
					psnBuf.toString(), new String[] { PsndocVO.PK_PSNDOC });
			// 社保信息
			AllocateOutHVO alloHvo = ((AggAllocateOutVO) datas[0])
					.getParentVO();
			StringBuilder bmdataBuf = new StringBuilder();
			bmdataBuf.append(" dr <> 1 and pk_org='")
					.append(alloHvo.getPk_org()).append("' ");
			bmdataBuf
					.append(" and pk_psnjob in (select pk_psnjob from hi_psnjob where begindate <= '"
							+ getLastDayOfMonth(
									alloHvo.getCperiod().substring(0, 4),
									alloHvo.getCperiod().substring(4, 6))
							...
							+ getFirstDayOfMonth(
									...
									alloHvo.getCperiod().substring(4, 6))
									.toString().substring(0, 10) + "') ");
			Map<String, List<BmDataVO>> bmdataVOMap = queryUnionVOList(
					BmDataVO.class, bmdataBuf.toString(), new String[] {
							BmDataVO.PK_PSNDOC, BmDataVO.PK_PSNJOB });
			// 险种信息
			StringBuilder bmClassBuf = new StringBuilder(
					" pk_bm_class in (select pk_bm_class from bm_data where ");
			bmClassBuf.append(bmdataBuf).append(" ) ");
			Map<String, BmClassVO> bmClassVOMap = queryBasicInfo(
					BmClassVO.class, bmClassBuf.toString(),
			...
			for (Object obj : datas) {
				if (obj instanceof AggAllocateOutVO) {
					...
					WaItemVO waItemVO = (null != itemVO ? pkItemVOMap
							.get(itemVO.getPk_wa_item()) : null);
					if (itemVO == null || waItemVO == null) {
						continue;
					}
					DefdocVO shareDefVO = shareVOMap.get(itemVO.getDef1());
					String shareCode = (shareDefVO == null ? "" : shareDefVO
							.getCode());
					String projtext = hvo.getCperiod()
							+ MultiLangHelper.getName(itemVO);
					UFDouble shareMny = (hvo.getDef2() == null ? UFDouble.ZERO_DBL
							: new UFDouble(hvo.getDef2()));
					CostCenterVO costCenVO = costCenterVOMap.get(hvo
							.getPk_costcenter());
					String costcenter = (costCenVO == null ? "" : costCenVO
							.getCccode());
					String orderCode = "";
					if (shareCode.equals(WaConstant.SHARE_MONTH_WORKHOURS)
							...
							|| shareCode
									.equals(WaConstant.SHARE_PROJ_OVERHOURS)) {
						if (StringUtils.isNotEmpty(hvo.getDef1())
								&& null != projVOMap.get(hvo.getDef1())) {
					...
					DefdocVO debitAccDefVO = debitAccVOMap
							.get(itemVO.getDef2());
					String debitAccCode = (debitAccDefVO == null ? ""
							: debitAccDefVO.getCode());
					DefdocVO debitVenDefVO = debitVenVOMap
							.get(itemVO.getDef3());
					String debitVenDefCode = (debitVenDefVO == null ? ""
							: debitVenDefVO.getCode());
					String debitVenCode = getVenderCode(debitVenDefCode, hvo,
							psnVOMap, ssmfVOMap, bmdataVOMap, waItemVO,
							smvenVOMap, bmClassVOMap);

					AllocateCsvVO debitCsvVO = new AllocateCsvVO(debitAccCode,
							shareMny, costcenter, orderCode, "", projtext,
							debitVenCode);
					setDirecMny(debitCsvVO, waItemVO, false);

					if (null != unionIDMap.get(debitCsvVO.getUnionID())) {
						AllocateCsvVO aCsvVO = unionIDMap.get(debitCsvVO
								.getUnionID());
						aCsvVO.setVouchmny(aCsvVO.getVouchmny().add(
								debitCsvVO.getVouchmny()));
					} else {
						unionIDMap.put(debitCsvVO.getUnionID(), debitCsvVO);
					}

					DefdocVO creditAccDefVO = creditAccVOMap.get(itemVO
							.getDef4());
					String creditAccCode = (creditAccDefVO == null ? ""
							: creditAccDefVO.getCode());
					DefdocVO creditVenDefVO = creditVenVOMap.get(itemVO
							.getDef5());
					String creditVenDefCode = (creditVenDefVO == null ? ""
							: creditVenDefVO.getCode());
					String creditVenCode = getVenderCode(creditVenDefCode, hvo,
							psnVOMap, ssmfVOMap, bmdataVOMap, waItemVO,
							smvenVOMap, bmClassVOMap);
					...
					AllocateCsvVO creditCsvVO = new AllocateCsvVO(
							creditAccCode, shareMny, "", "", "", projtext,
							creditVenCode);
					setDirecMny(creditCsvVO, waItemVO, true);

					if (null != unionIDMap.get(creditCsvVO.getUnionID())) {
						AllocateCsvVO aCsvVO = unionIDMap.get(creditCsvVO
								.getUnionID());
						aCsvVO.setVouchmny(aCsvVO.getVouchmny().add(
								creditCsvVO.getVouchmny()));
					} else {
	...
	private void setDirecMny(AllocateCsvVO csvVO, WaItemVO waItemVO,
			boolean isCredit) {
	...
	private String getVenderCode(String venCode, AllocateOutHVO hvo,
			Map<String, PsndocVO> psnVOMap, Map<String, DefdocVO> ssmfVOMap,
			Map<String, List<BmDataVO>> bmdataVOMap, WaItemVO waItemVO,
			Map<String, DefdocVO> smvenVOMap,
			Map<String, BmClassVO> bmClassVOMap) {
		String retCode = "";
		...
		if (venCode.toUpperCase().equals(WaConstant.VENDER_SIFI)) {
			bmdataVO = (null != bmdataList && !bmdataList.isEmpty()) ? bmdataList
					.get(0) : null;
	...
	private BmDataVO getDataVOByItem(List<BmDataVO> bmdataList,
			WaItemVO waItemVO, Map<String, BmClassVO> bmClassVOMap) {
		if ((null == bmdataList || bmdataList.isEmpty()) || null == waItemVO
				|| MapUtils.isEmpty(bmClassVOMap)) {
			return null;
		}
		...
		if (StringUtils.isNotBlank(itemName)) {
			for (BmDataVO dataVO : bmdataList) {
				if (null == dataVO
						|| StringUtils.isEmpty(dataVO.getPk_bm_class())) {
					continue;
				}
				...
				if (itemName.contains("大病")) {
					...
					if (itemName.substring(0, 4).indexOf(
							bmClass.getName().trim().substring(0, 4)) != -1) {
						return dataVO;
					}
				}
			}
	...
	private String getRoundNumber(UFDouble amt, boolean isTWArea)
			throws BusinessException {
		String retStr = "";
		if (isTWArea) {
