...
public class PsndocSubInfoService4JFSImpl implements IPsndocSubInfoService4JFS {
	...
	private RangeTableAggVO[] getRangeTables(String pk_org, UFDate date)
	...
	private void writeBackPsndoc(Map<String, GroupInsDetailVO> calcRst)
		...
		if (calcRst.size() > 0) {
			...
			for (Entry<String, GroupInsDetailVO> rst : calcRst.entrySet()) {
				...
				// 重新整理recordnum
				strSQL = getRecountRecordnumSQL(PsndocDefTableUtil
	...
	private String getRecountRecordnumSQL(String tableName)
			throws BusinessException {
		String strSQL;
		strSQL = "";
		strSQL += " UPDATE ";
		...
		strSQL += "                         WHERE ";
		strSQL += "                             def.pk_psndoc=" + tableName
				+ ".pk_psndoc) - row_number() over ";
		strSQL += "                     (partition BY pk_psndoc ORDER BY begindate) rowno ";
		strSQL += "                 FROM ";
		strSQL += "                     " + tableName + ") tmp ";
		strSQL += "         WHERE ";
		strSQL += "             tmp.pk_psndoc_sub=" + tableName
				+ ".pk_psndoc_sub) ";
		return strSQL;
	}
			...
			for (Entry<String, Map<String, GroupInsSettingVO>> grpSets : groupSettings
				...
				for (Entry<String, GroupInsSettingVO> psnSet : grpSets
					...
					boolean isCalc = (psnSet.getValue().getdStartDate()
							.isSameDate(curMonthEnd) || psnSet.getValue()
							.getdStartDate().before(curMonthEnd))
							&& (psnSet.getValue().getdEndDate()
									.isSameDate(curMonthEnd) || psnSet
									.getValue().getdEndDate()
									.after(curMonthEnd));
	...
	public void dismissPsnNHI(String pk_org, String pk_psndoc,
		...
		strSQL = "UPDATE " + PsndocDefTableUtil.getPsnLaborTablename()
				+ " SET glbdef15='" + enddate.toString()
				+ "' WHERE ISNULL(glbdef15, '9999-12-31') > '"
				+ enddate.toString() + "' AND dr=0 AND pk_psndoc = '"
				+ pk_psndoc + "'";
		this.getBaseDao().executeUpdate(strSQL);
	...
	public void renewRangeEx(String pk_org, String[] pk_psndocs,
		...
		if (pk_psndocs != null && pk_psndocs.length > 0) {
			...
			// 取指定期間有效薪資項目
			getValidWaItemByPeriod(pk_wa_class, startperiod, endperiod,
					periodItem);
			for (String pk_psndoc : pk_psndocs) {
				...
				// 取指定期間平均薪資
				UFDouble avgSalary = getAvgSalary(pk_wa_class, periodItem,
						pk_psndoc);
				if (avgSalary == null) {
					continue;
				}
				...
				if (avgSalary.equals(UFDouble.ZERO_DBL)) {
					IPsndocQueryService psnQry = NCLocator.getInstance()
							.lookup(IPsndocQueryService.class);
					PsndocVO[] vos = psnQry
							.queryPsndocByPks(new String[] { pk_psndoc });
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
							.getNCLangRes().getStrByID("twhr_personalmgt",
									"068J61035-0031", null,
									new String[] { vos[0].getCode() })/*
																	 * 員工 [{0}]
																	 * 在指定的期間範圍內沒有薪資資料
																	 * ，
																	 * 無法計算平均薪資。
				...
				// 處理劳保劳退
				List<PsndocDefVO> subInfoLines = dealLaborInfoset(
						effectivedate, pk_psndoc, avgSalary, updateSQLs);

				// 處理健保
				dealHealInfoset(effectivedate, pk_psndoc, avgSalary,
						updateSQLs, subInfoLines);
	...
	private void checkExistDateValid(String[] pk_psndocs, UFDate effectivedate)
			throws BusinessException {
		...
		strSQL = "select pk_psndoc from "
				+ PsndocDefTableUtil.getPsnLaborTablename()
				+ " where pk_psndoc in (" + strSQL + ") and begindate >= '"
				+ effectivedate.toDate().toString() + "'";
		String existsPsn = (String) this.getBaseDao().executeQuery(strSQL,
				new ColumnProcessor());
		if (!StringUtils.isEmpty(existsPsn)) {
			PsndocVO[] vos = psnQry
					.queryPsndocByPks(new String[] { existsPsn });
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("twhr_personalmgt",
							"068J61035-0029", null,
							new String[] { vos[0].getCode() })/*
		...
		strSQL = strSQL.replace(PsndocDefTableUtil.getPsnLaborTablename(),
				PsndocDefTableUtil.getPsnHealthTablename());
		existsPsn = (String) this.getBaseDao().executeQuery(strSQL,
				new ColumnProcessor());
		if (!StringUtils.isEmpty(existsPsn)) {
			PsndocVO[] vos = psnQry
					.queryPsndocByPks(new String[] { existsPsn });
	...
	private void checkPeriodValid(String startperiod, String endperiod)
			throws BusinessException {
		// 校驗薪資期間有效性
		if ((new UFDate(startperiod + "-01")).after(new UFDate(endperiod
				+ "-01"))) {
			throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
					.getNCLangRes().getStrByID("twhr_personalmgt",
	...
	private void dealHealInfoset(UFDate effectivedate, String pk_psndoc,
			UFDouble avgSalary, List<String> updateSQLs,
			List<PsndocDefVO> subInfoLines) throws BusinessException,
			DAOException {
		String strSQL;
		strSQL = "select * from " + PsndocDefTableUtil.getPsnHealthTablename()
				+ " where pk_psndoc='" + pk_psndoc
				+ "' and dr=0 and begindate <= '" + effectivedate.toString()
				+ "' and isnull(enddate, '9999-12-31') >='"
				+ effectivedate.toString() + "' and glbdef14='Y'";
		List<Map> healResult = (List<Map>) this.getBaseDao().executeQuery(
				strSQL, new MapListProcessor());

		if (healResult != null && healResult.size() > 0) {
			List<PsndocDefVO> newSubLines = new ArrayList<PsndocDefVO>();
			for (Map healset : healResult) {
				UFDouble originHealLevel = healset.get("glbdef16") == null ? UFDouble.ZERO_DBL
						: new UFDouble((BigDecimal) healset.get("glbdef16"));
				RangeLineVO newRangeLine = findRangeLine(rangeTables,
						RangeTableTypeEnum.NHI_RANGETABLE.toIntValue(),
						avgSalary);
				UFDouble newHealLevel = newRangeLine.getRangeupper().equals(
						UFDouble.ZERO_DBL) ? newRangeLine.getRangelower()
						.sub(1) : newRangeLine.getRangeupper();
				if (!originHealLevel.equals(newHealLevel)) {
					// 更新原有健保行
					updateSQLs.add("update "
							+ PsndocDefTableUtil.getPsnHealthTablename()
							+ " set enddate = '"
							+ effectivedate.getDateBefore(1).toLocalString()
							+ "', lastflag = 'N' where pk_psndoc_sub='"
							+ (String) healset.get("pk_psndoc_sub") + "'");
					updateSQLs
							.add("update "
									+ PsndocDefTableUtil
											.getPsnHealthTablename()
									+ " set recordnum=isnull(recordnum,0)+1 where pk_psndoc = '"
									+ pk_psndoc + "'");
					// 新增新的健保行
					PsndocDefVO newVO = getNewSubInfoLineByOrigin(healset,
							effectivedate, avgSalary, newHealLevel, null);
	...
	private List<PsndocDefVO> dealLaborInfoset(UFDate effectivedate,
			String pk_psndoc, UFDouble avgSalary, List<String> updateSQLs)
			throws BusinessException, DAOException {
		String strSQL = "select * from "
				+ PsndocDefTableUtil.getPsnLaborTablename()
				+ " where pk_psndoc = '" + pk_psndoc
		...
		Map<String, Object> laborResult = (Map<String, Object>) this
		...
		if (laborResult != null && !laborResult.isEmpty()) {
			UFDouble originLaborLevel = laborResult.get("glbdef4") == null ? UFDouble.ZERO_DBL
					: new UFDouble((BigDecimal) laborResult.get("glbdef4"));
			UFDouble originRetireLevel = laborResult.get("glbdef7") == null ? UFDouble.ZERO_DBL
					: new UFDouble((BigDecimal) laborResult.get("glbdef7"));
			RangeLineVO newRangeLine = findRangeLine(rangeTables,
					RangeTableTypeEnum.LABOR_RANGETABLE.toIntValue(), avgSalary);
			UFDouble newLaborLevel = newRangeLine.getRangeupper().equals(
					UFDouble.ZERO_DBL) ? newRangeLine.getRangelower().sub(1)
					: newRangeLine.getRangeupper();
			newRangeLine = findRangeLine(rangeTables,
					RangeTableTypeEnum.RETIRE_RANGETABLE.toIntValue(),
					avgSalary);
			UFDouble newRetireLevel = newRangeLine.getRangeupper().equals(
					UFDouble.ZERO_DBL) ? newRangeLine.getRangelower().sub(1)
					: newRangeLine.getRangeupper();
			if (!originLaborLevel.equals(newLaborLevel)
					|| !originRetireLevel.equals(newRetireLevel)) {
				// 更新原有劳保劳退行
				updateSQLs.add("update "
						+ PsndocDefTableUtil.getPsnLaborTablename()
						+ " set enddate = '"
				...
				// 新增新的劳保劳退行
				subInfoLines
						.add(getNewSubInfoLineByOrigin(laborResult,
								effectivedate, avgSalary, newLaborLevel,
								newRetireLevel));
			}
		}
		return subInfoLines;
	...
	private UFDouble getAvgSalary(String pk_wa_class,
			Map<String, List<String>> periodItem, String pk_psndoc)
			throws DAOException {
		String strSQL;
		String strTotelSQL = "";
		...
		if (periodItem.size() > 0) {
			for (Entry<String, List<String>> entry : periodItem.entrySet()) {
				...
				strSQL = "select " + keys + " nhisalary from wa_data "
						+ " where cyear||'-'||cperiod = '" + entry.getKey()
						+ "' and pk_psndoc = '" + pk_psndoc
						+ "' and pk_wa_class = '" + pk_wa_class + "' and dr=0 ";
			...
			Object rstObj = this.getBaseDao().executeQuery(
					"select sum(nhisalary) nhisalary from (" + strTotelSQL
							+ ") A", new ColumnProcessor());
			if (rstObj != null) {
				sumRst = new UFDouble((BigDecimal) rstObj);
			} else {
				Logger.error("SKIP RENEW: psn group insurance salary sum is null: "
						+ pk_psndoc);
				return null;
			}
		}
	...
	private void getValidWaItemByPeriod(String pk_wa_class, String startperiod,
			String endperiod, Map<String, List<String>> periodItem)
			throws DAOException {
		String strSQL = "select cyear||'-'||cperiod cyearperiod, itemkey from wa_classitem where pk_wa_item in ("
				...
				+ " where tw.isnhiitem_30 = 'Y') and pk_wa_class = '"
				+ pk_wa_class
				+ "' and cyear||'-'||cperiod >= '"
				+ startperiod
				+ "' and cyear||'-'||cperiod <= '" + endperiod + "' ";
		List<Map> rst = (List<Map>) this.getBaseDao().executeQuery(strSQL,
	...
	private void getValidWaItemByPeriod4GroupIns(String pk_org,
			String pk_wa_class, String startperiod, String endperiod,
	...
	private PsndocDefVO getNewSubInfoLineByOrigin(
			Map<String, Object> originValues, UFDate effectivedate,
			UFDouble avgSalary, UFDouble newMainLevel, UFDouble newRetireLevel)
			throws BusinessException {
		if (newRetireLevel != null) {
			...
			newLaborVO.setBegindate(new UFLiteralDate(effectivedate.asBegin()
					.toString()));
			newLaborVO
					.setEnddate(originValues.get("enddate") == null ? null
							: new UFLiteralDate(originValues.get("enddate")
									.toString()));
			newLaborVO
					.setAttributeValue("glbdef1", originValues.get("glbdef1"));
			...
			newLaborVO
					.setAttributeValue("glbdef5", originValues.get("glbdef5"));
			newLaborVO.setAttributeValue("glbdef6", UFBoolean.FALSE);
			newLaborVO.setAttributeValue("glbdef7", newRetireLevel);
			newLaborVO
					.setAttributeValue("glbdef8", originValues.get("glbdef8"));
			newLaborVO
					.setAttributeValue("glbdef9", originValues.get("glbdef9"));
			newLaborVO.setAttributeValue("glbdef10",
					originValues.get("glbdef10"));
			newLaborVO.setAttributeValue("glbdef11",
					originValues.get("glbdef11"));
			newLaborVO.setAttributeValue("glbdef12",
					originValues.get("glbdef12"));
			newLaborVO.setAttributeValue("glbdef13",
					originValues.get("glbdef13"));
			// 勞退開始日期處理
			UFDate originStartDate = originValues.get("glbdef14") == null ? null
					: new UFDate((String) originValues.get("glbdef14"));
			UFDate originEndDate = originValues.get("glbdef15") == null ? null
					: new UFDate((String) originValues.get("glbdef15"));
			if (originStartDate == null
					|| originStartDate.after(effectivedate)
					|| (originEndDate != null && originEndDate
							.before(effectivedate))) {
				newLaborVO.setAttributeValue("glbdef14", originStartDate);
			} else {
				newLaborVO.setAttributeValue("glbdef14",
						effectivedate.asBegin());
			}
			// 勞退結束日期
			newLaborVO.setAttributeValue("glbdef15",
					originValues.get("glbdef15"));
			//
			return newLaborVO;
		} else {
			PsndocDefVO newHealVO = PsndocDefUtil.getPsnHealthVO();
			...
			if (savedVOs != null && savedVOs.length > 0) {
				for (PsndocDefVO vo : savedVOs) {
					if (vo.getClass().equals(
						...
						DefdocVO defvo = getHIPsnTypeDefVO((String) vo
								.getAttributeValue("glbdef1"));
						vo.setAttributeValue("glbdef16", null);
						vo.setAttributeValue("glbdef17",
						...
						// 投保人或眷属姓名 glbdef1
						int suffix = -1;
						LanguageVO[] langs = MultiLangContext.getInstance()
								.getEnableLangVOs();
						...
						String psnName = "";
						if (suffix != -1) {
							psnName = (String) psnVO
									.getAttributeValue(PsndocVO.NAME
						...
						vo.setAttributeValue(
								"glbdef17",
								(defvos == null || defvos.size() == 0) ? "~"
										: ((DefdocVO) defvos.toArray()[0])
												.getPk_defdoc());
					}
				}
			}
	...
	private DefdocVO getHIPsnTypeDefVO(String pk_psntype)
			throws BusinessException {
		// ssx added for 勞健退申報
		// 提繳身份別
	...
	public void generateGroupIns(String pk_org, String[] pk_psndocs)
		...
		for (String pk_psndoc : pk_psndocs) {
			PsndocDefVO[] vos = service.queryByCondition(PsndocDefUtil
					.getGroupInsuranceVO().getClass(), " pk_psndoc='"
					+ pk_psndoc + "' ");
			if (vos != null && vos.length > 0) {
		...
		// 取團保費率表
		IGroupinsuranceMaintain settingSrv = NCLocator.getInstance().lookup(
				IGroupinsuranceMaintain.class);
		...
		for (String pk_psndoc : pk_psndocs) {
			...
			UFDouble grpSum = getGroupInsWadocBaseSalaryByPsnDate(pk_org,
					pk_psndoc, earlist);
			...
			// 新建團保資料
			PsndocDefVO[] savedVOs = getNewGroupInsVO(pk_psndoc, pk_org,
					psnSum, setting, psnVO);
	...
	@Override
	public UFDouble getGroupInsWadocBaseSalaryByPsnDate(String pk_org,
			String pk_psndoc, UFLiteralDate salaryDate)
			throws BusinessException {
		// 檢查勞健保參數是否有團保保薪基數加總項目
		List<BaseDocVO> grpWaitems = (List<BaseDocVO>) this.getBaseDao()
				.retrieveByClause(BaseDocVO.class,
						"code like 'GRPSUM%' and pk_org='" + pk_org + "'",
						"code");
		...
		String strSQL = "select sum(nmoney) grpsum from hi_psndoc_wadoc where pk_wa_item in ("
				+ waitems
				+ ")  and pk_org= '"
				+ pk_org
				+ "' and pk_psndoc = '"
		...
		UFDouble grpSum = new UFDouble(String.valueOf(this.getBaseDao()
				.executeQuery(strSQL, new ColumnProcessor())));
		return grpSum;
	}

	private PsndocDefVO[] getNewGroupInsVO(String pk_psndoc, String pk_org,
			UFDouble psnSum, GroupInsuranceSettingVO[] setting,
		...
		for (GroupInsuranceSettingVO setvo : setting) {
			...
			newGrpInsVO.setLastflag(i == setting.length ? UFBoolean.TRUE
					: UFBoolean.FALSE);
			newGrpInsVO.setAttributeValue("glbdef1", psndocVO.getName());
			newGrpInsVO.setAttributeValue("glbdef2", psndocVO.getId());
			newGrpInsVO.setAttributeValue("glbdef3", new UFDate(psndocVO
					.getBirthdate().toDate()));
	...
	public void renewGroupIns(String pk_org, String[] pk_psndocs,
		...
		if (pk_psndocs != null && pk_psndocs.length > 0) {
			...
			Map<String, List<String>> periodItem = new HashMap<String, List<String>>();
			// 取指定期間有效薪資項目
			getValidWaItemByPeriod4GroupIns(pk_org, pk_wa_class, startperiod,
					endperiod, periodItem);
			for (String pk_psndoc : pk_psndocs) {
				if (periodItem.size() == 0) {
					throw new BusinessException(nc.vo.ml.NCLangRes4VoTransl
							.getNCLangRes().getStrByID("twhr_personalmgt",
				...
				if (baseOnAverageSalary) {
					// 取六个月的平均薪资
					baseSalary = getAvgSalary(pk_wa_class, periodItem,
							pk_psndoc);
					if (baseSalary == null) {
						continue;
					}
					...
					// 取定调资
					IPsndocSubInfoService4JFS salaryQry = NCLocator
							.getInstance().lookup(
									IPsndocSubInfoService4JFS.class);
					baseSalary = salaryQry.getGroupInsWadocBaseSalaryByPsnDate(
							pk_org, pk_psndoc,
							new UFLiteralDate(effectivedate.toString()));
				}

				List<String> updateSQLs = new ArrayList<String>();

				// 處理劳保劳退
				List<PsndocDefVO> subInfoLines = dealGroupInsInfoset(
						effectivedate, pk_psndoc, baseSalary, updateSQLs);
	...
	private void checkExistDateValid4GroupIns(String[] pk_psndocs,
			UFDate effectivedate) throws BusinessException {
		IPsndocQueryService psnQry = NCLocator.getInstance().lookup(
				IPsndocQueryService.class);
		String strSQL = "";
		...
		if (!StringUtils.isEmpty(existsPsn)) {
			PsndocVO[] vos = psnQry
					.queryPsndocByPks(new String[] { existsPsn });
			throw new BusinessException("員工 [" + vos[0].getCode()
					+ "] 的團保信息中已存在生效日期之後的有效資料，無法進行同步作業。");
		}
	}

	private List<PsndocDefVO> dealGroupInsInfoset(UFDate effectivedate,
			String pk_psndoc, UFDouble avgSalary, List<String> updateSQLs)
			throws BusinessException {
		String strSQL;
		List<PsndocDefVO> subInfoLines = new ArrayList<PsndocDefVO>();
		...
		strSQL = "select * from "
				...
				+ "' and dr=0 and begindate <= '" + effectivedate.toString()
				+ "' and isnull(enddate, '9999-12-31') >='"
				+ effectivedate.toString() + "' and glbdef7='N'";
		List<Map> groupInsResult = (List<Map>) this.getBaseDao().executeQuery(
				strSQL, new MapListProcessor());

		if (groupInsResult != null && groupInsResult.size() > 0) {
			List<PsndocDefVO> newSubLines = new ArrayList<PsndocDefVO>();
			for (Map groupSet : groupInsResult) {
				UFDouble originSalaryBase = groupSet.get("glbdef6") == null ? UFDouble.ZERO_DBL
						: new UFDouble((BigDecimal) groupSet.get("glbdef6"));
				...
				if (!originSalaryBase.equals(newSalaryBase)) {
					...
					updateSQLs
							.add("update "
									+ PsndocDefTableUtil
											.getGroupInsuranceTablename()
									+ " set recordnum=isnull(recordnum,0)+1 where pk_psndoc = '"
									+ pk_psndoc + "'");
					// 新增新的團保設定行
					PsndocDefVO newVO = getNewSubInfoLineByOrigin4GroupIns(
							groupSet, effectivedate, newSalaryBase);
					newSubLines.add(newVO);
				}
			}
			int rowno = newSubLines.size() - 1;
	...
	private PsndocDefVO getNewSubInfoLineByOrigin4GroupIns(
			Map<String, Object> originValues, UFDate effectivedate,
		...
		newHealVO.setLastflag(UFBoolean.TRUE);
		newHealVO.setBegindate(new UFLiteralDate(effectivedate.toString()));
		newHealVO.setEnddate(originValues.get("enddate") == null ? null
				: new UFLiteralDate(originValues.get("enddate").toString()));
		newHealVO.setAttributeValue("glbdef1", originValues.get("glbdef1"));
		newHealVO.setAttributeValue("glbdef2", originValues.get("glbdef2"));
		newHealVO.setAttributeValue("glbdef3", originValues.get("glbdef3"));
		newHealVO.setAttributeValue("glbdef4", originValues.get("glbdef4"));
