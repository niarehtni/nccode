...
//			check(vos[0].getPk_org(), vos);//保存前的提示性校验中已经校验过了，为了效率此处不再校验
		...
		for(LeaveRegVO vo : vos) {
			...
			if(psndocVO == null)
				throw new BusinessException(ResHelper.getString("6017psndoc","06017psndoc0131"/*@res "保存失败,人员{0}的考勤档案时间范围不包含该时间段"*/,psndocService.getPsnName(vo.getPk_psndoc())));
			// 设置管理组织
...
//		validatePeriodSeal(vo.getPk_org(),beginDate,endDate,false);
	...
	private void validatePeriodSeal(String pk_org,UFLiteralDate beginDate,UFLiteralDate endDate,boolean isApply) throws BusinessException{
		...
		if(vos==null||vos.length<1){
			String msg = isApply?ResHelper.getString("6017leave","06017leave0219")/*@res "未获得指定申请日期段内的考勤期间!"*/:
				ResHelper.getString("6017leave","06017leave0220")/*@res "未获得指定实际日期段内的考勤期间!"*/;
			throw new BusinessException(msg);
		}
		//MOD 取消封存限制 James
//		for(PeriodVO periodVO : vos){
//			if(periodVO.getSealflag()!=null&&periodVO.getSealflag().booleanValue()) {
//				String msg = isApply?ResHelper.getString("6017leave","06017leave0221")/*@res "指定申请日期段内的考勤期间已经封存,不能销假!"*/:
//					ResHelper.getString("6017leave","06017leave0222")/*@res "指定实际日期段内的考勤期间已经封存,不能销假!"*/;
//				throw new BusinessException(msg);
//			}
//		}
	...
	private void validateRegCalculated(LeaveRegVO vo) throws BusinessException{
		...
		if(vo.getWaperiod() !=null && !StringUtil.isEmpty(vo.getWaperiod())){
			String msg = ResHelper.getString("leave","2leaveoffExt-00001")/*@res "未获得指定实际日期段内的考勤期间!"*/;
			throw new BusinessException(msg);
		}
	...
	private void validateAndUpdateBanlance(LeaveRegVO vo) throws BusinessException{
		...
		if(balanceVO.getIssettlement().booleanValue()||balanceVO.getIsuse().booleanValue())
			throw new BusinessException(ResHelper.getString("6017leave","06017leave0223")/*@res "无法完成操作,当前休假类别和休假期间的结余数据已结算!"*/);
...
//		getServiceTemplate().update(true, balanceVO);
	...
	public LeaveRegVO[] insertData(SplitBillResult<LeaveRegVO> splitResult)
		...
		//获取休假类别 @author xiepch
		Map<String,Integer> typecoypMap = new HashMap<String,Integer>();//休假类别copy主键与结算周期
		...
		for(int i=0;i<result.length;i++){
			...
			if(result[i].getBegindate().getYear()< Integer.valueOf(result[i].getLeaveyear()) 
					&& typecoypMap.get(result[i].getPk_leavetypecopy()) == 1){//休假类别为按年结算时才转考勤年度为自然年
