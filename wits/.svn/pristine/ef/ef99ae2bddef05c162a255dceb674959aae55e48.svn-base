...
/**
 * Get Sum(value) 新税改_累计已扣税 (新税改_累计应纳税所得额 - 本期应纳税所得额 - (5000 * （发薪月数 - 1）)) *
 * 适用税率 - 速算扣除数
 * 
 * 汇总薪资方案已经审核
 * 
...
public class TaxfunTotalTaxedAmtProcesser extends AbstractWAFormulaParse implements IFormulaAli {
	...
	public FunctionReplaceVO getReplaceStr(String formula) throws BusinessException {
		...
		if (new BaseDAO().getDBType() == DBUtil.SQLSERVER) {
			sqlBuffer
					.append(" SELECT  isnull (sum(wadata.f_5),0) value from wa_data wadata         ");
		} else {
			sqlBuffer
					.append(" SELECT  nvl (sum(wadata.f_5),0) value from wa_data wadata      ");
		}
		//过滤掉汇总薪资类别方案,按月计税方案
