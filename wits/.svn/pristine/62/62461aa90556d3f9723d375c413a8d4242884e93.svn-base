...
import nc.vo.hrwa.watbmdaysalary.TbmDaySalaryVO;
...
public class WaTbmdaysalaryServiceImpl implements IWaTbmdaysalaryService {
	...
	// 查询参数设置-考勤日薪计算天数取值方式
	int daynumtype = getSysintValue(pk_hrorg, DaySalaryEnum.TBMSYSINT);
	// 考勤日薪计算结果
	List<TbmDaySalaryVO> listTbmDaySalaryVOs = new ArrayList<TbmDaySalaryVO>();
	// 获取日薪计算天数
	...
	for (Map.Entry<Object, List<GeneralVO>> e : psndocWadocMap.entrySet()) {
		...
		TbmDaySalaryVO salaryVO = new TbmDaySalaryVO();
		...
		salaryVO.setTbmdaysalary(daysalary);
		salaryVO.setTbmhoursalary(hoursalary);
		listTbmDaySalaryVOs.add(salaryVO);
	    }
	}
	// 在插入數據之前，做清空處理
	getDao().deleteByClause(TbmDaySalaryVO.class,
    ...
    public void checkTbmDaysalaryAndRecalculate(String[] pk_psndocs, UFLiteralDate begindate, UFLiteralDate enddate)
	    throws BusinessException {
	String inpsndocsql = new InSQLCreator().getInSQL(pk_psndocs, true);
	// 查询出定调资存在新增、修改的记录以及记录为空的数据,同时需要排除没有考勤档案的数据 tbmpsndoc.pk_org is not null
	String checkSql = "SELECT DISTINCT\n" + "	tbmpsndoc.pk_org,\n" + "	calendar.calendardate,\n"
		+ "	wadoc.pk_psndoc,\n" + "	wadoc.pk_psnjob,\n" + "	wadoc.pk_psndoc_sub,\n" + "	wadoc.ts AS wadocts,\n"
		+ "	wadoc.pk_wa_item,\n" + "	waitem.isattend,\n" + "	waitem.taxflag,\n" + "	wadoc.nmoney,\n"
		+ "	salary.pk_tbm_daysalary\n" + "FROM\n" + "	hi_psndoc_wadoc wadoc\n"
		...
		+ "LEFT JOIN wa_tbmdaysalary salary ON (\n" + "	salary.pk_psndoc = wadoc.pk_psndoc\n"
		...
		+ begindate.toStdString() + "'\n" + " and tbmpsndoc.pk_org is not null AND wadoc.pk_psndoc in (" + inpsndocsql + ")\n" + "AND (\n"
	...
	for (Map.Entry<Object, List<GeneralVO>> e : psndocWadocMap.entrySet()) {
		...
		salaryVO.setTbmhoursalary(hoursalary);
		listTbmDaySalaryVOs.add(salaryVO);
		String pk_tbm_salary = generalVO.getAttributeValue("pk_tbm_daysalary") == null ? "" : generalVO
			.getAttributeValue("pk_tbm_daysalary").toString();
	...
	// 删除已经不存在的定调资记录
	String deletesql = "DELETE FROM wa_tbmdaysalary WHERE pk_psndoc_sub NOT IN (SELECT pk_psndoc_sub FROM hi_psndoc_wadoc)";
	getDao().executeUpdate(deletesql);
	if (deletePks.size() > 1) {
	    getDao().deleteByPKs(TbmDaySalaryVO.class, deletePks.toArray(new String[0]));
	}
	getDao().insertVOList(listTbmDaySalaryVOs);
    }
}
