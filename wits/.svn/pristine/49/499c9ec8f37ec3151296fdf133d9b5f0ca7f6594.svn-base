...
//			return false;
	...
	public void checkBeforeDeleteTBMPsnodc(String pk_org, String[] pkTbmpsndocs)
		...
		List<String> sealedPeriodPsnList = new ArrayList<String>();//?õ•????????????????????????????list
...
/*@res "???????????????????????????????,???????:{0}????????????????????????"*/, getPsnNames(sealedPeriodPsnList)));
	...
	public void checkDateScope(String pk_org, IDateScope[] dateScopes)
		...
		if(ArrayUtils.isEmpty(allPeriodVOs)){
			...
			throw new BusinessException(ResHelper.getString("6017basedoc", "06017basedoc1861"/*@res"???{0}¶ƒ??????????"*/,orgName));
...
//			/*@res "¶ƒ???è˝?????!"*/);
		...
		if(!ArrayUtils.isEmpty(dateScopeHaveNotPeriod)){
			...
			throw new BusinessException(ResHelper.getString("6017basedoc","06017basedoc1633"/*@res "?????¶∂??{0}????????????????¶∂??{1}??!"*/, allScopeDesc,periodScopeDesc));
		}
		//?????????????????
...
//		}
		...
		if(!ArrayUtils.isEmpty(intersectionScopes))
			throw new BusinessException(ResHelper.getString("6017basedoc","06017basedoc1634")/*@res "?????¶∂???????????????!"*/);
	}
...
//			/*@res "¶ƒ???è˝?????!"*/);
		...
		List<String> sealedPeriodPsnList = new ArrayList<String>();//?????????????????????ßﬂ??????????????list
...
//		}
		for(SuperVO vo:tbmPsndocVOs){
			...
			//??????????????????
			IDateScope[] intersectionScopes = DateScopeUtils.intersectionDateScopes(new TBMPsndocVO[]{psndocVO}, sealedPeriodVOs);
			if(!ArrayUtils.isEmpty(intersectionScopes) && !noPeriodPsnList.contains(pk_psndoc))//?ßﬂ???????????noPeriodPsnList????????
				sealedPeriodPsnList.add(pk_psndoc);
		}
...
/*@res "?????????????????????????????,???????????????:{0}?????????????????????????"*/, getPsnNames(sealedPeriodPsnList));
	...
	public void checkBeforeUpdateTBMPsndoc(String pk_org, SuperVO[] updatePsndocVOs)
		...
		List<String> changedDateInSealedPeriodPsnList = new ArrayList<String>();//?????????????????????ßﬂ????????list
	...
	private String getPsnNames(List<String> psndocList) throws BusinessException{
		...
		try {
			...
			if (ArrayUtils.isEmpty(psndocVOs))
				return null;
			//???????????ß÷??????????????filteredPsndocs?ß÷????????????filteredPsndocs?????????????????????map
