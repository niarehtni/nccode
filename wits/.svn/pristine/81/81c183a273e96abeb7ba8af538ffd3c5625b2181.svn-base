...
public class CaculateAction extends PayDataBaseAction {
	...
	public void doAction(ActionEvent e) throws Exception{
		...
		if(isgetlicense<0){
			MessageDialog.showErrorDlg(getEntranceUI(), null, "个人所得税增强包未授权！");
			return ;
		}
		//end 
		super.doAction(e);
			...
			new SwingWorker<Boolean, Void>() {
				...
				protected Boolean doInBackground() throws Exception {
					try {
						...
						// MOD(由于日薪计算消耗较大，不能在薪资计算过程中进行日薪计算)
						...
						ShowStatusBarMsgUtil.showStatusBarMsg("預處理日薪數據耗時："
								+ spent.setScale(3, UFDouble.ROUND_HALF_UP).toString() + "秒", getContext());
