...
public class ExportBankInfoAction extends HrAction {
	...
	String[] textArr = getService().getBankReportText(getPk_org(), getYear() + getPeriod(), getPk_wa_class());
	...
	if (textArr != null && textArr.length > 2) {
	    ...
	    fileChooser.setSelectedFile(new File(fileChooser.getCurrentDirectory().getAbsolutePath() + "\\BANK_RPT_"
		...
		if (!fileChooser.getSelectedFile().getAbsoluteFile().toString().toUpperCase().endsWith(".TXT")) {
