...
public class QuitInsuranceExportAction extends HrAction {
	...
	public QuitInsuranceExportAction() {
		super();
		super.setBtnName(ResHelper.getString("twhr_datainterface", "DataInterface-00080")); // 劳健退三合一退保
		super.setCode("AddInsuranceExportAction");
		super.putValue(Action.SHORT_DESCRIPTION, ResHelper.getString("twhr_datainterface", "DataInterface-00080")); // 劳健退三合一退保
	}
	...
	public void doAction(ActionEvent arg0) throws Exception {
		...
		if (isGen == UIDialog.ID_OK) {
			...
			for (String pk_legalOrg : pkRealOrgSet) {
				...
				String filepath = baseFilePath+ java.io.File.separator +"三合一退保_" + org.getCode() + ".xls";
				if (null == filepath) {
					return;
				}
				...
				//微...调以兼容只退健保 Ares.Tank 2018-11-1 11:27:28
				...
				String sql = "select 2 as move, '" + TWNHHI01.getTextvalue() + "' as insnum,"
						...
						+ " 2 as inssort,1 as insured, " + " case doc2.code  " + " when 'LT06' then 'Y' "
						+ " when 'LT13' then '1'  " + " when 'LT14' then '2' " + " else '' end as insforeign, "
						+ " ps.name2 as insname, ps.id as id, " + " case doc2.code  " + " when 'LT06' then ps.id "
						+ " when 'LT13' then ps.id  " + " when 'LT14' then ps.id " + " else '' end as wjid, "
						+ " ps.birthdate as birthdate, "
						+ " doc3.code as yyb,doc4.code as yysmb,doc4.name as yysm,g3.enddate as yydate "
						+ " from bd_psndoc ps " + " left join org_orgs org on org.pk_org = ps.pk_org "
						+ " left join org_hrorg hr on org.code = hr.code " + " left join bd_defdoc doc1 on "
						+ " doc1.pk_defdoc ='" +TWNHHI04.getRefvalue()+ "' left join "
						...
						+ " left join bd_defdoc doc4 on doc4.pk_defdoc = g3.glbdef19 " + " where ps.name2 = g3.glbdef1"
						// + " and g3.recordnum=0 "//少了一个人注释掉
						+ " and (isnull(g2.begindate,g3.begindate)=g3.begindate)  "
						+ " and isnull(g2.insuranceform,2) = 2 and g3.insuranceform = 2 "
						// + " and hr.pk_hrorg ='"+ pk_org
						+ " and isnull(g2.legalpersonorg,'"+pk_legalOrg+"') ='" + pk_legalOrg + "'" + " and g3.legalpersonorg ='" + pk_legalOrg
						+ "'" + " and  isnull(g2.enddate,'"+startPeriod+"')='" + startPeriod + "' and g3.enddate='"+ startPeriod +"' ";
				...
				if (list != null && list.size() > 0) {
					for (int i = 0; i < list.size(); i++) {
						...
						for (int j = 0; j < 16; j++) {
							...
							case 11:
								cell.setCellValue(
										AddInsuranceExportAction.adYearToRepublicOfChina(map.get("birthdate")));
								break;
							case 12:
