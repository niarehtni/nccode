...
import nc.jdbc.framework.exception.MSSQLException;
...
public class LeaveBalanceMaintainImpl implements ILeaveBalanceManageMaintain,
	 ...
	 */
	protected void generatePartWhLeaveTemp(String pk_org, String pk_leavetype,
			String year, String month, LeaveBalanceVO[] vos) throws BusinessException, ParseException {
		...
		/**
		 *  // 1 取日期  算工龄  按 年月到 考勤期间 取结束日期减去  workagestartdate 薪资起算日（入职日期）
		 *	// 2 取特休身份标志
		 *	// 3 计算 每个人 应获取数据的 工作日历开始时间和结束时间   
		 *	// 4 特休人员计算百分比，非特休人员 返回1 （100%）
		 * 
		 */
		...
		String queryFlagSql = "select t1.pk_psndoc,t1.is_special_timer,t2.workagestartdate from bd_psndoc "
				...
				+ "where t2.pk_psndoc = t3.pk_psndoc) and t1.pk_psndoc in ("
				+ inSql + ")";
		@SuppressWarnings("unchecked")
		List<Map<String,String>> resultList = (List<Map<String,String>>)basedao.executeQuery(queryFlagSql, new MapListProcessor());
		...
		String kaoqinEndDateStr = year + "-" + month + "-25";//设置个默认值为 当前计算年月 的 25 号
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		String queryEndDateSql = "select t.enddate from tbm_period t where t.pk_org = '"+pk_org+"' and t.accyear = '"+year+"' and t.accmonth = '"+month+"'";
		...
		try {
			...
			for (int i = 0; i < psnList.size(); i++) {
			    ...
			    //parameter.addParam(new UFDateTime().toString());//ts  NC JdbcSession 在 insert时 自动添加插入字段  ts
				...
				if ("Y".equals(psnToFlagMap.get(pk_psndoc)[0])) {//是特休人员身份
					...
					if (addSixWorkStartDate.after(kaoqinEndDate)) {
						 ...
						 * 即入职时间大于6个月且小于1年，则汇总工时开始日期为 薪资起算日期workStartDate，结束日期为addSixWorkStartDate
						 * 到员工工作日历中进行工时汇总，然后计算出比例
						 */
						//获取工时汇总
						String sumSql = "select sum(t1.gzsj) from tbm_psncalendar t1 where t1.pk_psndoc = '"+pk_psndoc+"' and t1.calendar >= '"+workStartDateStr+"' and t1.calendar <= '"+addSixWorkStartDateStr+"' and t1.pk_org = '"+pk_org+"'";
						double sumHour = 0;
						...
						if (addTwelveWorkStartDate.before(kaoqinEndDate) && addTwentyWorkStartDate.after(kaoqinEndDate)) {
							/*3.1 薪资起算日期加上12个月后小于当前考勤期间结束日期，且薪资起算日期加上24个月大于当前考勤期间结束日期
							 * 即入职时间大于1年但小于2年，汇总工时开始日期为 薪资起算日期workStartDate
							 * 结束日期为addTwelveWorkStartDate（一年后）
							 */
							//获取工时汇总
							String sumSql = "select sum(t1.gzsj) from tbm_psncalendar t1 where t1.pk_psndoc = '"+pk_psndoc+"' and t1.calendar >= '"+workStartDateStr+"' and t1.calendar <= '"+addTwelveWorkStartDateStr+"' and t1.pk_org = '"+pk_org+"'";
							double sumHour = 0;
							Object obj = basedao.executeQuery(sumSql, new ColumnProcessor());
							if (null != obj) {
								sumHour = Double.valueOf(obj.toString());
							...
							/*3.2 最后一种情况 ：入职时间 大于 2 年
							 * 汇总结束日期为  当前考勤期间结束日期 对应年的 入职日期
							 * 例如：薪资起算日（入职日）2016-03-21  当前考勤期间结束日期 为 2018-05-25
							 * 则 汇总结束日期为  2018-03-21  开始日期为 此日期 往前一年  即  2017-03-21
							 */
							//计算结束日期
							...
							//获取工时汇总
							String sumSql = "select sum(t1.gzsj) from tbm_psncalendar t1 where t1.pk_psndoc = '"+pk_psndoc+"' and t1.calendar >= '"+sumStartDateStr+"' and t1.calendar <= '"+sumEndDateStr+"' and t1.pk_org = '"+pk_org+"'";
							double sumHour = 0;
							Object obj = basedao.executeQuery(sumSql, new ColumnProcessor());
							if (null != obj) {
								sumHour = Double.valueOf(obj.toString());
		...
		for (LeaveBalanceVO vo : vos) {
			...
			if(null != isSpecialrest && isSpecialrest.booleanValue()){
				...
				//按照年结算
					...
					String tbmPeriodSql = " select min(begindate)begindate,max(enddate)enddate from tbm_period where accyear = '"+year+"' and pk_org = '"+vo.getPk_org()+"' ";
					List<Object[]> tbmPeriodList = (List<Object[]>) baseDao.executeQuery(tbmPeriodSql, new ArrayListProcessor());
				...
				if(!"".equals(startDate) && !"".equals(overDate)){
					String specialSql = " select specialrest from tbm_psndoc where pk_psndoc = '"+vo.getPk_psndoc()+"' and pk_org" +
				            " = '"+vo.getPk_org()+"' and isnull(enddate,'9999-01-01') >= '"+overDate+"' order by begindate desc";
...
//	//判断是否为年休假
//	public Boolean isSpecialHoliday(LeaveTypeCopyVO typeVO) throws BusinessException{
//		
//		if(null == typeVO){
//			return false;
//		}
//		
//		//获取系统认定的年休假的编码
//		String itemcode = String.valueOf(SysInitQuery.getParaString(typeVO.getPk_org(), "TBMTWHR01"));
//		//获取当前假期类别的编码
