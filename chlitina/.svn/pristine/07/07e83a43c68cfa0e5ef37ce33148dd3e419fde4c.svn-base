...
public abstract class AbstractTWHolidayOffdayValidator  implements ITWHolidayOffdayValidate {
    ...
    public Map<Map<String, String>, String> validate(String pk_org, String pk_psndoc, UFLiteralDate checkDate, List<AggPsnCalendar> volists)
	...
	for (IDateScope scope : checkScope) {
		...
		if (!StringUtils.isEmpty(strMessage)) {
			holidaymessage.put(strMessage, psnvo.getCode());
		...
		if (!StringUtils.isEmpty(strMessage)) {
			hoursmessage.put(strMessage, psnvo.getCode());
	...
	List<String> calendars = new ArrayList<String>();
		for(AggPsnCalendar vo : volist){
			if(((PsnCalendarVO)vo.getParentVO()).getGzsj()!= null && ((PsnCalendarVO)vo.getParentVO()).getGzsj().toDouble() > dailyWorkLimit){
