	...
	public void wizardFinish(WizardEvent event) throws WizardActionException {
		...
		if (changeDayTypeCheckBox) {
			...
			try{
				...
				//在页面做一个弹出框
				List<List<String>> strMessage = manageValidate.updateteamValidate(wizardModel.getModel().getContext().getPk_org(),pk_teams, firstDate, secondDate);
			...
			try{
				//IHRHolidayQueryService
				//在页面做一个弹出框
				List<List<String>> strMessage = manageValidate.updateteamValidate(wizardModel.getModel().getContext().getPk_org(),pk_teams, changeDate, panel3.getafterChangeDateType());
				UFBoolean isStrcheck = SysInitQuery.getParaBoolean(legal_pk_org, "TWHRT03");
				if(null != strMessage ){
					for(List<String> strs : strMessage){
						if(isStrcheck.booleanValue()){
