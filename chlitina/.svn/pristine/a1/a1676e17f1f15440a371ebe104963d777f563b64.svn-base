...
public class SegDetailServiceImpl implements ISegDetailService {
	...
	if (!leaveTypeVOMap.containsKey(pk_org)) {
	    String leaveTypeRefCode = SysInitQuery.getParaString(pk_org, "TWHRT08");
	...
	if (!psndocMap.containsKey(pk_org)) {
	    TBMPsndocVO psndoc = (TBMPsndocVO) this.getBaseDao().retrieveByPK(TBMPsndocVO.class, pk_psndoc);
	    if (psndoc != null) {
    ...
    @SuppressWarnings("unchecked")
    @Override
    public AggOTLeaveBalanceVO[] getOvertimeToRestHoursByType(String pk_org, String[] pk_psndocs,
	    UFLiteralDate beginDate, UFLiteralDate endDate, String pk_overtimetype) throws BusinessException {
	...
	if (pk_psndocs != null && pk_psndocs.length > 0) {
		...
		if (curNode == null) {
		    ...
		    OTLeaveBalanceVO headVo = createNewHeadVO(pk_org, pk_psndoc, totalAmount, spentAmount,
		...
		AggOTLeaveBalanceVO aggvo = new AggOTLeaveBalanceVO();
		// 创建表头
		OTLeaveBalanceVO headVo = createNewHeadVO(pk_org, pk_psndoc, totalAmount, spentAmount, remainAmount,
			frozenAmount, useableAmount);
		aggvo.setParent(headVo);
		...
		if (detailVOs.size() > 0) {
		    aggvo.setChildren(OTBalanceDetailVO.class, detailVOs.toArray(new OTBalanceDetailVO[0]));
		}
		aggVos.add(aggvo);
	    }
	}
