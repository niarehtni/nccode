...
public class LeaveRegCardView extends TBMPubBillCardForm implements FocusListener, BillCardBeforeEditListener {
    ...
    // getBillCardPanel().getHeadItem("pk_psnjob").getComponent();
    // //
    // psnRef.getRefModel().addWherePart(PubPermissionUtils.getTBMPsnjobPermission());//这种方法多用了远程连接
    // AllParams allParams =
    // ((TALoginContext)getModel().getContext()).getAllParams();
	...
	if (!isYear() && !islactation()) {
	    UIComboBox monthComboBox = (UIComboBox) getBillCardPanel().getHeadItem(LeaveRegVO.LEAVEMONTH)
		    .getComponent();
	...
	if (LeaveRegVO.ISLACTATION.equals(e.getKey())) {
	    ...
	    // 同时记录相应修改字段的日期
	    LeaveRegVO regVO = (LeaveRegVO) getBillCardPanel().getBillData().getHeaderValueVO(
		    LeaveRegVO.class.getName());
	    if (regVO == null)
		return;
	    if (LeaveRegVO.LEAVEBEGINTIME.equals(e.getKey())) {
		getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "time", "date"),
			regVO.getLeavebegintime() == null ? null : regVO.getLeavebegintime());
	    } else {
		getBillCardPanel().setHeadItem(StringUtils.replace(e.getKey(), "time", "date"),
			regVO.getLeaveendtime() == null ? null : regVO.getLeaveendtime());
	    }
	    synTimeLong();
		...
		getBillCardPanel().setHeadItem(
			...
			regVO.getLeavebegindate() == null ? null : (new UFDateTime(regVO.getLeavebegindate().toDate()
				.getTime())));
	    } else {
		getBillCardPanel().setHeadItem(
			StringUtils.replace(e.getKey(), "date", "time"),
			regVO.getLeaveenddate() == null ? null : (new UFDateTime(regVO.getLeaveenddate().toDate()
				.getTime())));
	    }
	    synTimeLong();
	...
	try {
	    LeaveRegVO leaveVO = (LeaveRegVO) getBillCardPanel().getBillData().getHeaderValueVO(
		    LeaveRegVO.class.getName());

	    LeaveBalanceVO leaveBalanceVO = null;
	    boolean isYear = isYear();
	    // 只有在用户录入了年度期间的情况下，才去计算结余时长等信息
	    if ((isYear && !StringUtils.isEmpty(leaveVO.getLeaveyear()))
		    || (!isYear && !StringUtils.isEmpty(leaveVO.getLeaveyear()) && !StringUtils.isEmpty(leaveVO
			    .getLeavemonth()))) {
		...
		leaveBalanceVO = MapUtils.isEmpty(balanceMap) ? null : balanceMap.get(leaveVO.getPk_psnorg()
			+ leaveVO.getPk_leavetype() + leaveVO.getLeaveyear() + (isYear ? "" : leaveVO.getLeavemonth()));
	    }
	    ...
	    // V61增加，冻结时长和可用时长
	    setHeadItemValue(LeaveRegVO.FREEZEDAYORHOUR,
		    isNull ? UFDouble.ZERO_DBL : leaveBalanceVO.getFreezedayorhour());
	    setHeadItemValue(LeaveRegVO.USEFULDAYORHOUR,
		    isNull ? UFDouble.ZERO_DBL : leaveBalanceVO.getUsefulrestdayorhour());
	...
	try {
	    ...
	    getBillCardPanel().setHeadItem(LeaveRegVO.LEAVEHOUR,
		    leaveVO.getLeavehour() == null ? new UFDouble(0) : leaveVO.getLeavehour());
	    setValue(leaveVO);
	...
	if (!islactation() && getCurPeriodVO() != null) {
		...
		if (isPreHolidayFirst()) {
		    ...
		    getBillCardPanel().getHeadItem(LeaveRegVO.LEAVEMONTH).setValue(
			    isYear() ? null : getCurPeriodVO().getAccmonth());
		}
	    }
	...
	if (changeLabelVO == null) {
	    ...
	    changeLabelVO.setChangeLabelKeys(new String[] { LeaveRegVO.LEAVEHOUR, LeaveRegVO.REALDAYORHOUR,
		    LeaveRegVO.RESTDAYORHOUR, LeaveRegVO.RESTEDDAYORHOUR, LeaveRegVO.FREEZEDAYORHOUR,
		    LeaveRegVO.USEFULDAYORHOUR,// V61增加，冻结时长和可用时长
    ...
    /**
     * 取当前考勤期间 2012-04-27改为懒加载方式 在使用时再加载，切换组织时在initYear中置为空
     * 
     * @return
     */
