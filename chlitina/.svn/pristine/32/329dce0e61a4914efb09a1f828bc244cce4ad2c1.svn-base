...
public class SaveAction extends RdsBaseAction {
	...
	private boolean saveAddData(PsndocAggVO aggVO, ActionEvent event) throws Exception {
		...
		if (null == newPkOrg) {
			...
			if(null != resultMap && null != resultMap.get(oldPkOrg)
					&& null != resultMap.get(newPkOrg)
		...
		if (TRNConst.Table_NAME_DEPTCHG.equals(curTabCode)) {
			...
			String sqlStr = "select pk_psndoc from " + PsndocDefTableUtil.getPsnLaborTablename() + " WHERE ISNULL(glbdef15, '9999-12-31') > '" + endDate.toString() + "' AND dr=0 AND pk_psndoc = '"
			...
			sqlStr = "select pk_psndoc from " + PsndocDefTableUtil.getPsnHealthTablename() +" WHERE ISNULL(glbdef15, '9999-12-31') > '" + endDate.toString() + "' AND dr=0 AND pk_psndoc = '"
					+ aggVO.getParentVO().getPk_psndoc() + "'" + " and lastflag = 'Y'";
