			...
			for(String dates : datelist){
				...
				for(int i =0; i<days+1; i++ ){
					...
					if(null != weekForm){
						...
						if(null != validator.validate(pk_org, pk_psndoc, checkdate, volists)){
							strlist.put(validator.validate(pk_org, pk_psndoc, checkdate, volists), null);
						}
					}
				}
			}
			...
			try {
				...
				for(String pk_psndoc: pk_psndocs){
					...
					for(int i =0; i<str.length; i++ ){
						...
						WorkWeekFormEnum weekForm = getWeekFormFromTBMPsndoc(pk_psndoc, checkdate);
						ITWHolidayOffdayValidate validator = TWHolidayOffdayValidatorFactory.getValidator(weekForm);
						strlists.put(validator.validate(pk_org, pk_psndoc, checkdate, infomaps), null);
					}
				}
