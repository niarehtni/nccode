 ...
 * 
 */
public class ShowTaxDig extends UIDialog implements ActionListener,
	...
	private UILabel getStartdatelbl() {
		if (startdatelbl == null) {
			try {
				...
				startdatelbl.setText(ResHelper.getString("twhr_personalmgt",
						"068J61035-0090")/* "开始日期" */);
	...
	private UILabel getEnddatelbl() {
		if (enddatelbl == null) {
			try {
				...
				enddatelbl.setText(ResHelper.getString("twhr_personalmgt",
						"068J61035-0091")/* "结束日期" */);
	...
	private UILabel getContactTelLabel() {
		if (contactTelLabel == null) {
			try {
				...
				contactTelLabel.setBounds(0, 90, 100, 30);
			} catch (Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private UILabel getContactNameLabel() {
		if (contactNameLabel == null) {
			try {
				...
				contactNameLabel.setBounds(0, 130, 100, 30);
			} catch (Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private void onButtonOKClicked() {
		this.startdate = new UFDate(this.getRefStartdate().getValueObj()
				.toString());
		this.enddate = new UFDate(this.getRefEnddate().getValueObj()
				.toString());
		if(this.startdate.after(this.enddate)){
			MessageDialog
			.showWarningDlg(this, ResHelper.getString("incometax",
					"2incometax-n-000004"), ResHelper.getString(
					"twhr_personalmgt", "068J61035-0092")  );// "开始日期不能大于结束日期"
			return;
		}
