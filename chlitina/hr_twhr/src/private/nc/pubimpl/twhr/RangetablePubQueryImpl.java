...
public class RangetablePubQueryImpl implements IRangetablePubQuery {
	...
	@Override
	public RangeTableAggVO[] queryRangetableByType(int tableType,
			UFDate queryDate) throws BusinessException {
		BaseDAO dao = new BaseDAO();
		String strCondition = "dr=0 "
				+ (tableType == -1 ? "" : "' and tabletype="
						+ String.valueOf(tableType)) + " and startdate<='"
				+ queryDate.toString() + "' and enddate>='"
				+ queryDate.toString() +"' and pk_group = 'GLOBLE00000000000000' "
		...
		if (rangeTables != null && rangeTables.size() > 0) {
			for (RangeTableVO rangeTable : rangeTables) {
				...
				Collection<RangeLineVO> rangeLines = dao.retrieveByClause(
						RangeLineVO.class, " pk_org = 'GLOBLE00000000000000' "
								+ " and pk_group = 'GLOBLE00000000000000' "
								+ " and dr=0 and pk_rangetable='"
	...
	@Override
	public RangeLineVO queryRangeLineByAmount(int tableType, UFDate queryDate,
			UFDouble amount) throws BusinessException {
		RangeTableAggVO[] rangeTables = queryRangetableByType(
				tableType, queryDate);
