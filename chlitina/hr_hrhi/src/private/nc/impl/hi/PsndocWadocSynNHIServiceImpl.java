...
public class PsndocWadocSynNHIServiceImpl implements IPsndocWadocSynNHIService {
	...
	if (nhiVOs != null && nhiVOs.length > 0) {
		...
		if ((vo.getBegindate().isSameDate(UFDate.getDate(nhivo.getBegindate().toDate())) || vo.getBegindate()
			.after(UFDate.getDate(nhivo.getBegindate().toDate())))
			&& (vo.getEnddate().isSameDate(UFDate.getDate(nhivo.getEnddate().toDate())) || vo.getEnddate()
				.before(UFDate.getDate(nhivo.getEnddate().toDate())))) {
		    return nhivo;
		}
	    }
	}
	...
	if (nhiVOs != null) {
		...
		newVO.setNmoney((vo.getAttributeValue("glbdef2") == null || vo.getAttributeValue("glbdef2").equals("~")) ? UFDouble.ZERO_DBL
