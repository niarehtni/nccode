...
public class PsndocModel extends BillManageModel implements IQueryMode {
	...
	try {
	    if (!hashBusinessInfoSet.contains(PsndocDefTableUtil.getPsnNHIDetailTablename())
		    && PsndocDefTableUtil.getPsnNHIDetailTablename() != null) {
		hashBusinessInfoSet.add(PsndocDefTableUtil.getPsnNHIDetailTablename());
	    }

	    if (!hashBusinessInfoSet.contains(PsndocDefTableUtil.getPsnNHISumTablename())
		    && PsndocDefTableUtil.getPsnNHISumTablename() != null) {
		hashBusinessInfoSet.add(PsndocDefTableUtil.getPsnNHISumTablename());
	    }

	    if (!hashBusinessInfoSet.contains(PsndocDefTableUtil.getPsnHealthInsExTablename())
		    && PsndocDefTableUtil.getPsnHealthInsExTablename() != null) {
		hashBusinessInfoSet.add(PsndocDefTableUtil.getPsnHealthInsExTablename());
	    }
	...
	if (blContractStarted == null) {
	    blContractStarted = Boolean.valueOf(HiCacheUtils.isModuleStarted("6011"));
	}
	...
	try {
	    Integer para = SysInitQuery.getParaInt(getContext().getPk_org(), "HI0003");
	    if (para == null) {
	...
	try {
	    return HiCacheUtils.getBillCodeContext("6007psndoc_clerkcode", getContext().getPk_group(), getContext()
		    .getPk_org());
	...
	try {
	    return HiCacheUtils.getBillCodeContext("6007psndoc_code", getContext().getPk_group(), getContext()
		    .getPk_org());
	} catch (BusinessException ex) {
	    Debug.error(ex.getMessage(), ex);
	}
	...
	try {
	    Integer para = SysInitQuery.getParaInt(getContext().getPk_org(), "HI0002");
	    if (para == null) {
	...
	IPsndocQryService queryService = (IPsndocQryService) NCLocator.getInstance().lookup(IPsndocQryService.class);
	try {
	...
	if (ArrayUtils.contains(strUniqueFields, "idtype")) {
	    strUniqueFields = ((String[]) ArrayUtils.removeElement(strUniqueFields, "idtype"));
	}

	if (ArrayUtils.contains(strUniqueFields, "id")) {
	    strUniqueFields = ((String[]) ArrayUtils.removeElement(strUniqueFields, "id"));
	}

	if (ArrayUtils.contains(strUniqueFields, "name")) {
	    strUniqueFields = ((String[]) ArrayUtils.removeElement(strUniqueFields, "name"));
	}

	String[] defaultfieldcodes = { "name", "idtype", "id" };

	strUniqueFields = ((String[]) ArrayUtils.addAll(defaultfieldcodes, strUniqueFields));
	...
	for (Iterator iterator = usedTablesOfQuery.iterator(); iterator.hasNext();) {
		...
		if ((filterMeta instanceof MDFilterMeta)) {
			...
			hashUsedTablesOfQuery.put(replaceTableName(strTableName), mdFilterMeta.getTableAlias());
			break;
		    }
		}
	    }
	...
	if (hiddenKeys == null) {
	    hiddenKeys = ((IPsndocQryService) NCLocator.getInstance().lookup(IPsndocQryService.class))
		    .queryHiddenKeys(getContext());
	}
	return hiddenKeys;
    }
	...
	if ((isListShow) && (getSelectedOperaRows() != null) && (getSelectedOperaRows().length > 0)) {
	    deleteMultiRows();
	} else {
	    deleteSeletedRow();
	}
	...
	PsndocAggVO[] agg = ((IPsndocService) NCLocator.getInstance().lookup(IPsndocService.class)).saveKeyPsn(refPKs,
	...
	for (int i = 0; (aggVOs != null) && (i < aggVOs.length); i++) {
		...
		IBDObject bdObj = getBusinessObjectAdapterFactory().createBDObject(aggVOs[i]);
		if (bdObj != null) {
	...
	fireEvent(new AppEvent("Data_Inserted", this, new RowOperationInfo(indexes, aggVOs)));
	...
	PsndocAggVO[] agg = ((IPsndocService) NCLocator.getInstance().lookup(IPsndocService.class))
		.saveKeyPsnByCondition(wherePart, (KeyPsnGrpVO) getCurrTypeOrgVO());
	...
	for (int i = 0; (objs != null) && (i < objs.length); i++) {
	    String pk_psnorg = ((PsndocAggVO) objs[i]).getParentVO().getPsnOrgVO().getPk_psnorg();
	...
	((IPsndocService) NCLocator.getInstance().lookup(IPsndocService.class)).deleteKeyPsn(
		(PsndocVO[]) vos.toArray(new PsndocVO[0]), (String[]) pks.toArray(new String[0]),
	...
	return ((IPsndocQryService) NCLocator.getInstance().lookup(IPsndocQryService.class)).queryNotNullSubset(
		psndocAggVO, showTab);
    }

    public String[] validateSubNotNull(PsndocAggVO psndocAggVO, HashMap<String, String> showTab)
	    throws BusinessException {
	return ((IPsndocQryService) NCLocator.getInstance().lookup(IPsndocQryService.class)).validateSubNotNull(
		psndocAggVO, showTab);
    }
