...
public class LeaveBalanceMaintainImpl implements ILeaveBalanceManageMaintain,
	...
	public LeaveBalanceVO[] calculate(String pk_org, String pk_leavetype,
		...
		if (canCalList.size() == 0)
			return vos;
		// 对休假数据进行每100个人分批执行一次计算，否则数据量过大导致数据丢失，计算不出有些人的已休时长
...
//		}
	...
	private LeaveFormulaCalParam createFormulaPara(String pk_org,
		...
		if (periodEndDate != null) {
			if (periodEndDate.after(calDate)) {// 若计算日期在期间的后面则人员的状态取期间的最后一天的状态
				calPsnDate = calDate;
			} else {
	...
	public UnSettlementResult unSettlement(String pk_org, String pk_leavetype,
		...
		for (LeaveBalanceVO vo : canUnSettlementVOs) {
			...
			int leaveSet = vo.getSettlementmethod().intValue();// 上次结算此记录时的结算方式：作废，转下期，转工资
	 ...
	 * 2.如果最晚的结算日期
	 * <期间最后一天，则从最晚的结算日期+1天到期间最后一天内，pk_psnorg有考勤档案记录，则需要用代码new一条，且此条的leaveindex
	 * =最晚的结算日期的index+1
	 * 
	 * @param context
	 * @param pk_leavetype
...
//		//MOD 张恒｛21746｝获取特休假人员 2018/8/29
...
//				result, salaryYear, salaryMonth);
		...
		//MOD 张恒｛21746｝ 结算时，在原来的基础上，加上特休假人员
		settlement(pk_org, typeVO, year, null, previousPeriod, nextPeriod,
				canSettleVOs, createLineNoMap(canSettleVOs), settlementDate,
				result, salaryYear, salaryMonth);
	...
	//BEGIN 张恒｛21746｝获取特休假人员 2018/8/29
	@SuppressWarnings("unchecked")
...
//		//BEGIN 张恒｛21746｝ 把特休假人员和入职日人员融合在一起  2018/8/29
...
//			}
//		}
//		//END 张恒｛21746｝ 把特休假人员和入职日人员融合在一起  2018/8/29
		...
		for (LeaveBalanceVO vo : vos) {
			...
			//MOD 张恒   找到年资起算日和年资结束日，如果不为空，则当前对象可以结算 2018/8/23
			...
			// 如果leaveindex=1，且考勤期间有上期/或者上一年度，则还要检查此人在上期/上一年度是否有考勤档案，若有，则要求：若数据库没有上期结算记录，则抛异常；若有记录，要求全部都已结算
				...
				//END 张恒    将65年资起算日的整合到63   2018/8/28	
					...
					//BEGIN 张恒｛21746｝ 将入职和年资分开计算 2018/8/29
					//按照入职来算
					if(leaveSetPeriod == LeaveTypeCopyVO.LEAVESETPERIOD_DATE){
					...
					//END 张恒｛21746｝ 将入职和年资分开计算 2018/8/29
				}
				if (existsTBMPsndoc) {
					para.clearParams();
					para.addParam(pk_org);
			...
			if (leaveSet == LeaveTypeCopyVO.LEAVESETTLEMENT_NEXT) {
				...
				if (!ArrayUtils.isEmpty(nextPeriodVOs)) {
					LeaveBalanceVO updateVO = nextPeriodVOs[0];
					updateList.add(updateVO);
					updateVO.setLastdayorhour(vo.getRestdayorhour());// 将本期结余更新到下期的“上期结余”
					if (vo.getRestdayorhour() != null
							&& vo.getRestdayorhour().doubleValue() != 0) {// 如果本期结余不为0，才有必要重算下期的结余
					...
					if (leaveSetPeriod != LeaveTypeCopyVO.LEAVESETPERIOD_DATE 
							// ssx added on 2018-03-16
							// for changes of start date of company age
							&& leaveSetPeriod != LeaveTypeCopyVO.LEAVESETPERIOD_STARTDATE) {// 按年和期间结算的，下期的日期范围是固定的
	...
	//END 张恒｛21746｝获取特休假人员 2018/8/29
...
//		//BEGIN 张恒｛21746｝ 满足特休假的员工，可以结算   2018/8/29
//		List<LeaveBalanceVO> specialPsnList = new ArrayList<LeaveBalanceVO>();
//		//END 张恒｛21746｝ 满足特休假的员工，可以结算   2018/8/29
