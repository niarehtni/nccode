...
public class AddSumIncomeTaxAction extends NCAction {
	...
	public void CreateSumIncomeTax() throws BusinessException {
		...
		for (int i = 0; i < objects.length; i++) {
			...
			if (!mapSum.containsKey(key)) {
				...
				sumTaxVO.setTaxbase((sumTaxVO.getTaxbase() == null ? UFDouble.ZERO_DBL : sumTaxVO.getTaxbase())
						.add(incomeVo.getTaxbase()));
				sumTaxVO.setCacu_value((sumTaxVO.getCacu_value() == null ? UFDouble.ZERO_DBL : sumTaxVO.getCacu_value())
						.add(incomeVo.getCacu_value()));
				sumTaxVO.setNetincome((sumTaxVO.getNetincome() == null ? UFDouble.ZERO_DBL : sumTaxVO.getNetincome())
						.add(incomeVo.getNetincome()));
				sumTaxVO.setPickedup((sumTaxVO.getPickedup() == null ? UFDouble.ZERO_DBL : sumTaxVO.getPickedup())
						.add(incomeVo.getPickedup()));
		...
		for (Entry<String, SumIncomeTaxVO> e : mapSum.entrySet()) {
			...
			sumTaxVO.setIdtypeno(getServices.getIdtypeno(sumTaxVO.getPk_psndoc(), mapIncome.get(key).get(0)
					.getPk_wa_data(), sumTaxVO.getEndperiod()));// 获取证别号
			sumTaxVO.setBilldate(new UFDate());// 单据日期
			sumTaxVO.setIsmanualdec(getManualDescFlag(sumTaxVO));
	...
	Map<String, PsndocVO> psnCache = new HashMap<String, PsndocVO>();

	private UFBoolean getManualDescFlag(SumIncomeTaxVO sumTaxVO) throws BusinessException {
		if (!psnCache.containsKey(sumTaxVO.getPk_psndoc())) {
			IUAPQueryBS query = NCLocator.getInstance().lookup(IUAPQueryBS.class);
			PsndocVO psn = (PsndocVO) query.retrieveByPK(PsndocVO.class, sumTaxVO.getPk_psndoc());
			psnCache.put(psn.getPk_psndoc(), psn);
		}

		if (!"0001Z010000000079UJJ".equals(psnCache.get(sumTaxVO.getPk_psndoc()).getCountry())) {
			// 非中國
			if ("3".equals(sumTaxVO.getIdtypeno())) {
				// 已滿183
	...
	public SumIncomeTaxVO getSumIncomeTaxVO(CIncomeTaxVO incomeVo, String granttype, String declarenum, String reason,
		...
		sumTaxVO.setIsforeigndeprdec(incomeVo.getIsforeigndeprdec());
		sumTaxVO.setPaydate(incomeVo.getCpaydate() == null ? null : incomeVo.getCpaydate().toUFLiteralDate(
				UFLiteralDate.BASE_TIMEZONE));
		return sumTaxVO;
	}
