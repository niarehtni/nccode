...
public class CaculateAction extends PayDataBaseAction {
	...
	public CaculateAction() {
		...
		putValue(Action.SHORT_DESCRIPTION,
				ResHelper.getString("60130paydata", "060130paydata0331")/*
																		 * @res
																		 * "计算"
																		 */
						+ "(Ctrl+A)");
	...
	public void doActionForExtend(ActionEvent e) throws Exception {
		if (!getPaydataModel().getIsCompute()) {
			if (showYesNoMessage(ResHelper.getString("60130paydata",
					"060130paydata0332")/* @res "有员工调薪，应先进行计算,是否继续?" */) != MessageDialog.ID_YES) {
				return;
			}
		}
		final RecacuTypeChooseDialog chooseDialog = new RecacuTypeChooseDialog(
				getParentContainer(), WA);
			...
			new SwingWorker<Boolean, Void>() {
				...
				protected Boolean doInBackground() throws Exception {
					try {
						...
						dialog.setStartText(ResHelper.getString("60130paydata",
								"060130paydata0333")/* @res "薪资计算过程中，请稍等..." */);
						dialog.start();
						// 取消二代健保计算的结果 Ares.Tank 2018年10月26日17:01:47
						ICalculateTWNHI nhiSrv = NCLocator.getInstance()
								.lookup(ICalculateTWNHI.class);
						nhiSrv.delExtendNHIInfo(getContext().getPk_group(),
								getContext().getPk_org(), getWaContext()
										.getPk_wa_class(), getWaContext()
										.getWaLoginVO().getPeriodVO()
										.getPk_wa_period());
						...
						getPaydataManager().onCaculate(caculateTypeVO,
								payfileVos.toArray(new SuperVO[0]));
					} catch (LockFailedException le) {
						error = ResHelper.getString("60130paydata",
								"060130paydata0334")/* @res "你操作的数据正被他人修改！" */;
				...
				protected void done() {
					if (error != null) {
						ShowStatusBarMsgUtil.showErrorMsg(ResHelper.getString(
								"60130paydata", "060130paydata0335")/*
																	 * @res
																	 * "计算过程存在错误"
																	 */, error,
								getContext());
