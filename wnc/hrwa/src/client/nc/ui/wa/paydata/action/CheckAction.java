...
public class CheckAction extends PayDataBaseAction {
	...
	public CheckAction() {
		...
		// setDisplayHotKey("s");
		putValue(Action.SHORT_DESCRIPTION, ResHelper.getString("common", "UC001-0000027")/*
																						 * @
																						 * res
																						 * "审核"
																						 */+ "(Ctrl+M)");
	...
	public void doActionForExtend(ActionEvent e) throws Exception {
		...
		// if
		// (showYesNoMessage(ResHelper.getString("60130paydata","060130paydata0332")/*@res
		// "有员工调薪，应先进行计算,是否继续?"*/) != MessageDialog.ID_YES) {
		// return;
		// }
		// }
		// end
			...
			protected Boolean doInBackground() throws Exception {
				try {
					...
					if (WACLASSTYPE.COLLECTCLASS != WaLoginVOHelper.getClassType(getWaLoginVO())) {
						...
						// AggWaTaxGroupVO aggtaxgroupvo =
						// NCLocator.getInstance().lookup(ITaxGroupQuery.class).queryAggWaTaxGroupVOByWaClassPk(getWaContext().getPk_wa_class());
						// if (aggtaxgroupvo != null)
						{
							// shenliangc 20140830
							// 合并计税方案部分审核只计算界面上查询出来的人员数据，需要传入过滤条件。
							// 不在界面上的人员可能并不想现在审核，所以项目公式可能不合法，此时部分审核会报公式不合法错误。需要只重新计算界面上人员。
							if (!rangeAll) {
								// 部分审核，部分重新计算
								WADelegator.getPaydata().reCaculate(getWaContext(),
										((WadataAppDataModel) getModel()).getWhereCondition());
							} else {
								// 全部审核，全部重新计算
					...
					// 薪资项目预警
					String keyName = ResHelper.getString("common", "UC001-0000027")/*
																					 * @
																					 * res
																					 * "审核"
																					 */;// 审核
					...
					if (messagevo != null
							&& (messagevo.getLisCorpWarns().size() > 0 || messagevo.getLisDeptWarns().size() > 0)) {
					...
					error = e.getMessage();
					// 20151103 shenliangc NCdp205490557 并发审核提示错误：LockFailed
					// key:1001A810000000GVOV6V begin
