...
public class SaveItemInterceptor extends DefaultWaActionInterceptor {
	...
	public boolean beforeDoAction(Action action, ActionEvent e) {
		...
		if (action instanceof SaveAction) {
			...
			vo = (WaItemVO) ((SaveAction) ((SaveAddAction) action)
					.getSaveAction()).getEditor().getValue();
		}

		if (vo != null) {
			if (vo.getIitemtype().equals(TypeEnumVO.FLOATTYPE.value())) {
				if (vo.getIflddecimal() == null) {
					showErrorDlg(ResHelper.getString("60130glbitem",
							"060130glbitem0011")/* @res "数值型的小数位数不能为空" */);
					return false;
				}
				...
				// }
				try {
					if (itemMustInTotalItem()
							&& StringUtils.isBlank(vo.getTotalitem())) {
						showErrorDlg(ResHelper.getString("60130glbitem",
								"060130glbitem0013")/*
													 * @res
					...
					showErrorDlg(ResHelper.getString("60130glbitem",
							"060130glbitem0014")/* @res "判断公共薪资项目是否必须纳入预算失败" */);
					return false;
				}
			}
		}
		if (vo != null && vo.getIproperty() == PropertyEnumVO.SYSTEM.value()) {
			showErrorDlg(ResHelper.getString("60130glbitem",
					"060130glbitem0015")/* @res "增减属性不能是:系统项" */);
			return false;
		}
		...
		if (vo.getFromEnumVO().equals(FromEnumVO.FORMULA)) {
			...
			if (StringUtil.isEmpty(vo.getVformula())) {
				showErrorDlg(ResHelper.getString("60130glbitem",
						"060130glbitem0019")/* @res "薪资规则表不能为空" */);
				return false;
			}
			// liangxr如果来源是薪资规则表，判断项目的小数位数是否
			ItemDataSourcePanel idp = null;
			if (action instanceof SaveAction) {
				idp = ((ItemBillFormEditor) ((SaveAction) action).getEditor())
						.getItemDataSourcePanel();
			} else {
				idp = ((ItemBillFormEditor) ((SaveAction) ((SaveAddAction) action)
						.getSaveAction()).getEditor()).getItemDataSourcePanel();
			}

			int ruleDecimal = (Integer) idp.getWageForm().getRefModel()
					.getValue("iflddecimal");
			int itemDecimal = vo.getIflddecimal();
			if (itemDecimal < ruleDecimal) {
				String msg = ResHelper.getString("60130glbitem",
						"060130glbitem0020")/*
											 * @res
											 * "薪资项目的小数位数小于薪资规则表的小数位数，是否继续?"
											 */;
				if (MessageDialog.showYesNoDlg(getContainer(), ResHelper
						.getString("60130glbitem", "060130glbitem0021")/*
																		 * @res
																		 * "提示"
																		 */,
						msg) == UIDialog.ID_NO) {
			...
			if (StringUtil.isEmpty(vo.getVformula())) {
				showErrorDlg(ResHelper.getString("60130glbitem",
						"060130glbitem0022")/* @res "薪资标准表不能为空" */);
				return false;
			}
		}
		...
		if (isAddSave(vo)) {
			...
			try {
				if (!util.expandTable()) {
					showErrorDlg(ResHelper.getString("60130glbitem",
							"060130glbitem0023")/* @res "薪资发放表扩展失败" */);
					return false;
				}
		...
		// add by ward 2018年9月6日16:01:02
		// 是否考勤需要勾选是否薪酬体系
		if ((vo != null) && !vo.getIsinhi().booleanValue()) {

			if (vo.getIsattend().booleanValue()) {
				showErrorDlg(nc.vo.ml.NCLangRes4VoTransl.getNCLangRes()
						.getStrByID("waitem", "2waitem-twhr-000001")/* "未纳入薪酬体系，不允许勾选是否考勤" */);
				return false;
			}
		}
		// end
	...
	private boolean itemMustInTotalItem() throws BusinessException {
		...
		must = SysInitBO_Client.getParaBoolean(getModel().getContext()
				.getPk_group(), ParaConstant.ITEM_INTO_AMOUNT);
		// must =
