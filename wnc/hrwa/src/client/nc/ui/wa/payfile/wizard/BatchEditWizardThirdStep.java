...
public class BatchEditWizardThirdStep extends WizardStep implements IWizardStepListener, BillEditListener {
	...
	public BatchEditWizardThirdStep(PayfileAppModel model, PayfileModelDataManager dataManager) {
		super();
		this.model = model;
		setTitle(ResHelper.getString("60130payfile", "060130payfile0331")/*
																		 * @res
																		 * "批量修改"
																		 */);
	...
	public void afterEdit(BillEditEvent e) {
		if (e.getKey().equals("taxedit")) {
			...
			if (((UICheckBox) e.getSource()).isSelected()) {
				...
				((UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_LIABILITYORG).getComponent()).getRefModel()
						.setPk_org(model.getContext().getPk_org());
	...
	public void validate() throws WizardStepValidateException {
		...
		// ssx modified on 2020-10-27
		// 补充保费计费方式
		Integer exnhiedit = (Integer) getHeadItem(PayfileVO.EXNHITYPE).getValueObject();
		// end

		WizardStepValidateException e = new WizardStepValidateException();
		if (!taxedit && !taxfreeedit && !stopedit && !fipedit && !libedit && feetype == null && pj == null
				&& biz == null && (exnhiedit == null || exnhiedit == 0)) {
			e.addMsg(ResHelper.getString("60130payfile", "060130payfile0256")/*
																			 * @res
																			 * "错误"
																			 */,
					ResHelper.getString("60130payfile", "060130payfile0297")/*
																			 * @res
																			 * "没有要修改的薪资项目！"
																			 */);
		}
		if (taxedit) {
			...
			if (taxtypeObj == null) {
				e.addMsg(ResHelper.getString("60130payfile", "060130payfile0245")/*
																				 ...
																				 * "提示"
																				 */,
						ResHelper.getString("60130payfile", "060130payfile0298")/*
																				 * @
																				 * res
																				 * "请选择扣税方式！"
																				 */);
				...
				if (getHeadItem(PayfileVO.TAXTABLEID).getValueObject() == null) {
					e.addMsg(ResHelper.getString("60130payfile", "060130payfile0245")/*
																					 ...
																					 * "提示"
																					 */,
							ResHelper.getString("60130payfile", "060130payfile0299")/*
																					 * @
																					 * res
																					 * "请选择税率表！"
																					 */);
		...
		if (taxfreeedit && (Boolean) getHeadItem(PayfileVO.ISDERATE).getValueObject()) {
			if (getHeadItem(PayfileVO.DERATEPTG).getValueObject() == null) {
				e.addMsg(ResHelper.getString("60130payfile", "060130payfile0245")/*
																				 ...
																				 * "提示"
																				 */,
						ResHelper.getString("60130payfile", "060130payfile0274")/*
																				 * @
																				 * res
																				 * "请输入减税比例！"
																				 */);
				...
				if (UFDouble.ZERO_DBL.compareTo(dera) > 0 || new UFDouble(100f).compareTo(dera) < 0) {
					e.addMsg(ResHelper.getString("60130payfile", "060130payfile0256")/*
																					 ...
																					 * "错误"
																					 */,
							ResHelper.getString("60130payfile", "060130payfile0300")/*
																					 * @
																					 * res
																					 * "减税比例必须在0-100之间！"
																					 */);
