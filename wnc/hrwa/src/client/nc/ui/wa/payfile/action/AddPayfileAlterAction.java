 ...
 * @修改日期:
 */
public class AddPayfileAlterAction extends AddPayfileAction {

	private static final long serialVersionUID = -8031876023153530124L;
	...
	public AddPayfileAlterAction() {
		super();
		setBtnName(ResHelper.getString("60130payfile", "060130payfile0236")/*
																			 * @res
																			 * "新增变动人员"
																			 */);
		putValue(INCAction.CODE, "AddAlter");
		putValue(Action.ACCELERATOR_KEY, KeyStroke.getKeyStroke(KeyEvent.VK_A, Event.CTRL_MASK + Event.SHIFT_MASK));
		putValue(Action.SHORT_DESCRIPTION, ResHelper.getString("60130payfile", "060130payfile0237")/*
																									 * @
																									 * res
																									 * "新增变动人员(Ctrl+Shif+A)"
																									 */);
	...
	public void doActionForExtend(ActionEvent e) throws Exception {
		if (!WaCacheUtils.isModuleStarted(IMODELCODE)) {
			MessageDialog.showHintDlg(getParentContainer(), "",
					ResHelper.getString("60130payfile", "060130payfile0334")/*
																			 * @res
																			 * "未安装人事模块，该功能不可用！"
																			 */);
			return;
		}
		((PayfileAppModel) getModel()).setPType(PersonType.CHANGE);
		...
		if (((PayfileFormEditor) getEditor()) != null) {
