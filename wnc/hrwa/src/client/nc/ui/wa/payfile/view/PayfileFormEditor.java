...
import nc.vo.wa.payfile.ExNhiTypeEnum;
...
public class PayfileFormEditor extends HrBillFormEditor {
	...
	protected void onNotEdit() {
		super.onNotEdit();
		getBillCardPanel().getHeadItem(PayfileVO.EXNHITYPE).setEnabled(false);
	}
	...
	public void handleEvent(AppEvent event) {
		...
		// && (getPayfileAppModel().getUiState() ==
		// UIState.EDIT||getPayfileAppModel().getUiState() == UIState.ADD)) {
		// PayfileVO vo = (PayfileVO) getValue();
		// //扣税方式为“不扣税”，税率表可以为空。
		// //20151207 shenliangc NCdp205555686 打开薪资档案节点，第一次点击新增报未知错误
		// if (vo != null && vo.getTaxtype()!=null &&(vo.getTaxtype() ==
		// Taxtype.TAXFREE.toIntValue())) {
		// isTaxTableCanNull = true;
		// }
		// }
		...
		if (AppEventConst.MODEL_INITIALIZED.equalsIgnoreCase(event.getType())) {
			...
			AbstractRefModel refmodel = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.TAXTABLEID).getComponent())
					.getRefModel();
			if (refmodel != null) {
				refmodel.setPk_org(context.getPk_org());
			}
			((UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_LIABILITYORG).getComponent()).setMultiCorpRef(true);
		...
		// 新增时根据人员类型设置参照
		if (AppEventConst.UISTATE_CHANGED == event.getType() && getPayfileAppModel().getUiState() == UIState.ADD) {
		...
		if (AppEventConst.UISTATE_CHANGED == event.getType()) {
			...
			if (getPayfileAppModel().getUiState() == UIState.EDIT || getPayfileAppModel().getUiState() == UIState.ADD) {
				refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.FIPORGVID).getComponent()).getRefModel();
				if (refmodel2 != null) {
					refmodel2.setRefTitle(ResHelper.getString("60130payfile", "160130payfile0026")/*
																								 * @
																								 * res
																								 * "财务组织"
																								 */);
				...
				refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_LIABILITYORG).getComponent())
						.getRefModel();
				if (refmodel2 != null) {
					refmodel2.setRefTitle(ResHelper.getString("60130payfile", "160130payfile0024")/*
																								 * @
																								 * res
																								 * "成本中心"
																								 */);
			...
			if (getPayfileAppModel().getUiState() == UIState.EDIT) {
				getBillCardPanel().getHeadItem(PayfileVO.PK_PSNJOB).setEnabled(false);

				// 如果是多次发放，查询其他子方案有无此人;如果有，则扣税设置不能修改。
				if (context.getPk_prnt_class() != null) {
					String pk_psndoc = (String) getBillCardPanel().getHeadItem(PayfileVO.PK_PSNDOC).getValueObject();
					try {
						boolean existInSub = WADelegator.getPayfileQuery().existInSubClass(context, pk_psndoc);

						if (existInSub) {
							getBillCardPanel().getHeadItem(PayfileVO.TAXTYPE).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.TAXTABLEID).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.ISNDEBUCT).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.ISDERATE).setEnabled(false);
							getDerateptg().setEnabled(false);
							// getBillCardPanel().getHeadItem(PayfileVO.STOPFLAG)
							// .setEnabled(false);

							getBillCardPanel().getHeadItem(PayfileVO.FIPDEPTVID).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.FIPORGVID).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.LIBDEPTVID).setEnabled(false);
							getBillCardPanel().getHeadItem(PayfileVO.PK_LIABILITYORG).setEnabled(false);

						} else {
							getBillCardPanel().getHeadItem(PayfileVO.TAXTYPE).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.TAXTABLEID).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.ISNDEBUCT).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.ISDERATE).setEnabled(true);
							getDerateptg().setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.STOPFLAG).setEnabled(true);

							getBillCardPanel().getHeadItem(PayfileVO.FIPDEPTVID).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.FIPORGVID).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.LIBDEPTVID).setEnabled(true);
							getBillCardPanel().getHeadItem(PayfileVO.PK_LIABILITYORG).setEnabled(true);

							refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.FIPDEPTVID).getComponent())
									.getRefModel();
							if (refmodel2 != null) {
								Object o = getRefPane(PayfileVO.FIPORGVID).getRefModel().getValue(
								...
								refmodel2.setRefTitle(ResHelper.getString("60130payfile", "160130payfile0023")/*
																											 * @
																											 * res
																											 * "财务部门"
																											 */);
							}
							refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.LIBDEPTVID).getComponent())
									.getRefModel();
							if (refmodel2 != null) {
								String pk_costcenter = (String) getRefPane(PayfileVO.PK_LIABILITYORG).getRefModel()
								...
								if (pk_costcenter != null) {
									...
									((CostDeptVersionRefModel) getRefPane(PayfileVO.LIBDEPTVID).getRefModel())
											.setPk_costcenter(pk_costcenter);
								} else {
									getRefPane(PayfileVO.LIBDEPTVID).setWhereString("1=2");
								}
								refmodel2.setRefTitle(ResHelper.getString("60130payfile", "160130payfile0025")/*
																											 * @
																											 * res
																											 * "成本部门"
																											 */);
				...
				// 如果是不扣税，设置税相关项目不可修改
				int taxType = (Integer) getBillCardPanel().getHeadItem(PayfileVO.TAXTYPE).getValueObject();
				if (Taxtype.TAXFREE.equalsValue(taxType)) {
					...
					Boolean isDerate = (Boolean) getBillCardPanel().getHeadItem(PayfileVO.ISDERATE).getValueObject();
					if (!isDerate) {
						getDerateptg().setEnabled(false);
					} else {
						getDerateptg().setEnabled(true);
				...
				if (ptype == PersonType.PARTTIME) {// 兼职人员
					billCardPanel.getHeadItem(PayfileVO.PARTFLAG).setValue(UFBoolean.TRUE);
				}
				refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.FIPDEPTVID).getComponent()).getRefModel();
				if (refmodel2 != null) {
					refmodel2.setPk_org(null);
					refmodel2.setRefTitle(ResHelper.getString("60130payfile", "160130payfile0023")/*
																								 * @
				...
				refmodel2 = ((UIRefPane) billCardPanel.getHeadItem(PayfileVO.LIBDEPTVID).getComponent()).getRefModel();
				if (refmodel2 != null) {
	...
	private void setExNhiTypeEnabled() {
		...
		if (!StringUtils.isEmpty(getPayfileAppModel().getWaContext().getPk_wa_class())) {
			IUAPQueryBS query = NCLocator.getInstance().lookup(IUAPQueryBS.class);
			try {
				WaClassVO waClassVO = (WaClassVO) query.retrieveByPK(WaClassVO.class, getPayfileAppModel()
						.getWaContext().getPk_wa_class());
		...
		getBillCardPanel().getHeadItem(PayfileVO.EXNHITYPE).setEnabled(exnhiEnabled);
	}
	...
	public void afterEdit(BillEditEvent e) {
		...
		if (PayfileVO.PK_PSNJOB.equals(e.getKey())) {
			String pk_psnjob = billCardPanel.getHeadItem(PayfileVO.PK_PSNJOB).getValueObject().toString();
			UIRefPane refpane = (UIRefPane) getBillCardPanel().getHeadItem(PayfileVO.PK_PSNJOB).getComponent();
			PersonType ptype = getPayfileAppModel().getPType();
			...
			try {
				if (context.getPk_prnt_class() != null) {
					PayfileVO vo = WADelegator.getPayfileQuery().getPsnInPrntClass(context, pk_psnjob);
					if (vo != null) {
				...
				// 设置版本信息
				Object temp = getRefPane("pk_psnjob.pk_dept.pk_vid").getValueObj();
				if (temp != null) {
					// 任职部门有可能为空
					billCardPanel.getHeadItem(PayfileVO.WORKDEPTVID).setValue(((String[]) temp)[0]);
				}
				// 任职组织不为空
				temp = getRefPane("pk_psnjob.pk_org.pk_vid").getValueObj();
				if (temp != null) {
					billCardPanel.getHeadItem(PayfileVO.WORKORGVID).setValue(((String[]) temp)[0]);
				}
				...
				// 查询人员帐号
				bankAccVOs = getBankQuery().queryPsnBankaccUnionVOsByCon(wheresql.toString(), false);
				if (null != bankAccVOs) {
			...
			if (payOrgVO != null) {
				getRefPane(PayfileVO.PK_FINANCEORG).setPK(payOrgVO.getPk_financeorg());
				getRefPane(PayfileVO.PK_FINANACEDEPT).setPk_org(payOrgVO.getPk_financeorg());
				getRefPane(PayfileVO.PK_FINANACEDEPT).setPK(payOrgVO.getPk_financedept());

				getRefPane(PayfileVO.FIPORGVID).setPK(payOrgVO.getFiporgvid());
				getRefPane(PayfileVO.FIPDEPTVID).setPk_org(payOrgVO.getPk_financeorg());
				getRefPane(PayfileVO.FIPDEPTVID).setPK(payOrgVO.getFipdeptvid());
			...
			IUAPQueryBS query = NCLocator.getInstance().lookup(IUAPQueryBS.class);
			try {
				PsnJobVO psnjobVO = (PsnJobVO) query.retrieveByPK(PsnJobVO.class, pk_psnjob);
				if (psnjobVO != null) {
					PsnOrgVO psnorgVO = (PsnOrgVO) query.retrieveByPK(PsnOrgVO.class, psnjobVO.getPk_psnorg());
					if (psnorgVO != null) {
						if (UFBoolean.TRUE.equals(psnorgVO.getEndflag())) {
							// 離職人員
							if (UFBoolean.TRUE.equals(SysInitQuery4TWHR.getParaBoolean(psnorgVO.getPk_hrorg(),
									"TWEP0006"))) {
								// Y：補充保費費基
								billCardPanel.setHeadItem(PayfileVO.EXNHITYPE, ExNhiTypeEnum.EXNHIBASE.toIntValue());
							} else {
								// N：兼職所得
								billCardPanel.setHeadItem(PayfileVO.EXNHITYPE, ExNhiTypeEnum.PARTTIME.toIntValue());
							}
						} else {
							// 非離職人員：補充保費費基
							billCardPanel.setHeadItem(PayfileVO.EXNHITYPE, ExNhiTypeEnum.EXNHIBASE.toIntValue());
			...
			for (PsnBankaccUnionVO bankAccVO : bankAccVOs) {
				if (IPsnbankaccConst.PAYACC_MAINCARD.equals(bankAccVO.getPayacc())) {
					getBankAccPane().setValue(bankAccVO.getAccnum());
					getBankname().setPK(bankAccVO.getBankaccbasVO().getPk_banktype());
				} else if (IPsnbankaccConst.PAYACC_SUBCARD1.equals(bankAccVO.getPayacc())) {
					getBankAccPane2().setValue(bankAccVO.getAccnum());
					getBankname2().setPK(bankAccVO.getBankaccbasVO().getPk_banktype());
				} else if (IPsnbankaccConst.PAYACC_SUBCARD2.equals(bankAccVO.getPayacc())) {
					getBankAccPane3().setValue(bankAccVO.getAccnum());
					getBankname3().setPK(bankAccVO.getBankaccbasVO().getPk_banktype());
				}

			}
		} else if (PayfileVO.PK_LIABILITYORG.equals(e.getKey())) {
			String pk_costcenter = (String) getRefPane(PayfileVO.PK_LIABILITYORG).getRefModel().getValue(
					"pk_costcenter");
			...
			Object pk_dept = getRefPane(PayfileVO.LIBDEPTVID).getRefModel().getValue("pk_dept");
			if (pk_dept != null) {
				getRefPane(PayfileVO.PK_LIABILITYDEPT).setPK(pk_dept.toString());
			} else {
				getRefPane(PayfileVO.PK_LIABILITYDEPT).setPK(null);
			}
		} else if (PayfileVO.FIPORGVID.equals(e.getKey())) {
			Object pk_org = getRefPane(PayfileVO.FIPORGVID).getRefModel().getValue("pk_financeorg");
			...
			Object pk_dept = getRefPane(PayfileVO.FIPDEPTVID).getRefModel().getValue("pk_dept");
			if (pk_dept != null) {
	...
	private UIRefPane getTaxtableid() {
		return (UIRefPane) billCardPanel.getHeadItem(PayfileVO.TAXTABLEID).getComponent();
	}
	...
	private UIRefPane getBankname() {
		return (UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_BANKTYPE1).getComponent();
	}

	private UIRefPane getBankname2() {
		return (UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_BANKTYPE2).getComponent();
	}

	private UIRefPane getBankname3() {
		return (UIRefPane) billCardPanel.getHeadItem(PayfileVO.PK_BANKTYPE3).getComponent();
	}
	...
	public Object getValue() {
		...
		if (UIState.ADD.equals(getPayfileAppModel().getUiState())) {
			String pk_psndoc = (String) getPsnRefPane().getRefModel().getValue("bd_psndoc.pk_psndoc");
			payfileVO.setPk_psndoc(pk_psndoc);
			String pk_psnorg = (String) getPsnRefPane().getRefModel().getValue("hi_psnorg.pk_psnorg");
			...
			payfileVO.setPk_wa_class(getPayfileAppModel().getWaContext().getPk_wa_class());
			payfileVO.setCyear(getPayfileAppModel().getWaContext().getWaYear());
			payfileVO.setCperiod(getPayfileAppModel().getWaContext().getWaPeriod());
			payfileVO.setCyearperiod(getPayfileAppModel().getWaContext().getCyear()
		...
		payfileVO.setPk_prnt_class(getPayfileAppModel().getWaContext().getWaLoginVO().getPk_prnt_class());
		return payfileVO;
	}

	public IPsnBankaccQueryService getBankQuery() {
		if (bankQuery == null) {
			bankQuery = NCLocator.getInstance().lookup(IPsnBankaccQueryService.class);
		}
		return bankQuery;
	}
