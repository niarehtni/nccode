...
public class WaAdjustManageServiceImpl implements IWaAdjustManageService,
	...
	public AggPsnappaproveVO doSubmit(AggPsnappaproveVO psnappaproveAggVO)
		...
		/*
		 * if
		 * (INCSystemUserConst.NC_USER_PK.equals(psnappaproveVO.getApprover())
		 * || psnappaproveVO.getApprover() == null) {
		 * psnappaproveVO.setApprover(PubEnv.getPk_user()); }
		 */
		/*
		 * if
		 * (INCSystemUserConst.NC_USER_PK.equals(psnappaproveVO.getOperator())
	...
	public AggPsnappaproveVO doUnapprove(AggPsnappaproveVO aggVO)
		...
		// boolean blnEdit =
		// this.getAdjustDao().isPsnAppBByPkForUnapprove(strPKPsnapp);
		// if (blnEdit)
		// {
	 ...
	 */
	/*
	 * public PsnappaproveBVO[] findOldAndApplayInfo4Psn(PsnappaproveBVO[]
	 * psnappaproveBVOs, boolean OldAndApplay) throws BusinessException {
	 * getOldInfos(psnappaproveBVOs,OldAndApplay);
	 * 
	 * if (OldAndApplay) { // 默认申请的档别级别
	 * getAdjustDao().getDefultApplys4Psn(psnappaproveBVOs); } return
	 * psnappaproveBVOs; }
	 */
	 ...
	 */
	/*
	 * private PsnappaproveBVO getOldInfo(PsnappaproveBVO psnappaproveBVO,
	 * boolean OldAndApplay) throws BusinessException { String pk_wa_item =
	 ...
	 * null && pk_wa_item != null) { boolean negotiation_wage = false;
	 * IPsndocwadocQueryService psndocwadocQueryService =
	 * lookupPsndocwadocQueryService(); PsndocWadocVO psndocWadocVO =
	 * psndocwadocQueryService.queryAllVOsByPsnPK(pk_psndoc, pk_wa_grd,
	 * pk_wa_item); if (psndocWadocVO != null) { negotiation_wage =
	 * psndocWadocVO.getNegotiation_wage().booleanValue();
	 * psnappaproveBVO.setWa_old_money(psndocWadocVO.getNmoney());
	 * 
	 * if (!StringUtils.isEmpty(psndocWadocVO.getPk_wa_prmlv())) { // String[]
	 * oldCrtInfos =
	 * getAdjustDao().queryVOByCrtPK(psndocWadocVO.getPk_wa_crt());
	 ...
	 * psnappaproveBVO
	 * .setPk_wa_crt_old_showname(getAdjustDao().getCrtName(psndocWadocVO
	 * .getPk_wa_prmlv(),psndocWadocVO.getPk_wa_seclv(),
	 * psndocWadocVO.getIsmultsec().booleanValue())); } } if (OldAndApplay) { //
	 * 对于修改的时候， 有申请的级别 // 默认申请同原来的状态一致
	 * psnappaproveBVO.setNegotiation(UFBoolean.valueOf(negotiation_wage)); }
	 * else { if (psnappaproveBVO.getNegotiation() == null) {
	 * psnappaproveBVO.setNegotiation(UFBoolean.valueOf(false)); } } } return
	 * psnappaproveBVO; }
	 */
	/**
	 * 取得原始的薪资信息
	...
	public AggPsnappaproveVO insert(AggPsnappaproveVO vo)
		...
		// PsnappaproveBVO[] psnappaproveBVOs = (PsnappaproveBVO[])
		// vo.getChildrenVO();
		// for (PsnappaproveBVO psnappaproveBVO : psnappaproveBVOs)
		// {
		...
		// NCLocator.getInstance().lookup(IBillcodeManage.class).getBillCodeContext("6301",
		// ppprove.getPk_group(), ppprove.getPk_org());
		// // 如果是自动生成编码，则需要提交
		// if (billCodeContext != null)
		// {
		// getHrBillCode().commitPreBillCode("6301", ppprove.getPk_group(),
		// ppprove.getPk_org(), ppprove.getBillcode());
		// }
	...
	// {
	// AggPsnappaproveVO[] psnappaproveAggVOs =
	// BTOBXVOConversionUtility.retChangeBusiVO(psnappaproveAggVO);
	//
	// if (ArrayUtils.isEmpty(psnappaproveAggVOs))
	// {
	// return null;
	...
	public String noWorkflowApproveBill(AggPsnappaproveVO vo, String oper,
		...
		if (blApproved == IBillStatus.CHECKPASS) {
			...
			/*
			 * String[] period =
			 * this.getAdjustDao().getPeriod(psnappaproveVO.getUsedate
			 * ().toString()); if (period == null || period[0] == null ||
			 * period[1] == null) { throw new
			 * BusinessException(ResHelper.getString
			 * ("60130adjapprove","060130adjapprove0119")@res "生效日期没有对应的薪资期间");
			 * }
			 */
			// validateUsedate(psnappaproveVO.getUsedate());
	...
	private void saveDefaultConfirmValue(AggregatedValueObject hrAggVO)
		...
		// 为审批记录设定
		// PsnappaproveBVO.PK_WA_PRMLV_COFM，PsnappaproveBVO.PK_WA_SECLV_COFM
		// PsnappaproveBVO.WA_PRMLV_COFM，WA_SECLV_COFM WA_COFM_MONEY
		// WA_CRT_COFM_MONEY
	...
	// LoginContext context) throws BusinessException
	// {
	// return this.getAdjustDao().queryPsnappaproveByPsnPks(strPKpsndoc,
	// context);
	// }
	...
	public void validateMoneyLimit(AggPsnappaproveVO aggVO, boolean isApprove)
		...
		for (PsnappaproveBVO psnappaproveVO : psnappaproveVOs) {
			...
			// {
			// String info =
			// MessageFormat.format(ResHelper.getString("60130adjapprove","060130adjapprove0120")/*@res
			// "第 {0}行：薪资标准不能为空, 请修改."*/, rowCnt);
			// Logger.debug(info);
			...
			criterionVo = criterionVoMap.get(strPrmlvPK + strSeclvPK);
			// 不是宽带
	...
	private void validateUsedate(UFLiteralDate usedate)
		...
		if (usedate != null && usedate.toString().length() > 0) {
			...
			// NCLocator.getInstance().lookup(IConfigFileService.class.getName());
			// try
			// {
			...
			// throw new
			// BusinessException(ResHelper.getString("60130adjapprove","060130adjapprove0124")/*@res
			// "生效日期不在系统定义的基准期间之内, 请修改."*/);
			// }
			// }
