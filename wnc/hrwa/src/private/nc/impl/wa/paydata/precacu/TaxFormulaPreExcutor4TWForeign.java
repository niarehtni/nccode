...
public class TaxFormulaPreExcutor4TWForeign extends AbstractFormulaExecutor {
	...
	public void excute(Object paramObject, WaLoginContext context)
		...
		List<String> pk_psndocs = (ArrayList<String>) getDao().executeQuery(
				qryPsndocPK, new ColumnListProcessor());
		if (pk_psndocs == null || pk_psndocs.size() < 1) {
			return;
		}
		String twhr08 = getSysinit(context.getPk_org(), "TWHR08");
		UFDouble expireNum = getBaseDocUFDoubleValue(context.getPk_org(),
				"TWSP0013");
		// 根据人员主键查询出所有员工信息
		List<PsndocVO> psnList = getListPsndocVO(pk_psndocs
				.toArray(new String[0]));
		List<String> forePsndocs = new ArrayList<String>();// 外籍员工扣税
		for (int i = 0; i < psnList.size(); i++) {
			PsndocVO psnVo = psnList.get(i);
			String isResident = psnVo.getAttributeValue(LocalizationSysinitUtil
					.getTwhrlPsn("TWHRLPSN02")) != null ? psnVo
					.getAttributeValue(
							LocalizationSysinitUtil.getTwhrlPsn("TWHRLPSN02"))
					.toString() : "N";
			if ("N".equals(isResident)
					&& !isExpire(twhr08, expireNum, psnVo, context)) {
		...
		UFDouble basePay = getBaseDocUFDoubleValue(context.getPk_org(),
				"TWSP0009");// 基本工资
		UFDouble baseMultiple = getBaseDocUFDoubleValue(context.getPk_org(),
				"TWSP0010");// 基本工资倍数
		UFDouble taxRate = getBaseDocUFDoubleValue(context.getPk_org(),
				"TWSP0011");// 基本税率
		UFDouble excessTaxRate = getBaseDocUFDoubleValue(context.getPk_org(),
				"TWSP0012");// 超额扣税税率

		StringBuffer updateSql = new StringBuffer();
		updateSql.append("UPDATE wa_cacu_data\n");
		updateSql.append("SET cacu_value = CASE\n");
		updateSql.append("WHEN tax_base < " + basePay.toString() + " * "
				+ baseMultiple.toString() + " THEN\n");
		...
		updateSql.append("	wa_cacu_data.pk_wa_class = '"
				+ context.getPk_wa_class() + "'\n");
		updateSql.append("AND wa_cacu_data.creator = '"
	 ...
	 * @throws DAOException
	 */
	private String getSysinit(String pk_org, String initcode)
			throws DAOException {
	...
	private List<PsndocVO> getListPsndocVO(String[] pks)
			throws BusinessException {
		InSQLCreator isc = new InSQLCreator();
	 ...
	 * @throws BusinessException
	 */
	private boolean isExpire(String twhr08, UFDouble expireNum, PsndocVO psn,
