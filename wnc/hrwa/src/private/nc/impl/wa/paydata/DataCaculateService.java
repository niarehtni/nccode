...
public class DataCaculateService extends AbstractCaculateService {
	...
	public void doCaculate() throws BusinessException {
		try {
			...
			// ssx rebuild INDEXes
			// (do NOT need to be unremarked on normal runtime)
			// rebuildIndexes();
			// end
	...
	protected void initFeeCalculateTable() throws BusinessException {
		...
		if (!this.getBaseDao().isTableExisted("wa_cacu_overtimefee")) {
			executeSQLs("CREATE TABLE wa_cacu_overtimefee  ( intcomp int, amounttaxable NUMBER(28,8),"
					+ " amounttaxfree NUMBER(28,8), taxfreehours NUMBER(28,8), taxablehours NUMBER(28,8), "
		...
		if (!this.getBaseDao().isTableExisted("wa_cacu_leavehour")) {
			executeSQLs("CREATE TABLE wa_cacu_leavehour  (intcomp int, cyear VARCHAR(4), cperiod VARCHAR(2), pk_timeitem VARCHAR(20), hours NUMBER(28,8), curmon_hours NUMBER(28,8),"
					+ " pk_wa_class VARCHAR2(20),  creator VARCHAR2(20),"
					+ " pk_psndoc VARCHAR2(20), TS CHAR(19) DEFAULT TO_CHAR(SYSDATE,'yyyy-mm-dd hh24:mi:ss'),"
	...
	private void initWaCacuPsnJob() throws BusinessException {
		String strSQL = "insert into wa_cacu_psnjob(pk_wa_class, creator, pk_psnjob, lastdate, pk_psndoc) "
				...
				// 同日离职入职过滤掉离职记录
				+ " and hi_psnjob.trnsevent<>4 "
				// end MOD
				+ " AND hi_psnjob.begindate = (SELECT max(pj.begindate) FROM hi_psnjob pj WHERE  pj.pk_psndoc = hi_psnjob.pk_psndoc and pj.trnsevent<>4 "
	...
	public void doCaculate(WaClassItemVO[] classItemVOs) throws BusinessException {
		...
		for (WaClassItemVO waClassItemVO : classItemVOs) {
			...
			if(null!=waClassItemVO.getVformula() && waClassItemVO.getVformula().toUpperCase().startsWith("WASTATICSFIXPRDNO")){
				clearSalaryMidData();
			}
	 ...
	 * @throws DAOException
	 */
	private void updateBmItem(WaClassItemVO itemVO) throws BusinessException {
		...
		sbd.append("  and wa_data.pk_wa_data in ( select pk_wa_data from wa_data where pk_wa_class ='"
				+ itemVO.getPk_wa_class() + "' and cyear= '" + itemVO.getCyear() + "' and cperiod= '"
				+ itemVO.getCperiod() + "'");
	...
	private void execute(WaClassItemVO itemVO, SqlFragment sqlFragment) throws BusinessException {
		try {
			if (!StringUtils.isBlank(sqlFragment.getValue())) {
				executeSQLs(translate2ExecutableSql(itemVO, sqlFragment));
			}
			updateDigits(itemVO);
		} catch (Exception e) {
	 ...
	 * @throws DAOException
	 */
	private void updateConstantItem(WaClassItemVO itemVO) throws BusinessException {
		String where = getCacuRangeWhere();
		String sql = "update wa_data set wa_data." + itemVO.getItemkey() + " =" + itemVO.getVformula() + where;
		executeSQLs(sql);
	}
	 ...
	 * // 薪资计算完毕。删除对应中间表数据
	 */
	public void clearSalaryMidData() throws BusinessException {
		// 薪资计算完毕。删除对应中间表数据
		StringBuffer deleteSalaryMidData = new StringBuffer();
		deleteSalaryMidData.append("delete from SALARYCALCULATION  ");
		if(null!=getLoginContext().getPk_loginUser()){
			deleteSalaryMidData.append(" where pk_user = '" + getLoginContext().getPk_loginUser() + "' ");
		}
		executeSQLs(deleteSalaryMidData);
		// this.getBaseDao().executeUpdate(deleteWa_cacu_data.toString());
	}
