...
public abstract class AbstractCaculateService extends AppendBaseDAO {
	...
	public void setWaDataCacuRange4Class(String where) throws BusinessException {
		...
		this.getBaseDao().executeUpdate(insert2Wa_cacu_data.toString());
	...
	public void doEncrypt() throws BusinessException {
		// 初始化已解密數據集
		this.getDEUtil().initDataDecrptedMap();
		// 加密人員信息子集
	...
	public void doDecryptWaData() throws BusinessException {
		...
		// 處理其他期間
		dealOtherPeriodWhere(this.getLoginContext());
	}

	// end

	private void dealOtherPeriodWhere(WaLoginContext context) throws BusinessException {
		WaClassItemVO[] classItemVOs = getClassItemVOs();
		for (WaClassItemVO itemVO : classItemVOs) {
			if (itemVO.getVformula() != null && itemVO.getVformula().contains("waOtherPeriodData")) {
				String[] formulaItems = itemVO.getVformula().replace("waOtherPeriodData", "").replace("(", "")
						.replace("\"", "").replace(")", "").split(",");

				if (formulaItems[2] != null && formulaItems[2].length() == 6) {
					// 取期间号
					this.getDEUtil().decryptWaDataByCondition(
							"pk_wa_class = '" + formulaItems[0] + "' and cyearperiod='" + formulaItems[2]
									+ "' and pk_psndoc in (select pk_psndoc from wa_cacu_data where pk_wa_class = '"
									+ getLoginContext().getWaLoginVO().getPk_wa_class() + "' and creator='"
									+ getLoginContext().getPk_loginUser() + "')");
				} else if (formulaItems[2] != null) {
					String cyearperiod = "";
					// 取上期或本期
					if (formulaItems[2].equals("0")) {
						// 本期
						cyearperiod = context.getCyear() + context.getCperiod();
					} else if (formulaItems[2].equals("1")) {
						// 上期
						IWaPeriodQuery pQry = NCLocator.getInstance().lookup(IWaPeriodQuery.class);
						AccPeriodVO[] pds = pQry.getAllAccPeriodBef(context.getCyear() + context.getCperiod());
						if (pds != null && pds.length > 0) {
							AccPeriodVO lastPd = null;
							for (AccPeriodVO pd : pds) {
								if ((pd.getPeriodyear() + pd.getAccperiodmth()).equals(context.getCyear()
										+ context.getCperiod())) {
									cyearperiod = lastPd.getPeriodyear() + lastPd.getAccperiodmth();
								} else {
									lastPd = pd;
								}
							}
					...
					this.getDEUtil().decryptWaDataByCondition(
							"pk_wa_class = '" + formulaItems[0] + "' and cyearperiod='" + cyearperiod
									+ "' and pk_psndoc in (select pk_psndoc from wa_cacu_data where pk_wa_class = '"
									+ getLoginContext().getWaLoginVO().getPk_wa_class() + "' and creator='"
									+ getLoginContext().getPk_loginUser() + "')");
				}
	...
	public String translate2ExecutableSql(WaClassItemVO itemVO, SqlFragment sqlFragment) throws BusinessException {
		...
		if ("f_300".equals(itemVO.getItemkey())) {
			value = value.replace("#salaryyear#", itemVO.getCyear());
			value = value.replace("#salarymonth#", itemVO.getCperiod());
		}
		// 2019-05-25 ended
