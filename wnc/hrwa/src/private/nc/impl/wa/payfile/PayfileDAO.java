...
public class PayfileDAO extends AppendBaseDAO {
	 ...
	 *            业务委托sql
	 * @param type
	 *            1:正式人员；2:兼职人员；3：相关人员
	 * @return
	 * @throws BusinessException
	 */
	public PayfileVO[] queryBatchAddPayfileVO(WaLoginContext loginContext, String condition, String orderCondtion,
			String msSql, int type) throws BusinessException {

		InSQLCreator inSQLCreator = new InSQLCreator();

		try {
			PeriodVO periodVO = WADelegator.getPeriodQueryService().queryBySchemeYP(
					loginContext.getWaLoginVO().getPk_periodscheme(), loginContext.getWaYear(),
					loginContext.getWaPeriod());
			...
			isEnable = PubEnv.isModuleStarted(PubEnv.getPk_group(), PubEnv.MODULE_HRHI);
			if (!isEnable) {
			...
			if (type == 1 || type == 2) {
				...
				if (msSql != null) {
					if (type == 1) {
						...
						msSql = msSql.replaceAll("and hi_psnjob.lastflag = 'Y'", "");
						sqlBuffer
				...
				IAOSQueryService aosQueryService = NCLocator.getInstance().lookup(IAOSQueryService.class);
				OrgVO[] orgVOs = aosQueryService.queryOrgByHROrgPK(loginContext.getPk_org(), null, null,
			...
			// 过滤未启用组织职能的人员
			sqlBuffer.append(" and hi_psnjob.pk_org in (select pk_adminorg from org_admin_enable) ");
			if (!StringUtil.isEmptyWithTrim(condition)) {
				sqlBuffer.append(" and hi_psnjob.pk_psnjob in (select pk_psnjob from hi_psnjob where ")
			...
			String leavepsndoc = " select  wa_data.pk_psndoc from  wa_data  where wa_data.pk_psndoc =bd_psndoc.pk_psndoc and pk_wa_class in (  select wa_inludeclass.pk_childclass  from wa_inludeclass where pk_parentclass = '"
					+ pk_prnt_class
					+ "' and cyear = '"
					+ waYear
					+ "' and cperiod = '"
			...
			String powerSql = WaPowerSqlHelper.getWaPowerSql(loginContext.getPk_group(),
					HICommonValue.RESOUCECODE_6007PSNJOB, IHRWADataResCode.WADEFAULT, "hi_psnjob");
	...
	private String createTempTable(String sql) throws BusinessException {
		...
		try {
			...
			tableName = inSQLCreator.createTempTable(tableName, columns, pkindex);
	 ...
	 * @throws BusinessException
	 */
	public PayfileVO[] queryTransferPayfileVO(WaLoginContext loginContext, String condition, String orderCondtion)
			throws BusinessException {
		...
		if (!StringUtil.isEmptyWithTrim(condition)) {
			sqlBuffer
					.append(" and wa_data.pk_wa_data in (select pk_wa_data from wa_data " + "where "
							+ WherePartUtil.getCommonWhereCondtion4Param(loginContext.getWaLoginVO()) + " and ")
					.append(condition).append(")");
		}
		String powerSql = WaPowerSqlHelper.getWaPowerSql(loginContext.getPk_group(),
	...
	public String[] queryPKSByCondition(WaLoginContext context, String condition, String orderCondtion)
		...
		// 20150716 xiejie3
		// 由于只需要查询pk_wa_data所以可以只查询这个字段，减少返回数据量，减少查询时间。方法querypkPayfileVO为新增的专为查询pk

		// PayfileVO[] vos = queryTransferPayfileVO(context, condition,
		// orderCondtion);
	...
	public PayfileVO[] querypkTransferPayfileVO(WaLoginContext loginContext, String condition, String orderCondtion)
			throws BusinessException {

		StringBuffer sqlBuffer = new StringBuffer(" select wa_data.pk_wa_data from wa_data "
				+ getPayfileSelectField()[1]);
	...
	public String[] getPayfileSelectField() {
		...
		sqlBuffer.append("			wa_data.exnhitype ");
		sqlBuffer.append("  from wa_data ");
	 ...
	 * @throws DAOException
	 */
	public boolean isAllCaculated(String gzlbId, String waYear, String waPeriod, String cond) throws DAOException {
	...
	public PeriodVO queryStartEndDate(PeriodVO periodVO) throws DAOException {
		...
		StringBuffer sqlB = new StringBuffer();
		sqlB.append("select wa_period.pk_wa_period,wa_period.cyear,wa_period.cperiod,wa_period.cstartdate, wa_period.cenddate "); // 1
	 ...
	 * @throws DAOException
	 */
	public PeriodVO queryPeriodByWaClass(String pk_wa_class, String waYear, String waPeriod) throws DAOException {
	 ...
	 * @param psns
	 */
	public void delDataByClass(PayfileVO[] psns, String pk_wa_class, String cyear, String cperiod) throws DAOException {
	 ...
	 * @throws DAOException
	 */
	public void updatePsnHintFlag(WaLoginContext context, String whereSql, UFBoolean isHint) throws DAOException {
		StringBuffer sqlB = new StringBuffer();
	...
	private void deleteRelationTable(PayfileVO[] vos) throws DAOException {
		...
		String sql2 = "delete from wa_datas where pk_wa_data in(select pk_wa_data from wa_data where pk_psndoc in("
				+ pkdocs + ") and pk_wa_class = ? and cyear = ? and cperiod = ?)";
		String sql3 = "delete from wa_redata where pk_psndoc in(" + pkdocs
				+ ") and pk_wa_class = ? and cyear = ? and cperiod = ?";
		String sql4 = "delete from wa_data where pk_psndoc in(" + pkdocs
				+ ") and pk_wa_class = ? and cyear = ? and cperiod = ?";
		SQLParameter param = new SQLParameter();
	...
	public void updateAccount(PayfileVO[] addPsn) throws BusinessException {
		...
		if (getDBType() == DBConsts.ORACLE || getDBType() == DBConsts.DB2) {
			...
			sbd.append(" where wa_data.checkflag = 'N'     and wa_data.pk_wa_class = '" + pk_wa_class
			...
			sbd.append(" and  wa_data.checkflag = 'N'     and wa_data.pk_wa_class = '" + pk_wa_class
					+ "'     and wa_data.cyear = '" + cyear + "'     and wa_data.cperiod = '" + cperiod + "'  ");
	 ...
	 * @throws BusinessException
	 */
	public void updateTaxset2Total(String psnjobs, String pk_prnt_class, String pk_wa_class, String cyear,
			String cperiod, UFBoolean stopflag, String[] pk_psndocs) throws BusinessException {
		// guoqt
	 ...
	 * @throws BusinessException
	 */
	public void updateTaxsetByClass(String addSql, String pk_prnt_class, String pk_wa_class, String cyear,
			String cperiod) throws BusinessException {
		// 20151123 shenliangc
		// 喻程:直接在数据库里执行此sql耗时很少，但SPR中耗时则达到5511ms，可能是因为新增档案后，表结构变化导致需要重置执行计划。
		...
		String sql = "update a set a.isndebuct=b.isndebuct,a.taxtableid = b.taxtableid,a.isderate = b.isderate,"
				...
				+ " a.pk_wa_class = '" + pk_prnt_class + "' and a.cyear = '" + cyear + "' and a.cperiod = '" + cperiod
				+ "' and b.pk_wa_class ='" + pk_wa_class + "' and b.cyear = '" + cyear + "' and b.cperiod = '"
				+ cperiod + "'  " + addSql + " and a.pk_psndoc = b.pk_psndoc ";
	 ...
	 * @throws BusinessException
	 */
	public void updateTaxsetByClassStopflag(String addSql, String pk_prnt_class, String pk_wa_class, String cyear,
			String cperiod, UFBoolean stopflag, String[] pk_psndocs) throws BusinessException {
		InSQLCreator inSQLCreator = new InSQLCreator();
		String sql = "update a set stopflag='"
				+ stopflag
				+ "' ,isndebuct=b.isndebuct,taxtableid = b.taxtableid,isderate = b.isderate,"
				+ "derateptg = b.derateptg,taxtype = b.taxtype,taxorg=b.taxorg,taxsumuid=b.taxsumuid from wa_data b,wa_data a where"
				+ " a.pk_wa_class = '" + pk_prnt_class + "' and a.cyear = '" + cyear + "' and a.cperiod = '" + cperiod
				+ "' and b.pk_wa_class ='" + pk_wa_class + "' and b.cyear = '" + cyear + "' and b.cperiod = '"
				+ cperiod + "'  " + addSql + " and a.pk_psndoc = b.pk_psndoc " + " and a.pk_psndoc in("
				+ inSQLCreator.getInSQL(pk_psndocs) + ") ";
	 ...
	 * @throws BusinessException
	 */
	public void deleteFromTotal(String psndocs, String pk_prnt_class, String pk_wa_class, String cyear, String cperiod)
			throws BusinessException {
	 ...
	 * @throws DAOException
	 */
	public PayfileVO[] getPsnNotInPrntClass(String pk_prnt_class, String pk_wa_class, String cyear, String cperiod)
			throws DAOException {
	 ...
	 * @throws BusinessException
	 */
	public boolean existInSubClass(String pk_prnt_class, String pk_wa_class, String cyear, String cperiod, String psndoc)
			throws BusinessException {
	 ...
	 * @throws BusinessException
	 */
	public HashMap<String, PayfileVO> existInSubClass(String pk_prnt_class, String pk_wa_class, String cyear,
			String cperiod, PayfileVO[] vos) throws BusinessException {
		InSQLCreator isc = new InSQLCreator();
		String[] fields = new String[] { PayfileVO.PK_PSNDOC };
		String tablename = isc.insertValues(HRWACommonConstants.WA_TEMP_DATAPSN, fields, fields, vos);
		...
		sql.append(String
				.format(" where wa_data.pk_wa_class in(select pk_childclass from wa_inludeclass where pk_parentclass ='%s') and wa_data.pk_wa_class!='%s'  ",
						pk_prnt_class, pk_wa_class));
	...
	/**
	 * guoqt 判断一个人在其他子方案中是否存在（有且不停发）
	 * 
	 * @param vo
	 * @return
	 * @throws BusinessException
	 */
	public boolean existInSubClassStopFlag(String pk_prnt_class, String pk_wa_class, String cyear, String cperiod,
			String psndoc) throws BusinessException {
		String sql = "select 1 from wa_data where pk_psndoc = ? and cyear = ? and cperiod = ? "
	 ...
	 * @throws BusinessException
	 */
	public void updateLeaveFlag(String pk_wa_class, String cyear, String cperiod, boolean isleave)
			throws BusinessException {
		String sql = " update wa_waclass set leaveflag = '" + (isleave ? "Y" : "N") + "' where" + " pk_wa_class = '"
				+ pk_wa_class + "'";
		getBaseDao().executeUpdate(sql);
		sql = " update wa_periodstate set classtype = " + WACLASSTYPE.PARENTCLASS.getValue() + " where"
				+ " pk_wa_class = '" + pk_wa_class + "' and exists "
	 ...
	 * @throws BusinessException
	 */
	public PayfileVO getPsnInPrntClass(String classpk, String cyear, String cperiod, String pk_psnjob)
			throws BusinessException {
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("select  ");
		sqlBuffer.append("       wa_data.pk_wa_data, ");
	 ...
	 * @throws BusinessException
	 */
	public PayfileVO getPsnInPrntClassAll(String classpk, String cyear, String cperiod, String pk_psnjob)
			throws BusinessException {
		StringBuffer sqlBuffer = new StringBuffer();
		sqlBuffer.append("select  ");
	...
	// * @throws BusinessException
	// */
	// public PayfileVO[] getPkFinanceOrg(PayfileVO[] payfileVOs) throws
	// BusinessException{
	...
	// PubEnv.MODULE_RESA);
	// String sql =
	// "select job.pk_psnjob as pk_psnjob,org.pk_org as pk_org, org.pk_vid as pk_vid,"
	// +
	...
	// }
	// if(isEnable){
	// ICostCenterPubService orgQuery =
	// NCLocator.getInstance().lookup(ICostCenterPubService.class);
	// for(WaBmFileOrgVO payOrgVO:payOrgVOs){
	// //加上默认成本中心和成本部门
	// String pk_costcenter =
	// orgQuery.queryCostCenterByDept(payOrgVO.getPk_dept(), false);
	// for(PayfileVO payfileVO:payfileVOs){
	// if(payOrgVO.getPk_psnjob().equals(payfileVO.getPk_psnjob())){
	...
	public void synFiAndCostOrg(PayfileVO[] addPsn) throws BusinessException {
		...
		try {
			...
			if (getDBType() == DBConsts.ORACLE || getDBType() == DBConsts.DB2) {
				...
				sbd.append("			)  where wa_data.pk_psnjob in (" + inC.getInSQL(addPsn, PayfileVO.PK_PSNJOB)
				...
				sbd2.append("            )  where wa_data.pk_psnjob in (" + inC.getInSQL(addPsn, PayfileVO.PK_PSNJOB)
						+ ")  and  wa_data.pk_wa_class = '" + pk_wa_class + "' and wa_data.cyear = '" + cyear
						+ "' and wa_data.cperiod  = '" + cperiod + "' ");
				...
				sbd2.append("  where  resa_costcenter.enablestate = '2'  and wa_data.pk_psnjob in ("
						+ inC.getInSQL(addPsn, PayfileVO.PK_PSNJOB) + ")  and  wa_data.pk_wa_class = '" + pk_wa_class
						+ "' and wa_data.cyear = '" + cyear + "' and wa_data.cperiod  = '" + cperiod + "' ");
	 ...
	 * @throws BusinessException
	 */
	public HashMap<String, PayfileVO> existInCurrentClass(PayfileVO[] vos) throws BusinessException {
		HashMap<String, PayfileVO> map = new HashMap<String, PayfileVO>();
		InSQLCreator isc = new InSQLCreator();
		...
		if (tstr != null && tstr.length > 0) {
			for (PayfileVO str : tstr) {
				map.put(str.getPk_wa_class() + str.getPk_psndoc() + str.getCyear() + str.getCperiod(), str);
			}
		}
		return map;
	}
	 ...
	 * @throws DAOException
	 */
	public void updateParentClassPayOff(String pk_prnt_class, String cyear, String cperiod, String payoffflag)
			throws DAOException {
		String sql = "update wa_periodstate set payoffflag = '" + payoffflag + "' where  pk_wa_class = '"
				+ pk_prnt_class + "' and exists " + "(select wa_period.pk_wa_period  from wa_period  "
				+ "  where wa_period.pk_wa_period = wa_periodstate.pk_wa_period" + "    and wa_period.cyear =  '"
				+ cyear + "'   and wa_period.cperiod =  '" + cperiod + "'   and  " + "wa_periodstate.pk_wa_class =  '"
				+ pk_prnt_class + "')";
		getBaseDao().executeUpdate(sql);
	}
	...
	public void synTaxOrg(String pk_wa_class, String cyear, String cperiod, String inPsnjob) throws BusinessException {
		...
		sbd.append(" and  wa_data.pk_wa_class = '" + pk_wa_class + "' and wa_data.cyear = '" + cyear
				+ "' and wa_data.cperiod  = '" + cperiod + "' ");
		if (inPsnjob != null) {
	...
	public void synTaxOrgByPeriod(String pk_wa_class, String cyear, String cperiod) throws BusinessException {
		...
		sbd.append("			 where job.pk_psnjob = wa_data.pk_psnjob ) ");
		sbd.append("			  where  (taxorg is null or taxorg='~' )");
		sbd.append(" and  wa_data.pk_wa_class in (" + pk_wa_class + ") and wa_data.cyear||wa_data.cperiod >= '" + cyear
				+ cperiod + "' ");
		getBaseDao().executeUpdate(sbd.toString());
	...
	public void clearTaxsumuid() throws BusinessException {
		...
		sbd.append("update wa_data set taxsumuid = null where taxsumuid is not null and pk_group='"
				+ PubEnv.getPk_group() + "' ");
		getBaseDao().executeUpdate(sbd.toString());
	...
	public HashMap<String, String> queryLastTaxuid(PayfileVO[] addPsn) throws BusinessException {
		...
		// 上一期间与当前期间sql
		String yearperiodSql = " (lastdata.cyear||lastdata.cperiod = '" + addPsn[0].getCyear() + addPsn[0].getCperiod()
				+ "' and lastdata.pk_wa_class<>'" + addPsn[0].getPk_wa_class() + "') ";
	...
	public String getPrePeriodVO(String pk_wa_class, String cyear, String cperiod) throws BusinessException {
		...
		// sqlB.append(" ( select pk_periodscheme from wa_waclass where  pk_wa_class = '"+
		// pk_wa_class + "' ) and  ");
