...
public class OvertimeImportExecutor extends DataImportExecutor implements IDataExchangeExternalExecutor {
			...
			for (Map<String, Object> rowNCMap : this.getNcValueObjects()) {
				try {
					...
					if (StringUtils.isEmpty(begindate)) {
						begindate = getDateString((String) rowNCMap.get(rowNo + ":overtimebegindate") + " 00:00:00");
					}
					...
					String startdate = getDateString((String) rowNCMap.get(rowNo + ":overtimebegindate"));

					// 加班开始时刻 OVERTIMEBEGINTIME
					String starttime = (String) rowNCMap.get(rowNo + ":overtimebegintime");
					if (!StringUtils.isEmpty(starttime) && starttime.length() == 4) {
						starttime = starttime.substring(0, 2) + ":" + starttime.substring(2, 4) + ":00";
					} else {
						starttime = "00:00:00";
					}
					rowNCMap.put(rowNo + ":overtimebegintime", startdate + " " + starttime);

					Double addTime = Double.parseDouble((String) rowNCMap.get(rowNo + ":overtimehour"));
					if (addTime == null) {
						addTime = 0.0;
					}

					SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
					java.util.Date dStart = formatter.parse(startdate + " " + starttime);
					...
					rowNCMap.put(rowNo + ":overtimeenddate", getDateString(nDate.toString().substring(0, 10)));

					// 加班结束时刻 OVERTIMEENDTIME
					rowNCMap.put(rowNo + ":overtimeendtime", getDateString(nDate.toString()));
					...
					rowNCMap.put(rowNo + ":pk_overtimetype", otType);
