...
public class LeaveImportExecutor extends DataImportExecutor implements IDataExchangeExternalExecutor {
		...
		if (this.getNcValueObjects() != null && this.getNcValueObjects().size() > 0) {
			...
			for (Map<String, Object> rowNCMap : this.getNcValueObjects()) {
				try {
					...
					// CREATIONTIME
					String begindate = getDateString((String) rowNCMap.get(rowNo + ":"
							+ this.getReservedPropertyName("MADEDATE")));
					if (StringUtils.isEmpty(begindate)) {
						begindate = getDateString((String) rowNCMap.get(rowNo + ":leavebegindate") + " 00:00:00");
					}
					...
					// LEAVEBEGINDATE
					String startdate = getDateString((String) rowNCMap.get(rowNo + ":leavebegindate"));

					// LEAVEBEGINTIME
					String starttime = (String) rowNCMap.get(rowNo + ":leavebegintime");
					if (!StringUtils.isEmpty(starttime) && starttime.length() == 4) {
						starttime = starttime.substring(0, 2) + ":" + starttime.substring(2, 4) + ":00";
					} else {
						starttime = "00:00:00";
					}

					String endtime = (String) rowNCMap.get(rowNo + ":leaveendtime");
					if (!StringUtils.isEmpty(endtime) && endtime.length() == 4) {
						endtime = endtime.substring(0, 2) + ":" + endtime.substring(2, 4) + ":00";
					}

					rowNCMap.put(rowNo + ":leavebegintime", startdate + " " + starttime);

					// LEAVEREMARK
					rowNCMap.put(rowNo + ":leaveremark",
							rowNCMap.get(rowNo + ":" + this.getReservedPropertyName("REMARK")));

					// LEAVEENDDATE
					if (new UFDateTime(startdate + " " + endtime).before(new UFDateTime(startdate + " " + starttime))) {
						rowNCMap.put(rowNo + ":leaveenddate", getDateString(new UFDateTime(startdate + " " + endtime)
								.getDateTimeAfter(1).toString().substring(0, 10)));
						rowNCMap.put(rowNo + ":leaveendtime", getDateString(new UFDateTime(startdate + " " + endtime)
								.getDateTimeAfter(1).toString()));
					} else {
						rowNCMap.put(rowNo + ":leaveenddate", startdate);
						rowNCMap.put(rowNo + ":leaveendtime", startdate + " " + endtime);
					}

					// LEAVEHOUR
					Double addTime = Double.parseDouble((String) rowNCMap.get(rowNo + ":leavehour"));
					...
					// PK_PSNDOC
					String pk_psndoc = this.getPsndocByCodeDate(this.getRowPsnCode().get(rowNo), startdate);
					...
					if (psnjob != null && psnjob.size() > 0 && !StringUtils.isEmpty((String) psnjob.get("pk_psnjob"))) {
						rowNCMap.put(rowNo + ":pk_psnjob", psnjob.get("pk_psnjob"));
						rowNCMap.put(rowNo + ":pk_dept_v", psnjob.get("pk_dept_v"));
					} else {
						throw new BusinessException("未找到員工工作記錄");
					}

					// PK_PSNORG
					rowNCMap.put(rowNo + ":pk_psnorg", (String) psnjob.get("pk_psnorg"));
					...
					if (addTime < 0) {
						...
						if (!this.getRowLeaveType().get(rowNo).equals("F02")) {
							...
							String strSQL = getUpdateSQLByCancelInfo(this.getRowLeaveType().get(rowNo),
									this.getPk_org(), pk_psndoc, leaveType,
									(String) rowNCMap.get(rowNo + ":leavebegintime"),
									(String) rowNCMap.get(rowNo + ":leaveendtime"), addTime);
	...
	private String getUpdateSQLByCancelInfo(String leaveTypeCode, String pk_org, String pk_psndoc, String leaveType,
			String newleavestarttime, String newleaveendtime, Double leaveHour) throws BusinessException {
		String strSQL = "select pk_leavereg, leavebegintime, leaveendtime, leavehour from tbm_leavereg where pk_org="
				+ getStringValue(pk_org) + " and pk_psndoc=" + getStringValue(pk_psndoc) + " and pk_leavetype="
				+ getStringValue(leaveType) + " and (leavebegintime<=" + getStringValue(newleavestarttime)
				+ " and leaveendtime>=" + getStringValue(newleaveendtime) + ")";
		Map<String, Object> originLeave = (Map<String, Object>) this.getBaseDAO().executeQuery(strSQL,
				new MapProcessor());

		if (originLeave != null && originLeave.size() > 0) {
			UFDateTime originBeginTime = new UFDateTime((String) originLeave.get("leavebegintime"));
			UFDateTime originEndTime = new UFDateTime((String) originLeave.get("leaveendtime"));
			Double originLeaveHour = Double.valueOf(String.valueOf(originLeave.get("leavehour")));
			...
			if (!leaveTypeCode.equals("F02")) {
				if (originBeginTime.equals(new UFDateTime(newleavestarttime))) {
					if (originBeginTime.after(new UFDateTime(newleaveendtime))) {
						// 舊的開始時間晚於新的開始時間：跨天
						newLeaveStartDate = new UFDateTime(newleaveendtime).getDateTimeAfter(1).toString()
								.substring(0, 10);
						newLeaveStartTime = new UFDateTime(newleaveendtime).getDateTimeAfter(1).toString();
					} else {
						newLeaveStartDate = new UFDateTime(newleaveendtime).toString().substring(0, 10);
						newLeaveStartTime = new UFDateTime(newleaveendtime).toString();
					}
					newLeaveEndDate = originEndTime.toString().substring(0, 10);
					newLeaveEndTime = originEndTime.toString();
				} else {
					newLeaveStartDate = originBeginTime.toString().substring(0, 10);
					newLeaveStartTime = originBeginTime.toString();
					if (new UFDateTime(newleavestarttime).before(originBeginTime)) {
						newLeaveEndDate = new UFDateTime(newleavestarttime).getDateTimeAfter(1).toString()
								.substring(0, 10);
						newLeaveEndTime = new UFDateTime(newleavestarttime).getDateTimeAfter(1).toString();
					} else {
						newLeaveEndDate = new UFDateTime(newleavestarttime).toString().substring(0, 10);
						newLeaveEndTime = new UFDateTime(newleavestarttime).toString();
					}
				}
			}
			strSQL = "UPDATE tbm_leavereg SET ";
			if (!leaveTypeCode.equals("F02")) {
				strSQL += "leavebegindate=" + getStringValue(newLeaveStartDate) + ", leavebegintime="
						+ getStringValue(newLeaveStartTime) + ", leaveenddate=" + getStringValue(newLeaveEndDate)
						+ ", leaveendtime=" + getStringValue(newLeaveEndTime) + ", ";
			}
			strSQL += "leavehour=" + String.valueOf(newLeaveHour) + ", isleaveoff='Y' WHERE pk_leavereg="
			...
			for (Map<String, Object> jsonobj : this.getJsonValueObjects()) {
				...
				for (Entry<String, Object> entry : jsonobj.entrySet()) {
					if (entry.getKey().equals("ROWNO")) {
						...
						this.getRowLeaveType().put(rowNo, (String) entry.getValue());
