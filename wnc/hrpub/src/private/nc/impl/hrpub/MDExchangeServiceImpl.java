...
import nc.impl.hrpub.dataexchange.businessprocess.DepartmentHistoryAppendImportExecutor;
...
public class MDExchangeServiceImpl implements IMDExchangeService, IBackgroundWorkPlugin {
	...
	private void taskImportJsonFile(String fullFilename) throws BusinessException {
		Logger.error("Import Json file via Interface");
		try {
			byte[] buffer = Files.readAllBytes(Paths.get(fullFilename));
			String jsonString = new String(buffer, StandardCharsets.UTF_8);
			String strMessage = new WNCDataExchangeImpl().doTransfer(jsonString);
		...
		if (exType.equals(ExchangeTypeEnum.CUSTOMIZE) || newImportExecutor == null) {
			if (classname.toLowerCase().startsWith("hi_psndoc_")) {
				...
				case "hrdept":
					...
					break;
				case "depthistory":
					// newImportExecutor = new
					// DepartmentHistoryImportExecutor();
					newImportExecutor = new DepartmentHistoryAppendImportExecutor();
					break;
				case "bd_psndoc":
