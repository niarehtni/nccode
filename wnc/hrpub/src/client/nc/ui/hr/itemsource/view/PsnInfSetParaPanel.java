...
public class PsnInfSetParaPanel extends UIPanel implements IParaPanel {
	...
	// #21106 汇总条件UIComboBox yejk 2018-09-13
	private nc.ui.pub.beans.UIComboBox isSumComBox = null;
	// #21106 汇总条件UILabel yejk 2018-09-13
	private nc.ui.pub.beans.UILabel isSumLabel = null;
	...
	class IvjEventHandler implements java.awt.event.ActionListener, nc.ui.pub.beans.ValueChangedListener {
		public void actionPerformed(java.awt.event.ActionEvent e) {
			...
			// on 2018-10-06
			// begin
			else if (e.getSource() == PsnInfSetParaPanel.this.getIsSumComBox()) {
				if (PsnInfSetParaPanel.this.getrefHifld().getRefModel() instanceof ParaInfSetandFldRefModel) {
					if (PsnInfSetParaPanel.this.getIsSumComBox().getSelectedIndex() == 1) {
						String strWhere = " infosetitem.data_type in ('18')  and item_code <> 'pk_psndoc' and item_code <> 'addr' "
								+ " and infoset.pk_infoset in (select pk_infoset from hr_infoset_item where ref_model_name = '0001Z710000000000ZO4')";
						PsnInfSetParaPanel.this.getrefHifld().getRefModel().setWherePart(strWhere);
					} else if (PsnInfSetParaPanel.this.getIsSumComBox().getSelectedIndex() == 2
							|| PsnInfSetParaPanel.this.getIsSumComBox().getSelectedIndex() == 3) {
						String strWhere = " infosetitem.data_type in ('18')  and item_code <> 'pk_psndoc' and item_code <> 'addr' "
								+ " and infoset.pk_infoset in (select pk_infoset from hr_infoset_item where ref_model_name = '0001Z01000000001BVPV')";
						PsnInfSetParaPanel.this.getrefHifld().getRefModel().setWherePart(strWhere);
					} else {
						PsnInfSetParaPanel.this.getrefHifld().setRefModel(new ParaInfSetandFldRefModel());
					}
				}
			}
			// end
	...
	public void checkPara(int dataType) throws java.lang.Exception {
		...
		if (getrefHifld().getRefPK() == null) {
			throw new Exception(ResHelper.getString("6001itemsrce", "06001itemsrce0013")
					/* @res "请选择要取数的员工信息项！" */);
		}

		// 再判断类型是否匹配
		int l_datatype = Integer.parseInt(getrefHifld().getRefValue(DATATYPE).toString());
		...
		if (!checkType(dataType, l_datatype)) {
			throw new Exception(ResHelper.getString("6001itemsrce", "06001itemsrce0014")
					/* @res "所选信息项数据类型不合法！" */);
		}
	...
	private boolean checkType(int dataType, int l_datatype) {
		return dataType == 0
				&& (l_datatype == IBillItem.DECIMAL || l_datatype == IBillItem.INTEGER || l_datatype == IBillItem.MONEY || l_datatype == IBillItem.FRACTION)

				|| dataType == 1
				&& (l_datatype == IBillItem.STRING || l_datatype == IBillItem.UFREF || l_datatype == IBillItem.COMBO
				|| l_datatype == IBillItem.USERDEF || l_datatype == IBillItem.BOOLEAN || l_datatype == IBillItem.MULTILANGTEXT)
				|| dataType == 2 && l_datatype == IBillItem.LITERALDATE || dataType == 3
				&& l_datatype == IBillItem.BOOLEAN;
	 ...
	 * Comment
	 */
	public void chkIfPeriod_ActionPerformed(java.awt.event.ActionEvent actionEvent) {
		if (actionEvent.getSource().equals(getchkIfPeriod())) {
			if (getchkIfPeriod().isSelected()) {// 按照期间设置日期
				...
				getlblMsg().setText(ResHelper.getString("6001itemsrce", "06001itemsrce0015")
						/* @res "默认：期间为空则按当前期间" */);
			} else {// 按原来的日期设置
				...
				getlblMsg().setText(ResHelper.getString("6001itemsrce", "06001itemsrce0016")
						/* @res "默认：日期为空取最新记录的数据" */);
			}
		}
		return;
	...
	private nc.ui.pub.beans.UIComboBox getcmbDate() {
		if (ivjcmbDate == null) {
			try {
				...
				ivjcmbDate.setBounds(155, 120, 75, 22);
				ivjcmbDate.setTranslate(true);

				/*
				 * String[] ml = new String[2]; ml[0] =
				 * ResHelper.getString("common", "UC000-0003900")
				 *
				 * @res "起始日期" ; ml[1] = ResHelper.getString("common",
				 * "UC000-0003203")
				 *
				 * @res "终止日期" ; String[] mlDefault = new String[] {
				 * ResHelper.getString("common", "UC000-0003900")
				 *
				 * @res "起始日期" , ResHelper.getString("common", "UC000-0003203")
				 *
				 * @res "终止日期" }; -= notranslate = -
				 */
				 ...
				 * "包含于薪资期间"
				 */;
				String[] mlDefault = new String[] { ResHelper.getString("common", "UC000-0003900")/*
																								 * @
																								 * res
																								 * "起始日期"
																								 */,
	 ...
	 * @return nc.ui.pub.beans.UIComboBox
	 */
	private nc.ui.pub.beans.UIComboBox getIsSumComBox() {
		if (isSumComBox == null) {
			try {
				isSumComBox = new nc.ui.pub.beans.UIComboBox();
				isSumComBox.setName("isSumComBox");
				isSumComBox.setBounds(90, 0, 140, 22);
				isSumComBox.setTranslate(true);

				String[] mlDefault = new String[4];
				mlDefault[0] = "0";
				mlDefault[1] = "1";
				mlDefault[2] = "2";
				mlDefault[3] = "3";
				String[] ml = new String[] { " ",// 空选项
						ResHelper.getString("common", "UC001-0000160"),// 按法人组织汇总
						ResHelper.getString("common", "UC001-0000161"),// 按人力资源组织汇总
						ResHelper.getString("common", "UC001-0000162") // 全部汇总
				};
				PairFactory mPairFactory = new PairFactory(ml, mlDefault);
				isSumComBox.addItems(mPairFactory.getAllConstEnums());
			} catch (java.lang.Throwable ivjExc) {
	...
	/**
	 *
	 * @Description: 汇总条件label
	 * @author yejk
	 * @date 2018-9-16
	 * @return
	 * @return nc.ui.pub.beans.UILabel
	 */
	private nc.ui.pub.beans.UILabel getIsSumLabel() {
		if (isSumLabel == null) {
			try {
				isSumLabel = new nc.ui.pub.beans.UILabel();
				isSumLabel.setName("lblHiFld");
				isSumLabel.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0032")
						/* @res "汇总条件" */);
				isSumLabel.setForeground(java.awt.Color.black);
				isSumLabel.setILabelType(0/** Java默认(自定义) */
				);
				isSumLabel.setFont(new java.awt.Font("dialog", 0, 12));
				isSumLabel.setBounds(5, 0, 70, 22);
				isSumLabel.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private nc.ui.pub.beans.UIComboBox getcmbRefdata() {
		if (ivjcmbRefdata == null) {
			try {
				...
				ivjcmbRefdata.setBounds(90, 160, 140, 22);

				ivjcmbRefdata.setTranslate(true);// 国际化时要翻译
				String[] ml = new String[3];
				ml[0] = "";
				ml[1] = ResHelper.getString("common", "UC000-0003279")/*
				 * @res "编码"
				 */;
				ml[2] = ResHelper.getString("common", "UC000-0001155")/*
				 * @res "名称"
				 */;
				String[] mlDefault = new String[] { "", ResHelper.getString("common", "UC000-0003279")/*
																									 * @
																									 * res
																									 * "编码"
																									 */,
	...
	private nc.ui.pub.beans.UILabel getlblDate() {
		if (ivjlblDate == null) {
			try {
				...
				ivjlblDate.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0018")
						/* @res "基准日期" */);
				ivjlblDate.setBounds(5, 120, 70, 22);
				ivjlblDate.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
	...
	private nc.ui.pub.beans.UILabel getlblHiFld() {
		if (ivjlblHiFld == null) {
			try {
				...
				ivjlblHiFld.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0019")
						/* @res "员工信息项" */);
				...
				ivjlblHiFld.setBounds(5, 80, 70, 22);
				ivjlblHiFld.setHorizontalAlignment(javax.swing.SwingConstants.LEFT);
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private nc.ui.pub.beans.UILabel getlblMsg() {
		if (ivjlblMsg == null) {
			try {
				...
				ivjlblMsg.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0020")
						/* @res "注：基准日期为空默认为系统登录日期" */);
				ivjlblMsg.setBounds(8, 240, 186, 22);
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private nc.ui.pub.beans.UILabel getlblRefdata() {
		if (ivjlblRefdata == null) {
			try {
				...
				ivjlblRefdata.setText(ResHelper.getString("601315common", "0601315common0017")/*
				 * @
				 * res
				 * "参照取值"
				 */);
				ivjlblRefdata.setBounds(5, 160, 70, 22);
				ivjlblRefdata.setHorizontalAlignment(javax.swing.SwingConstants.RIGHT);
	...
	private nc.ui.pub.beans.UICheckBox getchkIfPeriod() {
		if (ivjchkIfPeriod == null) {
			try {
				...
				ivjchkIfPeriod.setText("  " + ResHelper.getString("6001itemsrce", "06001itemsrce0017")
						/* @res "日期按业务期间设置" */);
				ivjchkIfPeriod.setBounds(5, 40, 168, 22);
				ivjchkIfPeriod.setActionCommand(ResHelper.getString("6001itemsrce", "06001itemsrce0017")
						/* @res "日期按业务期间设置" */);
	...
	public java.lang.String[] getPara() throws java.lang.Exception {
		...
		String isSum = "";
		isSum = getIsSumComBox().getSelectdItemValue().toString();
		...
		if (getrefHifld().isVisible()) {
			table_code = getrefHifld().getRefModel().getValue(TABLE_CODE).toString();
			table_Fld = getrefHifld().getRefModel().getValue(FLDCODE).toString();

			// "@登录日期@", "@BFV_LOGDATE@"
			if (getchkIfPeriod().isVisible() && getchkIfPeriod().isSelected()) {

				if (gettxtSPeriod().getText() == null || gettxtSPeriod().getText().trim().length() == 0) {
					l_date = "0";
				} else {
			...
			} else if (getrefDate().getText() != null && getrefDate().getText().length() > 0) {
				Object object = getrefDate().getValueObj();
				if (object instanceof UFDate) {
					UFDate date = (UFDate) object;
					l_date = date.getYear() + IFormulaConst.splitSymbol + date.getMonth() + IFormulaConst.splitSymbol
							+ date.getDay();
				}
			}
			...
			if (getrefHifld().getRefModel().getValue("ref_model_name") != null) {
				String ref_model_name = getrefHifld().getRefModel().getValue("ref_model_name").toString();
				...
				if (model != null) {
					...
					if (ref_table == null || ref_item_code == null) {
						throw new BusinessException(ResHelper.getString("6001itemsrce", "06001itemsrce0021")
								/* @res "不能取该值， 具体原因是该数据对应的模型没有表名或者没有对应的列." */);
					}
				}
			}
			ref_infosetpk = getrefHifld().getRefModel().getValue("infoset.pk_infoset").toString();
			// if (getcmbRefdata().isVisible() &&
			...
			if (l_datatype == IBillItem.COMBO) {
				Object obj = getrefHifld().getRefModel().getValue("pk_infoset_item");
				if (obj != null) {
			...
			if (getchkIfPreLeave().isVisible() && getchkIfPreLeave().isSelected()
					&& ref_infosetpk.equals(ItemPropertyConst.FULLTIME_INFSET_PK)) {
			...
			if (isMutlTableJoin(ref_table)) {
				...
				if (model.getClass().getName().equals("nc.ui.hrcm.templet.model.ContModelRef")) {
			...
			return new String[] { table_code, getFldAfterdot(table_Fld), getFldAfterdot(refcodeORname), l_date,
					ref_table, getFldAfterdot(ref_pk_field), conditon, isSum };
		}
		return null;
	}
	...
	private nc.ui.pub.beans.UIRefPane getrefDate() {
		if (ivjrefDate == null) {
			try {
				...
				ivjrefDate.setBounds(90, 120, 140, 22);
	...
	private nc.ui.pub.beans.UIRefPane getrefHifld() {
		if (ivjrefHifld == null) {
			try {
				...
				ivjrefHifld.setBounds(90, 120, 140, 22);

				try {
					Class refmodel = Class.forName("nc.ui.hr.itemsource.view.ParaInfSetandFldRefModel");
					ivjrefHifld.setRefModel((nc.ui.bd.ref.AbstractRefModel) refmodel.newInstance());
					...
					ivjrefHifld.getRefModel().setWherePart(
							"   infosetitem.data_type in ('" + IBillItem.STRING + "','" + IBillItem.INTEGER + "','"
									+ IBillItem.DECIMAL + "','" + IBillItem.UFREF + "','" + IBillItem.COMBO + "','"
									+ IBillItem.USERDEF + "','" + IBillItem.MULTILANGTEXT + "','" + IBillItem.MONEY
									+ "','" + IBillItem.FRACTION + "','" + IBillItem.LITERALDATE + "','"
									+ IBillItem.BOOLEAN + "') " +
	...
	private UIAsteriskPanelWrapper getrefHifldUI() {
		...
		if (refHifldUI == null) {
			...
			refHifldUI.setBounds(83, 80, 147, 22);
	...
	private nc.ui.pub.beans.UITextField gettxtSPeriod() {
		if (ivjtxtSPeriod == null) {
			try {
				...
				ivjtxtSPeriod.setBounds(95, 120, 30, 20);
	...
	private nc.ui.pub.beans.UILabel getUILabel5() {
		if (ivjUILabel5 == null) {
			try {
				...
				ivjUILabel5.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0025")
						/* @res " 前" */);
				ivjUILabel5.setBounds(80, 120, 28, 22);
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private nc.ui.pub.beans.UILabel getUILabel52() {
		if (ivjUILabel52 == null) {
			try {
				...
				ivjUILabel52.setText(ResHelper.getString("common", "UC000-0002560")
						/* @res "期间" */);
				ivjUILabel52.setBounds(128, 120, 28, 22);
			} catch (java.lang.Throwable ivjExc) {
				handleException(ivjExc);
			}
		}
	...
	private void initConnections() throws java.lang.Exception {
		...
		getIsSumComBox().addActionListener(ivjEventHandler);
	}
	...
	private void initialize() {
		try {
			...
			// #21106 添加汇总条件label和汇总条件ComBox yejk 2018-09-13
			add(getIsSumLabel(), getIsSumLabel().getName());
			add(getIsSumComBox(), getIsSumComBox().getName());
	...
	public void updateDis(java.lang.String newFuncName) {
		...
		// MOD ssx modified for TWLC on 2018-10-06
		getIsSumLabel().setVisible(true);
		getIsSumComBox().setVisible(true);
		//
		...
		getchkIfPreLeave().setVisible(false);
		getlblDate().setText(ResHelper.getString("common", "UC000-0002313")/*
		 * @res
		 * "日期"
		 */);
		getlblMsg().setText(ResHelper.getString("6001itemsrce", "06001itemsrce0016")
	...
	private nc.ui.pub.beans.UICheckBox getchkIfPreLeave() {
		if (ivjchkIfPreLeave == null) {
			try {
				...
				ivjchkIfPreLeave.setText(ResHelper.getString("6001itemsrce", "06001itemsrce0030")
						/* @res "取离职前工作记录" */);
				ivjchkIfPreLeave.setBounds(5, 200, 168, 22);
				ivjchkIfPreLeave.setActionCommand(ResHelper.getString("6001itemsrce", "06001itemsrce0030")
						/* @res "取离职前工作记录" */);
