...
public class LeaveBalanceDAO {
	...
	/**
	 * 将一批人员的在year年的入职年开始日期和结束日期插入临时表 要求psndocVOs的pk_psnorg没有重复的
	 * 
	 * @param year
	 * @param psndocVOs
	 * @param hireDateMap
	 ...
	 */
	public void initHireDateTempTable(String year, TBMPsndocVO[] psndocVOs, Map<String, UFLiteralDate> hireDateMap)
			throws BusinessException {
		...
		List<String> addedPsnorgs = new ArrayList<String>();
		try {
	...
	public LeaveBalanceVO[] queryByCondition4HireDateSettlementWithHireDateTempTable(String pk_org,
			LeaveTypeCopyVO typeVO, String year, FromWhereSQL fromWhereSQL) throws DAOException {
		// 需要查询组织和部门版本
		...
		String deptversionAlias = fromWhereSQL.getTableAliasByAttrpath("pk_psnjob." + PsnJobVO.PK_DEPT_V
				+ FromWhereSQLUtils.getAttPathPostFix());
		String[] extraSelFields = new String[] { "hiredate.hirebegindate", "hiredate.hireenddate",
				"leavebalance.pk_timeitem", "leavebalance.pk_leavebalance", "leavebalance.curyear",
				"leavebalance.translatein", "leavebalance.translateout", "leavebalance.transflag",
				"leavebalance.lastdayorhour", "leavebalance.curdayorhour", "leavebalance.realdayorhour",
				"leavebalance.restdayorhour", "leavebalance.yidayorhour", "leavebalance.freezedayorhour",
				"leavebalance.calculatetime", "leavebalance.issettlement", "leavebalance.settlementdate",
				"leavebalance.leaveindex", "leavebalance.isuse", "leavebalance.ts", "leavebalance.changelength",
				"leavebalance.salaryyear", "leavebalance.salarymonth",
		...
		para.addParam(year);
		para.addParam(pk_org);
		String sql = FromWhereSQLUtils.createSelectSQL(fromWhereSQL, TBMPsndocVO.getDefaultTableName(), null,
				extraSelFields, extraJoins, extraConds, null);
		LeaveBalanceVO[] vos = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class,
				(Collection) new BaseDAO().executeQuery(sql, para, new BeanListProcessor(LeaveBalanceVO.class)));
		if (!ArrayUtils.isEmpty(vos)) {
			...
			for (LeaveBalanceVO vo : vos) {
				...
				vo.setPeriodextendenddate(extendDateCounts == 0 ? vo.getHireenddate() : vo.getHireenddate()
						.getDateAfter(extendDateCounts));
	 ...
	 * 关联条件是pk_psnorg,不是pk_psndoc 由于是考勤档案left join
	 * leavebalance，因此即使leavebalance中无记录，也会查出一条来。如果已经存在，则有几条就查询出几条
	 * 
	 * @param pk_org
	 * @param fromWhereSQL
	 * @param beginDate
	 ...
	 */
	@SuppressWarnings("unchecked")
	public LeaveBalanceVO[] queryByDateScope(String pk_org, FromWhereSQL fromWhereSQL, UFLiteralDate beginDate,
			UFLiteralDate endDate, String pk_timeitem, String year, String month) throws BusinessException {
		// 需要查询组织和部门版本
		...
		String deptversionAlias = fromWhereSQL.getTableAliasByAttrpath("pk_psnjob." + PsnJobVO.PK_DEPT_V
				+ FromWhereSQLUtils.getAttPathPostFix());
		SQLParamWrapper wrapper = TBMPsndocSqlPiecer.selectJoinByPsnorgFieldAndDateArea(pk_org, new String[] {
				TBMPsndocVO.PK_GROUP, TBMPsndocVO.PK_ORG, TBMPsndocVO.PK_PSNDOC, TBMPsndocVO.PK_PSNORG,
				TBMPsndocVO.PK_PSNJOB, TBMPsndocVO.PK_TBM_PSNDOC }, new String[] { "tbm_leavebalance.pk_timeitem",
				"tbm_leavebalance.pk_leavebalance", "tbm_leavebalance.curyear", "tbm_leavebalance.curmonth",
				"tbm_leavebalance.translatein", "tbm_leavebalance.translateout", "tbm_leavebalance.transflag",
				"tbm_leavebalance.lastdayorhour", "tbm_leavebalance.curdayorhour", "tbm_leavebalance.realdayorhour",
				"tbm_leavebalance.restdayorhour", "tbm_leavebalance.yidayorhour", "tbm_leavebalance.freezedayorhour",
				"tbm_leavebalance.calculatetime", "tbm_leavebalance.issettlement", "tbm_leavebalance.settlementdate",
				"tbm_leavebalance.leaveindex", "tbm_leavebalance.isuse", "tbm_leavebalance.ts",
				"tbm_leavebalance.changelength", "tbm_leavebalance.salaryyear", "tbm_leavebalance.salarymonth",
				orgversionAlias + "." + AdminOrgVersionVO.PK_VID + " as " + LeaveBalanceVO.PK_ORG_V,
				deptversionAlias + "." + DeptVersionVO.PK_VID + " as " + LeaveBalanceVO.PK_DEPT_V },
				"tbm_leavebalance", "tbm_leavebalance.pk_org", "tbm_leavebalance.pk_psnorg",
				"tbm_leavebalance.pk_org=? and tbm_leavebalance.pk_timeitem = ? and tbm_leavebalance.curyear=? "
						+ (month == null ? " and tbm_leavebalance.curmonth is null "
								: ("and tbm_leavebalance.curmonth=?")),// 2014-04-26修改按年度结算的月份必须为空
				beginDate.toString(), endDate.toString(), null, false, fromWhereSQL);
		BaseDAO dao = new BaseDAO();
		...
		LeaveBalanceVO[] vos = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class,
				(Collection) dao.executeQuery(sql, para, new BeanListProcessor(LeaveBalanceVO.class)));
		if (ArrayUtils.isEmpty(vos))
			return vos;
		// 处理结算的部门版本信息
	...
	protected void syncFromDB4DataExists(LeaveBalanceVO[] vos) throws BusinessException {
		...
		try {
			...
			LeaveBalanceVO[] dbVOs = CommonUtils.retrieveByClause(LeaveBalanceVO.class, LeaveBalanceVO.PK_LEAVEBALANCE
					+ " in(" + pkInSQL + ")");
	 ...
	 * 
	 * @param vos
	 *            需要同步的vos，不要求已经存在于数据库中
	 * @throws BusinessException
	 */
	@SuppressWarnings("unchecked")
	protected void syncFromDB(LeaveTypeCopyVO typeVO, String year, String month, LeaveBalanceVO[] vos)
			throws BusinessException {
		if (ArrayUtils.isEmpty(vos))
			return;
		...
		try {
			...
			dbVOs = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class,
					new BaseDAO().retrieveByClause(LeaveBalanceVO.class, cond, para));
		} finally {
			if (isc != null)
				isc.clear();
		}
	 ...
	 */
	@SuppressWarnings("unchecked")
	protected void ensureData(String pk_org, LeaveTypeCopyVO typeVO, String year, String month, LeaveBalanceVO[] vos)
			throws BusinessException {
		String[] pk_psnorgs = SQLHelper.getStrArray(vos, LeaveBalanceVO.PK_PSNORG);
		BaseDAO dao = new BaseDAO();
		String cond = "pk_org=? and " + LeaveBalanceVO.PK_TIMEITEM + "=? " + "and " + LeaveBalanceVO.CURYEAR + "=? ";
		...
		try {
			...
			result = (LeaveBalanceVO[]) CommonUtils.toArray(LeaveBalanceVO.class,
					dao.retrieveByClause(LeaveBalanceVO.class, cond, para));
		} finally {
			if (isc != null)
				isc.clear();
		}
	 ...
	 * @throws BusinessException
	 */
	public Map<String, List<LeaveBalanceVO>> queryLeaveBalanceMapByPsnOrgs(String pk_hrorg, LeaveTypeCopyVO typeVO,
			String[] pk_psnorgs, String year, String month) throws BusinessException {
		...
		try {
			...
			String cond = "pk_org=? and pk_timeitem=? and pk_psnorg in (" + isc.getInSQL(pk_psnorgs)
					+ ") and curyear =? ";
			SQLParameter para = new SQLParameter();
