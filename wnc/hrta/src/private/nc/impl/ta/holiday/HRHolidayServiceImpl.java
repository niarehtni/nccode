	...
	private Map<String, Map<String, Integer>> checkPsnCalendarDay(String pk_hrorg,UFLiteralDate beginDate,
			UFLiteralDate endDate,Map<String, Map<String, Integer>> returnMap) {
				...
				String sql = "select tbm_psncalendar.pk_psndoc,tbm_psncalendar.calendar,tbm_psncalendar.date_daytype "
						...
						+ "where tbm_psncalendar.calendar >= '"+beginDate+"' "
						+ "and tbm_psncalendar.calendar <= '"+endDate+"' "
						+ "and tbm_psncalendar.pk_org = '"+pk_hrorg+"'"
						+ "and dr=0";
					...
					for(HashMap<String,Object> colMap :result){
						if(null!=colMap.get("calendar")&&null!=colMap.get("pk_psndoc")
							...
							if(resultPackage.containsKey(colMap.get("calendar"))){
								resultPackage.get(colMap.get("calendar"))
								.put(colMap.get("pk_psndoc").toString(), (Integer)colMap.get("date_daytype"));
							}else{
...
//		return returnMap;
	...
	public Map<String, Map<String, Integer>> queryTeamWorkDayTypeInfos4View(String[] pkTeams, String[] pkOrgs,
		...
		//首先查询一下整个组织默认的工作日历
		//Map<业务单元，<日期，工作日类型>>
		Map<String, Map<String, Integer>> defualtOrgMap =
				queryTeamWorkDayTypeInfos(pkOrgs,beginDate,endDate);
	 ...
	 * @param pk_hrorgs Map<业务单元，<日期，工作日类型>>
	 * @param beginDate
	 * @param endDate
	 * @param returnMap
...
//	}
	 ...
	 */
	@Override
	public Map<String, Map<String, Integer>> queryPsnWorkDayTypeInfo4PsnCalenderInsert(
			String pk_hrorg, String[] pk_psndocs, UFLiteralDate beginDate,
			UFLiteralDate endDate,boolean onlyAbnormalDayInMap) throws BusinessException {
		Map<String, Map<String, Integer>> returnMap = new HashMap<String, Map<String, Integer>>();
		if(null == pk_psndocs){
		...
		if (isLocation.booleanValue()) {
			...
			Map<String, Map<String, Integer>> orgDateType
				= findWorkDayType(pk_hrorgs, beginDate, endDate);
	 ...
	 */
	@SuppressWarnings("unchecked")
	public Map<String, Map<String, Integer>> findWorkDayType(String [] pk_orgs, UFLiteralDate startDate, UFLiteralDate endDate)
			throws BusinessException{
		...
		for(String pk_org : pk_orgs){
			...
			String sqlStr = " SELECT workcld.calendardate,workcld.datetype "
					+ " FROM bd_workcalendardate workcld "
					+ " WHERE workcld.pk_workcalendar = (select org.workcalendar "
					+ " from org_orgs org where org.pk_org = '"+pk_org+"') "
					+ " and workcld.calendardate <='"+endDate.getYear() +"-"+ endDate.getStrMonth() +"-"+ endDate.getStrDay()+"' "
					+ " and workcld.calendardate >='"+startDate.getYear() +"-"+ startDate.getStrMonth() +"-"+ startDate.getStrDay()+"' "
					+ " AND workcld.dr = 0 ";
			...
			for(Map<String,Object> columMap : qresult){
				...
				org2DayType.put((String)columMap.get("calendardate"), dayTypeAdapter((Integer)columMap.get("datetype")));
			}
	 ...
	 * 这两个地方的日历天类型是用的不同的枚举值(mmp 要命啊,谁设计的)
	 * 所以需要进行适配
	 * 规则:
	 * 工作日历--员工工作日历
	 * 工作日0	-	工作日0
