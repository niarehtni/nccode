...
public class HRHolidayServiceImpl implements IHRHolidayManageService, IHRHolidayQueryService {
	 ...
	 * 如果hrorg下的多个业务单元使用的假日还不一样
	 * ，则采用复杂模式：不存pk_holidayenjoyh，只存tbm_holidayenjoyb，逻辑是：如果一个人某一天的工作日
	 ...
	 * @throws BusinessException
	 */
	private void createEnjoyDetail0(String pk_hrorg, String[] pk_psndocs, UFLiteralDate beginDate, UFLiteralDate endDate)
			throws BusinessException {
		clearEnjoyDetail();
		...
		Map<String, String> HolidayRules = workcalendarService.queryCalendarHolidayRule(StringPiecer.getStrArray(
				orgVOs, OrgVO.PK_ORG));
		...
		HolidayVO[] holidayVOs = NCLocator.getInstance().lookup(IHolidayQueryService.class)
				.queryHolidayVOs(holidayType, beginDate, endDate);
	...
	private Map<String, HRHolidayVO[]> queryHolidayVOsByOrgs(String[] pk_orgs) throws BusinessException {
		...
		try {
			...
			OrgVO[] orgVOs = (OrgVO[]) NCLocator.getInstance().lookup(IPersistenceRetrieve.class)
					.retrieveByClause(null, OrgVO.class, orgCondition);
			// 从业务单元上取假日分类的主键
	...
	private TimeZone getTimeZone(String pk_org, OrgVO[] orgVOs) throws BusinessException {
		...
		for (OrgVO orgVO : orgVOs) {
			if (!pk_org.equals(orgVO.getPk_org()))
				continue;
			return StringUtils.isEmpty(orgVO.getPk_timezone()) ? ICalendar.BASE_TIMEZONE : TimezoneUtil
	 ...
	 * @param beginDate
	 * @param endDate
	 * @param onlyAbnormalDayInMap
	 *            ,true,则map里面只会出现有异常的天(即假日和对调日),否则出现所有的天
	 * @return
	 * @throws BusinessException
	 */
	@Override
	public Map<String, Map<String, Integer>> queryPsnWorkDayTypeInfo(String pk_hrorg, String[] pk_psndocs,
		...
		for (String pk_org : pk_orgs) {
			orgPsnEnjoyMap.put(
					pk_org,
					queryPsnWorkDayTypeInfo4SingleOrg(pk_org, pk_psndocs, dates, holidayInfoMap.get(pk_org),
		...
		for (String pk_psndoc : pk_psndocs) {
			...
			Map<String, Map<String, Integer>> orgEnjoyMap = psnOrgEnjoyMap == null ? new HashMap<String, Map<String, Integer>>()
					: psnOrgEnjoyMap.get(pk_psndoc);
			...
			if (orgGroupPsndocVOMap.size() == 1) {
				...
				returnMap.put(
						pk_psndoc,
						queryPsnWorkDayTypeInOrg(pk_psndoc, pk_org,
								queryHolidayInfo(pk_org, holidayInfoMap, beginDate, endDate), dates,
								onlyAbnormalDayInMap));
				continue;
			}
			...
			for (String psnpk_org : psnpk_orgs) {// 按组织循环
				// 人员在此组织内的工作日类型
				Map<String, Integer> psnWorkDayTypeInOrgMap = orgEnjoyMap.containsKey(psnpk_org) ? orgEnjoyMap
						.get(psnpk_org) : queryPsnWorkDayTypeInOrg(pk_psndoc, psnpk_org,
	...
	private Map<String, Map<String, Integer>> checkPsnCalendarDay(String pk_hrorg, UFLiteralDate beginDate,
			UFLiteralDate endDate, Map<String, Map<String, Integer>> returnMap) throws BusinessException {
		// Map<String, Map<String, Integer>> Map<pk_psndoc,Map<日期,工作日类型>>

		// 查找某些人员和某些日期相对应的datetype
		String sql = "select pk_psndoc,calendar,date_daytype "
				+ "FROM tbm_psncalendar "
				+ "where calendar >= '"
				+ beginDate
				+ "' "
				+ "and calendar <= '"
				...
				+ pk_hrorg
				+ "'"
				// ssx added on 2019-09-05
				// 写这段的肯定没想到一个拥有6000人的组织半年的排班记录有多少条吗？1,080,000
				// 你知道吗？UAP平台30万就放弃你了！
				// 这段注释是一座碑，下面埋着我的无奈。
				+ (returnMap.size() > 0 ? (" and pk_psndoc in ("
						+ new InSQLCreator().getInSQL(returnMap.keySet().toArray(new String[0])) + ")") : "")
				// end
				+ "and dr=0";
	 ...
	 * 
	 * @see
	 * nc.itf.ta.IHRHolidayQueryService#queryPsnWorkDayTypeInfo(java.lang.String
	 ...
	 * @throws BusinessException
	 */
	private Map<String, Integer> queryTeamWorkDayTypeInfo(String pk_org, UFLiteralDate beginDate,
			UFLiteralDate endDate, HolidayInfo<HRHolidayVO> holidayInfo) throws BusinessException {
		Map<String, Integer> returnMap = new HashMap<String, Integer>();
	...
	private Map<String, Map<String, Integer>> checkTeamCalendarDay(Map<String, Map<String, Integer>> defaultOrgMap,
		...
		for (String pk_hrorg : pk_hrorgs) {
			...
			String sql = "select bd_teamcalendar.pk_team,bd_teamcalendar.calendar,bd_teamcalendar.date_daytype "
					...
					+ "and bd_teamcalendar.calendar <='" + endDate + "'  " + "and  bd_teamcalendar.pk_org ='"
					+ pk_hrorg + "' " + "and dr = 0 ";
			List<HashMap<String, Object>> result = new ArrayList<>();// 数据库返回结果
	 ...
	 *            ，此业务单元的假日信息
	 * @param onlyAbnormalDayInMap
	 *            ，true，返回的map里面，只包含异常天（即假日和对调日）；否则包含所有天
	...
	protected <T extends HolidayVO> Map<String, Map<String, Integer>> queryPsnWorkDayTypeInfo4SingleOrg(String pk_org,
		...
		Map<String, Map<String, Boolean>> enjoyMap = CommonUtils.transferMap(queryHolidayEnjoyInfo(pk_org, pk_psndocs,
				holidayInfo.getHolidayVOs()));
		for (String pk_psndoc : pk_psndocs) {
			...
			for (UFLiteralDate date : continuousDates) {
				...
				if (!isSwitch && onlyAbnormalDayInMap)// 如果无对调，且不返回正常天，则不用处理此天
					continue;
				// 如果对调了，或者返回所有天，则要处理此天
				UFLiteralDate realDate = switchMap.containsKey(date.toString()) ? UFLiteralDate.getDate(switchMap
						.get(date.toString())) : date;
				dayTypeMap.put(date.toString(), HolidayVO.getSimpleWorkDayType(realDate));
			}
			returnMap.put(pk_psndoc, dayTypeMap);
		}
	...
	public Map<String, Map<String, Boolean>> queryHolidayEnjoyInfo(String pk_hrorg, String[] pk_psndocs,
		...
		UFLiteralDate[] earliestLatestDates = HolidayVO
				.getEarliestLatestDate(CollectionUtils.isEmpty(holidayList) ? null : holidayList
						.toArray(new HRHolidayVO[0]));
		UFLiteralDate firstDate = earliestLatestDates[0];// 假日信息的最早天
		UFLiteralDate latestDate = earliestLatestDates[1];// 假日信息的最晚天
		Map<String, List<TBMPsndocVO>> psndocMap = NCLocator.getInstance().lookup(ITBMPsndocQueryService.class)
	...
	@Override
	public Map<String, Integer> queryPsnWorkDayTypeInfo(String pk_psndoc, UFLiteralDate beginDate, UFLiteralDate endDate)
			throws BusinessException {
		// 首先查询此人在日期范围内所有的考勤档案
		ITBMPsndocQueryService psndocService = NCLocator.getInstance().lookup(ITBMPsndocQueryService.class);
	...
	public Map<String, Map<String, Integer>> queryPsnWorkDayTypeInfo4PsnCalenderInsert(String pk_hrorg,
		...
		String legal_pk_org = LegalOrgUtilsEX.getLegalOrgByOrgs(orgs).get(pk_hrorg);
