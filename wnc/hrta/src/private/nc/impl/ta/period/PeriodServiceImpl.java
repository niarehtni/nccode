...
public class PeriodServiceImpl implements IPeriodQueryService, IPeriodManageService {
	...
	@Override
	public void checkDateScope(String pk_org, UFLiteralDate beginDate, UFLiteralDate endDate) throws BusinessException {
		...
		// throw new
		// BusinessException(ResHelper.getString("6017basedoc","06017basedoc1628"));
		/* @res "开始日期所在期间已封存!" */
		...
		// throw new
		// BusinessException(ResHelper.getString("6017basedoc","06017basedoc1630"));
		/* @res "结束日期所在期间已封存!" */
	}
	...
	public void checkDateB4Modify(String pk_org, UFLiteralDate[] dates) throws BusinessException {
		...
		// if(beginPeriod.getSealflag()!=null&&beginPeriod.getSealflag().booleanValue())
		// throw new
		// BusinessException(ResHelper.getString("6017hrta","0hrta0052"));
		/* @res "日期所在考勤期间已封存！" */
		PeriodVO endPeriod = queryByDate(pk_org, endDate);
		if (endPeriod == null)
			throw new BusinessException(ResHelper.getString("6017hrta", "0hrta0051")
	...
	@Override
	public boolean checkDateScopeReturnsBoolean(String pk_org, UFLiteralDate beginDate, UFLiteralDate endDate)
	...
	@Override
	public PeriodVO queryByDate(String pk_org, UFLiteralDate date) throws BusinessException {
		return new PeriodDAO().queryByDate(pk_org, date);
	}

	@Override
	public PeriodVO queryByYearMonth(String pk_org, String year, String month) throws BusinessException {
		return new PeriodDAO().queryByYearMonth(pk_org, year, month);
	}

	@Override
	public PeriodVO queryNextPeriod(String pk_org, String year, String month) throws BusinessException {
	...
	@Override
	public PeriodVO queryNextPeriod(String pk_org, UFLiteralDate date) throws BusinessException {
	...
	@Override
	public PeriodVO queryPreviousPeriod(String pk_org, String year, String month) throws BusinessException {
		PeriodVO vo = queryByYearMonth(pk_org, year, month);
		if (vo == null)
			return null;
	...
	@Override
	public PeriodVO queryPreviousPeriod(String pk_org, UFLiteralDate date) throws BusinessException {
		PeriodVO vo = queryByDate(pk_org, date);
		if (vo == null)
			return null;
		return new PeriodMaintainImpl().queryPreviousPeriod(vo);
	...
	@Override
	public String[] queryPeriodYearsByOrg(String pk_org) throws BusinessException {
		String sql = "select " + PeriodVO.TIMEYEAR + " from " + PeriodVO.getDefaultTableName() + " where "
				+ IBaseServiceConst.PK_ORG_FIELD + "='" + pk_org + "' group by " + PeriodVO.TIMEYEAR + " order by "
				+ PeriodVO.TIMEYEAR;
	...
	@Override
	public PeriodVO[] queryPeriodsByDateScope(String pk_org, UFLiteralDate beginDate, UFLiteralDate endDate)
			throws BusinessException {
		return new PeriodDAO().queryPeriodsByDateScope(pk_org, beginDate, endDate);
	}

	@Override
	public Map<String, String[]> queryPeriodYearAndMonthByOrg(String pk_org) throws BusinessException {
	...
	@Override
	public PeriodVO[] queryByYear(String pk_org, String year) throws BusinessException {
	...
	public PeriodVO queryCurPeriod(String pk_org) throws BusinessException {
		...
		PeriodVO[] periodVOs = (PeriodVO[]) CommonUtils.toArray(PeriodVO.class,
				dao.retrieveByClause(PeriodVO.class, cond, orderBy, para));
		if (!ArrayUtils.isEmpty(periodVOs))
			return periodVOs[0];
		...
		periodVOs = (PeriodVO[]) CommonUtils.toArray(PeriodVO.class,
				dao.retrieveByClause(PeriodVO.class, cond, orderBy, para));
		if (ArrayUtils.isEmpty(periodVOs))
			return null;
	...
	@Override
	public void checkBeforeDeleteTBMPsnodc(String pk_org, String[] pkTbmpsndocs) throws BusinessException {
		...
		for (TBMPsndocVO psndocVO : tbmpsndocVOs) {
			if (!ArrayUtils.isEmpty(DateScopeUtils.intersectionDateScopes(new TBMPsndocVO[] { psndocVO },
					allSealedPeriodVOs))) {
	...
	@Override
	public void checkDateScope(String pk_org, IDateScope[] dateScopes) throws BusinessException {
		...
		if (ArrayUtils.isEmpty(allPeriodVOs)) {
			String orgName = MultiLangHelper.getName(NCLocator.getInstance().lookup(IAOSQueryService.class)
					.queryByPK(pk_org));
			throw new BusinessException(ResHelper.getString("6017basedoc", "06017basedoc1861"/*
																							 * @
																							 * res
																							 * "组织{0}未设置考勤期间"
																							 */, orgName));
			// throw new
			// ValidationException(ResHelper.getString("6017basedoc","06017basedoc1632")
			// /*@res "未定义考勤期间!"*/);
		}
		...
		if (!ArrayUtils.isEmpty(dateScopeHaveNotPeriod)) {
			...
			String periodScopeDesc = allPeriodVOs[0].getBegindate() + " - "
					+ allPeriodVOs[allPeriodVOs.length - 1].getEnddate();
			throw new BusinessException(ResHelper.getString("6017basedoc", "06017basedoc1633"/*
																							 * @
																							 * res
																							 * "日期范围【{0}】超过了考勤期间的范围【{1}】!"
																							 */, allScopeDesc,
		...
		if (sealedPeriodList.size() == 0)
			return;
		IDateScope[] intersectionScopes = DateScopeUtils.intersectionDateScopes(dateScopes,
				sealedPeriodList.toArray(new PeriodVO[0]));
		if (!ArrayUtils.isEmpty(intersectionScopes))
			throw new BusinessException(ResHelper.getString("6017basedoc", "06017basedoc1634")/*
																							 * @
																							 * res
																							 * "日期范围所在的考勤期间已封存!"
																							 */);
	...
	@Override
	public void checkBeforeInsertTBMPsndoc(SuperVO tbmPsndocVO) throws BusinessException {
	...
	public void checkBeforeInsertTBMPsndoc(PsndocVO[] psndocvos, SuperVO tbmPsndocVO) throws BusinessException {
		...
		// }
		// //考勤档案与封存期间的交集
		// IDateScope[] intersectionScopes =
		// DateScopeUtils.intersectionDateScopes(new TBMPsndocVO[]{psndocVO},
		// sealedPeriodVOs);
		// if(!ArrayUtils.isEmpty(intersectionScopes))
		...
		for (SuperVO vo : tbmPsndocVOs) {
			...
			// ??????????????????
			IDateScope[] intersectionScopes = DateScopeUtils.intersectionDateScopes(new TBMPsndocVO[] { psndocVO },
					sealedPeriodVOs);
			if (!ArrayUtils.isEmpty(intersectionScopes) && !noPeriodPsnList.contains(pk_psndoc))// ?薪???????????noPeriodPsnList????????
				sealedPeriodPsnList.add(pk_psndoc);
		}
		...
		if (noPeriodPsnList.size() > 0) {
			...
			/* @res "??????????????????????????????,???????????????:{0}" */,
					(name == null || name.length() == 0) ? getPsnNames(noPeriodPsnList) : name);
		}
		...
		if (sealedPeriodPsnList.size() > 0) {
			 ...
			 * @res
			 * "?????????????????????????????,???????????????:{0}?????????????????????????"
			 */, (name == null || name.length() == 0) ? getPsnNames(sealedPeriodPsnList) : name);
		}
		String message = noPeriodMessage == null ? sealedPeriodMessage : (sealedPeriodMessage == null ? noPeriodMessage
				: (noPeriodMessage + "\r\n" + sealedPeriodMessage));
	...
	@Override
	public void checkBeforeInsertTBMPsndoc(String pk_org, SuperVO[] tbmPsndocVOs) throws BusinessException {
		if (ArrayUtils.isEmpty(tbmPsndocVOs))
			return;
		PeriodVO[] allPeriodVOs = new PeriodDAO().queryPeriodsByOrg(pk_org);
		if (ArrayUtils.isEmpty(allPeriodVOs)) {
		...
		PeriodVO[] sealedPeriodVOs = sealedPeriodList.size() == 0 ? null : sealedPeriodList.toArray(new PeriodVO[0]);
		UFLiteralDate periodBeginDate = allPeriodVOs[0].getBegindate();
		List<String> noPeriodPsnList = new ArrayList<String>();// ?????????????????????????list
		List<String> sealedPeriodPsnList = new ArrayList<String>();// ?????????????????????薪??????????????list
		// for(SuperVO vo:tbmPsndocVOs){
		// TBMPsndocVO psndocVO = (TBMPsndocVO)vo;
		// String pk_psndoc = psndocVO.getPk_psndoc();
		// if(psndocVO.getBegindate().before(periodBeginDate)){
	...
	@Override
	public void checkBeforeUpdateTBMPsndoc(String pk_org, SuperVO[] updatePsndocVOs) throws BusinessException {
		...
		if (changedDateInSealedPeriodPsnList.size() > 0) {
			...
			/* @res "下列人员考勤档案变动的日期所在考勤期间已封存,不能修改考勤档案:{0}。可以将考勤期间解封后再修改。" */,
					getPsnNames(changedDateInSealedPeriodPsnList));
		}
		String message = noPeriodMessage == null ? sealedPeriodMessage : (sealedPeriodMessage == null ? noPeriodMessage
				: (noPeriodMessage + "\r\n" + sealedPeriodMessage));
		throw new ValidationException(message);
	...
	private String getPsnNames(List<String> psndocList) throws BusinessException {
		...
		try {
			...
			PsndocVO[] psndocVOs = (PsndocVO[]) CommonUtils.toArray(PsndocVO.class,
					new BaseDAO().retrieveByClause(PsndocVO.class, PsndocVO.PK_PSNDOC + " in(" + inSQL + ")"));
	...
	@Override
	public PeriodVO checkCurPeriod(String pk_org, String year, String month) throws BusinessException {
		PeriodVO curPeriod = queryCurPeriod(pk_org);
		if (curPeriod == null)
		...
		if (!year.equals(curPeriod.getTimeyear()) || !month.equals(curPeriod.getTimemonth()))
			...
			/* @res "期间{0}不是当期，当期是{1}!" */, year + "-" + month,
					curPeriod.getTimeyear() + "-" + curPeriod.getTimemonth()));
		return curPeriod;
	}
	...
	public String[] queryPsnAllYears(String pk_psndoc) throws BusinessException {
		...
		String condition = " exists (select 1 from tbm_psndoc where tbm_psndoc.pk_psndoc='" + pk_psndoc
				+ "' and tbm_psndoc.pk_org=tbm_period.pk_org "
		...
		// BaseDAO dao = new BaseDAO();
		// TBMPsndocVO[] psndocVOs = (TBMPsndocVO[])
		// CommonUtils.toArray(TBMPsndocVO.class,
		// dao.retrieveByClause(TBMPsndocVO.class, cond, order,para));
		// if(ArrayUtils.isEmpty(psndocVOs))
		// return null;
		// //按pk_org分组
		// Map<String, TBMPsndocVO[]> psndocVOMap =
		// CommonUtils.group2ArrayByField(TBMPsndocVO.PK_ORG, psndocVOs);
		// String periodCond =
		// PeriodVO.BEGINDATE+"<=? and "+PeriodVO.ENDDATE+">=?";
		...
		// PeriodVO[] periodVOs = (PeriodVO[])
		// CommonUtils.toArray(PeriodVO.class,
		// dao.retrieveByClause(PeriodVO.class, periodCond, periodOrder, para));
		// if(ArrayUtils.isEmpty(periodVOs))
		// continue;
	...
	@SuppressWarnings("unchecked")
	@Override
	public void createDatePeriod(String pk_org, UFLiteralDate beginDate, UFLiteralDate endDate)
			throws BusinessException {
		clearDatePeriod();
		...
		PeriodVO[] periodVOs = (PeriodVO[]) CommonUtils.toArray(PeriodVO.class,
				new BaseDAO().retrieveByClause(PeriodVO.class, cond, PeriodVO.BEGINDATE, paras));
		if (ArrayUtils.isEmpty(periodVOs))
			return;
		// 涉及到的所有考勤年度
		...
		for (int year = firstYear; year <= lastYear; year++) {
			...
			if (ArrayUtils.isEmpty(yearPeriodVOs))
				continue;
			yearDateMap.put(Integer.toString(year), new UFLiteralDate[] { yearPeriodVOs[0].getBegindate(),
					yearPeriodVOs[yearPeriodVOs.length - 1].getEnddate() });
		}
	...
	@Override
	public PeriodVO queryByDateWithCheck(String pk_org, UFLiteralDate date) throws BusinessException {
	...
	@Override
	public PeriodVO queryByAccYearMonth(String pk_org, String accYear, String accMonth) throws BusinessException {
		return new PeriodDAO().queryByAccYearMonth(pk_org, accYear, accMonth);
	}

	@Override
	public Map<UFLiteralDate, PeriodVO> queryPeriodMapByDateScope(String pk_org, UFLiteralDate beginDate,
	...
	@Override
	public Map<UFLiteralDate, PeriodVO> queryPeriodMapByDateScopes(String pk_org, IDateScope[] dateScopes)
			throws BusinessException {
		if (ArrayUtils.isEmpty(dateScopes))
			return null;
		...
		for (IDateScope mergedScope : mergedScopes) {
			Map<UFLiteralDate, PeriodVO> tempMap = queryPeriodMapByDateScope(pk_org, mergedScope.getBegindate(),
					mergedScope.getEnddate());
	...
	@Override
	public Map<String, PeriodVO[]> queryByYear(String pk_org, String[] years) throws BusinessException {
		if (ArrayUtils.isEmpty(years))
			return null;
	...
	@Override
	public Map<String, PeriodVO[]> queryByYear(String pk_org, String beginYear, String endYear)
			throws BusinessException {
	...
	@Override
	public DateScopeCheckResult checkDateScopeEnable(String pk_org, UFLiteralDate beginDate, UFLiteralDate endDate)
			throws BusinessException {
