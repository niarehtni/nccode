...
//			return false;
	...
	public void checkBeforeDeleteTBMPsnodc(String pk_org, String[] pkTbmpsndocs)
		...
		List<String> sealedPeriodPsnList = new ArrayList<String>();//?洢????????????????????????????list
...
/*@res "???????????????????????????????,???????:{0}????????????????????????"*/, getPsnNames(sealedPeriodPsnList)));
	}
	...
	public void checkDateScope(String pk_org, IDateScope[] dateScopes)
		...
		if(ArrayUtils.isEmpty(allPeriodVOs)){
			...
			throw new BusinessException(ResHelper.getString("6017basedoc", "06017basedoc1861"/*@res"???{0}δ??????????"*/,orgName));
...
//			/*@res "δ???忼?????!"*/);
		...
		if(!ArrayUtils.isEmpty(dateScopeHaveNotPeriod)){
			...
			throw new BusinessException(ResHelper.getString("6017basedoc","06017basedoc1633"/*@res "?????Χ??{0}????????????????Χ??{1}??!"*/, allScopeDesc,periodScopeDesc));
		}
		//?????????????????
...
//		}
		...
		if(!ArrayUtils.isEmpty(intersectionScopes))
			throw new BusinessException(ResHelper.getString("6017basedoc","06017basedoc1634")/*@res "?????Χ???????????????!"*/);
	}
...
//			/*@res "δ???忼?????!"*/);
		...
		List<String> sealedPeriodPsnList = new ArrayList<String>();//?????????????????????н??????????????list
...
//		}
		for(SuperVO vo:tbmPsndocVOs){
			...
			//??????????????????
			IDateScope[] intersectionScopes = DateScopeUtils.intersectionDateScopes(new TBMPsndocVO[]{psndocVO}, sealedPeriodVOs);
			if(!ArrayUtils.isEmpty(intersectionScopes) && !noPeriodPsnList.contains(pk_psndoc))//?н???????????noPeriodPsnList????????
				sealedPeriodPsnList.add(pk_psndoc);
		}
...
/*@res "??????????????????????????????,???????????????:{0}"*/, (name == null || name.length() == 0) ? getPsnNames(noPeriodPsnList) : name);
		}
...
/*@res "?????????????????????????????,???????????????:{0}?????????????????????????"*/, (name == null || name.length() == 0) ? getPsnNames(sealedPeriodPsnList) : name);
		}
...
//		}
		...
		if(noPeriodPsnList.size()>0){
			noPeriodMessage = ResHelper.getString("6017basedoc","06017basedoc1635"
/*@res "??????????????????????????????,???????????????:{0}"*/, getPsnNames(noPeriodPsnList));
		}
...
/*@res "?????????????????????????????,???????????????:{0}?????????????????????????"*/, getPsnNames(sealedPeriodPsnList));
	...
	public void checkBeforeUpdateTBMPsndoc(String pk_org, SuperVO[] updatePsndocVOs)
		...
		List<String> beginDateNoPeriodPsnList = new ArrayList<String>();//????????????????????????????????????????????list
		List<String> changedDateInSealedPeriodPsnList = new ArrayList<String>();//?????????????????????н????????list
	...
	private String getPsnNames(List<String> psndocList) throws BusinessException{
		...
		try {
			...
			if (ArrayUtils.isEmpty(psndocVOs))
				return null;
			//???????????е??????????????filteredPsndocs?е????????????filteredPsndocs?????????????????????map
...
/*@res "???{0}??????????????{1}!"*/, year+"-"+month,curPeriod.getTimeyear()+"-"+curPeriod.getTimemonth()));
		return curPeriod;
	}
...
//		dao.executeUpdate("truncate table tbm_dateperiod");
	...
	public void createDatePeriod(String pk_org, UFLiteralDate beginDate,
		...
		if(ArrayUtils.isEmpty(periodVOs))
			return;
		//?漰???????п??????
		int firstYear = Integer.parseInt(periodVOs[0].getTimeyear())-1;//??????????????????????????????????
		int lastYear = Integer.parseInt(periodVOs[periodVOs.length-1].getTimeyear());
		Map<String, UFLiteralDate[]> yearDateMap = new HashMap<String, UFLiteralDate[]>();//??????????????????map??key??2011?????value???????????????????????????????
