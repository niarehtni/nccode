...
/**
 * 实现拆单算法的类 所谓拆单，就是根据前置假规则以及往期结余假优先参数，将用户填写的申请单/登记单转换为正确的休假类别和期间（
 * 有可能拆单的结果与用户填写的完全一致，例如填写了2011年度的事假，拆单的结果也是2011年度的事假，这种情况下 就是未拆单）
 * 对于单个新增申请单或登记单的情况，若发生了拆单（即最后拆单的结果与用户的潜意识不一致）时，需要用 对话框提示用户，将拆单结果显示在对话框中。
 * 而对于批量新增，直接将拆单结果显示在最后一步的对话框中（此步原来就有，将用户填写的批量信息生成具体的休假记录）
 * 那么，问题是，在单个新增时，什么情况下，才需要用对话框提示用户？ 分两种情况讨论。
 ...
 * 出现其他的类别，但本类别被拆成了多个期间】，则可以认为存在拆单了。 例如，2011.1.3日填写了一个事假单，有可能 拆单的结果是：
 * 2010年病假，视为拆单了，需提示用户 2011年病假，2011年事假，视为拆单了，需提示用户
 * 2010年事假，未拆单，不提示用户，生成的2010年度自动带到界面上 2011年事假，未拆单，不提示用户，生成的2011年度自动带到界面上
 * 2010年事假，2011年事假，视为拆单了，需提示用户，此时休假类别虽然没变，但用户的潜意识是自己只填了一张休假单，生成了 两张单子，当然要提示用户
 * 用简练的语言来描述，就是 对于“往期结余假优先”参数为N的情况，只有拆单结果为本类别本期间的时候，才不用提示用户，其他情况都要提示用户
...
public class SplitLeaveBillUtils {
	 ...
	 *            休假类别的主键与其copyvo的对应map，<pk_timeitem,LeaveTypeCopyVO>
	 * @param dependTypeVOMap
	 *            ，休假类别主键与其依赖的copyvo数组的对应，<pk_timeitem,LeaveTypeCopyVO[]>
	 * @return 相同休假类别，相同期间的合并为一张申请单
	 * @throws BusinessException
	 */
	static SplitBillResult<AggLeaveVO> split(AggLeaveVO aggVO, TimeRuleVO timeRuleVO,
			Map<String, LeaveTypeCopyVO> leaveTypeVOMap, Map<String, LeaveTypeCopyVO[]> dependMap)
			throws BusinessException {
		...
		// UFBoolean canOverYear = SysInitQuery.getParaBoolean(pk_org,
		// LeaveConst.OVERYEAR_PARAM);//跨年度单据是否可以保存参数
		...
		// Map<String, List<LeaveBalanceVO>> balanceMap =
		// NCLocator.getInstance().lookup(ILeaveBalanceManageService.class).queryAndCalLeaveBalanceVOBatchForPreHoliday(pk_org,
		// dependMap, leaveTypeVOMap, timeRuleVO, bvos);
		Map<String, List<LeaveBalanceVO>> balanceMap = NCLocator.getInstance().lookup(ILeaveBalanceManageService.class)
				.queryAndCalLeaveBalanceVOBatchForPreHoliday(pk_org, dependMap, leaveTypeVOMap, timeRuleVO, toSplitVOs);
		LeaveBalanceVO[] balanceVOs = MapUtils.isEmpty(balanceMap) ? null : (CollectionUtils.isEmpty(balanceMap
				.get(pk_psnorg)) ? null : balanceMap.get(pk_psnorg).toArray(new LeaveBalanceVO[0]));
		// 此处的休假数据不应该包含已经结算的数据
		...
		// if(null == aggShiftMap){
		// aggShiftMap =
		// ShiftServiceFacade.queryShiftAggVOMapByHROrg(leavehVO.getPk_org());
		...
		CalParam4OnePerson leaveLengthCalParam = BillProcessHelperAtServer.initParam(leavehVO.getPk_psndoc(),
				timeRuleVO, BillMutexRule.createBillMutexRule(timeRuleVO.getBillmutexrule()), aggShiftMap,
				CommonMethods.createShiftMapFromAggShiftMap(aggShiftMap), aggVO.getBodyVOs(), BillMutexRule.BILL_LEAVE,
				2, 2);
		...
		if (!tempResult.isRealSplit()) {
			...
			splitResult.setSplit(false);
			AggLeaveVO[] splitVOs = new AggLeaveVO[] { SplitLeaveBillHelper.syncBodyToHeadAndCreateAggVO(
					tempResult.getSplitVOs(), hVO) };
		...
		AggLeaveVO[] splitVOs = SplitLeaveBillHelper.groupLeavebVOs(aggVO.getHeadVO(), tempResult.getSplitVOs(),
				leaveTypeVOMap, canOverYear != null && canOverYear.booleanValue());
	...
	/**
	 * 将一个登记单执行拆单操作 如果未执行拆单，则返回null，否则返回拆单结果
	 * 
	 * @param regVO
	 * @return
	 * @throws BusinessException
	...
	public static SplitBillResult<LeaveRegVO> split(LeaveRegVO regVO) throws BusinessException {
		...
		// ssx added on 2019-11-14
		// 年度補休及加班轉調休如果有定義則跳過拆單
		String otl = SysInitQuery.getParaString(pk_org, "TWHRT08");
		String anl = SysInitQuery.getParaString(pk_org, "TWHRT10");

		if (regVO.getPk_leavetypecopy().equals(otl) || regVO.getPk_leavetypecopy().equals(anl)) {
			splitResult.setSplit(false);
			splitResult.setSplitResult(new LeaveRegVO[] { regVO });
			return splitResult;
		}
		...
		Map<String, TimeZone> psnjobTimeZoneMap = new HashMap<String, TimeZone>();
		// 获取与休假时段有交集的所有可能的结算记录，包含前置假和用户填写的类别，已经按规则排好顺序
		Map<String, List<LeaveBalanceVO>> balanceMap = NCLocator
				.getInstance()
				.lookup(ILeaveBalanceManageService.class)
				.queryAndCalLeaveBalanceVOBatchForPreHoliday(pk_org, dependMap, leaveTypeVOMap, timeRuleVO,
						new LeaveCommonVO[] { regVO });
		LeaveBalanceVO[] balanceVOs = MapUtils.isEmpty(balanceMap) ? null : (CollectionUtils.isEmpty(balanceMap
				.get(pk_psnorg)) ? null : balanceMap.get(pk_psnorg).toArray(new LeaveBalanceVO[0]));
		LeaveBalanceVO[] balanceVOsClone = SplitLeaveBillHelper.clone(balanceVOs);// 将结算记录复制一下,因为后面需要用结算记录的结余时长等信息
		tempResult.setBalanceVOs(balanceVOs);
		Map<String, AggShiftVO> aggShiftMap = ShiftServiceFacade.queryShiftAggVOMapByHROrg(pk_org);
		CalParam4OnePerson leaveLengthCalParam = BillProcessHelperAtServer.initParam(regVO.getPk_psndoc(), timeRuleVO,
				BillMutexRule.createBillMutexRule(timeRuleVO.getBillmutexrule()), aggShiftMap,
				CommonMethods.createShiftMapFromAggShiftMap(aggShiftMap), new LeaveRegVO[] { regVO },
		...
		// ssx added on 2020-02-25
		// 啟碁指定年度時，視為不做拆單處理
		if (!StringUtils.isEmpty(regVO.getLeaveyear())) {
			SplitLeaveBillHelper.processVOStatus(regVO, new LeaveRegVO[] { regVO });
			SplitLeaveBillHelper.syncRestLength(new LeaveRegVO[] { regVO }, balanceVOsClone, leaveTypeVOMap);
			splitResult.setSplitResult(new LeaveRegVO[] { regVO });
		} else {
	 ...
	 * @param pk_org
	 * @param appDate
	 *            ,申请日期，批增申请单时有意义，批增登记单时为null
	 * @param batchVOs
	 * @return
	 * @throws BusinessException
	 */
	@SuppressWarnings("unchecked")
	public static <T extends LeaveCommonVO> T[] split(String pk_org, UFLiteralDate appDate, T[] batchVOs)
			throws BusinessException {
		...
		Map<String, AggShiftVO> aggShiftMap = ShiftServiceFacade.queryShiftAggVOMapByHROrg(pk_org);
		Map<String, CalParam4OnePerson> paramMap = BillProcessHelperAtServer.initParam(timeRuleVO, mutexRule,
				aggShiftMap, CommonMethods.createShiftMapFromAggShiftMap(aggShiftMap), batchVOs,
				BillMutexRule.BILL_LEAVE, 2, 2);
		...
		// Map<String, T[]> psndocGrpMap =
		// CommonUtils.group2ArrayByField(LeaveCommonVO.PK_PSNDOC, batchVOs);
		LeaveComparator<T> comparator = new LeaveComparator<T>(sortedTypeVOs);
		ILeaveBalanceManageService leaveBalanceService = NCLocator.getInstance().lookup(
				ILeaveBalanceManageService.class);
		Map<String, List<LeaveBalanceVO>> balanceMap = leaveBalanceService.queryAndCalLeaveBalanceVOBatchForPreHoliday(
				pk_org, dependMap, leaveTypeVOMap, timeRuleVO, batchVOs);
		Map<String, TimeZone> psnjobTimeZoneMap = BillMethods.getPsnjobTimeZoneMap(
				StringPiecer.getStrArrayDistinct(batchVOs, LeaveCommonVO.PK_PSNJOB), timeRuleVO.getTimeZoneMap());// 组织时区Map
		...
		for (String pk_psnorg : psnorgGrpMap.keySet()) {// 按人员组织关系循环处理
			...
			for (T batchVO : psnBatchVOs) {
				...
				LeaveBalanceVO[] balanceVOs = MapUtils.isEmpty(balanceMap) ? null : (CollectionUtils.isEmpty(balanceMap
						.get(batchVO.getPk_psnorg())) ? null : balanceMap.get(batchVO.getPk_psnorg()).toArray(
						new LeaveBalanceVO[0]));
	 ...
	 * @throws BusinessException
	 */
	private static <T extends LeaveCommonVO> void processSplit(TemporarySplitResult<T> temporarySplitResult,
			CalParam4OnePerson leaveLengthCalParam, Map<String, TimeZone> psnjobTimeZoneMap// pk_psnjob对应的时区的map,<pk_psnjob,TimeZone>
	) throws BusinessException {
		...
		Map<LeaveBalanceVO, ITimeScope> effectiveScopeMap = SplitLeaveBillHelper.createBalanceVOEffectiveScopeMap(
				balancedVOs, leaveLengthCalParam);
		...
		for (int i = 0; i < balancedVOs.length; i++) {
			...
			T[] intersectionScopes = TimeScopeUtils.intersectionTimeScopesRemainsOriType(effectivePeriodTimeScope,
					temporarySplitResult.getToSplitVOs());

			// ssx added on 2020-02-25
			// 啟碁指定年度時，視為不做拆單處理
			if (temporarySplitResult.getToSplitVOs() != null && temporarySplitResult.getToSplitVOs().length == 1) {
				if (temporarySplitResult.getToSplitVOs()[0] instanceof LeaveRegVO) {
					if (!StringUtils.isEmpty(((LeaveRegVO) temporarySplitResult.getToSplitVOs()[0]).getLeaveyear())) {
						// 當假單中指定了年度時，認為WNC強行歸屬到該年，不應用拆單邏輯
						break;
					}
				}
			}
			...
			// double usefulRestLen =
			// balanceVO.getUsefulRestDayOrHour();//此类别此期间的可用结余时长，由结余时长减去冻结时长
			// 2013-07-16
			// 可用是时长是0.33，但是考勤规则中取0位小数，向下取整，会导致拆单的时长为0，先在此根据考勤规则处理可用时长防止这种拆单
			double usefulRestLen = leaveLengthCalParam.timeruleVO.getTimeRuleUFDouble(
					balanceVO.getUsefulrestdayorhour()).doubleValue();// 此类别此期间的可用结余时长，由结余时长减去冻结时长
	 ...
	 * @throws BusinessException
	 */
	private static <T extends LeaveCommonVO> void postProcessSplit(TemporarySplitResult<T> temporarySplitResult,
			CalParam4OnePerson leaveLengthCalParam, Map<String, TimeZone> psnjobTimeZoneMap// pk_psnjob对应的时区的map,<pk_psnjob,TimeZone>
	) throws BusinessException {
		...
		LeaveBalanceVO[] balancedVOs = temporarySplitResult.getBalanceVOs();
		TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(toSplitVOs[0].getPk_psnjob(),
				psnjobTimeZoneMap);
		for (int i = 0; i < balancedVOs.length; i++) {
			if (ArrayUtils.isEmpty(temporarySplitResult.getToSplitVOs()))// toSplitVOs为空，表示已经拆单完毕
				return;
			...
			// 如果此时间范围与休假时段没有交集，则不用continue。intersectionScopes是此类别在此年度/期间能支持的休假的最大范围
			T[] intersectionScopes = TimeScopeUtils.intersectionTimeScopesRemainsOriType(effectiveScope,
					temporarySplitResult.getToSplitVOs());
			...
			if (balanceVO.isSettlement()) {// 如果已经结算，则如果交集与独占区相交，则说明相交的部分已经不可能被支持了，只能抛异常
				...
				ITimeScope[] exclusionScopes = SplitLeaveBillHelper.getExclusionScopes(balanceVO, temporarySplitResult,
						true);
				if (TimeScopeUtils.isCross(exclusionScopes, intersectionScopes)) {
					String psnName = getPsnName(pk_psndoc);
					String period = balanceVO.getCuryear()
							+ (oriType.isSetPeriodYearORDate() ? "" : balanceVO.getCurmonth());
					String periodName = oriType.isSetPeriodYearORDate() ? PublicLangRes.YEAR2() : PublicLangRes
							.PERIOD();
			...
			BillProcessHelperAtServer.calLeaveLength4OnePerson(oriType.getPk_org(), pk_psndoc, intersectionScopes,
					oriType, leaveLengthCalParam);
			...
			T[] mergedSplitVOs = SplitLeaveBillHelper.mergeScopesByBalanceVO(temporarySplitResult.getSplitVOs(),
					intersectionScopes, temporarySplitResult.isCanOverYear(), balanceVO, oriType, leaveLengthCalParam,
					timeZone);
			temporarySplitResult.setSplitVOs(mergedSplitVOs);
			temporarySplitResult.setToSplitVOs(SplitLeaveBillHelper.minus(temporarySplitResult.getToSplitVOs(),
					intersectionScopes, timeZone));
		}
	...
	private static <T extends LeaveCommonVO> void processSplit0(
			...
			TemporarySplitResult<T> temporarySplitResult, double maxAffordLen, CalParam4OnePerson leaveLengthCalParam,
			Map<String, TimeZone> psnjobTimeZoneMap, boolean splitBreakAppDateRule,// 是否要进行申请日超期拆单校验
		...
		if (ArrayUtils.isEmpty(curProecessScopes) || maxAffordLen < 0.00001)
			return;
		// 当前处理的休假类别
		LeaveTypeCopyVO curProcessLeaveTypeCopyVO = temporarySplitResult.getLeaveTypeVOMap().get(
				curProcessBalanceVO.getPk_timeitem());
		// 对剩下的时段，还要依次进行两项校验：申请日期超过开始日期多少天的校验(只对申请单)和休假时段是否可以跨年度保存的校验
		// T vo = temporarySplitResult.getToSplitVOs()[0];
		TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(curProecessScopes[0].getPk_psnjob(),
				psnjobTimeZoneMap);
		if (splitBreakAppDateRule) {
			// 首先进行申请日期的校验,将时间太早以至于违反了休假类别规定的休假时间段拆出来
			T[][] splitVO = SplitLeaveBillHelper.splitTimeScopeByApplyDatePara(curProecessScopes,
					temporarySplitResult.getApplyDate(), timeZone, curProcessLeaveTypeCopyVO);
			...
			if (!ArrayUtils.isEmpty(splitVO[0])) {
				...
				processSplit0(curProcessBalanceVO, temporarySplitResult, maxAffordLen, leaveLengthCalParam,
						psnjobTimeZoneMap, false, splitBreakOverYearRule);
				temporarySplitResult.setToSplitVOsForOneBalanceVO(SplitLeaveBillHelper.reviveToArray(
						temporarySplitResult.getToSplitVOsForOneBalanceVO(), splitVO[0]));// 暂时扣除的加回去
				return;
			}
			processSplit0(curProcessBalanceVO, temporarySplitResult, maxAffordLen, leaveLengthCalParam,
					psnjobTimeZoneMap, false, splitBreakOverYearRule);
		...
		if (splitBreakOverYearRule) {
			...
			T[] splitVO = SplitLeaveBillHelper.splitTimeScopeByOverYearPara(curProecessScopes, timeZone,
					leaveLengthCalParam);
			if (splitVO.length == curProecessScopes.length) {// 如果按自然年度拆分后，长度没变，表示不需要拆分
				processSplit0(curProcessBalanceVO, temporarySplitResult, maxAffordLen, leaveLengthCalParam,
						psnjobTimeZoneMap, splitBreakAppDateRule, false);
				return;
			}
		...
		processSplitLengthNotEnough(curProcessBalanceVO, temporarySplitResult, maxAffordLen, leaveLengthCalParam,
				psnjobTimeZoneMap);
	}

	/**
	 * 某个类别期间结余时长不够的情况下，部分地支持休假时间段 此时处理的休假时段都没有违反申请日超期、跨年度保存规定，可以放心地处理
	 * 由于结余时长不够，只能支持一部分休假时长。具体支持哪一部分，是有讲究的， 原则是，优先支持“独占区”的休假时段
	 * "独占区"的意思是，本类别本期间能支持，但循环后续的类别期间已经不可能支持的区域。为了不使算法过于复杂，我们约定如下：
	 * 将时段分为三种：完全在独占区里面的，部分在独占区里的，完全在独占区之外的 优先用从左至右的顺序处理第一种，其次第二种，最后从左至右处理第三种
	 * 
	 ...
	 * 如果是用户录入的类别，且（不控制时长||不严格控制时长），则将scopes1都设为此类别此期间
	 * ，返回（即scopes2不予支持），并且，如果是不严格控制时长，还要提示用户超限
	 * 若可以全部支持scopes1,则依次将scopes2中的时段合并到scopes1中去
	 * ，若合并后时长不超限，则表明可以支持此scopes2中的时段。若合并后超限了，需要
	...
	private static <T extends LeaveCommonVO> void processSplitLengthNotEnough(
		...
		// 首先得到此类别期间的"独占区"
		ITimeScope[] exclusionScopes = SplitLeaveBillHelper.getExclusionScopes(curProcessBalanceVO,
				temporarySplitResult, false);
		processSplitLengthNotEnoughComplexly(curProcessBalanceVO, temporarySplitResult, maxAffordLen,
				leaveLengthCalParam, psnjobTimeZoneMap, exclusionScopes);
	}
	...
	private static <T extends LeaveCommonVO> void processSplitLengthNotEnoughComplexly(
			LeaveBalanceVO curProcessBalanceVO,// 当前处理的结算记录
			TemporarySplitResult<T> temporarySplitResult, double maxAffordLen, CalParam4OnePerson leaveLengthCalParam,
			Map<String, TimeZone> psnjobTimeZoneMap, ITimeScope[] exclusionScopes// 此结算记录的独占区
	) throws BusinessException {
		// 首先将待支持的休假时段分为三类：完全在独占区里的，部分在独占区里的，完全在独占区外的，第一类和第三类都可以从左至右支持，第二类需要进一步判断
		T[][] groupVOs = SplitLeaveBillHelper.groupScopesByExclusionScopes(
				temporarySplitResult.getToSplitVOsForOneBalanceVO(), exclusionScopes);
		...
		if (!ArrayUtils.isEmpty(inExclusionScopes)) {
			maxAffordLen = processSplitLengthNotEnoughInExclusionScopes(curProcessBalanceVO, temporarySplitResult,
					maxAffordLen, leaveLengthCalParam, psnjobTimeZoneMap, inExclusionScopes, splitVOList, errMsg);
		}
		...
		if (!ArrayUtils.isEmpty(partInExclusionScopes)) {
			maxAffordLen = processSplitLengthNotEnoughPartInExclusionScopes(curProcessBalanceVO, temporarySplitResult,
					maxAffordLen, leaveLengthCalParam, psnjobTimeZoneMap, exclusionScopes, partInExclusionScopes,
					splitVOList, errMsg);
		}
		// 最后处理完全在独占区之外的部分
		T[] outExclusionScopes = groupVOs[2];// 完全在独占区外的，leavebalance记录可以完全支持的
		if (!ArrayUtils.isEmpty(outExclusionScopes)) {
			maxAffordLen = processSplitLengthNotEnoughOutExclusionScopes(curProcessBalanceVO, temporarySplitResult,
					maxAffordLen, leaveLengthCalParam, psnjobTimeZoneMap, outExclusionScopes, splitVOList, errMsg);
		}
		// 代码走到这里，可以被支持的时段都放入了splitVOList里面
		if (splitVOList.size() > 0) {
			TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(splitVOList.get(0).getPk_psnjob(),
					psnjobTimeZoneMap);
			temporarySplitResult.setSplitVOsForOneBalanceVO(splitVOList.toArray((T[]) Array.newInstance(
					temporarySplitResult.getToSplitVOsForOneBalanceVO().getClass().getComponentType(),
					splitVOList.size())));
			temporarySplitResult.setToSplitVOsForOneBalanceVO(SplitLeaveBillHelper.minus(
					temporarySplitResult.getToSplitVOsForOneBalanceVO(),
					temporarySplitResult.getSplitVOsForOneBalanceVO(), timeZone));
		}
	}

	/**
	 * 处理完全在独占区之外的休假时段 基本思想是量力而行，能支持多少支持多少
	 * 
	 * @param <T>
	 * @param curProcessBalanceVO
	 * @param temporarySplitResult
	...
	private static <T extends LeaveCommonVO> double processSplitLengthNotEnoughOutExclusionScopes(
			LeaveBalanceVO curProcessBalanceVO,// 当前处理的结算记录
			TemporarySplitResult<T> temporarySplitResult, double maxAffordLen, CalParam4OnePerson leaveLengthCalParam,
			Map<String, TimeZone> psnjobTimeZoneMap, T[] outExclusionScopes,// 休假记录中，完全在独占区外的时段
	...
	/**
	 * 处理完全在独占区之内的休假时段 处理思想： 如果能够完全支持，则完全支持
	 * 如果不能完全支持，但当前的leavebalance是用户录入的类别，且（不控制时长||不严格控制时长），也能完全支持（即使用超限时长）
	 * 如果不能完全支持，且用户录入的类别严格控制时长，则抛异常，(因为已经没有leavebalance记录可以支持了)
	 * 如果不能完全支持，且当前的leavebalance不是用户录入的类别
	 * ，且用户录入的类别（不控制时长||不严格控制时长），则要进行拆单，将能支持的时段拆出来
	 * 不能支持的时段，等到全部leavebalance记录循环完毕之后
	 * ，再从头循环，寻找用户录入的类别的leavebalance，看它们能不能用超限时长来支持
	 * 
	 * @param <T>
	 * @param curProcessBalanceVO
	 * @param temporarySplitResult
	...
	private static <T extends LeaveCommonVO> double processSplitLengthNotEnoughInExclusionScopes(
			LeaveBalanceVO curProcessBalanceVO,// 当前处理的结算记录
			TemporarySplitResult<T> temporarySplitResult, double maxAffordLen, CalParam4OnePerson leaveLengthCalParam,
			Map<String, TimeZone> psnjobTimeZoneMap, T[] inExclusionScopes,// 休假记录中，完全在独占区内的时段
			List<T> splitVOList,// 将可以支持的部分放入此list
			String errMsg) throws BusinessException {
		...
		boolean isUserInputType = temporarySplitResult.getOriginalLeaveTypeCopyVO().getPk_timeitem()
				.equals(curProcessBalanceVO.getPk_timeitem());
		// 用户录入的类别是否严格控制时长
		boolean isUerInputTypeStrictLimit = temporarySplitResult.getOriginalLeaveTypeCopyVO().isLeaveLimit()
				&& temporarySplitResult.getOriginalLeaveTypeCopyVO().isRestrictLimit();
		...
		if (isUerInputTypeStrictLimit) {
			...
			TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(inExclusionScopes[0].getPk_psnjob(),
					psnjobTimeZoneMap);
			throw new BusinessException(MessageFormat.format(errMsg, psnName, inExclusionScopes[0].getLeavebegintime()
					.toStdString(timeZone), inExclusionScopes[inExclusionScopes.length - 1].getLeaveendtime()
					.toStdString(timeZone), typeVO.getMultilangName()));
		}
	...
	/**
	 * 处理部分在独占区里，部分在独占区外的时段 这些时段是最难处理的 基本思想是，若能够全部支持，则全部支持即可。
	 * 若不能全部支持，则将这些脚踏两条船的时段，拆成两个部分：独占区之内的部分和独占区之外的部分Pin，优先处理独占区之内的部分Pout
	 * 若Pin能全部支持，则全部支持Pin，Pout部分则量力而行，能支持多少就支持多少
	 * 若Pin不能全部支持，则参考processSplitLengthNotEnoughInExclusionScopes方法的处理方式处理Pin
	 * ，Pout就不予支持了
	 * 
	 * @param <T>
	 * @param curProcessBalanceVO
	...
	private static <T extends LeaveCommonVO> double processSplitLengthNotEnoughPartInExclusionScopes(
		...
		LeaveTypeCopyVO typeVO = temporarySplitResult.getLeaveTypeVOMap().get(curProcessBalanceVO.getPk_timeitem());
		TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(partInExclusionScopes[0].getPk_psnjob(),
				psnjobTimeZoneMap);
		// 交切处理成两部分：独占区之内的部分，和独占区之外的部分
		T[][] splitVOs = SplitLeaveBillHelper.splitScopesByExclusionScopes(partInExclusionScopes, exclusionScopes,
				timeZone);
		// 独占区的交集，计算其总时长
		T[] intersectionScopes = splitVOs[0];
		BillProcessHelperAtServer.calLeaveLength4OnePerson(curProcessBalanceVO.getPk_org(),
				partInExclusionScopes[0].getPk_psndoc(), intersectionScopes, typeVO, leaveLengthCalParam);
		...
		if (intersctionLen > maxAffordLen) {
			double remainsAffordLen = processSplitLengthNotEnoughInExclusionScopes(curProcessBalanceVO,
					temporarySplitResult, maxAffordLen, leaveLengthCalParam, psnjobTimeZoneMap, intersectionScopes,
					splitVOList, errMsg);
		...
		for (T outScope : splitVOs[1]) {// 循环尝试着支持独占区之外的部分
			...
			MergeResultDescriptor<T> result = SplitLeaveBillHelper.mergeScopeToScopes(toMergeScopes, outScope,
					temporarySplitResult.isCanOverYear(), timeZone);
			BillProcessHelperAtServer.calLeaveLength4OnePerson(curProcessBalanceVO.getPk_org(),
					partInExclusionScopes[0].getPk_psndoc(), result.getVos(), typeVO, leaveLengthCalParam);
			...
			if ((leftJoinIndex >= 0 && rightJoinIndex < 0) || (leftJoinIndex < 0 && rightJoinIndex >= 0)) {
				...
				double canAffordLen2SplitVO = maxAffordLen
						- SplitLeaveBillHelper.sumLengthExclude(toMergeScopes, keyIndex);
				T[] spltVOs = inverseCalculate(toSplitVO, typeVO, canAffordLen2SplitVO, leaveLengthCalParam,
						psnjobTimeZoneMap, leftJoinIndex >= 0);
			...
			// 对于这段，能提供的时长
			double canAffordLen2SplitVO = maxAffordLen
					- SplitLeaveBillHelper.sumLengthExclude(toMergeScopes, leftJoinIndex);
			T[] spltVOs = inverseCalculate(toSplitVO, typeVO, canAffordLen2SplitVO, leaveLengthCalParam,
					psnjobTimeZoneMap, true);
			for (int n = 0; n < toMergeScopes.length; n++) {
				if (n == leftJoinIndex) {
					splitVOList.add(spltVOs[0]);
					double len = spltVOs[0].getLeaveHourValue();
	 ...
	 *            申请日期，对于申请单有意义
	 * @param <T>
	 * @param CalParam4OnePerson
	 *            ,计算个人休假时长需要的参数
	 * @throws BusinessException
	 */
	private static <T extends LeaveCommonVO> void processSplit(LeaveBalanceVO curProcessBalanceVO,
			TemporarySplitResult<T> temporarySplitResult, double maxAffordLen, CalParam4OnePerson leaveLengthCalParam,
			Map<String, TimeZone> psnjobTimeZoneMap) throws BusinessException {
		processSplit0(curProcessBalanceVO, temporarySplitResult, maxAffordLen, leaveLengthCalParam, psnjobTimeZoneMap,
				temporarySplitResult.getApplyDate() != null, !temporarySplitResult.isCanOverYear());
		// 走到这里，此leavebalance所有能支持的时段都放入了splitVOsForOneBalanceVO里面，所有不能支持的都放入了toSplitVOsForOneBalanceVO里面
		TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(
				temporarySplitResult.getToSplitVOs()[0].getPk_psnjob(), psnjobTimeZoneMap);
	 ...
	 * isFromLeft2Right决定了拆的方向
	 * ，为true的话，从左往右支持，即可以支持的部分在vo左部，为false的话，从右往左支持，即可以支持的部分在vo右部 大致逻辑：
	 * 首先用简单算法算出一个粗略的结束/开始时间T
	 * 然后用T计算休假时长L，定义step=(L<=可提供时长)?正向推10分钟:反向推10分钟（正向是会使休假时长变长的方向
	 * ，对于从左到右拆，是往after方向，反之是往before方向） 然后开始下面的循环： location: T=T+step，计算休假时长L
	 * 若L<=可提供时长 则 如果 step为正向-->step不变，goto location
	 * 如果step=反向推10分钟-->step=正向推1分钟，goto location
	 * 如果step=反向推1分钟，则循环停止，T就是确切的休假结束/开始时间 若L>可提供时长 则 如果 step为反向-->step不变，goto
	 * location 如果 step为正向推10分钟-->step=反向推1分钟，goto location
	 * 如果step为正向推1分钟，则循环停止，T=T-step
	 * 此算法的基本思想是，先算一个大概的时间点，如果此时间点计算的时长还没超限，则用大步子(10分钟)往前冲；如果超限了，则用大步子(10分钟)往后撤
	 * 大步冲了一个step之后，再次计算时长，若仍然没有超，则继续大步(10分钟)冲 若超了，则往回小步(1分钟)退
	 * 大步撤了一个step之后，再次计算时长，若仍然超限，则继续大步(10分钟)撤 若没有超限，则改变方向，小步（1分钟）往前走
	 ...
	 * @param leaveLengthCalParam
	 * @param psnjobTimeZoneMap
	 * @param isFromLeft2Right
	 *            ,true表示从开始时间开始往结束时间的方向拆，false表示从结束时间往开始时间的方向拆
	 * @return
	 * @throws BusinessException
	 */
	@SuppressWarnings("unchecked")
	private static <T extends LeaveCommonVO> T[] inverseCalculate(T vo, LeaveTypeCopyVO leaveTypeCopyVO,
			double maxAffordLen, CalParam4OnePerson leaveLengthCalParam, Map<String, TimeZone> psnjobTimeZoneMap,
			boolean isFromLeft2Right) throws BusinessException {
		// 休假结余时长能提供的休假小时数（maxAffordLen的单位有可能是天，要统一转换为小时，便于计算）
		double leaveHours = leaveTypeCopyVO.getTimeItemUnit() == TimeItemCopyVO.TIMEITEMUNIT_HOUR ? maxAffordLen
				: maxAffordLen * leaveLengthCalParam.timeruleVO.getDaytohour2();
		// 初步计算的休假结束/开始时间。这只是一个粗略计算的时间，需要进一步的精确处理
		UFDateTime testEndOrBeginTime = simpleCalculateEndOrBeginTime(
				isFromLeft2Right ? vo.getLeavebegintime() : vo.getLeaveendtime(),
				isFromLeft2Right ? vo.getLeaveendtime() : vo.getLeavebegintime(), leaveHours, leaveTypeCopyVO,
				leaveLengthCalParam, isFromLeft2Right);
		...
		TimeZone timeZone = SplitLeaveBillHelper.queryTimeZoneByPkPsnjob(tempCanAffordVO.getPk_psnjob(),
				psnjobTimeZoneMap);
		...
		BillProcessHelperAtServer.calLeaveLength4OnePerson(pk_org, pk_psndoc, tempVOs, leaveTypeCopyVO,
				leaveLengthCalParam);
	 ...
	 * 基本思想是，从fromTime开始往后沿着时间轴往后
	 * /前走，走过的工作日历的工作时间段的长度刚好等于leaveHours时，停住，返回停止地的时间点
	 * 
	 * @param fromTime
	 * @param endTimeLimit
	 *            ,计算的时间点的下限/上限，计算出来的结束/开始时间不能晚于/早于此时间
	 * @param leaveHours
	 * @param leaveTypeCopyVO
	 * @param leaveLengthCalParam
	 * @param fromLeft2Right
	 *            true，表示从开始时间出发往结束方向找，false相反
	 * @return
	 * @throws BusinessException
	 */
	private static UFDateTime simpleCalculateEndOrBeginTime(UFDateTime fromTime, UFDateTime toTimeLimit,
			double leaveHours, LeaveTypeCopyVO leaveTypeCopyVO, CalParam4OnePerson leaveLengthCalParam,
			boolean fromLeft2Right) throws BusinessException {
		// 取出开始/结束时间的所属日期，然后从所属日期开始循环处理工作日历。保险起见，所属日期往前/后推三天
		UFLiteralDate curDate = fromTime.getDate().toUFLiteralDate(ICalendar.BASE_TIMEZONE)
				.getDateBefore(fromLeft2Right ? 3 : -3);
		...
		while (true) {// 若fromLeft2Right为true，则是从前往后推算，否则是从后往前推算
			...
			// 前一天的班次
			ShiftVO preShift = preCalendar == null ? null : ShiftServiceFacade.getShiftVOFromMap(shiftMap, preCalendar
			...
			// 后一天的班次
			ShiftVO nextShift = nextCalendar == null ? null : ShiftServiceFacade.getShiftVOFromMap(shiftMap,
					nextCalendar.getPsnCalendarVO().getPk_shift());
			// 后一天的时区
			...
			// 当天的工作时间段
			ITimeScope[] workScopes = BillProcessHelper.getWorkTimeScopes(curDate.toString(), curCalendar, preShift,
			...
			if (fromLeft2Right) {
				if ((!lastWorkScope.isContainsLastSecond() && !lastWorkScope.getScope_end_datetime().after(fromTime))
						|| (lastWorkScope.isContainsLastSecond() && lastWorkScope.getScope_end_datetime().before(
								fromTime)))
					continue;
			} else {
			...
			// 班次的开始/结束时间
			UFDateTime shiftBeginEndTime = fromLeft2Right ? firstWorkScope.getScope_start_datetime() : lastWorkScope
			...
			if ((fromLeft2Right && !fromTime.after(shiftBeginEndTime))
					|| (!fromLeft2Right && !fromTime.before(shiftBeginEndTime))) {
				...
				ITimeScope[] workScopesAfterBeforeFromTime = TimeScopeUtils.intersectionTimeScopes(
						workScopes,
						new ITimeScope[] { fromLeft2Right ? new DefaultTimeScope(fromTime, lastWorkScope
								.getScope_end_datetime(), lastWorkScope.isContainsLastSecond()) : new DefaultTimeScope(
								firstWorkScope.getScope_start_datetime(), fromTime, false) });
				shiftSecondsAfterBeforeFromTime = curIsGX ? Math.min(workDaySeconds,
						TimeScopeUtils.getLength(workScopesAfterBeforeFromTime)) : TimeScopeUtils
						.getLength(workScopesAfterBeforeFromTime);
			}
			...
			if (shiftSecondsAfterBeforeFromTime == remainSeconds) {// 如果remainSeconds刚好能包含此工作段，则返回工作段最后一秒/第一秒
				return fromLeft2Right ? (DateTimeUtils.min(
						lastWorkScope.isContainsLastSecond() ? DateTimeUtils.getDateTimeAfterMills(
								lastWorkScope.getScope_end_datetime(), 1000) : lastWorkScope.getScope_end_datetime(),
						toTimeLimit)) : DateTimeUtils.max(firstWorkScope.getScope_start_datetime(), toTimeLimit);
			}
			...
			return fromLeft2Right ? DateTimeUtils.min(spreadTime, toTimeLimit) : DateTimeUtils.max(spreadTime,
					toTimeLimit);
		}
	}

	private static <T extends LeaveCommonVO> boolean checkApplyDatePara(T vo, UFLiteralDate applyDate,
			TimeZone timeZone, LeaveTypeCopyVO leaveTypeCopyVO, boolean throwsException) throws BusinessException {
		...
		if (isBreak && throwsException) {
			...
			if (limitDates == 0)
				...
				/* @res "人员{0}休假类别{1}的申请日期{2}比休假开始日期{3}晚了{4}天,按规定应该当天申请!" */, psnName, typeName, applyDate, beginDate,
						lateDates));
			throw new BusinessException(MessageFormat.format(ResHelper.getString("6017leave", "06017leave0230")
			/* @res "人员{0}休假类别{1}的申请日期{2}比休假开始日期{3}晚了{4}天,按规定不得超过{5}天!" */, psnName, typeName, applyDate, beginDate,
					lateDates, limitDates));
		}
		return isBreak;
	}
	 ...
	 * @param timeZone
	 * @param isSetEndTime
	 *            ，true表示设置结束时间，false表示设置开始时间
	 */
	private static <T extends LeaveCommonVO> void setEndOrBeginTime(T vo, UFDateTime endOrBeginTime, TimeZone timeZone,
