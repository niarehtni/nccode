...
public class OTLeaveBalancePubServiceImpl {
	...
	public OTLeaveBalanceVO[] queryOTLeaveAggvos(String pk_org, String[] pk_psndocs, String[] pk_depts,
			UFBoolean isTermLeave, String pk_leavetypecopy, String maxdate, String queryYear, String beginDate,
		...
		if (!StringUtils.isEmpty(pk_org)) {
			...
			if (pk_leavetypecopy.equals(initotleavetype)) {
				if (StringUtils.isEmpty(queryYear)) {
					...
					ret = new SegDetailServiceImpl().getOvertimeToRestHoursByType(pk_org, pk_psndocs, pk_depts,
							isTermLeave, new UFLiteralDate(beginDate), new UFLiteralDate(endDate), null);
				} else {
					ret = new SegDetailServiceImpl().getOvertimeToRestHoursByType(pk_org, pk_psndocs, pk_depts,
							isTermLeave, queryYear, pk_leavetypecopy);
				}
			} else if (pk_leavetypecopy.equals(initexleavetype)) {
				if (StringUtils.isEmpty(queryYear)) {
					ret = new LeaveExtraRestServiceImpl().getLeaveExtHoursByType(pk_org, pk_psndocs, pk_depts,
							isTermLeave, new UFLiteralDate(beginDate), new UFLiteralDate(endDate), pk_leavetypecopy,
							false, false);
				} else {
					ret = new LeaveExtraRestServiceImpl().getLeaveExtHoursByType(pk_org, pk_psndocs, pk_depts,
							isTermLeave, queryYear, pk_leavetypecopy, false);
				}
			}
		}
		return ret;
