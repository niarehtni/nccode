...
public class LeaveTypeCardPanel extends TimeItemCardPanel{
	...
	//结算周期
	private UIComboBox wayofresult;
	...
	private UICheckBox isleaveplan;
	...
	private UICheckBox isspecialrest;
	//有效期延长时限
...
//	private UIPanel leavePanel;
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==this.getIsleaveplan()){
			getIsleaveplan().setSelected(false);
			getIsleaveplan().setEnabled(getIsleaveplan().isSelected());
			return;
		}
		//点击公式设置按钮
...
//			getConvertrule().setVisible(false);
		...
		if(e.getSource()==this.getWayofresult()){
			if(getWayofresult().getSelectedIndex() == 1||getWayofresult().getSelectedIndex() == 2){
				getLeavescale().setEnabled(false);
				getLeavesetperiod().setEnabled(false);
		        getLeavescale().setSelectedItem(Integer.valueOf(1));
		        getLeavesetperiod().setSelectedItem(Integer.valueOf(0));
			} else if (getModel().getUiState() != UIState.NOT_EDIT) {
		        getLeavescale().setEnabled(true);
		        getLeavesetperiod().setEnabled(true);
		      }
			return;
		}
	}
...
//			UILabel settlementLabel = new UILabel(ResHelper.getString("common","UC000-0003249")/*@res "结算方式"*/);
			...
			//dq
			//结算周期
			builder.append("加班结算方式");
			builder.append(getWayofresult());
			...
			builder.append("");
			builder.append(getIsspecialrest());
			//享有是否在自助中显示
			builder.append("");
...
/*@res "按{0}结算"*/;
			...
			leavesetperiod.addItem(new DefaultConstEnum(
					TimeItemCopyVO.LEAVESETPERIOD_STARTDATE, "按年資起算日結算"));
			//
	...
	//dq
	@SuppressWarnings("unchecked")
	public UIComboBox getWayofresult() {
		if(wayofresult==null){
			wayofresult = new UIComboBox();
//			String s = ResHelper.getString("6017basedoc","06017basedoc1522")
/*@res "按{0}结算"*/;
			wayofresult.addItem(new DefaultConstEnum(0,""));
			wayofresult.addItem(new DefaultConstEnum(1,"固定月结算"));
			wayofresult.addItem(new DefaultConstEnum(2,"滚动月结算"));
			getComponentList().add(wayofresult);
			wayofresult.addItemListener(this);
		}
		return wayofresult;
	}
...
/*@res "有效期延长时限(天)"*/);
	...
	public UICheckBox getIsspecialrest() {
		if(isspecialrest==null){
			isspecialrest = new UICheckBox(ResHelper.getString("6017item","type20180905-0000")
/*@res "是否个别递延与结算假期"*/);
			getComponentList().add(isspecialrest);
			isspecialrest.addActionListener(this);
		}
		return isspecialrest;
	}
...
/*@res "按班次时长折算"*/));
	...
	public UIComboBox getMonthunit() {
		if(monthunit==null){
			monthunit =  new UIComboBox();;
			monthunit.addItem(new DefaultConstEnum(0,"0"));
			monthunit.addItem(new DefaultConstEnum(1,"1"));
			monthunit.addItem(new DefaultConstEnum(2,"2"));
			monthunit.addItem(new DefaultConstEnum(3,"3"));
			monthunit.addItem(new DefaultConstEnum(4,"4"));
			monthunit.addItem(new DefaultConstEnum(6,"6"));
			monthunit.addItem(new DefaultConstEnum(12,"12"));
...
//			formulaset.setPreferredSize(new Dimension(100,20));
	...
	public UICheckBox getIsleaveplan(){
		if(isleaveplan==null){
			isleaveplan = new UICheckBox(ResHelper.getString("", "使用休假计划"));
			getComponentList().add(isleaveplan);
		}
		return isleaveplan;
	}
}