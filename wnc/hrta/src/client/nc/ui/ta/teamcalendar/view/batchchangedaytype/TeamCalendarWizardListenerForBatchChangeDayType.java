...
public class TeamCalendarWizardListenerForBatchChangeDayType implements IWizardDialogListener {
	...
	public void wizardFinish(WizardEvent event) throws WizardActionException {
		...
		/*
		 * UFLiteralDate beginDate = panel1.getBeginDate(); UFLiteralDate
		 * endDate = panel1.getEndDate();
		 */
		String pk_org = panel1.getPK_BU();
		// 取出班组主键
		ConfirmTeamStepForBatchChangeDayType step2 = (ConfirmTeamStepForBatchChangeDayType) wizardModel.getSteps().get(
				1);
		...
		if (changeDayTypeCheckBox) {
			...
			try {
				...
				// 在页面做一个弹出框
				List<List<String>> strMessage = manageValidate.updateteamValidate(wizardModel.getModel().getContext()
						.getPk_org(), pk_teams, firstDate, secondDate);
				...
				if (null != strMessage) {
					for (List<String> strs : strMessage) {
						if (isStrcheck.booleanValue()) {
							...
							if (2 == MessageDialog.showOkCancelDlg(wizardModel.getModel().getContext().getEntranceUI(),
									"校验", "以下员工" + strs + "是否继续保存?")) {
								return;
							}
			...
			if (null == firstDate || null != secondDate) {
				...
				try {
					...
					ITeamCalendarManageMaintain queryMaintain = NCLocator.getInstance().lookup(
							ITeamCalendarManageMaintain.class);

					queryMaintain.batchChangeDateType(pk_org, pk_teams, firstDate, secondDate);
			...
			try {
				...
				// 在页面做一个弹出框
				List<List<String>> strMessage = manageValidate.updateteamValidate(wizardModel.getModel().getContext()
						.getPk_org(), pk_teams, changeDate, panel3.getafterChangeDateType());
				UFBoolean isStrcheck = SysInitQuery.getParaBoolean(legal_pk_org, "TWHRT03");
				if (null != strMessage) {
					for (List<String> strs : strMessage) {
						if (isStrcheck.booleanValue()) {
			...
			if (null != changeDate) {
				try {
					...
					// if (!StringUtils.isEmpty(changedayorhourStr)
					// && !UFDouble.ZERO_DBL.equals(new
					// UFDouble(changedayorhourStr))) {
					queryMaintain.batchChangeDateType4OneDay(pk_org, pk_teams, changeDate,
							panel3.getafterChangeDateType(), changedayorhourStr);
					// }
				} catch (BusinessException e) {
	...
	private void refreshData(UFLiteralDate firstDate, UFLiteralDate secondDate) {
		...
		if (firstDate.before(secondDate)) {
			...
			try {
				// calendarVOs = queryMaintain.queryCalendarVOByPsndocs(pk_org,
				// pk_psndocs, firstDate, secondDate);
	...
	/*
	 * @Override public void wizardFinish(WizardEvent event) throws
	 * WizardActionException { HRWizardModel wizardModel =
	 * (HRWizardModel)event.getModel(); //取出日期范围
	 * SelectTeamStepForBatchChangeDayType step1 =
	 * (SelectTeamStepForBatchChangeDayType)wizardModel.getSteps().get(0);
	 * SelectTeamPanelForBatchChangeDayType panel1 =
	 * step1.getTeamPanelForBatchChangeDayType(); UFLiteralDate beginDate =
	 * panel1.getBeginDate(); UFLiteralDate endDate = panel1.getEndDate();
	 * String pk_org = panel1.getPK_BU(); //取出班组主键
	 * ConfirmTeamStepForBatchChangeDayType step2 =
	 * (ConfirmTeamStepForBatchChangeDayType)wizardModel.getSteps().get(1);
	 * ConfirmTeamPanelForBatchChangeDayType panel2 =
	 * step2.getTeamPanelForBatchChangeDayType(); TeamHeadVO[] teamVOs =
	 * panel2.getSelTeamVOs(); String[] pk_teams =
	 * StringPiecer.getStrArray(teamVOs, TeamHeadVO.CTEAMID); //取出遇假日排班取消标志
	 * ShiftSetStepForBatchChangeDayType step3 =
	 * (ShiftSetStepForBatchChangeDayType)wizardModel.getSteps().get(2);
	 * ShiftSetPanelForBatchChangeDayType panel3 =
	 * step3.getShiftSetPanelForBatchChangeDayType(); //取出班次主键 String
	 * old_Pk_shift = panel3.getOldShiftPk();//老的班次主键 String new_Pk_shift =
	 * panel3.getNewShiftPk();//新的班次主键 Boolean
	 * withOldShift=panel3.getOldShiftCheckBox().isSelected();//原班次是否被选中
	 * ITeamCalendarManageMaintain manageMaintain =
	 * NCLocator.getInstance().lookup(ITeamCalendarManageMaintain.class);
	 * ITeamCalendarQueryMaintain queryMaintain =
	 * NCLocator.getInstance().lookup(ITeamCalendarQueryMaintain.class);
	 * TeamInfoCalendarVO[] calendarVOs = null; try { //批量更新
	 * manageMaintain.batchChangeShiftNew(pk_org, pk_teams, beginDate,
	 * endDate,withOldShift, old_Pk_shift,new_Pk_shift); //更新后调用查询返回结果 String
	 * pk_hrorg =
	 * NCLocator.getInstance().lookup(IAOSQueryService.class).queryHROrgByOrgPK
	 * (pk_org).getPk_org(); // calendarVOs =
	 * queryMaintain.queryCalendarVOByPKTeams(pk_org, pk_teams, beginDate,
	 * endDate); calendarVOs = queryMaintain.queryCalendarVOByPKTeams(pk_hrorg,
	 * pk_teams, beginDate, endDate); } catch (BusinessException e) {
	 * Debug.error(e.getMessage(), e); WizardActionException wae = new
	 * WizardActionException(e); wae.addMsg("1", e.getMessage()); throw wae; }
	 * TeamCalendarAppModel appModel =
	 * (TeamCalendarAppModel)wizardModel.getModel();
	 * appModel.setBeginEndDate(beginDate, endDate);
	 * appModel.initModel(calendarVOs);
	 * event.getModel().gotoStepForwardNoValidate(0); }
	 * 
	 * @Override public void wizardCancel(WizardEvent event) throws
	 * WizardActionException {}
	 * 
	 * @Override public void wizardFinishAndContinue(WizardEvent event)throws
	 * WizardActionException {}
	 */
}
