...
public class HRTANatualOrderFormulaTabBuilder extends HRNatualOrderFormulaTabBuilder implements AppEventListener {
	...
	public String preConvert(String formula) {
		...
		if (formula.contains("directsum")) {
			String dayitemname = ResHelper.getString("hr_formula_config", "ta_formula_000500")/*
																							 * @
																							 * res
																							 * "日报项目"
																							 */;
			for (ViewOrderVO item : items) {
				...
				ViewOrderVOWithItemTypeName newVO = new ViewOrderVOWithItemTypeName(item);
				// MOD
				// (PM26331:公式校验时，应使用replace进行简单替换，标准产品中使用replaceAll实为正则表达式替换，导致表达式偶发性错误)
				// ssx modified on 2019-04-19
				// tips: replaceAll -> replace
				formula = formula.replace(newVO.toString(), newVO.getField_name());
				// end
			}
			for (ViewOrderVO item : items) {
				String itemName = "[" + dayitemname + "-" + item.getMultilangName() + "]";
	...
	/*
	 * 将脚本表达式转换为业务表达式 HR中不需要进行这个转换，因为涉及到公式的地方都将业务表达式存到数据库了，因此这个方法应该不会调到
	 * (non-Javadoc)
	 * 
	...
	public String postConvert(String formula) {
		...
		for (ViewOrderVO item : items) {
			...
			// ssx modified on 2019-04-19
			// tips: replaceAll -> replace
			formula = formula.replace(newVO.getField_name(), newVO.toString());
			// end
		}
		return formula;
	}
	...
	// //Invaild syntax of formula! Formula is
	// directsum("f_i_10",0,[日报项目.日报日期]<=[日报项目.调薪日期])
	// if(!formula.contains("directsum"))
	// return formula;
	// ViewOrderVO[] items = getItemVOs();
	// if(ArrayUtils.isEmpty(items))
	// return formula;
	// String dayitemname =
	// ResHelper.getString("hr_formula_config","ta_formula_000500")/*@res
	// "日报项目"*/;
	// for(ViewOrderVO item:items){
	// String itemName = "["+dayitemname+"."+item.getMultilangName()+"]";
	// formula = org.springframework.util.StringUtils.replace(formula, itemName,
	// item.getField_name());
	//
	// ViewOrderVOWithItemTypeName newVO = new
	// ViewOrderVOWithItemTypeName(item);
	...
	public String replaceFuncCodeByName(String formula, HRFormulaItem hrItem) {
		...
		if (ArrayUtils.isEmpty(leftBracketIdx) || ArrayUtils.isEmpty(rightBracketIdx)) {
			...
			// ssx modified on 2019-04-19
			// tips: replaceAll -> replace
			return formula.replace(recode, hrItem.getInputSigRes());
			//
		}
	...
	public ViewOrderVO[] getItemVOs() {
		if (ArrayUtils.isEmpty(itemVOs) || !getModel().getContext().getPk_org().equals(oldPkorg)) {
			...
			try {
				itemVOs = NCLocator
						.getInstance()
						.lookup(IViewOrderQueryService.class)
						.queryAllViewOrder(getModel().getContext(), ViewOrderVO.FUN_TYPE_DAY,
								ViewOrderVO.REPORT_TYPE_PSN);
