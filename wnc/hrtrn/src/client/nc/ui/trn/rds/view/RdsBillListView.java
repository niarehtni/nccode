...
public class RdsBillListView extends HrBillListView implements BillEditListener2, ChangeListener {
	...
	private void setDimiCellEditable(BillModel billModel, int selRow) throws BusinessException {
		// 可编辑的字段 wanglqh
		String[] defaultAttr = new String[] { PsnJobVO.BEGINDATE, PsnJobVO.ENDDATE, PsnJobVO.DEPOSEMODE,
				PsnJobVO.SERIES, PsnJobVO.PK_PSNCL, PsnJobVO.PK_DEPT, PsnJobVO.MEMO, PsnJobVO.TRNSTYPE,
				PsnJobVO.TRNSREASON, PsnJobVO.MEMO, PsnJobVO.PK_POST, PsnJobVO.PK_JOBRANK, PsnJobVO.PK_JOBGRADE,
				PsnJobVO.PK_POSTSERIES, PsnJobVO.PK_JOB, PsnJobVO.PK_JOB_TYPE, PsnJobVO.PK_JOBRANK,
	...
	private void afterEditPsnjob(BillEditEvent e) {
		...
		try {
			if (PsnJobVO.PK_GROUP.equals(e.getKey())) {
				...
				// //wanglqh专项补丁部门联动成本中心
				// DeptVO deptVO = psn.getPk_dept() == null ? null :
				// getService().queryByPk(DeptVO.class, psn.getPk_dept());
				// if(deptVO != null){
				// setValue(deptVO.getAttributeValue("glbdef3"), e,
				// "jobglbdef9");// 成本中心
				// }
				...
				// ssx remarked on 2020-06-04
				// WNC不需要這個功能
				/*
				 * // 职务类别 String series = (String) e.getValue(); String pk_job
				 * = (String)
				 * getBillListPanel().getBodyBillModel(e.getTableCode(
				 * )).getValueAt(e.getRow(), PsnJobVO.PK_JOB + "_ID"); String
				 * pk_post = (String)
				 * getBillListPanel().getBodyBillModel(e.getTableCode
				 * ()).getValueAt(e.getRow(), PsnJobVO.PK_POST + "_ID"); if
				 * (StringUtils.isBlank(pk_job) &&
				 * StringUtils.isNotBlank(series)) { String defaultlevel = "";
				 * String defaultrank = ""; Map<String, String> resultMap =
				 * NCLocator.getInstance().lookup(IPsndocQryService.class)
				 * .getDefaultLevelRank(series, pk_job, null, pk_post, null); if
				 * (!resultMap.isEmpty()) {
				 * 
				 * defaultlevel = resultMap.get("defaultlevel"); defaultrank =
				 * resultMap.get("defaultrank"); }
				 * 
				 * setValue(defaultlevel, e, PsnJobVO.PK_JOBGRADE);// 职级
				 * setValue(defaultrank, e, PsnJobVO.PK_JOBRANK);// 职等 } else if
				 * (StringUtils.isBlank(pk_job) && StringUtils.isBlank(series))
				 * { setValue(null, e, PsnJobVO.PK_JOBGRADE);// 职级
				 * setValue(null, e, PsnJobVO.PK_JOBRANK);// 职等 } // 设置职级是否可以编辑
				 * setJobGradeItemEdit(e.getTableCode(), e.getRow());
				 */
				// end
