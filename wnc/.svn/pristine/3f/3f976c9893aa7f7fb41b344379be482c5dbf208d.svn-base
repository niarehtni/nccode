...
public class DayStatRecordCreator extends AbstractRecordCreator {
	...
	protected String[] getSubTableFkFields() {
		return new String[]{DayStatbVO.PK_DAYSTAT,DayStatbVO.PK_DAYSTAT,DayStatbVO.PK_DAYSTAT};
	}
	//MOD James
	@Override
	protected String[] getSubTableNames() {
		return new String[]{"tbm_daystatb","tbm_daystatb_notcurrmonth","tbm_daystatbb"};
	}
	...
	protected void deleteGarbageData(String pk_org, UFLiteralDate beginDate,
		...
		String othWhere = " where pk_org <> '"+pk_org+"' and calendar between '"+beginDate.toString()+"' and '"+endDate.toString()+"' " + 
			" and exists(select 1 from tbm_psndoc where tbm_psndoc.pk_org = '"+pk_org+"' and tbm_daystat.calendar between tbm_psndoc.begindate and tbm_psndoc.enddate )";
