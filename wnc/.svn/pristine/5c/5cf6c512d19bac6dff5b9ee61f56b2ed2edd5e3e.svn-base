...
public class NHISysInitVO implements ICheckParaFinal {
	...
	@Override
	public CheckParaVO paraEditabilityCheck(SysInitVO paramSysInitVO,
			SysInitVO[] paramArrayOfSysInitVO) {
		return null;
	}
	...
	@Override
	public CheckParaVO paraBeforeSavingCheck(SysInitVO paramSysInitVO,
			SysInitVO[] paramArrayOfSysInitVO) {
		CheckParaVO chkvo = new CheckParaVO();
		if (paramSysInitVO.getInitcode().substring(0, 3).equals("TWHR")) {
			if (paramSysInitVO.getInitcode().equals("TWHR01")
					&& "Y".equals(String.valueOf(paramSysInitVO.getValue()))) { // 啟用台灣人力資源
				// 读预置脚本
				StringBuilder[] sbPreset = readSQL("/ierp/twhr/*.sql",
						paramSysInitVO);
	...
	private StringBuilder[] readSQL(String filename, SysInitVO paramSysInitVO) {
		...
		try {
			...
			File filelistObj = new File(RuntimeEnv.getInstance().getNCHome()
					+ filename);
			...
			for (File fileobj : fileobjs) {
				...
				while (line != null) {
					...
					if (!line.trim().substring(0, 2).equals("--")) { // 注释行不读取
						line = replaceRefs(line, paramSysInitVO,
								fileobj.getName(), lineno);
	...
	private String replaceRefs(String line, SysInitVO paramSysInitVO,
			String filename, long lineno) {
		// pk_group
		line.replaceAll("$$PKGROUP$$",
				getPkGroupByOrg(paramSysInitVO.getPk_org()));
		// pk_org
		line.replaceAll("$$PKORG$$", paramSysInitVO.getPk_org());
		// time
		line.replaceAll("$$TS$$", paramSysInitVO.getModifiedTime()
				.toLocalString());
		line.replaceAll("$$CREATETIME$$", paramSysInitVO.getModifiedTime()
				.toLocalString());
		// user
	...
	private String replaceQuery(String line, String filename, long lineno) {
		...
		while (begin >= 0) {
			...
			if (begin >= 0) {
				end = line.indexOf(mark, begin + mark.length()) + mark.length()
						- 1;
				if (end < 0) {
					ExceptionUtils.wrappBusinessException("預置腳本發生錯誤，查詢標記不匹配："
							+ filename + ": 第" + String.valueOf(lineno) + "行");
				}
				...
				if (!queryValueMap.containsKey(strQuery)) {
					try {
						String value = (String) dao.executeQuery(strQuery,
								new ColumnProcessor());
	...
	private String getPkGroupByOrg(String pk_org) {
		if (grouporgMap.containsKey(pk_org)) {
			...
			String sql = "select pk_group from org_orgs where pk_org='"
					+ pk_org + "'";
			try {
				String pk_group = (String) dao.executeQuery(sql,
						new ColumnProcessor());
	...
	private String generateAutoAddID() {
		for (int i = autoid.length() - 1; i >= 0; i--) {
			...
			autoid = autoid.substring(0, i)
					+ nxt
					+ (i == autoid.length() - 1 ? "" : autoid.substring(i + 1,
							autoid.length()));
