		...
		if (changeDayTypeCheckBox) {
			...
			if (null != changeDate) {
				...
				try {
					...
					//IHRHolidayQueryService
					//在页面做一个弹出框
					List<List<String>> strMessage = manageValidate.updatedayValidate(wizardModel.getModel().getContext().getPk_org(),psndocListWithNoDate.toArray(new String[0]), changeDate, panel3.getafterChangeDateType());
					UFBoolean isStrcheck = SysInitQuery.getParaBoolean(legal_pk_org, "TWHRT03");
					if(null != strMessage ){
						for(List<String> strs : strMessage){
							if(isStrcheck.booleanValue()){
