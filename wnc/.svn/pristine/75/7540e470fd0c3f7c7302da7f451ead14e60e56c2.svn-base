...
//			cardPanel.getLeavePanel().setVisible(true);
	...
	public Object getValue() {
		...
		if((Integer)cardPanel.getWayofresult().getSelectdItemValue()==1||(Integer)cardPanel.getWayofresult().getSelectdItemValue()==2){
			vo.setMoncounts(cardPanel.getMonthunit()==null?0:Integer.valueOf(cardPanel.getMonthunit().getSelectdItemValue().toString()));//台湾本地化  按固定、滚动月份计算假期使用情况
		}else{
			vo.setMoncounts(0);//台湾本地化  按固定、滚动月份计算假期使用情况
		}
