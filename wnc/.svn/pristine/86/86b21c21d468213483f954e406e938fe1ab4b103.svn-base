...
public class IntervalDurationParse extends AbstractWAFormulaParse {
	...
	public static void valueOfTBMPsn(WaLoginContext context, String replaceArg) throws BusinessException{
		...
		String sql = "update wa_cacu_data set cacu_value = 0 where  " + "pk_wa_class = '" + context.getPk_wa_class() + "' and creator = '" + context.getPk_loginUser() + "'";
		dao.executeUpdate(sql); 
		
		sql = "select DISTINCT pk_psndoc from wa_cacu_data where pk_wa_class= '" + context.getPk_wa_class() + "' and creator = '" + context.getPk_loginUser() + "'";

		ArrayList<String> pk_psndocs = (ArrayList<String>)dao.executeQuery(sql, new ColumnListProcessor());
		sql = "SELECT DISTINCT pk_psndoc FROM hi_psnjob WHERE trnstype = '" + refTransType + "'";//(SELECT pk_trnstype FROM hr_trnstype WHERE trnstypecode = '0305')
		ArrayList<String> id_psndocs = (ArrayList<String>)dao.executeQuery(sql, new ColumnListProcessor());
		String beginDate="",endDate="";
		if(id_psndocs!=null){
			sql = replaceArg+" AND wa_data.pk_psndoc='" + pk_psndocs.get(0) + "'";
			...
			" AND hi_psnjob.pk_psndoc IN " + str +
			...
			if (result!=null){
				...
				for(int i=0; i < result.size(); i++){
					...
					if(isExistPsn){
						firstNoExist = true;
						if(!idMap.containsKey(psn)) idMap.put(psn,(long)0);
						//判断单个人员信息是否读取完成
						...
						if(start&&isExistTrnstype && refTransType.equals((String)records[2])) {
							...
							if(!"".equals(idBeginDate)&&getDaySub(idBeginDate,endDate)>0) {
								if(getDaySub(idBeginDate,beginDate)>0) idBeginDate = beginDate;
								start = false;
								end = true;
								//最后一条记录判断
							...
							if(!"".equals(idEndDate)){
								if(getDaySub(beginDate,idEndDate)>0){
									if(getDaySub(endDate,idEndDate)>0) idEndDate = endDate;
			...
			try {
				...
				while (iterator.hasNext()) {  
					...
					sql = "update wa_cacu_data set cacu_value = " + entry.getValue() + " where  " + "pk_wa_class = '" + context.getPk_wa_class() + "' and creator = '"
							+ context.getPk_loginUser() + "'  and pk_psndoc = '" + entry.getKey() + "'";
