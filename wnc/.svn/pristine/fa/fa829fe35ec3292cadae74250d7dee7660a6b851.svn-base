...
public class ConfirmPsnStepListenerForOutJoin implements IWizardStepListener {

    @Override
    public void stepActived(WizardStepEvent event) throws WizardStepException {
	Debug.debug("multiselect step will be actived");
	ConfirmPsnStepForOutJoin currStep = (ConfirmPsnStepForOutJoin) event.getStep();
	HRWizardModel model = (HRWizardModel) currStep.getModel();
	if (!(model.getStepWhenAction() instanceof SelPsnStepForOutJoin))
	    return;
	...
	IQueryScheme scheme = selPsnPanel.getSelPsnPanelForOutJoin().getQueryEditor().getQueryScheme();
	...
	currStep.getConfirmPsnPanelForOutJoin().setFormVOs(vos);
    }
