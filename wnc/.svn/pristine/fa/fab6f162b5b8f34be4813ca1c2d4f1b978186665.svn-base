...
public class PFSubmitAdjustAction extends PFSubmitAction {
	...
	private SimpleDocServiceTemplate service = new SimpleDocServiceTemplate(
			getClass().getName());
	...
	public void doAction(ActionEvent evt) throws Exception {
		...
		headVO.setSum_apply_money(headVO.getSum_apply_money() == null ? null
				: new UFDouble(SalaryEncryptionUtil.encryption(headVO
						.getSum_apply_money().toDouble())));
		headVO.setSum_confim_money(headVO.getSum_confim_money() == null ? null
				: new UFDouble(SalaryEncryptionUtil.encryption(headVO
						.getSum_confim_money().toDouble())));
		...
		if (INCSystemUserConst.NC_USER_PK.equals(headVO.getOperator())
				|| headVO.getOperator() == null)
	...
	@Override
	protected PfProcessBatchRetObject submitValidate(Integer iApproveType)
		...
		for (int i = 0; i < pfAggVOs.length; i++) {
			...
			if (dbVO == null) {
				exInfo.putErrorMessage(i, pfAggVOs[i],
			...
			if (!checkDataPermission(operationCode, mdOperationCode,
					resourceCode, dbVO)) {
				exInfo.putErrorMessage(i, dbVO,
						ResHelper.getString("6001pf", "06001pf0074")
				/* @res "没有权限提交单据" */);
				continue;
			}
			...
			if (IPfRetCheckInfo.NOSTATE != flowBiz.getApproveStatus()) {
				exInfo.putErrorMessage(i, dbVO,
						ResHelper.getString("6001pf", "06001pf0075")
				/* @res "单据的的状态不为自由态" */);
				continue;
			}

			if (HRConstEnum.APPROVE_TYPE_FORCE_WORK_FLOW == iApproveType) {
				String billtype = StringUtils.isBlank(flowBiz.getTranstype()) ? flowBiz
						.getBilltype() : flowBiz.getTranstype();

				boolean blExistWorkFlow = getWorkflowQry()
						.isExistWorkflowDefinition(billtype,
								flowBiz.getPkorg(), flowBiz.getBillMaker(),
								WorkflowTypeEnum.Approveflow.getIntValue());

				if (!blExistWorkFlow) {
					exInfo.putErrorMessage(i, dbVO,
							ResHelper.getString("6001pf", "06001pf0065")
					/* @res "没有针对当前提交人的审批流" */);
					continue;
				}
			}
		...
		return new PfProcessBatchRetObject(
				al.toArray(new AggregatedValueObject[0]), exInfo);
	}
