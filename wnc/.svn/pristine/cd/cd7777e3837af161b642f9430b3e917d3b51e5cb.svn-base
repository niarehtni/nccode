...
public class SaveLeaveRegAction extends SaveAction {
    ...
    public void doAction(ActionEvent evt) throws Exception {
		...
		//add by chenklb@yonyou.com 2018.5.8  保存时校验生效时间begin
		nc.vo.pub.lang.UFLiteralDate nowDate=new UFLiteralDate();
		UFLiteralDate nowDate1=nowDate.before(vo.getLeaveenddate())?vo.getLeaveenddate():nowDate;
		vo.setEffectivedate(vo.getEffectivedate()==null?nowDate1:(vo.getEffectivedate().before(nowDate1)?nowDate1:vo.getEffectivedate()));
		//add by chenklb@yonyou.com 2018.5.8  保存时校验生效时间end
		if(islactation())
		{
...
/*@res "开始日期{0}晚于结束日期{1},请修改!"*/;
		...
		else
		{
			...
			try {
				period1 =NCLocator.getInstance().lookup(IPeriodQueryService.class).queryByDate(vo.getPk_org(),vo.getBegindate());
				period2 =  NCLocator.getInstance().lookup(IPeriodQueryService.class).queryByDate(vo.getPk_org(),vo.getEnddate());
				TimeRuleVO timeRule=((TALoginContext)getModel().getContext()).getAllParams().getTimeRuleVO();
				...
				if(!period1.getYear().equals(period2.getYear())){//判断开始时间和结束时间是否在同一年
					if(!canOverYear.booleanValue()||canOverYear==null){//考勤规则中的休假跨年是否保存
						hint=ResHelper.getString("6017leave","06017leave0265");/*休假时间段跨年不允许保存*/
					}
				}
