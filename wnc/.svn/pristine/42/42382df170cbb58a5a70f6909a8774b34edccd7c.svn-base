...
public class GenerateNHISettingAction extends PsndocWadocPubAction {
	...
	public GenerateNHISettingAction() {
		...
		putValue("ShortDescription", "為當前選定員工生成首筆勞保、勞退及健保投保設定資料" + "(Ctrl+Alt+G)");
		...
		new SwingWorker() {
			...
			protected Boolean doInBackground() throws Exception {
				try {
					...
					int selectRow = getMainComponent().getComponent().getBillCardPanel().getBillTable()
							.getSelectedRow();
					if (selectRow >= 0) {
						psnList.add((String) getMainComponent().getComponent().getBillCardPanel().getBillModel()
			...
			protected void done() {
				if (error != null) {
					ShowStatusBarMsgUtil.showErrorMsg("生成投保设定發生錯誤", error, getModel().getContext());
				} else {
					ShowStatusBarMsgUtil.showStatusBarMsg("生成投保設定成功", getModel().getContext());
				}
			}
		}.execute();
	...
	protected boolean isActionEnable() {
		...
		try {
			...
			this.putValue("message_after_action", "取台灣勞健保參數發生錯誤：" + e.getMessage());
		}
