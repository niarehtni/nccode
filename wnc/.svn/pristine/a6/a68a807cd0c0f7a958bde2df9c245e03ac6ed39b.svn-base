...
public class PsnjobImportExecutor extends DataImportExecutor implements
		...
		if (this.getNcValueObjects() != null
			...
			for (Map<String, Object> rowNCMap : this.getNcValueObjects()) {
				try {
					...
					rowNCMap.put(rowNo + ":clerkcode", code);

					String id = (String) rowNCMap.get(rowNo + ":"
							+ this.getReservedPropertyName("IDNO"));
					// pk_psndoc
					String pk_psndoc = "";
					if (this.getPsnIDPKMap().containsKey(id)
							&& this.getPsnIDPKMap().get(id) != null) {
						...
						if (StringUtils.isEmpty(pk_psndoc)
								|| "~".equals(pk_psndoc)) {
							throw new BusinessException("找不到身份證件號碼 [" + id
									+ "] 對應的員工");
						} else {
					...
					// 組織集團
					String orgCode = (String) rowNCMap.get(rowNo + ":"
							+ this.getReservedPropertyName("NCOMNO"));
					...
					// 異動事件
					String trnsReasonCode = (String) rowNCMap.get(rowNo + ":"
							+ this.getReservedPropertyName("ETSNO"));
					...
					// 生效日期
					String effectDate = getDateString((String) rowNCMap
							.get(rowNo + ":"
									+ this.getReservedPropertyName("CRDATE")));
					// 離職記錄開始日期+1
					if (trnsevent == 4) {
						effectDate = new UFDate(effectDate).getDateAfter(1)
								.toString();
					}
					rowNCMap.put(rowNo + ":begindate",
							effectDate.substring(0, 10));
					...
					// 異動原因
					String trnsreason = trnsevent == 4 ? getTransReason("J00")
							: (getTransReason(trnsReasonCode));
					rowNCMap.put(rowNo + ":trnsreason", trnsreason);

					String pk_psnjob = getPsnjobByEvent(pk_psndoc, trnsevent,
							effectDate);
					...
					this.getExtendSQLs()
							.get(rowNo)
							.addAll(dealPsnorg(rowNo, pk_psndoc, pk_org,
									effectDate, trnsevent, ts));
					rowNCMap.put(rowNo + ":pk_psnorg", this.getPsnOrgPKMap()
							.get(rowNo));

					// 職等
					if (this.isPropertyChanged(rowNCMap, rowNo,
							this.getReservedPropertyName("OCLASS1"),
					...
					// 班次
					if (this.isPropertyChanged(rowNCMap, rowNo,
							this.getReservedPropertyName("OCLSNO"),
					...
					// 職稱
					if (this.isPropertyChanged(rowNCMap, rowNo,
							this.getReservedPropertyName("OTILNO"),
					...
					// 工作地點
					if (this.isPropertyChanged(rowNCMap, rowNo,
							this.getReservedPropertyName("OPLACENO"),
							this.getReservedPropertyName("NPLACENO"))) {
						String pk_post = getPk_place((String) rowNCMap
								.get(rowNo
										+ ":"
										+ this.getReservedPropertyName("NPLACENO")));
					...
					// 備註
					String memo = (String) rowNCMap.get(rowNo + ":"
					...
					this.getExtendSQLs()
							.get(rowNo)
							.addAll(this.dealPsnwork(pk_psnjob, pk_psndoc,
									pk_dept_v, effectDate, pk_org, trnsevent,
									ts));
	...
	private String getPsnjobByEvent(String pk_psndoc, int trnsevent,
			String effectDate) throws BusinessException {
		String strSQL = "select pk_psnjob from hi_psnjob where pk_psndoc="
				+ getStringValue(pk_psndoc) + " and trnsevent="
				+ String.valueOf(trnsevent) + " and "
				+ getStringValue(effectDate)
				+ " between begindate and isnull(enddate, '9999-12-31');";
		String pk_psnjob = (String) this.getBaseDAO().executeQuery(strSQL,
				new ColumnProcessor());

		return pk_psnjob;
	}

	private List<String> dealPsnwork(String pk_psnjob, String pk_psndoc,
			String pk_dept_v, String effectDate, String pk_org, int trnsevent,
			String ts) throws BusinessException {
		List<String> sqls = new ArrayList<String>();
		String pk_psndoc_sub = null;

		if (!StringUtils.isEmpty(pk_psnjob)) {
			String strSQL = "select pk_psndoc_sub from hi_psndoc_work where pk_psnjob="
					+ getStringValue(pk_psnjob);

			pk_psndoc_sub = (String) this.getBaseDAO().executeQuery(strSQL,
					new ColumnProcessor());
		}
		...
		if (StringUtils.isEmpty(pk_psndoc_sub)) {
			sqls.add("INSERT INTO HI_PSNDOC_WORK ("
					...
					+ "WORK_ADDR, WORKACHIVE, WORKCORP, WORKDEPT, WORKDUTY, WORKJOB, WORKPOST) "
					+ "VALUES ("
					// BEGINDATE, BG_CHECK, CERTIFIER, CERTIPHONE, CREATIONTIME
					...
					+ ", null, 0, "
					+ (trnsevent == 4 ? getStringValue(effectDate.substring(0,
							10)) : "null")
					+ ", 'N', null, null, null, '~', "
					...
					+ ", "
					+ (StringUtils.isEmpty(pk_psnjob) ? newPK
							: getStringValue(pk_psnjob))
					...
					+ getStringValue(pk_dept_v) + "), null, null, null);");
		} else {
			if (trnsevent == 4) {
				sqls.add("UPDATE HI_PSNDOC_WORK SET ENDDATE="
						+ getStringValue(effectDate.substring(0, 10))
						+ " WHERE PK_PSNDOC_SUB="
						+ getStringValue(pk_psndoc_sub));
			} else {
				sqls.add("UPDATE HI_PSNDOC_WORK SET BEGINDATE="
						+ getStringValue(effectDate.substring(0, 10))
						+ " WHERE PK_PSNDOC_SUB="
						+ getStringValue(pk_psndoc_sub) + " AND BEGINDATE > "
						+ getStringValue(effectDate.substring(0, 10)));
			}
		}
	...
	private String getPk_Org(String orgCode) throws BusinessException {
		...
		String strSQL = "select pk_org from org_orgs where code="
	...
	private String getPk_dept_v(String pk_dept, String effectDate)
			throws BusinessException {
		String strSQL = "select pk_vid from org_dept_v where pk_dept = "
				+ getStringValue(pk_dept) + " and "
				+ getStringValue(effectDate)
	...
	private List<String> dealPsnorg(String rowNo, String pk_psndoc,
			String pk_org, String effectDate, int trnsevent, String ts)
			throws BusinessException {
		List<String> sqls = new ArrayList<String>();

		String strSQL = "select pk_psnorg from hi_psnorg where pk_psndoc = "
				+ getStringValue(pk_psndoc) + " and pk_org = "
				+ getStringValue(pk_org) + " and begindate <= "
				+ getStringValue(effectDate.substring(0, 10));
		String pk_psnorg = (String) this.getBaseDAO().executeQuery(strSQL,
				new ColumnProcessor());

		if (StringUtils.isEmpty(pk_psnorg)) {
			if (this.getPsnOrgMap().containsKey(pk_psndoc)) {
				for (Entry<UFDate, String> psnorg : this.getPsnOrgMap()
						.get(pk_psndoc).entrySet()) {
		...
		if (StringUtils.isEmpty(pk_psnorg)) {
			...
			if (!this.getPsnOrgMap().containsKey(pk_psndoc)) {
				this.getPsnOrgMap().put(pk_psndoc,
						new HashMap<UFDate, String>());
			}

			this.getPsnOrgMap().get(pk_psndoc)
					.put(new UFDate(effectDate), pk_psnorg);
			...
			sqls.add("UPDATE HI_PSNORG SET BEGINDATE="
					+ getStringValue(effectDate.substring(0, 10))
					+ " WHERE PK_PSNORG=" + getStringValue(pk_psnorg)
					+ " AND BEGINDATE > " + getStringValue(effectDate));
			this.getPsnOrgPKMap().put(rowNo, pk_psnorg);

			if (trnsevent == 4) {
				// 离职更新组织关系
				sqls.add("update hi_psnorg set enddate="
						+ getStringValue(effectDate.substring(0, 10))
						+ " where pk_psnorg=" + getStringValue(pk_psnorg));
			}
		}
	...
	private void reOrderPsnJobs(String pk_psndoc) throws BusinessException {
		String strSQL = "select * from hi_psnjob where pk_psndoc="
				+ getStringValue(pk_psndoc)
				+ " and dr=0 order by begindate desc;";
		List<Map<String, Object>> psnjobs = (List<Map<String, Object>>) this
		...
		if (psnjobs != null && psnjobs.size() > 0) {
			...
			for (int i = 0; i < psnjobs.size(); i++) {
				...
				if (i < psnjobs.size() - 1) {
					// 離職不更新Enddate
					enddate = new UFDate((String) psnjobs.get(i + 1).get(
							"begindate")).getDateBefore(1);
					strSQL += ", enddate="
							+ getStringValue(enddate.toString()
									.substring(0, 10));
				} else {
					strSQL += ", lastflag='Y'";
				}
	...
	private String getPKByID(String id) throws BusinessException {
		...
		if (this.getPsnIDPKMap().containsKey(id)) {
			...
			String strSQL = "select pk_psndoc from bd_psndoc where id = "
					+ getStringValue(id);
