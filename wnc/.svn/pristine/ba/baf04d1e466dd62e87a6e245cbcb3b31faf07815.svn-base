...
public class PsndocNHIDAO {
	 ...
	 * 同步處理離職人員勞健保記錄
	 * 
	 * @param dimissionPsnjob
	 * @throws BusinessException
	 */
	public static void dealNHIForDismissPsn(PsnJobVO dimissionPsnjob) throws BusinessException {
		// ssx added for Taiwan NHI on 2015-10-15
		// Stop NHI Info
		// 2017-05-16 upgrade to V65, from JD code
		// Moved from PersonRecordImpl on 2018-06-07 for multi-source calling
		IPsndocSubInfoService4JFS nhiService = NCLocator.getInstance().lookup(IPsndocSubInfoService4JFS.class);
		nhiService.dismissPsnNHI(dimissionPsnjob.getPk_org(), dimissionPsnjob.getPk_psndoc(), dimissionPsnjob
				.getBegindate().getDateBefore(1));
		// ssx added on 2017-12-19
		// 回寫團保結束日期
		nhiService.dismissPsnGroupIns(dimissionPsnjob.getPk_org(), dimissionPsnjob.getPk_psndoc(), dimissionPsnjob
				.getBegindate().getDateBefore(1));
	}
