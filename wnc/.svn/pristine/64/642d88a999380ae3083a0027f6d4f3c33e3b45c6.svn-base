 ...
 * 
 */
public class DepartmentImportExecutor extends DataImportExecutor implements
			...
			try {
				for (Map<String, Object> rowNCMap : this.getNcValueObjects()) {
					...
					String effectDate = getDateString((String) rowNCMap
							.get(rowNo + ":createdate"));
					getNewIDByCode(deptCode, effectDate);

					if (isNew) {
						// 部門新增時
						// 補充：版本PK
						rowNCMap.put(rowNo + ":pk_vid",
								newDeptVersionPK.replaceAll("'", ""));
						// 補充：集團PK
						...
						if (rowNCMap.containsKey(rowNo + ":creationtime")) {
							String madedate = (String) rowNCMap.get(rowNo
									+ ":creationtime");
					...
					if (rowNCMap.containsKey(rowNo + ":deptcanceldate")) {
						rowNCMap.put(rowNo + ":hrcanceled",
								isCanceled(rowNCMap, rowNo) ? "Y" : "N");
					}
					...
					if (isNew) {
						...
						this.getExtendSQLs()
								.get(rowNo)
								.addAll(getExtNewDeptVersionSQL(rowNCMap, rowNo));

						// 新增組織
						this.getExtendSQLs().get(rowNo)
								.addAll(getExtNewOrgSQL(rowNCMap, rowNo));

						// 新增組織版本
						this.getExtendSQLs()
								.get(rowNo)
								.addAll(getExtNewOrgVersionSQL(rowNCMap, rowNo));

						// 新增部門負責人
						this.getExtendSQLs().get(rowNo)
								.addAll(getExtNewPrincipleSQL(rowNCMap, rowNo));

						// 新增部門變更記錄
						this.getExtendSQLs().get(rowNo)
								.addAll(getExtNewHistorySQL(rowNCMap, rowNo));
	...
	private List<String> getExtNewHistorySQL(Map<String, Object> rowNCMap,
			String rowno) {
		...
		sqls.add("INSERT INTO OM_DEPTHISTORY "
				...
				+ ", "
				+ (rowNCMap.containsKey(rowno + ":name2") ? getStringValue(rowNCMap
						.get(rowno + ":name2")) : getStringValue(rowNCMap
						.get(rowno + ":name")))
				+ ", "
				+ (rowNCMap.containsKey(rowno + ":name3") ? getStringValue(rowNCMap
						.get(rowno + ":name3")) : getStringValue(rowNCMap
						.get(rowno + ":glbdef5"))) + ", null, null, null, "
				+ newDeptPK + ", '" + OidGenerator.getInstance().nextOid()
	...
	private List<String> getExtNewOrgVersionSQL(Map<String, Object> rowNCMap,
			String rowno) {
		List<String> sqls = new ArrayList<String>();
		sqls.add("INSERT INTO ORG_ORGS_V "
				...
				+ (isNewVersion(rowNCMap, rowno)
						&& !StringUtils.isEmpty(((String) rowNCMap.get(rowno
								+ ":createdate"))) ? "'"
						+ getDateString(
								(String) rowNCMap.get(rowno + ":createdate"))
				...
				+ ", '~', '~', null, "
				+ "null, '~', '~', '"
				+ rowNCMap.get(rowno + ":pk_org") + "', '~', '~', null, '~');");
		return sqls;
	}

	private List<String> getExtNewOrgSQL(Map<String, Object> rowNCMap,
			String rowno) {
		List<String> sqls = new ArrayList<String>();
	...
	private List<String> getExtNewDeptVersionSQL(Map<String, Object> rowNCMap,
			String rowno) {
		List<String> sqls = new ArrayList<String>();
		sqls.add("INSERT INTO ORG_DEPT_V "
				...
				+ "CHARGELEADER, DEPTLEVEL, ORGTYPE13, ORGTYPE17, DEPTDUTY) "
				+ "VALUES " + "('~',"
				+ getStringValue(rowNCMap.get(rowno + ":code"))
				+ ", "
				+ (getDateString((String) rowNCMap.get(rowno + ":createdate")) == null ? "null"
	...
	private void getNewIDByCode(String deptCode, String effectDate)
			throws BusinessException {
		String strSQL = "select pk_dept, pk_vid from org_dept where code = '"
				+ deptCode + "' and islastversion='Y'";
		Map<String, Object> pks = (Map<String, Object>) this.getBaseDAO()
				.executeQuery(strSQL, new MapProcessor());
	...
	private List<String> getExtDeptChangeSQL(Map<String, Object> rowNCMap,
			String rowno) throws BusinessException {
		List<String> sqls = new ArrayList<String>();
	...
	private String getDateString(Map<String, Object> rowNCMap, String rowno) {
		...
		if (!StringUtils.isEmpty(dateString)) {
			...
			dateString = (!StringUtils.isEmpty((String) rowNCMap.get(rowno
					+ ":createdate")) ? getDateString((String) rowNCMap
	...
	private List<String> getExtNewPrincipleSQL(Map<String, Object> rowNCMap,
			String rowno) throws BusinessException {
		List<String> sqls = new ArrayList<String>();
		if (deptOrgExists((String) rowNCMap.get(rowno + ":code"))) {
			// 部門主管
			String deptcode = (String) rowNCMap.get(rowno + ":code");
			String psncode = rowNCMap.get(rowno + ":principal") == null ? null
					: (String) rowNCMap.get(rowno + ":principal");
			if (psncode != null && !principleExists(deptcode, psncode)) {
				...
				sqls.add("INSERT INTO org_orgmanager "
						...
						+ psncode
						+ "'), 0, 0, (select pk_group from org_dept where code = '"
						+ deptcode
						+ "'), (select pk_org from org_dept where code = '"
						+ deptcode
						+ "'), '"
						+ OidGenerator.getInstance().nextOid()
						+ "', (select pk_dept from org_dept where code = '"
						+ deptcode
						+ "'), (select pk_psndoc from bd_psndoc where code = '"
						+ psncode
						+ "'), null, "
						+ "'Y', null, null "
						+ " from dual where (select count(pk_psndoc) from bd_psndoc where code = '"
						+ psncode + "') > 0;");
			}

			// 部門副主管
			if (!principleExists((String) rowNCMap.get(rowno + ":code"),
					(String) rowNCMap.get(rowno + ":principal"))) {
				// sqls.add("");
			}
		}
	...
	private boolean principleExists(String deptcode, String psncode)
			throws BusinessException {
	...
	private boolean isCanceled(Map<String, Object> rowNCMap, String rowno) {
		if (StringUtils.isEmpty((String) rowNCMap
				.get(rowno + ":deptcanceldate"))
				|| ((String) rowNCMap.get(rowno + ":deptcanceldate"))
	...
	private List<String> getExtDeptVersionSQL(Map<String, Object> rowNCMap,
			String rowno) throws BusinessException {
		List<String> sqls = new ArrayList<String>();
		if (deptDeptVersionExists((String) rowNCMap.get(rowno + ":code"),
			...
			sqls.add("UPDATE ORG_DEPT_V "
					...
					+ this.getCuserid()
					+ "'"
					+ (rowNCMap.containsKey(rowno + ":innercode") ? (", INNERCODE=" + getStringValue(rowNCMap
							.get(rowno + ":innercode"))) : "")
					...
					+ (rowNCMap.containsKey(rowno + ":name") ? (", NAME=" + getStringValue(rowNCMap
							.get(rowno + ":name"))) : "")
					+ (rowNCMap.containsKey(rowno + ":name2") ? (", NAME2=" + getStringValue(rowNCMap
							.get(rowno + ":name2"))) : ", NAME2="
							+ getStringValue(rowNCMap.get(rowno + ":name")))
					+ (rowNCMap.containsKey(rowno + ":name3") ? (", NAME3=" + getStringValue(rowNCMap
							.get(rowno + ":name3"))) : ", NAME3="
			...
			sqls.add("INSERT INTO ORG_DEPT_V "
					...
					+ "null, '9999-12-31 23:59:59', '初始版本', null, null, null, null, null, "
					+ (isNewVersion(rowNCMap, rowno)
							&& !StringUtils.isEmpty(((String) rowNCMap
									.get(rowno + ":createdate"))) ? "'"
							+ getDateString(
									(String) rowNCMap
											.get(rowno + ":createdate"))
									.replace("/", "-").substring(0, 7)
									.replace("-", "") + "'" : (!StringUtils
							.isEmpty((String) rowNCMap.get(rowno
									+ ":createdate")) ? getDateString(
							(String) rowNCMap.get(rowno + ":createdate"))
	...
	private boolean deptDeptVersionExists(String code, String effectDate)
			throws BusinessException {
		String strSQL = "select count(*) from org_dept_v where code = '"
				+ code.trim() + "' and createdate='" + effectDate + "'";
		Integer count = (Integer) this.getBaseDAO().executeQuery(strSQL,
				new ColumnProcessor());
		return count > 0;
	}

	private List<String> getExtOrgSQL(Map<String, Object> rowNCMap, String rowno)
			throws BusinessException {
		List<String> sqls = new ArrayList<String>();
		...
		if (deptOrgExists((String) rowNCMap.get(rowno + ":code"))) {
			...
			sqls.add("UPDATE ORG_ORGS "
					...
					+ (rowNCMap.containsKey(rowno + ":enablestate") ? (", ENABLESTATE='"
							+ String.valueOf(rowNCMap.get(rowno
									+ ":enablestate")) + "'")
							: "")
					+ (rowNCMap.containsKey(rowno + ":createdate") ? (", VSTARTDATE=" + ((getDateString((String) rowNCMap
							.get(rowno + ":createdate")) == null ? "null" : "'"
	...
	private boolean deptOrgExists(String code) throws BusinessException {
		String strSQL = "select count(*) from org_orgs where code = '"
				+ code.trim() + "'";
		Integer count = (Integer) this.getBaseDAO().executeQuery(strSQL,
				new ColumnProcessor());
		return count > 0;
	...
	private String getInnerCode(String code, String pk_fatherorg)
			throws BusinessException {
		String innercode = "";
		String rdnStr = "";
		...
		if (!StringUtils.isEmpty(pk_fatherorg)) {
			innercode = (String) this.getBaseDAO().executeQuery(
					"select innercode from org_dept where pk_dept='"
							+ pk_fatherorg + "'", new ColumnProcessor());
		}
