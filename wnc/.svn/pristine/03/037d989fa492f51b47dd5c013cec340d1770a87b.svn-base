...
public class CalAccumulatePaidFormulaExecutor extends AbstractFormulaExecutor {
	...
	public void excute(Object arg0, WaLoginContext context) throws BusinessException {
		...
		String calSql = "select sum("
				+ Cur_Time_Pay_Tax
				+ ") cal_sum,wa.pk_psndoc  from wa_data where pk_wa_class in ( select PK_WA_CLASS from wa_periodstate where PK_WA_PERIOD in (select pk_wa_period from wa_period where CONCAT(CYEAR,CPERIOD) >= '"
				+ start + "' and CONCAT(CYEAR,CPERIOD) < '" + end + "' ) and CACULATEFLAG = 'Y') or pk_wa_class = '"
				+ pk_wa_class + "')) and pk_psndoc in (select distinct pk_psndoc from wa_cacu_data where PK_WA_CLASS='"
