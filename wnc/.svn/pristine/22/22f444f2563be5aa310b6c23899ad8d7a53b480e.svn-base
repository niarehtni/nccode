...
public class DefaultExportAction extends HrAction {
			...
			new Thread() {
				...
				public void run() {
					...
					try {
						...
						List<Map<String,String[]>> textArr = getService().getIITXTextReport(pk_group, pk_org, showTaxDig.getStartdate(),
								showTaxDig.getEnddate(),showTaxDig.getContactName(), showTaxDig.getContactEmail(), showTaxDig.getContactTel());
	...
	private IDeclarationExportService getService() {
		if (service == null) {
			service = (IDeclarationExportService) NCLocator.getInstance().lookup(
					IDeclarationExportService.class);
		}
		return service;
	}
