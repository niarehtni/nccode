...
import nc.ref.twhr.refmodel.RangeLineRefModel;
...
public class JoinSetPanelForLabourJoin extends UIPanel implements ValueChangedListener, AppEventListener,
	ActionListener {
    ...
    /*
     * @Override public void valueChanged(ValueChangedEvent event) { int
     * daysCount = getCircularDaysCountInput().getValue(); int existsCount =
     * getShiftSetScrollPane().getTable().getRowCount();
     * if(daysCount==existsCount){ return; } if(daysCount>existsCount){
     * getShiftSetScrollPane().addLine(daysCount-existsCount); return; } int[]
     * delRows = new int[existsCount-daysCount]; for(int
     * i=0;i<delRows.length;i++){ delRows[i]=daysCount+i; }
	...
	if (labourPanel == null) {
	    ...
	    // DefaultFormBuilder(layout,shiftPanel);
	    ...
	    // shiftPanel.setBorder(new
	    // UITitledBorder(ResHelper.getString("6017psncalendar","06017psncalendar0076")
	    // /*@res "班次信息"*/));
	}
	return labourPanel;
    }
	...
	if (laoJiRefPane == null) {
	    ...
	    RangeLineRefModel refModel = new RangeLineRefModel(1, newDate);
	...
	if (jianJiRefPane == null) {
	    ...
	    RangeLineRefModel refModel = new RangeLineRefModel(3, newDate);
	...
	if (tuiJiRefPane == null) {
	    ...
	    RangeLineRefModel refModel = new RangeLineRefModel(2, newDate);
