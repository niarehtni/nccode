...
public class AddInsuranceExportAction extends HrAction {
		...
		if (isGen == UIDialog.ID_OK) {
			...
			for (String pk_legalOrg : pkRealOrgSet) {
				...
				if (null == org) {
				 ...
				 filepath = getExportFilePath("三合一加保_"
						+ org.getCode() + ".xls");
				if (null == filepath) {
					return;
				}
				try {
					String s = LocalizationSysinitUtil
							.getTwhrzzxx("TWHRZZXX01");
					if (null == s) {
						throw new BusinessException("參數TWHRZZXX01未找到!");
					}
					s = LocalizationSysinitUtil.getTwhrzzxx("TWHRZZXX02");
					if (null == s) {
						throw new BusinessException("參數TWHRZZXX02未找到!");
					}
					s = LocalizationSysinitUtil.getTwhrzzxx("TWHRZZXX03");
					if (null == s) {
						throw new BusinessException("參數TWHRZZXX03未找到!");
					}
					s = LocalizationSysinitUtil.getTwhrzzxx("TWHRZZXX04");
					if (null == s) {
						throw new BusinessException("參數TWHRZZXX04未找到!");
					}
				} catch (Exception e) {
					throw new BusinessException("參數TWHRZZXX未找到!");
				}
				...
				String sql = "select 4 as move, hr."
						...
						+ " inner join (select * from hi_psnjob where lastflag = 'Y') psnjob on psnjob.PK_PSNDOC = a.PK_PSNDOC "
						+ " inner join bd_defdoc dfc on  dfc.PK_DEFDOC=psnjob.JOBGLBDEF8 "
						+ " where g2.begindate = '"
						+ startPeriod
						+ "'"
						+ " and g2.recordnum=(select max(gl.recordnum) from bd_psndoc ps left join "
						+ PsndocDefTableUtil.getPsnLaborTablename()
						+ " gl on gl.pk_psndoc = ps.pk_psndoc where ps.pk_psndoc=a.pk_psndoc) "
						+ " and g2.begindate = g3.begindate "
						+ " and g3.glbdef14='Y' and g2.glbdef10 = 'Y' "
					...
					for (int i = 0; i < list.size(); i++) {
						...
						for (int j = 0; j < 26; j++) {
							...
							cell.setCellValue((map.get("wage") == null ? dfInt
										.format(0) : dfInt.format(
										getDecryptedDecimalValue(map
											.get("wage")))));
								break;
							case 11:
							...
							case 20:
								...
								break;
							case 21:
								cell.setCellValue(map.get("sex") == null ? ""
	...
	private BigDecimal getDecryptedDecimalValue(Object number) {
		return BigDecimal.valueOf((number == null) ? 0 : SalaryDecryptUtil
				.decrypt((new UFDouble(String.valueOf(number)).doubleValue())));
	}
