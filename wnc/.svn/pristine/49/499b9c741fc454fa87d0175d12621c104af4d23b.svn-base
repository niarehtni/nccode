...
public class PayLeaveServiceImpl implements IPayLeaveManageService,
		IPayLeaveQueryService {
	...
	public PeriodVO queryPeriodDate(String pk_periodscheme, String cyear,
		...
		return getPayLeaveDao()
				.queryPeriodDate(pk_periodscheme, cyear, cperiod);
	}
	...
	public PayfileVO[] queryLeavePsnInfo(String pk_waclass, String cyear,
		...
		return getPayLeaveDao().queryLeavePsnInfo(pk_waclass, cyear, cperiod,
				startDate, endDate);
	}
	...
	@Override
	public void processNewLeave(WaLoginContext context, PayfileVO[] psns,
			Object[] vos) throws BusinessException {
		WaClassVO childVO = getNotCheckLeaveClass(context.getPk_prnt_class());
		if (childVO == null) {
			if (getPayLeaveDao().isExistNotPayLeaveClass(
					context.getPk_prnt_class())) {
				throw new BusinessException(ResHelper.getString(
						"60130payleave", "060130payleave0022")/*
															 * @res
															 * "存在未完成的离职发薪，请处理后再进行操作。"
															 */);
			}
			// 查询父方案
			WaClassVO parentClass = WADelegator.getWaClassQuery()
					.queryWaClassByPK(context.getPk_prnt_class());
		...
		service.regenerateSystemFormula(childVO.getPk_org(),
				childVO.getPk_wa_class(), childVO.getCyear(),
				childVO.getCperiod());
	...
	public WaClassVO getNotCheckLeaveClass(String pk_waclass)
			throws BusinessException {
		return getPayLeaveDao().getNotCheckLeaveClass(pk_waclass);
	}

	public WaClassVO createLeaveClass(WaClassVO parentClass)
			throws BusinessException {
	...
	public void addLeavePsn(WaClassVO childVO, PayfileVO[] psns)
			throws BusinessException {
		if (psns == null) {
			return;
		}
	...
	public void processUserClassItem(WaLoginContext context, WaClassVO childVO,
			Object[] vos) throws BusinessException {
	...
	public void addUserClassItem(WaClassVO childVO, Object[] vos)
			throws BusinessException {
		if (vos == null || vos.length == 0) {
			return;
		}
	...
	private WaClassVO createChildClass(WaClassVO parentClass)
			throws BusinessException {
		// 创建离职子方案
		...
		childVO.setCode(parentClass.getCode() + parentClass.getCyear()
				+ parentClass.getCperiod() + "_times_" + batch);
	...
	private void createClassRelation(WaClassVO parentClass, WaClassVO childVO,
			int batch) throws BusinessException {
		// 创建父子关系
	...
	private void createPeriodState(WaClassVO parentClass, WaClassVO childVO)
			throws BusinessException {
		// 为离职子方案创建期间状态
		PeriodVO periodVO = WADelegator.getPeriodQueryService()
				.queryBySchemeYP(parentClass.getPk_periodscheme(),
						parentClass.getCyear(), parentClass.getCperiod());
	...
	private void createDefClassItem(WaClassVO vo) throws BusinessException {
		// 查询出所有的发放项目
		WaClassItemVO[] newvos = WADelegator
				.getClassItemQuery()
				.queryItemInfoVO(
						vo.getPk_org(),
						vo.getPk_prnt_class(),
						vo.getCyear(),
						vo.getCperiod(),
						" wa_classitem.itemkey in(select itemkey from wa_item where pk_org = 'GLOBLE00000000000000')");
		// 替换薪资方案，薪资年月 状态 。
	...
	public AggPayDataVO queryAggPayDataVOByCondition(
		...
		// 有权限的薪资项目
		WaClassItemVO[] classItemVOs = WADelegator.getPaydataQuery()
		...
		if (StringUtils.isBlank(orderCondtion)) {
			...
			orderCondtion = StringUtils.isBlank(orderCondtion) ? " org_dept.code,hi_psnjob.clerkcode "
					: orderCondtion;
			;
		}
		...
		// end
		String[] pks = getPayLeaveDao().queryPKSByCondition(loginContext,
	...
	public DataVO[] queryDataVOByPks(String[] pk_wa_data)
		...
		try {
			...
			DataVO[] dataVOArrays = getPayLeaveDao().queryByPKSCondition(
					conditon, "");

			// 2016-12-12 zhousze 薪资加密：这里处理离职结薪界面数据刷新查询，数据解密 begin
	...
	@Override
	public WaInludeclassVO queryNewLeaveClasses(String pk_waclass,
			String cyear, String cperiod) throws BusinessException {
		return getPayLeaveDao()
				.queryNewLeaveClasses(pk_waclass, cyear, cperiod);
	}
	...
	public String queryUnCheckPsnInfo(WaLoginContext context, PayfileVO[] psns)
		...
		try {
			String sql = "SELECT distinct hi_psnjob.clerkcode, "
			...
			sql += " AND hi_psnjob.pk_psnjob IN ("
					+ inSQLCreator.getInSQL(psns, PayfileVO.PK_PSNJOB) + ") ";

			List result = (List) getPayLeaveDao().getBaseDao().executeQuery(
					sql, param, new MapListProcessor());
			if (result == null || result.size() == 0) {
				return null;
			}

			// 如果方案已审核不提示下面一句话
			if (context.getWaLoginVO().getPeriodVO().getCheckflag()
					.booleanValue()) {
				return null;
			}
			...
			msg.append(ResHelper.getString("60130payleave",
					"060130payleave0000"))/* @res "以下离职人员将从未审核的正常发薪中删除，是否继续？\r\n" */;
