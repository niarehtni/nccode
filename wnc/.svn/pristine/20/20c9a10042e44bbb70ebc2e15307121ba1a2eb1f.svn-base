...
public class ConfirmPsnStepListenerForQuitJoin implements IWizardStepListener {

    @Override
    public void stepActived(WizardStepEvent event) throws WizardStepException {
	Debug.debug("multiselect step will be actived");
	ConfirmPsnStepForQuitJoin currStep = (ConfirmPsnStepForQuitJoin) event.getStep();
	HRWizardModel model = (HRWizardModel) currStep.getModel();
	if (!(model.getStepWhenAction() instanceof SelPsnStepForQuitJoin))
	    return;
	SelPsnStepForQuitJoin selPsnStep = (SelPsnStepForQuitJoin) model.getSteps().get(0);
	SelPsnPanelForQuitJoin selPsnPanel = selPsnStep.getSelPsnPanelForQuitJoin();
	// FromWhereSQL fromWhereSQL = selPsnPanel.getQuerySQL();

	IQueryScheme scheme = selPsnPanel.getSelPsnPanelForQuitJoin().getQueryEditor().getQueryScheme();
	...
	try {
	    currStep.getConfirmPsnPanelForQuitJoin().setFormVOs(vos, laoJiJu, tuiJiJu, jianJiJu);
	} catch (BusinessException e) {
