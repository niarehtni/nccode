	...
	private void SaveNHIDetail() throws BusinessException {
		...
		if (nhiTotalVOs != null && nhiTotalVOs.size() > 0) {
			for (PsndocDefVO vo : nhiTotalVOs) {
				...
				// on 2018-11-07
				for (String fn : NhiCalcUtils.getNhiDetailEncryptionAttributes()) {
					UFDouble originValue = this.getUFDouble((UFDouble) vo.getAttributeValue(fn));
					double encryptedValue = SalaryEncryptionUtil
							.encryption(this.getUFDouble(originValue).doubleValue());
	...
	private void SaveNHITotal() throws BusinessException {
		...
		if (nhiTotalVOs != null && nhiTotalVOs.size() > 0) {
			for (PsndocDefVO vo : nhiTotalVOs) {
				...
				// ssx added for Encryption
				// on 2018-11-07
				for (String fn : NhiCalcUtils.getNhiSumEncryptionAttributes()) {
					UFDouble originValue = this.getUFDouble((UFDouble) vo.getAttributeValue(fn));
					double encryptedValue = SalaryEncryptionUtil
							.encryption(this.getUFDouble(originValue).doubleValue());
					vo.setAttributeValue(fn, new UFDouble(encryptedValue));
	...
	private List<PsndocDefVO> getNHITotalSaveVOs() throws BusinessException {
		...
		if (this.getNhiDataList() != null && this.getNhiDataList().size() > 0) {
			for (NhiCalcVO vo : this.getNhiDataList()) {
				...
				// glbdef4,劳保级距
				newVO.setAttributeValue("glbdef4", vo.getLaborrange());
				// glbdef5,劳保投保天数
				...
				// glbdef14,劳退级距
				newVO.setAttributeValue("glbdef14", vo.getRetirerange());
				// glbdef15,劳退金提缴金额(个人)
				...
				if (vo.getHealthstuff() != null && vo.getHealthhirer() != null) {
					// glbdef17,健保级距
					newVO.setAttributeValue("glbdef17", getUFDouble(vo.getHealthrange()));
					// glbdef18,眷属人数(含本人)
