...
public class TrnBillFormEditor extends HrBillFormEditor implements BillCardBeforeEditListener {
	...
	public void afterEdit(BillEditEvent e) {
		try {
			if (StapplyVO.PK_PSNJOB.equals(e.getKey())) {
				// 调配离职人员
				setPersonInfo();
				BillItem item = getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE);
				// 调出时清空调配后组织
				getRelAndEnd((String) getBillCardPanel().getHeadItem(StapplyVO.PK_HRCM_ORG).getValueObject(),
				...
				BillItem item = getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE);
				if (item != null && (Integer) item.getValueObject() == TRNConst.TRANSMODE_CROSS_OUT)// &&
				...
				{
					// 调出时清空调配后组织
					String pk_dept = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_DEPT).getValueObject();
				...
				// 岗位
				String pk_post = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST).getValueObject();
				PostVO post = pk_post == null ? null : getService().queryByPk(PostVO.class, pk_post, true);
				if (post != null) {
					setItemValueAndEnable(StapplyVO.NEWPK_POSTSERIES, post.getPk_postseries(), false);// 岗位序列
					setItemValueAndEnable(StapplyVO.NEWPK_JOB, post.getPk_job(), true);// 职务
					JobVO jobVO = post.getPk_job() == null ? null : getService().queryByPk(JobVO.class,
							post.getPk_job());
					boolean isJobTypeSeted = false;
					if (jobVO != null) {
						setItemValueAndEnable(StapplyVO.NEWSERIES, jobVO.getPk_jobtype(), false);// 职务类别
						isJobTypeSeted = true;
					}
					if (post.getEmployment() != null) {
						setItemValueAndEnable(StapplyVO.NEWOCCUPATION, post.getEmployment(), true);// 职业
					}
					if (post.getWorktype() != null) {
						setItemValueAndEnable(StapplyVO.NEWWORKTYPE, post.getWorktype(), true);// 工种
					}

					String defaultlevel = "";
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
							.getDefaultLevelRank(null, null, null, pk_post, null);
					if (!resultMap.isEmpty()) {
						defaultlevel = resultMap.get("defaultlevel");
						defaultrank = resultMap.get("defaultrank");
					}
					if (!isJobTypeSeted) {
						String newseries = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWSERIES)
								.getValueObject();
						setItemValueAndEnable(StapplyVO.NEWSERIES, newseries, false);// 职务类别
					}
					setItemValueAndEnable(StapplyVO.NEWPK_JOBGRADE, defaultlevel, true);// 职级
					setItemValueAndEnable(StapplyVO.NEWPK_JOBRANK, defaultrank, true);// 职等
				} else {
					setItemValueAndEnable(StapplyVO.NEWSERIES, null, true);// 职务类别
					setItemValueAndEnable(StapplyVO.NEWPK_POSTSERIES, null, true);// 岗位序列
					setItemValueAndEnable(StapplyVO.NEWPK_JOBGRADE, null, false);// 职级
				...
				if (post == null) {
					afterEdit(new BillEditEvent(getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOB).getComponent(),
							null, StapplyVO.NEWPK_JOB));
				}
			} else if (StapplyVO.NEWPK_JOB.equals(e.getKey())) {
				// 职务
				String pk_job = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOB).getValueObject();
				String pk_post = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST).getValueObject();
				...
				if (job != null) {
					...
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
							.getDefaultLevelRank(null, pk_job, null, pk_post, null);
					if (!resultMap.isEmpty()) {
						defaultlevel = resultMap.get("defaultlevel");
						defaultrank = resultMap.get("defaultrank");
					}

					setItemValueAndEnable(StapplyVO.NEWSERIES, job.getPk_jobtype(), true);// 职务类别
					setItemValueAndEnable(StapplyVO.NEWPK_JOBGRADE, defaultlevel, true);// 职级
					setItemValueAndEnable(StapplyVO.NEWPK_JOBRANK, defaultrank, true);// 职等
					setItemValueAndEnable(StapplyVO.NEWPK_POSTSERIES, null, false);// 岗位序列
				} else {
					setItemValueAndEnable(StapplyVO.NEWSERIES, null, (StringUtils.isNotBlank(pk_post) ? false : true));// 职务类别
					setItemValueAndEnable(StapplyVO.NEWPK_POSTSERIES, null, true);// 岗位序列
					setItemValueAndEnable(StapplyVO.NEWPK_JOBGRADE, null, (StringUtils.isBlank(pk_post) ? false : true));// 职级
					setItemValueAndEnable(StapplyVO.NEWPK_JOBRANK, null, true);// 职等
				}
			} else if (StapplyVO.NEWPK_JOBGRADE.equals(e.getKey())) {
				// 职级
				String pk_jobgrage = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBGRADE).getValueObject();
				JobLevelVO jobgrade = pk_jobgrage == null ? null : getService().queryByPk(JobLevelVO.class,
						pk_jobgrage, true);
				if (jobgrade != null) {
					String pk_postseries = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES)
							.getValueObject();
					String pk_job = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOB).getValueObject();
					String pk_post = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST).getValueObject();
					String series = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWSERIES).getValueObject();
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
				...
				if (StringUtils.isBlank(pk_job) && StringUtils.isNotBlank(series)) {
					...
					String defaultrank = "";
					Map<String, String> resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
							.getDefaultLevelRank(series, pk_job, null, pk_post, null);
					if (!resultMap.isEmpty()) {
						defaultlevel = resultMap.get("defaultlevel");
						defaultrank = resultMap.get("defaultrank");
					}
				...
				// 单据类型变化 同步ID
				UIRefPane ref = (UIRefPane) getBillCardPanel().getHeadItem(StapplyVO.TRANSTYPEID).getComponent();
				setItemValueAndEnable(StapplyVO.TRANSTYPE, ref.getRefCode(), false);
			} else if (StapplyVO.PK_HI_ORG.equals(e.getKey())) {// 调配后人事组织
				String pk_hi_org = (String) getBillCardPanel().getHeadItem(StapplyVO.PK_HI_ORG).getValueObject();
			...
			} else if (StapplyVO.EFFECTDATE.equals(e.getKey()))// 生效日期
			{
				...
				getRelAndEnd((String) getBillCardPanel().getHeadItem(StapplyVO.PK_HRCM_ORG).getValueObject(),
						(Integer) item.getValueObject());// 选择不同字段时处理解除和终止字段---参数其实为新合同管理组织，目前等于调配后人事组织，后面考虑是否修改
				// 选择完生效日期，清空调整后部门 by he
				setItemValueAndEnable(StapplyVO.NEWPK_DEPT, null, true);// 职务类别
			} else if (StapplyVO.PK_HRCM_ORG.equals(e.getKey())) {
				BillItem item = getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE);
				String pk_hrcm_org = (String) getBillCardPanel().getHeadItem(StapplyVO.PK_HRCM_ORG).getValueObject();
				getRelAndEnd(pk_hrcm_org, (Integer) item.getValueObject());
			}

			String pk_jobType = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWSERIES).getValueObject();// 职务类别
			String pk_job = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOB).getValueObject();// 职务
			String pk_postSeries = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES).getValueObject();// 岗位序列
			String pk_post = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST).getValueObject();// 岗位
			if (StringUtils.isBlank(pk_jobType) && StringUtils.isBlank(pk_postSeries) && StringUtils.isBlank(pk_job)
				...
				getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBGRADE).setEnabled(true);
			}
	...
	public void getRelAndEnd(String pk_hrcm_org, Integer inte) throws BusinessException {
		...
		// 调出时清空调配后组织
		String pk_old_hrcm_org = (String) getBillCardPanel().getHeadItem(StapplyVO.PK_OLD_HRCM_ORG).getValueObject();
	...
	public CtrtVO getCtrt() throws BusinessException {
		String pk_psnorg = (String) getBillCardPanel().getHeadItem(StapplyVO.PK_PSNORG).getValueObject();
	...
	public void getRelEffectDate(CtrtVO ctrtvo) throws BusinessException {
		UFLiteralDate effectdate = (UFLiteralDate) getBillCardPanel().getHeadItem(StapplyVO.EFFECTDATE)
				.getValueObject();
		if (effectdate != null) {
			String isshow = getRelOrEndShow(ctrtvo, effectdate);
			setItemValueAndEnable(StapplyVO.ISRELEASE, StapplyVO.ISRELEASE.equals(isshow) ? UFBoolean.TRUE
					: UFBoolean.FALSE, true);// 解除
			setItemValueAndEnable(StapplyVO.ISEND, StapplyVO.ISEND.equals(isshow) ? UFBoolean.TRUE : UFBoolean.FALSE,
					true);// 终止
		} else {
			// 当生效日期为空时，就不必要去判断这个人是否有合同或者应该解除或者终止合同
			boolean isrelease = (Boolean) getBillCardPanel().getHeadItem(StapplyVO.ISRELEASE).getValueObject();
			setItemValueAndEnable(StapplyVO.ISRELEASE, isrelease ? isrelease : null, true);// 解除
			boolean isend = (Boolean) getBillCardPanel().getHeadItem(StapplyVO.ISEND).getValueObject();
	...
	public CtrtVO getQueryCtrtVO(String pk_psnorg) throws BusinessException {
		CtrtVO ctrtvo = null;
		String condition = " pk_psnorg = '" + pk_psnorg
	...
	public boolean getQuerySeq() throws BusinessException {
		...
		SeqcontrolSetVO seqvo = NCLocator.getInstance().lookup(ISeqcontrolManageService.class).getQuerySet();
		if (seqvo != null) {
	...
	public boolean beforeEdit(BillItemEvent e) {
		...
		if (StapplyVO.TRANSTYPEID.equals(e.getItem().getKey())) {
			...
			String where = " and ( parentbilltype = '" + getModel().getBillType() + "' and pk_group = '"
					+ getModel().getContext().getPk_group() + "' )";
			String powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_TRANSTYPE,
			...
			String where = " and busiprop = 6  and ( isnull(pk_org,'~') = '~' or pk_org = '"
					+ getModel().getContext().getPk_org() + "') and validity =1 and primarybilltype like '"
					+ getModel().getBillType() + "%'";
			((UIRefPane) e.getItem().getComponent()).getRefModel().addWherePart(where);
		} else if (StapplyVO.PK_PSNJOB.equals(e.getItem().getKey())) {
			// 调配/离职人员
			// 在职条件
			String inJobSql = " and hi_psnjob.pk_psnjob in " + HiSQLHelper.getInJobSQL(true);
			UIRefPane psnRef = (UIRefPane) e.getItem().getComponent();
			String condition = "";
			String powerSql = null;
			boolean iscrossin = getModel().getStapply_mode() == TRNConst.TRANSMODE_CROSS_IN ? true : false;// 判断调配方式是否是调入,调入时为true,组织内和调出为false
			if (iscrossin)// 调入
			{
				psnRef.getRefModel().setUseDataPower(Boolean.FALSE);
				// 调入时,参照集团内其他HR组织的人员,使用"调入人员的特殊场景"
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_6007PSNJOB,
						HICommonValue.OPERATIONCODE_TRANSDEFAULT, "hi_psnjob");
				condition = " and hi_psnjob.pk_group = '" + getModel().getContext().getPk_group()
						+ "' and hi_psnjob.pk_hrorg <> '" + getModel().getContext().getPk_org()
						+ "' and hi_psnjob.pk_org <> '" + getModel().getContext().getPk_org()
						+ "' and hi_psnjob.lastflag = 'Y' and hi_psnjob.ismainjob = 'Y' " + inJobSql
						+ (StringUtils.isBlank(powerSql) ? "" : (" and " + powerSql));
			} else
			// 组织内和调出
			{
				...
				condition = inJobSql + (StringUtils.isBlank(powerSql) ? "" : (" and " + powerSql));
			}
			...
			try {
				...
				if (isset)// 设置了逐级管控才走这里面
				{
					if (!iscrossin)// 组织内和调出
					{
						((PsnjobSeqcontrolTreeModel) psnRef.getRefModel()).setOrgSeqcontrol("'"
								+ getModel().getContext().getPk_org() + "'");
						String pkSql = NCLocator.getInstance().lookup(IPsndocService.class)
								.queryControlSql("@@@@Z710000000006M7F", getModel().getContext().getPk_org(), false);
						if (!StringUtils.isEmpty(pkSql)) {
							((PsnjobSeqcontrolTreeModel) psnRef.getRefModel()).setOrgSeqcontrol(pkSql);
						}
					} else
					// 调入
					{
						((PsnjobSeqcontrolTreeModel) psnRef.getRefModel()).setOrgSeqcontrol("'"
								+ getModel().getContext().getPk_org() + "'");
						String pkSql = NCLocator.getInstance().lookup(IPsndocService.class)
								.queryControlSql("@@@@Z710000000006M4C", getModel().getContext().getPk_org(), false);
						if (!StringUtils.isEmpty(pkSql)) {
							((PsnjobSeqcontrolTreeModel) psnRef.getRefModel()).setOrgSeqcontrol(pkSql);
						}
					}
			...
			if (getModel().getStapply_mode() == TRNConst.TRANSMODE_CROSS_OUT
				...
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_ORG,
			...
			if (!StringUtils.isBlank(powerSql)) {
				whereSql = " and pk_adminorg in ( select pk_org from org_orgs where " + powerSql
			...
			} else if (TRNConst.BUSITYPE_DIMISSION.equals(getModel().getBillType())) {
				funcode = "@@@@Z710000000006M7F";
			}

			try {
				String gkSql = NCLocator.getInstance().lookup(IPsndocService.class)
						.queryControlSql(funcode, getModel().getContext().getPk_org(), true);
			...
			// 获取生效日期
			UFDate effectdate = getBillCardPanel().getHeadItem("effectdate").getValueObject() == null ? null
					: new UFDate(String.valueOf(getBillCardPanel().getHeadItem("effectdate").getValueObject()));

			// 部门--AOSorgRefModelNew
			UIRefPane deptRef = (UIRefPane) e.getItem().getComponent();
			// 不受UAP部门权限控制，受HR部门权限控制

			deptRef.getRefModel().setUseDataPower(Boolean.FALSE);
			String pk_org = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_ORG).getValueObject();
			deptRef.setPk_org(pk_org);
			String cond = " and hrcanceled = 'N' and depttype <> 1 ";
			if (null != effectdate) {
				// by he校验调配后部门的有效性,不能引用在生效日期之后成立的部门.同时刷新调配后部门参照的值.
				cond = " and hrcanceled = 'N' and depttype <> 1 and createdate<'" + effectdate + "' ";
			}
			String powerSql = "";
			if (getModel().getStapply_mode() == TRNConst.TRANSMODE_CROSS_OUT
					&& TRNConst.BUSITYPE_TRANSITION.equals(getModel().getBillType())) {
				// 只有在调配申请审批节点并且调配方式为调出时使用特殊场景
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_DEPT,
						HICommonValue.OPERATIONCODE_TRANSDEFAULT, "org_dept");// 数据权限中的使用场景为“变动后组织/部门引用”
			} else {
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_DEPT,
						IRefConst.DATAPOWEROPERATION_CODE, "org_dept");// 数据权限中的使用场景为“通用引用”
			}
			if (!StringUtils.isBlank(powerSql)) {
				cond += " and " + powerSql;
			}
			...
			if (getModel().getStapply_mode() == TRNConst.TRANSMODE_CROSS_OUT
					&& TRNConst.BUSITYPE_TRANSITION.equals(getModel().getBillType())) {
				// 只有在调配申请审批节点并且调配方式为调出时使用特殊场景
				psnclRef.getRefModel().setDataPowerOperation_code(HICommonValue.OPERATIONCODE_TRANSDEFAULT);
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_PSNCL,
						HICommonValue.OPERATIONCODE_TRANSDEFAULT, "bd_psncl");
			} else {
				psnclRef.getRefModel().setDataPowerOperation_code(IRefConst.DATAPOWEROPERATION_CODE);
				powerSql = HiSQLHelper.getPsnPowerSql(PubEnv.getPk_group(), HICommonValue.RESOUCECODE_PSNCL,
			...
			// 职级
			String pk_postseries = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES).getValueObject();
			String pk_job = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOB).getValueObject();
			String pk_post = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST).getValueObject();
			String series = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWSERIES).getValueObject();
			...
			if (item != null) {
				...
				try {
					resultMap = NCLocator.getInstance().lookup(IPsndocQryService.class)
			...
			String pk_jobrank = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBRANK).getValueObject();
			String pk_jobgrade = (String) getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBGRADE).getValueObject();
			...
			if (e.getItem().getTableCode().equals(TRNConst.TRNS_NEWINFO_TAB)) {
				if (ref instanceof UIRefPane) {
					...
					pk_org = item.getValueObject() == null ? getModel().getContext().getPk_org() : (String) item
							.getValueObject();
					if (!getModel().isApproveSite() && TRNConst.TRANSMODE_CROSS_OUT == transMode) {
	...
	public IExceptionHandler getDefaultExceptionHandler() {
		...
		if (defaultExceptionHandler == null) {
			defaultExceptionHandler = new DefaultExceptionHanler(getModel().getContext().getEntranceUI());
		}
	...
	private void hideAgreementSet() {
		...
		isCMStart = PubEnv.isModuleStarted(PubEnv.getPk_group(), PubEnv.MODULE_HRCM);
		if (isCMStart) {
			return;
		}
		// 没有启用合同模块 则不显示合同页签
	...
	protected void onAdd() {
		...
		UIRefPane psnRef = (UIRefPane) getBillCardPanel().getHeadItem(StapplyVO.PK_PSNJOB).getComponent();
		...
		psnRef.getRefModel();
		getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES).setEnabled(true);
		getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBGRADE).setEnabled(false);// 调配申请新增时职级不可编辑
		String trnstype = (String) getBillCardPanel().getHeadItem(StapplyVO.PK_TRNSTYPE).getValueObject();
		...
		try {
			...
			TrnstypeFlowVO[] flow = (TrnstypeFlowVO[]) NCLocator
					...
					.retrieveByClause(null, TrnstypeFlowVO.class,
							" pk_group = '" + PubEnv.getPk_group() + "' and pk_trnstype = '" + trnstype + "'");
			if (flow != null && flow.length > 0) {
				BillItem transtype = getBillCardPanel().getHeadItem(StapplyVO.TRANSTYPEID);
				BillItem transtypecode = getBillCardPanel().getHeadItem(StapplyVO.TRANSTYPE);
				if (transtype != null && transtypecode != null
				...
				BillItem busitype = getBillCardPanel().getHeadItem(StapplyVO.BUSINESS_TYPE);
	...
	protected void onEdit() {
		...
		getBillCardPanel().execHeadTailLoadFormulas();
		Integer stapply_mode = (Integer) getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE).getValueObject();
	...
	protected void setDefaultValue() {
		...
		this.getBillCardPanel().setHeadItem(StapplyVO.PK_TRNSTYPE, getModel().getTrnstype());
		Integer tranMode = getModel().getStapply_mode();
		if (tranMode != null && TRNConst.TRANSMODE_CROSS_OUT == tranMode) {
			// 调出,或变更离职后HR组织
			getBillCardPanel().getHeadItem(StapplyVO.PK_HI_ORG).setEnabled(true);
			getBillCardPanel().setHeadItem(StapplyVO.PK_HI_ORG, null);
		} else {
			// 组织内、调入、不变更离职后组织都是当前HR组织
			getBillCardPanel().getHeadItem(StapplyVO.PK_HI_ORG).setEnabled(false);

			getBillCardPanel().getHeadItem(StapplyVO.PK_HRCM_ORG).setEnabled(false);
			getBillCardPanel().getHeadItem(StapplyVO.ISRELEASE).setEnabled(false);
		...
		if (items != null) {
			for (BillItem item : items) {
				...
				if (value2 != null
						&& (item.getValueObject() == null || IBillItem.TEXTAREA == item.getDataType()
	...
	public void setEditable(boolean editable) {
		...
		if (getModel().getUiState() == UIState.ADD) {
			setHeadItemEnable(StapplyVO.BILL_CODE, getModel().isBillCodeEditable());
		}
	...
	public void setPersonInfo() throws BusinessException {
		...
		BillItem[] olditems = getBillCardPanel().getBillData().getHeadItems(TRNConst.TRNS_OLDINFO_TAB);
		BillItem[] newitems = getBillCardPanel().getBillData().getHeadItems(TRNConst.TRNS_NEWINFO_TAB);
		BillItem[] allItems = (BillItem[]) ArrayUtils.addAll(olditems, newitems);
		UIRefPane psnref = RefUtils.getRefPaneOfItem(getBillCardPanel().getHeadItem(StapplyVO.PK_PSNJOB));
		...
		// getService().queryByCondition(PsnJobVO.class,
		// "pk_psnjob='"+pk_psnjob+"' and eff")
		Object trnstype = getBillData().getHeadItem(StapplyVO.PK_TRNSTYPE).getValueObject();
		// 获取设置了默认的项目
		...
		if (defaultls.contains(StapplyVO.NEWPK_PSNCL)) {
			// 人员类别
			getBillCardPanel().getHeadItem(StapplyVO.NEWPK_PSNCL).setValue(hm.get(PsnJobVO.PK_PSNCL));
		}
		if (defaultls.contains(StapplyVO.NEWPK_DEPT)) {
			// 组织
			getBillCardPanel().getHeadItem(StapplyVO.NEWPK_ORG).setValue(hm.get(PsnJobVO.PK_ORG));
		}
		if (defaultls.contains(StapplyVO.NEWPK_DEPT)) {
			// 部门,没有组织权限就没有部门权限
			getBillCardPanel().getHeadItem(StapplyVO.NEWPK_DEPT).setValue(
					hm.get(PsnJobVO.PK_ORG) == null ? null : hm.get(PsnJobVO.PK_DEPT));
		}
		...
		if (jobItem == null || jobItem.getValueObject() == null) {
			...
			getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBGRADE).setValue(null);
		} else {
			// 职务不为空时
		...
		// 如果默认的没有岗位,则岗位相关的都为空
		BillItem postItem = getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POST);
		if (!defaultls.contains(StapplyVO.NEWPK_POST) || postItem == null || postItem.getValueObject() == null) {
			getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES).setValue(null);
		} else {
			...
			if (post == null) {
				...
				getBillCardPanel().getHeadItem(StapplyVO.NEWPK_POSTSERIES).setValue(post.getPk_postseries());
		...
		if (!defaultls.contains(StapplyVO.NEWPOSTSTAT) && bi != null && !bi.isShow()) {
			bi.setValue(UFBoolean.TRUE);
		}

		// 设置选中的人员的pk_psndoc
		getBillCardPanel().getHeadItem(StapplyVO.PK_PSNDOC).setValue(psn.getPk_psndoc());
		getBillCardPanel().getHeadItem(StapplyVO.PK_PSNORG).setValue(psn.getPk_psnorg());
		getBillCardPanel().getHeadItem(StapplyVO.ASSGID).setValue(psn.getAssgid());
		Integer transMode = null;
		if (getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE) != null) {
			transMode = (Integer) getBillCardPanel().getHeadItem(StapplyVO.STAPPLY_MODE).getValueObject();
		}
		...
		if (transMode == null || TRNConst.TRANSMODE_INNER == transMode || TRNConst.TRANSMODE_CROSS_OUT == transMode) {
			...
			getBillCardPanel().getHeadItem(StapplyVO.PK_OLD_HI_ORG).setValue(
					HiSQLHelper.getEveryHrorg(psn.getPk_psnorg(), psn.getAssgid(), ManagescopeBusiregionEnum.psndoc));
			getBillCardPanel().getHeadItem(StapplyVO.PK_OLD_HRCM_ORG).setValue(
					HiSQLHelper.getEveryHrorg(psn.getPk_psnorg(), psn.getAssgid(), ManagescopeBusiregionEnum.psnpact));
		} else {
			// 调入是要查委托关系
			getBillCardPanel().getHeadItem(StapplyVO.PK_OLD_HI_ORG).setValue(
		...
		if (transMode == null || TRNConst.TRANSMODE_INNER == transMode || TRNConst.TRANSMODE_CROSS_IN == transMode) {
			getBillCardPanel().getHeadItem(StapplyVO.PK_HI_ORG).setValue(getModel().getContext().getPk_org());
			getBillCardPanel().getHeadItem(StapplyVO.PK_HRCM_ORG).setValue(getModel().getContext().getPk_org());
		}
	...
	private void setJobInfo(String pk_job, List<String> defaultls) throws BusinessException {
		...
		if (job == null) {
			...
			// 设置职务类别
			getBillCardPanel().getHeadItem(StapplyVO.NEWSERIES).setValue(job.getPk_jobtype());

			String pk_jobgrade = (String) getHeadItemValue(StapplyVO.NEWPK_JOBGRADE);
			if (defaultls.contains(StapplyVO.NEWPK_JOBGRADE) && pk_jobgrade != null) {
				...
				if (null != grade) {
					getBillCardPanel().getHeadItem(StapplyVO.NEWPK_JOBRANK).setValue(grade.getPk_jobrank());
				}
			} else {
	...
	public void showAllSetItems(boolean editstate) {
		...
		BillTabVO oldtab = billdata.getTabVO(IBillItem.HEAD, TRNConst.TRNS_OLDINFO_TAB);
		BillTabVO newtab = billdata.getTabVO(IBillItem.HEAD, TRNConst.TRNS_NEWINFO_TAB);
		...
		for (IItemSetAdapter itemvo : itemvos) {
			...
			if (item != null) {
				...
				if (StapplyVO.NEWPK_ORG.equals(item.getKey()) || StapplyVO.NEWPK_DEPT.equals(item.getKey())
					...
					if (StapplyVO.NEWPK_DEPT.equals(item.getKey()) && !getModel().isApproveSite()) {
						item.setNull(false);
					}
					// end
				} else {
