...
public class OverTimeApplyView extends TaApplyBaseView implements IController {
	 ...
	 * 校驗加班類型是否是允許轉調休
	 */
	public String validisTorest(String pk_item) {
		String sql = "select ";
		return "";
	}
	...
	public void resetOvertimeType(Dataset dsDetail, Dataset ds) throws BusinessException {
		...
		if (allDetailRows.length > 0) {
			...
			overtimeType = GetOvertimeType(beiginTime, String.valueOf(selRow.getValue(ds.nameToIndex("pk_psndoc"))),
					String.valueOf(selRow.getValue(ds.nameToIndex("pk_org"))));
		}
		...
		for (int i = 1; i < allDetailRows.length; i++) {
			...
			String otType = GetOvertimeType(beiginTime, (String) selRow.getValue(ds.nameToIndex("pk_psndoc")),
					(String) selRow.getValue(ds.nameToIndex("pk_org")));
	...
	public void onAfterDataChange_hrtaovertimeb(DatasetCellEvent datasetCellEvent) {
		...
		if (colIndex == dsDetail.nameToIndex(OvertimebVO.OVERTIMEBEGINTIME)) {
			try {
				// String pk_hrorg =
				// rowMaster.getString(dsMaster.nameToIndex(OvertimehVO.PK_ORG));
				// if (isTWOrg(pk_hrorg)) {
		...
		// 選擇加班時間控制
		if (colIndex == dsDetail.nameToIndex(OvertimebVO.OVERTIMEENDTIME)) {
			String value = (String) rowMaster.getValue(dsMaster.nameToIndex(OvertimehVO.PK_OVERTIMETYPE));
			String pk_psndoc = (String) rowMaster.getValue(dsMaster.nameToIndex(OvertimehVO.PK_PSNDOC));
			List<List<String>> resultlist = new ArrayList<List<String>>();
			Row[] rows = dsDetail.getAllRow();
			for (Row row : rows) {
				if (row.getValue(colIndex) != null
						&& row.getValue(dsDetail.nameToIndex(OvertimebVO.OVERTIMEBEGINTIME)) != null) {
					List<String> timelist = new ArrayList<String>();
					UFDateTime beiginTime = (UFDateTime) rowDetail.getValue(dsDetail
							.nameToIndex(OvertimebVO.OVERTIMEBEGINTIME));
					UFDateTime endTime = (UFDateTime) rowDetail.getValue(dsDetail
							.nameToIndex(OvertimebVO.OVERTIMEENDTIME));
					timelist.add(beiginTime.toString());
			...
			if (resultlist != null && resultlist.size() > 0) {
				...
				try {
					overtype = NCLocator
							.getInstance()
							.lookup(IOvertimeQueryService.class)
							.queryOvertimeTypeByPsnDates(
									String.valueOf(rowMaster.getValue(dsMaster.nameToIndex("pk_org"))), pk_psndoc,
									resultlist, overtype);
				} catch (BusinessException e) {
